...
export function LeadManager() {
  ...
  async function find(event: FormEvent<HTMLFormElement>) {
    ...
    const data = new FormData(event.currentTarget);
    setFinding(true);
    setFindingStep('Connecting to Apollo...');
    setNotice('');
    setError('');
    try {
      await delay(120);
      ...
      setFindingStep('Saving results...');
      setLeads((items) => [...found, ...items.filter((item) => !found.some((lead) => lead.id === item.id))]);
      setFindingStep('Done.');
      setNotice(found.length ? `Imported ${found.length} Apollo companies and queued website analysis where possible.` : 'Apollo did not find matching companies. Try a broader industry, larger company size, or remove city filters.');
    } catch (nextError) {
      setFindingStep('');
      setError(friendlyErrorMessage(nextError, 'Apollo lead discovery could not be completed. Please adjust the filters and try again.'));
  ...
  if (simpleExperience) {
    return <div className="min-w-0 space-y-6">
      ...
      <section className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
        ...
        {loading ? <div className="mt-6"><Skeleton lines={4} /></div> : leads.length ? <div className="mt-5 space-y-3">{leads.map((lead) => <article key={lead.id || lead.company} className="rounded-lg border border-slate-200 p-4"><div className="flex flex-col gap-3 min-[430px]:flex-row min-[430px]:items-start min-[430px]:justify-between"><div><div className="flex flex-wrap items-center gap-2"><h3 className="font-bold text-ink">{lead.company}</h3>{lead.source === 'apollo' && <span className="rounded-full bg-indigo-50 px-2 py-1 text-xs font-bold text-indigo-700">Apollo</span>}</div><p className="mt-1 break-all text-sm text-slate-500">{lead.website || lead.domain || lead.email || 'Contact details not found yet'}</p></div><span className="w-fit rounded-full bg-teal-50 px-3 py-1 text-xs font-semibold text-brand">{lead.status}</span></div><dl className="mt-3 grid gap-2 text-sm min-[430px]:grid-cols-3"><div><dt className="text-slate-500">Industry</dt><dd>{lead.industry || 'Not found'}</dd></div><div><dt className="text-slate-500">Location</dt><dd>{[lead.city, lead.country].filter(Boolean).join(', ') || 'Not found'}</dd></div><div><dt className="text-slate-500">Contact</dt><dd>{lead.email || lead.contact || lead.phone || 'Not found'}</dd></div><div><dt className="text-slate-500">Employees</dt><dd>{lead.employee_count || 'Not found'}</dd></div><div><dt className="text-slate-500">Title</dt><dd>{lead.title || 'Not found'}</dd></div><div><dt className="text-slate-500">Confidence</dt><dd>{lead.confidence || 'Not found'}</dd></div></dl><div className="mt-4 flex flex-wrap gap-2"><button onClick={() => runLeadAi<SalesCopilot>(lead, 'copilot', (key, value) => setCopilot((items) => ({ ...items, [key]: value })))} className="min-h-11 rounded-md border border-slate-300 px-3 text-sm font-semibold">Review with AI</button><Link href="/dashboard/campaigns" className="inline-flex min-h-11 items-center justify-center rounded-md bg-ink px-3 text-sm font-semibold text-white">Use in campaign</Link></div></article>)}</div> : <div className="mt-5"><EmptyState title="No Apollo companies yet" copy="Search by country, city, and industry. If Apollo returns no results, broaden the city or company-size filter and try again." /></div>}
      </section>
      <section className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
...
export function SettingsAndProfile() {
  ...
  useEffect(() => {
    if (!ready) return;
    Promise.all([api<Profile>('/api/profile'), api<Settings>('/api/settings'), api<Workspace>('/api/workspace'), api<BillingPlan[]>('/api/billing/plans'), api<Usage>('/api/billing/usage'), api<ApolloIntegrationStatus>('/api/integrations/apollo/status')])
      .then(([p, s, w, nextPlans, nextUsage, nextApolloStatus]) => { setProfile(p); setSettings(s); setWorkspace(w); setPlans(Array.isArray(nextPlans) ? nextPlans : []); setUsage(nextUsage?.usage ? nextUsage : null); setApolloStatus(nextApolloStatus); })
