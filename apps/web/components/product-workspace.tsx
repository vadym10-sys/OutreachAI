...
const emptyMetrics: DashboardMetrics = { leads: 0, campaigns: 0, emails_sent: 0, open_rate: 0, reply_rate: 0, meetings: 0, revenue: 0, mrr: 0 };
...
function useClerkTokenApi() {
  ...
  return useCallback(async function api<T>(path: string, init: RequestInit = {}) {
    if (!isLoaded) throw new Error('Authentication is loading');
...
export function DashboardHome() {
  ...
  useEffect(() => {
    Promise.all([api<DashboardMetrics>('/api/dashboard'), api<Activity[]>('/api/activity'), api<Notification[]>('/api/notifications')])
...
export function CampaignBuilder() {
  ...
  const load = useCallback(() => { Promise.all([api<Campaign[]>('/api/campaigns'), api<{ items: Lead[] }>('/api/leads?page_size=100')]).then(([c, l]) => { setCampaigns(c); setLeads(l.items); if (c[0]) setSelectedCampaign(c[0].id); if (l.items[0]) setSelectedLead(l.items[0].id || ''); }).finally(() => setLoading(false)); }, [api]);
  useEffect(load, [load]);
...
export function LeadManager() {
  ...
  const load = useCallback(() => { Promise.all([api<{ items: Lead[] }>(`/api/leads?search=${encodeURIComponent(search)}&status=${encodeURIComponent(status)}&page_size=50`), api<Campaign[]>('/api/campaigns')]).then(([leadPage, nextCampaigns]) => { setLeads(leadPage.items); setCampaigns(nextCampaigns); }).finally(() => setLoading(false)); }, [api, search, status]);
  useEffect(load, [load]);
...
export function SettingsAndProfile() {
  ...
  const [notice, setNotice] = useState('');
  useEffect(() => { Promise.all([api<Profile>('/api/profile'), api<Settings>('/api/settings')]).then(([p, s]) => { setProfile(p); setSettings(s); }); }, [api]);
