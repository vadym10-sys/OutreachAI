...
import type { Activity, AdminSummary, BillingPlan, Campaign, DashboardMetrics, Email, Lead, Notification, Profile, Settings, Usage, Workspace } from '@/lib/types';
...
const emptyMetrics: DashboardMetrics = { leads: 0, campaigns: 0, emails_sent: 0, delivered: 0, opened: 0, replies: 0, bounces: 0, open_rate: 0, reply_rate: 0, conversion_rate: 0, meetings: 0, revenue: 0, mrr: 0, arr: 0, revenue_series: [], funnel: [], plan: 'Starter', usage: {} };
...
export function DashboardHome() {
  ...
  const cards = [
    ...
    ['Reply rate', `${metrics.reply_rate}%`], ['Conversion', `${metrics.conversion_rate}%`], ['Meetings', metrics.meetings], ['Revenue', `$${metricNumber(metrics.revenue).toLocaleString()}`], ['MRR', `$${metricNumber(metrics.mrr).toLocaleString()}`], ['ARR', `$${metricNumber(metrics.arr).toLocaleString()}`], ['Plan', metrics.plan || 'Starter']
  ];
...
export function CampaignBuilder() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const payload = {
      ...
      language: String(data.get('language') || 'English'), offer: String(data.get('offer') || ''), cta: String(data.get('cta') || ''), email_tone: String(data.get('email_tone') || 'Professional'), signature: String(data.get('signature') || ''), follow_up_days: Number(data.get('follow_up_days') || 3), timezone: String(data.get('timezone') || 'UTC'),
      sequence: [
