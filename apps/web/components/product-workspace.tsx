...
export function SettingsAndProfile() {
  ...
  async function testApollo() {
    ...
    try {
      ...
      setNotice(status.connected ? 'Apollo connection verified.' : status.last_error || 'Apollo is not connected.');
    } catch (nextError) {
      setNotice(friendlyErrorMessage(nextError, 'Apollo connection test could not be completed.'));
    } finally {
  ...
  async function testHunter() {
    ...
    try {
      ...
      setNotice(status.connected ? 'Hunter connection verified.' : status.last_error || 'Hunter is not connected.');
    } catch (nextError) {
      setNotice(friendlyErrorMessage(nextError, 'Hunter connection test could not be completed.'));
    } finally {
