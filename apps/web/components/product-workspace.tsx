...
export function DashboardHome() {
  ...
  if (simpleExperience) {
    const firstAction = metrics.leads > 0 ? { href: '/dashboard/campaigns', label: 'Create campaign' } : { href: '/dashboard/leads', label: 'Find leads' };
    const nextStep = metrics.leads > 0 ? 'You have leads. Next, create a simple campaign and let AI prepare the first email for review.' : 'Start by finding companies you want to contact. OutreachAI will analyze them and prepare the next step.';
    return <div className="min-w-0 space-y-6">
      <header className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
        <p className="text-sm font-semibold text-brand">Start here</p>
        <h1 className="mt-2 text-2xl font-bold min-[390px]:text-3xl">Dashboard</h1>
        <p className="mt-2 max-w-2xl text-slate-600">See what to do next and track the business results that matter.</p>
      ...
      {loading ? <Skeleton lines={4} /> : <section className="rounded-lg border border-teal-200 bg-teal-50 p-5">
        <p className="text-sm font-semibold text-brand">Next best action</p>
      ...
      <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        ].map(([label, value]) => <article key={label} className="rounded-lg border border-slate-200 bg-white p-4"><p className="text-sm text-slate-500">{label}</p><p className="mt-2 text-3xl font-bold text-ink">{value}</p></article>)}
      </section>
      <section className="grid gap-4 lg:grid-cols-3">
        {[
          ['1. Find leads', 'Choose a country, city, and industry. Keep it broad for the first search.', '/dashboard/leads', 'Find leads'],
          ['2. Review AI work', 'Open each company result and check the website analysis before sending anything.', '/dashboard/leads', 'Review leads'],
          ['3. Approve campaign', 'Create one simple campaign, generate an email, and approve only when it looks right.', '/dashboard/campaigns', 'Create campaign']
        ].map(([title, copy, href, label]) => <article key={title} className="rounded-lg border border-slate-200 bg-white p-5"><h2 className="font-bold text-ink">{title}</h2><p className="mt-2 text-sm text-slate-600">{copy}</p><Link href={href} className="mt-4 inline-flex min-h-11 items-center justify-center rounded-md border border-slate-300 px-4 py-2 text-sm font-semibold text-ink">{label}</Link></article>)}
      </section>
    </div>;
  }
    ...
    return <div className="min-w-0 space-y-6">
      ...
      <form id="lead-search-form" onSubmit={find} className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
        ...
        <div className="grid gap-4 md:grid-cols-3">
          <label className="block"><span className="text-sm font-semibold text-slate-700">Country</span><input required name="country" placeholder="Germany" className="mt-2 w-full rounded-md border border-slate-300 px-3 py-3" /></label>
          ...
          <label className="block"><span className="text-sm font-semibold text-slate-700">Industry</span><input name="industry" placeholder="Construction" className="mt-2 w-full rounded-md border border-slate-300 px-3 py-3" /></label>
        </div>
        <details className="mt-4 rounded-md border border-slate-200 bg-slate-50 p-3" open={leadAdvancedOpen} onToggle={(event) => setLeadAdvancedOpen(event.currentTarget.open)}>
          ...
          <div className="mt-3 grid gap-3 md:grid-cols-2">
            ...
            <input name="employee_count" placeholder="Employee count" className="rounded-md border border-slate-300 px-3 py-2" />
        ...
        <button disabled={finding} className="focus-ring mt-5 inline-flex min-h-11 w-full items-center justify-center gap-2 rounded-md bg-brand px-4 py-2 font-semibold text-white md:w-auto">{finding ? <Loader2 className="animate-spin" size={18} /> : <Search size={18} />} Find companies</button>
      </form>
      <section className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
