...
import type { Activity, AdminSummary, AISalesEmployee, ApolloIntegrationStatus, BillingPlan, Campaign, CampaignAnalytics, DashboardMetrics, Email, FollowUpSequence, GrowthEngine, HunterIntegrationStatus, Lead, MeetingPrep, Notification, Profile, SalesCopilot, SalesEmployeeLeadInsight, SalesEmployeeMemory, SalesEmployeePerformance, SalesEmployeeRun, SalesEmployeeTaskPlan, Settings, TeamRouterDashboard, TeamRouterPlan, Usage, WebsiteAudit, Workspace } from '@/lib/types';
...
function FirstMeetingGuide({ compact = false }: { compact?: boolean }) {
  ...
  const steps = [
    ...
    ['2', 'Find companies', 'Why: meetings start with the right accounts.', 'Next: find companies and verify emails.', 'Time: 1-2 minutes', 'Success: qualified companies appear.'],
...
function LeadSourceBadges({ lead }: { lead: Lead }) {
  return <div className="flex flex-wrap items-center gap-2">
    {lead.source === 'google_maps' && <span className="rounded-full bg-sky-50 px-2 py-1 text-xs font-bold text-sky-700">Local search</span>}
    {lead.source === 'apollo' && <span className="rounded-full bg-indigo-50 px-2 py-1 text-xs font-bold text-indigo-700">Lead database</span>}
    {lead.source === 'manual' && <span className="rounded-full bg-slate-100 px-2 py-1 text-xs font-bold text-slate-700">Manual</span>}
    {lead.hunter_verified ? <span className="rounded-full bg-teal-50 px-2 py-1 text-xs font-bold text-brand">Verified email</span> : null}
...
function leadIntelligence(lead: Lead) {
  ...
  const readable = firstLine.trim().startsWith('{') ? raw.split('\n').slice(1).join('\n').trim() : raw.trim();
  const value = (key: string, fallback = '') => {
    const direct = lead[key as keyof Lead];
    if (typeof direct === 'string' && direct.trim()) return direct;
    const meta = metadata[key];
    return typeof meta === 'string' && meta.trim() ? meta : fallback;
  };
  return {
    summary: value('ai_summary', readable || `${lead.company} matches this search because it fits the selected industry, location, and company profile.`),
    offer: value('suggested_offer'),
    strategy: value('outreach_strategy'),
    angle: value('sales_angle'),
    expectedReplyRate: value('expected_reply_rate'),
    icpScore: typeof metadata.icp_score === 'number' ? metadata.icp_score : null,
    language: value('detected_language'),
    geography: value('target_geography'),
  };
}
...
function PremiumLeadCard({ lead, copilot, aiLoading, draft, draftLoading, onReview, onGenerateEmail }: { lead: Lead; copilot?: SalesCopilot; aiLoading: boolean; draft?: Email; draftLoading?: boolean; onReview: () => void; onGenerateEmail: () => void }) {
  ...
  const score = copilot?.probability_to_reply ?? intelligence.icpScore ?? trustScoreFromLead(lead);
    ...
    <div className="grid gap-3 p-4 lg:grid-cols-[1fr_0.85fr]">
      ...
      <div className="rounded-lg border border-slate-200 bg-slate-50 p-4">
        ...
        {copilot && <div className="mt-3 grid gap-2 text-sm min-[430px]:grid-cols-2">
          ...
          <p className="rounded-md bg-white p-2"><span className="font-semibold">Revenue:</span> {formatEstimatedRevenue(copilot.estimated_revenue)}</p>
        </div>}
        ...
        {!copilot && (intelligence.offer || intelligence.strategy || intelligence.angle || intelligence.expectedReplyRate) && <div className="mt-3 grid gap-2 text-sm">
          ...
          {intelligence.strategy && <p className="rounded-md bg-white p-2"><span className="font-semibold">Outreach strategy:</span> {intelligence.strategy}</p>}
          {intelligence.expectedReplyRate && <p className="rounded-md bg-white p-2"><span className="font-semibold">Expected reply:</span> {intelligence.expectedReplyRate}</p>}
        </div>}
      </div>
    </div>
...
export function LeadManager() {
  ...
  const [findingStep, setFindingStep] = useState('');
  ...
  async function find(event: FormEvent<HTMLFormElement>) {
    ...
    const data = new FormData(event.currentTarget);
    setFinding(true);
    setFindingStep('Connecting to lead sources...');
    setLeadActionLogs(['Connecting to lead search, contact verification, and AI analysis.']);
    setNotice('');
    setError('');
    try {
      await delay(120);
      setFindingStep('Searching companies...');
      setLeadActionLogs((items) => [...items, 'Searching for companies that match the country, city, industry, and radius.']);
      ...
      setNotice(found.length ? `Imported ${found.length} companies. Verified ${verified} email${verified === 1 ? '' : 's'} and queued website analysis where possible.` : 'No matching companies were found. Try a broader industry, larger company size, or remove city filters.');
  ...
  async function create(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      await delay(120);
      setFindingStep('Checking verified emails...');
      setLeadActionLogs((items) => [...items, 'Checking decision-maker emails by domain.']);
      ...
      setLeadActionLogs((items) => [...items, 'AI is preparing the company summary, sales angle, offer, and outreach strategy.']);
      setLeads((items) => [lead, ...items.filter((item) => item.id !== lead.id)]);
      setFindingStep('Ready for review.');
      setNotice(lead.hunter_verified ? `${lead.company} is ready. An email was verified and AI prepared outreach intelligence.` : `${lead.company} is ready. No verified email was found yet, but AI analysis and manual email entry are available.`);
  ...
  if (simpleExperience) {
    return <div className="min-w-0 space-y-6">
      <header className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
        ...
        <p className="mt-2 max-w-2xl text-slate-600">Add one company website. OutreachAI checks for verified emails and analyzes the website so you can prepare a reviewed outreach draft today.</p>
      ...
      <section className="grid gap-3 md:grid-cols-4">
        {[
          ...
          ['What happens next', 'OutreachAI verifies emails, analyzes the website, and saves the company without duplicates.'],
      ...
      <form id="lead-search-form" onSubmit={find} className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
        ...
        {findingStep && <div role="status" className="mt-4 rounded-md border border-teal-200 bg-teal-50 px-4 py-3 text-sm font-semibold text-brand">{findingStep}</div>}
...
export function AISalesEmployees() {
  ...
  async function importText(path: 'websites' | 'google-maps' | 'business-export', field: 'websites' | 'export_text', event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    if (!employeeId) return;
    const data = new FormData(event.currentTarget);
    setBusy(path);
...
export function SettingsAndProfile() {
  ...
  const [apolloStatus, setApolloStatus] = useState<ApolloIntegrationStatus | null>(null);
  const [hunterStatus, setHunterStatus] = useState<HunterIntegrationStatus | null>(null);
  const [testingApollo, setTestingApollo] = useState(false);
  const [testingHunter, setTestingHunter] = useState(false);
  const [notice, setNotice] = useState('');
  useEffect(() => {
    if (!ready) return;
    Promise.all([api<Profile>('/api/profile'), api<Settings>('/api/settings'), api<Workspace>('/api/workspace'), api<BillingPlan[]>('/api/billing/plans'), api<Usage>('/api/billing/usage'), api<ApolloIntegrationStatus>('/api/integrations/apollo/status'), api<HunterIntegrationStatus>('/api/integrations/hunter/status')])
      .then(([p, s, w, nextPlans, nextUsage, nextApolloStatus, nextHunterStatus]) => { setProfile(p); setSettings(s); setWorkspace(w); setPlans(Array.isArray(nextPlans) ? nextPlans : []); setUsage(nextUsage?.usage ? nextUsage : null); setApolloStatus(nextApolloStatus); setHunterStatus(nextHunterStatus); })
  ...
  async function testApollo() {
    ...
    try {
      const status = await api<ApolloIntegrationStatus>('/api/integrations/apollo/test', { method: 'POST' });
      setApolloStatus(status);
      setNotice(status.connected ? 'Lead search connection verified.' : status.last_error || 'Lead search connection is not ready.');
    } catch (nextError) {
      setNotice(friendlyErrorMessage(nextError, 'Lead search connection test could not be completed.'));
    } finally {
  ...
  async function testHunter() {
    ...
    try {
      const status = await api<HunterIntegrationStatus>('/api/integrations/hunter/test', { method: 'POST' });
      setHunterStatus(status);
      setNotice(status.connected ? 'Email verification connection verified.' : status.last_error || 'Email verification is not ready.');
    } catch (nextError) {
      setNotice(friendlyErrorMessage(nextError, 'Email verification test could not be completed.'));
    } finally {
      ...
      {!settings ? <Skeleton lines={5} /> : <div className="grid gap-6 xl:grid-cols-[1fr_0.8fr]">
        ...
        <aside className="space-y-6">
          ...
          <section className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
            <div className="flex items-start justify-between gap-3">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-600">Manual company analysis and email verification are ready today. Automatic company search requires upgraded data access.</p>
              </div>
            ...
            <dl className="mt-4 grid gap-2 text-sm">
              ...
              <div className="rounded-md bg-slate-50 p-3"><dt className="font-semibold text-slate-600">What to do next</dt><dd>Add a company website in Lead Finder. Automatic search is unavailable until upgraded data access is enabled.</dd></div>
            </dl>
          ...
          <section className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
            <div className="flex items-start justify-between gap-3">
              <div>
                <h2 className="text-xl font-bold text-ink">Email verification</h2>
                <p className="mt-1 text-sm text-slate-600">Decision-maker emails are verified before you review outreach.</p>
              </div>
            ...
            <dl className="mt-4 grid gap-2 text-sm">
              ...
              <div className="rounded-md bg-slate-50 p-3"><dt className="font-semibold text-slate-600">What to do next</dt><dd>{hunterStatus?.connected ? 'Find companies and review verified emails.' : 'Ask the owner to reconnect email verification.'}</dd></div>
            </dl>
            <button type="button" onClick={testHunter} disabled={testingHunter} className="focus-ring mt-4 inline-flex min-h-11 w-full items-center justify-center gap-2 rounded-md border border-slate-300 px-4 py-2 text-sm font-semibold text-ink disabled:opacity-60">{testingHunter && <Loader2 className="animate-spin" size={16} />} Test email verification</button>
          </section>
          <details className="rounded-lg border border-slate-200 bg-white p-5">
            <summary className="cursor-pointer font-semibold text-ink">Advanced settings</summary>
      ...
      <div className="space-y-6">
        ...
        <section className="rounded-lg border border-slate-200 bg-white p-5">
          <div className="flex items-start justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-600">Manual company analysis is active. Automatic company search requires an upgraded data connection before it can search companies.</p>
            </div>
            <span className={`rounded-full px-3 py-1 text-xs font-bold ${showApolloLeadSearch && apolloStatus?.connected ? 'bg-teal-50 text-brand' : 'bg-orange-50 text-orange-700'}`}>{showApolloLeadSearch && apolloStatus?.connected ? 'Connected' : 'Requires Custom API Access'}</span>
          </div>
        ...
        <section className="rounded-lg border border-slate-200 bg-white p-5">
          ...
          <div className="mt-4 grid gap-3 text-sm md:grid-cols-3">
            ...
            <p className="rounded-md bg-slate-50 p-3">Next step: <span className="font-semibold">{hunterStatus?.connected ? 'Review verified emails' : 'Reconnect email verification'}</span></p>
          </div>
          <button type="button" onClick={testHunter} disabled={testingHunter} className="focus-ring mt-4 inline-flex min-h-11 items-center justify-center gap-2 rounded-md border border-slate-300 px-4 py-2 text-sm font-semibold disabled:opacity-60">{testingHunter && <Loader2 className="animate-spin" size={16} />} Test email verification</button>
        </section>
        <section className="rounded-lg border border-slate-200 bg-white p-5">
