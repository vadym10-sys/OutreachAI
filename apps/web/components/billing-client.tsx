...
export function PricingCheckoutButton({ plan, children = 'Subscribe' }: { plan: PlanName; children?: React.ReactNode }) {
  ...
  return <button onClick={startCheckout} disabled={loading} className="focus-ring inline-flex min-h-11 w-full items-center justify-center gap-2 rounded-md bg-brand px-5 py-3 text-center text-sm font-semibold text-white shadow-soft transition hover:bg-blue-700 disabled:opacity-60 min-[360px]:w-auto">{loading ? <Loader2 className="animate-spin" size={18} /> : <CreditCard size={18} />}{children}</button>;
}
  ...
  async function checkout(plan: string) {
    ...
    try {
      ...
      const session = current
        ...
        : await apiWithToken<BillingCheckoutSession>('/api/billing/checkout', authToken, { method: 'POST', body: JSON.stringify({ plan }) });
      safeRedirect(session.url);
    } catch (nextError) {
