...
export function TaskResultClient({ taskId }: { taskId: string }) {
  ...
  return <div className="min-w-0"><div className="flex flex-col gap-3 min-[430px]:flex-row min-[430px]:items-start min-[430px]:justify-between"><div><Link href="/dashboard/sales-employees" className="text-sm font-semibold text-brand">Back to AI Employees</Link><h1 className="mt-2 text-2xl font-bold min-[390px]:text-3xl">Task Results</h1><p className="mt-2 break-words text-slate-600">{result?.command || 'Loading task report...'}</p></div>{result && <span className="w-fit rounded-full bg-teal-50 px-3 py-1 text-xs font-bold text-brand">{result.status}</span>}</div>{error && <div className="mt-4 rounded-md border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">{error}</div>}{notice && <div className="mt-4 rounded-md border border-teal-200 bg-teal-50 px-4 py-3 text-sm text-brand">{notice}</div>}{loading ? <div className="mt-6 h-48 animate-pulse rounded-lg bg-slate-200" /> : result && report ? <div className="mt-6 space-y-6"><section className="rounded-lg border border-slate-200 bg-white p-5"><div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between"><div><p className="text-sm font-semibold text-brand">{result.employee_name || 'AI Sales Employee'}</p><h2 className="mt-1 text-xl font-bold">{report.final_summary}</h2><p className="mt-2 text-sm text-slate-600">Execution time: {Math.round(result.execution_time_ms / 1000)}s · Completed: {result.completed_at ? new Date(result.completed_at).toLocaleString() : 'Not completed'}</p></div><div className="flex flex-wrap gap-2"><button onClick={downloadCsv} disabled={busy === 'csv'} className="focus-ring inline-flex min-h-11 items-center gap-2 rounded-md border border-slate-300 px-4 py-2 text-sm font-semibold">{busy === 'csv' ? <Loader2 className="animate-spin" size={16} /> : <Download size={16} />} Download CSV</button><button onClick={() => action('export-crm')} disabled={busy === 'export-crm'} className="focus-ring inline-flex min-h-11 items-center gap-2 rounded-md border border-slate-300 px-4 py-2 text-sm font-semibold"><UploadCloud size={16} /> Export to CRM</button><button onClick={() => action('approve-send')} disabled={busy === 'approve-send'} className="focus-ring inline-flex min-h-11 items-center gap-2 rounded-md bg-ink px-4 py-2 text-sm font-semibold text-white"><Send size={16} /> Approve & Send</button></div></div><p className="mt-4 rounded-md bg-slate-50 p-3 text-sm text-slate-700"><span className="font-semibold">Next recommended action:</span> {report.next_recommended_action}</p></section>{report.failure_reason && <section className="rounded-lg border border-orange-200 bg-orange-50 p-5"><h2 className="font-bold text-orange-900">No companies found</h2><p className="mt-2 text-sm text-orange-800">{report.failure_reason}</p><p className="mt-3 text-sm text-orange-800">Suggested next command: {text(report.empty_result_details?.suggested_next_command, 'Broaden the search and try again.')}</p></section>}<section className="rounded-lg border border-slate-200 bg-white p-5"><h2 className="font-bold">Companies found</h2>{companies.length ? <div className="mt-4 space-y-3">{companies.map((company, index) => <article key={`${text(company.company_name)}-${index}`} className="rounded-md border border-slate-200 p-3"><div className="flex flex-col gap-2 min-[430px]:flex-row min-[430px]:items-start min-[430px]:justify-between"><div><p className="font-semibold">{text(company.company_name)}</p><p className="break-all text-sm text-slate-500">{text(company.website)}</p></div><span className="w-fit rounded-full bg-teal-50 px-3 py-1 text-xs font-bold text-brand">Confidence {numberValue(company.confidence_score)}%</span></div><dl className="mt-3 grid gap-2 text-sm min-[430px]:grid-cols-3"><div><dt className="text-slate-500">Country</dt><dd>{text(company.country)}</dd></div><div><dt className="text-slate-500">City</dt><dd>{text(company.city)}</dd></div><div><dt className="text-slate-500">Industry</dt><dd>{text(company.industry)}</dd></div><div><dt className="text-slate-500">Phone</dt><dd>{text(company.phone)}</dd></div><div><dt className="text-slate-500">Email</dt><dd>{text(company.email)}</dd></div><div><dt className="text-slate-500">Source</dt><dd>{text(company.source)}</dd></div></dl><p className="mt-3 text-sm text-slate-600">{text(company.short_description)}</p><p className="mt-2 text-sm font-semibold text-ink">{text(company.why_matched)}</p></article>)}</div> : <p className="mt-4 rounded-md bg-slate-50 p-3 text-sm text-slate-600">No companies were found. Review the empty-result guidance above.</p>}</section><section className="rounded-lg border border-slate-200 bg-white p-5"><h2 className="font-bold">Prepared outreach emails</h2>{emails.length ? <div className="mt-4 space-y-3">{emails.map((email, index) => <article key={index} className="rounded-md bg-slate-50 p-3 text-sm"><p className="font-semibold">{text(email.subject, 'Draft subject')}</p><p className="mt-1 text-slate-500">Target: {text(email.target_company)} · Tone: {text(email.tone, 'Professional')}</p><pre className="mt-3 whitespace-pre-wrap rounded-md bg-white p-3 text-slate-700">{text(email.body, 'Draft body not available')}</pre></article>)}</div> : <p className="mt-4 rounded-md bg-slate-50 p-3 text-sm text-slate-600">No outreach drafts were prepared for this task.</p>}</section><section className="grid gap-6 lg:grid-cols-2"><div className="rounded-lg border border-slate-200 bg-white p-5"><h2 className="font-bold">Tools used</h2><div className="mt-4 space-y-3">{tools.map((tool, index) => <div key={index} className="rounded-md bg-slate-50 p-3 text-sm"><p className="font-semibold">{text(tool.tool_name)}</p><p className="mt-1 text-slate-600">{text(tool.output_summary)}</p><p className="mt-2 text-xs font-bold text-brand">{text(tool.status)} · {numberValue(tool.duration_ms)}ms</p></div>)}</div></div><div className="rounded-lg border border-slate-200 bg-white p-5"><h2 className="font-bold">AI action log</h2><div className="mt-4 space-y-3">{log.map((item, index) => <div key={index} className="rounded-md bg-slate-50 p-3 text-sm"><p className="flex items-center gap-2 font-semibold"><CheckCircle2 size={16} className="text-brand" />{text(item.step)}</p><p className="mt-1 text-slate-600">{text(item.message)}</p><p className="mt-2 text-xs text-slate-500">{text(item.timestamp, 'No timestamp')} · {text(item.status)}</p></div>)}</div></div></section></div> : <div className="mt-6 rounded-lg border border-slate-200 bg-white p-6 text-sm text-slate-600">Task result not found.</div>}</div>;
}
