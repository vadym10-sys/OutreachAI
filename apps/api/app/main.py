...
from app.core.observability import (
    init_sentry,
    sentry_transaction_name,
    set_request_context,
)
from app.core.reliability import (
    required_environment_issues,
...
def _safe_error_message(status_code: int, detail: object) -> str:
    ...
    if raw in {
        ...
        "Outbound sending is disabled in this environment.",
    }:
        return raw
    ...
    if any(
        term in lower
        for term in ["google", "places", "apollo", "hunter", "lead search"]
    ):
        return "Lead search is temporarily unavailable. Please try again later."
    if any(
        term in lower for term in ["openai", "model", "ai analysis", "website analysis"]
    ):
...
def _safe_error_detail(status_code: int, detail: object) -> object:
    if (
        status_code == 402
        and isinstance(detail, dict)
        and detail.get("code") == "plan_limit_exceeded"
    ):
        return detail
    return _safe_error_message(status_code, detail)
...
def _is_enrichment_restart_path(request: Request) -> bool:
    ...
    return (
        ...
        and path.startswith("/api/workspace-app/companies/")
        and path.endswith("/enrichment/restart")
    )
...
async def sentry_request_context(request: Request, call_next) -> Response:
    ...
    sentry_sdk.set_context(
        "transaction",
        {"name": sentry_transaction_name(request), "request_id": request_id},
    )
    try:
    ...
    if duration_ms >= settings.slow_request_ms:
        logger.warning(
            "Slow request request_id=%s path=%s duration_ms=%s",
            request_id,
            request.url.path,
            duration_ms,
        )
...
async def audit_user_actions(request: Request, call_next) -> Response:
    ...
    if request.url.path.startswith(
        ("/api/health", "/api/live", "/api/ready", "/docs", "/openapi", "/webhooks")
    ):
        return response

    user_id = authenticated_user_id_from_authorization(
    ...
    try:
        ...
        logger.warning(
            "User action audit failed request_id=%s path=%s reason=%s",
...
async def sanitized_http_exception_handler(
    ...
    if exc.status_code >= 500 and _is_enrichment_restart_path(request):
        sentry_sdk.capture_message(
            "Enrichment restart downgraded from HTTP 5xx", level="warning"
        )
    ...
    return JSONResponse(
        status_code=exc.status_code,
        content={"detail": _safe_error_detail(exc.status_code, exc.detail)},
    )
...
async def sanitized_unhandled_exception_handler(
    ...
    if _is_enrichment_restart_path(request):
        sentry_sdk.capture_message(
            "Enrichment restart unhandled exception downgraded", level="warning"
        )
        return _enrichment_restart_fallback_response(request)
    return JSONResponse(
        status_code=500,
...
def api_readiness() -> JSONResponse:
        ...
        if not schema_status.ready:
            ...
            if schema_status.pending_migrations:
                critical_failures.append(
                    f"Pending migrations: {', '.join(schema_status.pending_migrations)}"
                )
            if schema_status.missing_tables:
                critical_failures.append(
                    f"Missing tables: {', '.join(schema_status.missing_tables)}"
                )
    ...
    if env_issues:
        critical_failures.append(
            f"Missing or invalid required environment: {', '.join(env_issues)}"
        )
    ...
    if not database_backups_configured:
        ...
        if production_runtime:
            critical_failures.append(
                "Production database backups are not configured or restore-verified."
            )
...
def api_queue_health(user: QueueHealthUser, db=Depends(get_db)) -> dict:
    del user
    summary = queue_health_summary(
        db, stale_after_seconds=settings.enrichment_worker_claim_timeout_seconds
    )
    return {
        **summary,
        "worker_claim_timeout_seconds": int(
            settings.enrichment_worker_claim_timeout_seconds or 900
        ),
...
app.include_router(
    ...
    tags=["ai-customer-finder"],
)
app.include_router(
    revenue_intelligence_router,
    prefix="/api/workspace-app/revenue-intelligence",
...
def startup() -> None:
    try:
        ...
        logger.info(
            ...
            ", ".join(
                f"{route.path}:{','.join(sorted(route.methods or []))}"
        ...
        if missing:
            logger.warning(
                "Customer-facing integrations not configured: %s", ", ".join(missing)
            )

        if not settings.auto_create_tables:
            logger.info(
                "Database table auto-creation disabled; /api/health is available without database connectivity"
            )
            return
        ...
        try:
            ...
            logger.exception(
                "Database schema migration failed; /api/ready will remain degraded until migrations succeed: %s",
                exc,
            )
            return
        with get_sessionmaker()() as db:
        ...
        if not backup_ready:
            logger.warning(
                "Database backup policy is not confirmed by runtime configuration."
            )
