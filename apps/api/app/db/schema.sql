...
CREATE INDEX IF NOT EXISTS idx_test_entitlements_workspace_user ON test_entitlements(workspace_id, user_id);
CREATE INDEX IF NOT EXISTS idx_test_entitlements_active_lookup ON test_entitlements(workspace_id, user_id, expires_at, revoked_at);
...
CREATE INDEX IF NOT EXISTS idx_plan_usage_reservations_workspace_id ON plan_usage_reservations(workspace_id);
CREATE INDEX IF NOT EXISTS idx_plan_usage_reservations_active ON plan_usage_reservations(workspace_id, period, metric, status, expires_at);
CREATE UNIQUE INDEX IF NOT EXISTS uq_plan_usage_reservation_idempotency ON plan_usage_reservations(workspace_id, period, metric, idempotency_key) WHERE status IN ('reserved', 'finalized');
...
CREATE INDEX IF NOT EXISTS idx_agent_runs_workspace_status_created ON agent_runs(workspace_id, status, created_at DESC);
CREATE UNIQUE INDEX IF NOT EXISTS uq_agent_runs_workspace_idempotency ON agent_runs(workspace_id, idempotency_key) WHERE idempotency_key <> '';
CREATE INDEX IF NOT EXISTS idx_agent_run_jobs_workspace_id ON agent_run_jobs(workspace_id);
CREATE INDEX IF NOT EXISTS idx_agent_run_jobs_workspace_status ON agent_run_jobs(workspace_id, status, available_at ASC);
CREATE INDEX IF NOT EXISTS idx_agent_run_jobs_run_operation ON agent_run_jobs(run_id, operation, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_agent_run_jobs_claim_token ON agent_run_jobs(claim_token);
CREATE UNIQUE INDEX IF NOT EXISTS uq_agent_run_jobs_active_operation ON agent_run_jobs(workspace_id, run_id, operation) WHERE status IN ('queued', 'running', 'retrying');
CREATE INDEX IF NOT EXISTS idx_agent_steps_workspace_id ON agent_steps(workspace_id);
CREATE INDEX IF NOT EXISTS idx_agent_steps_workspace_run ON agent_steps(workspace_id, run_id, step_index);
CREATE INDEX IF NOT EXISTS idx_agent_tool_calls_workspace_id ON agent_tool_calls(workspace_id);
CREATE INDEX IF NOT EXISTS idx_agent_tool_calls_workspace_run ON agent_tool_calls(workspace_id, run_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_agent_tool_calls_workspace_tool ON agent_tool_calls(workspace_id, tool_name, created_at DESC);
CREATE UNIQUE INDEX IF NOT EXISTS uq_agent_tool_calls_idempotency ON agent_tool_calls(workspace_id, idempotency_key) WHERE idempotency_key <> '';
CREATE INDEX IF NOT EXISTS idx_agent_approval_requests_workspace_id ON agent_approval_requests(workspace_id);
CREATE INDEX IF NOT EXISTS idx_agent_approval_requests_workspace_run ON agent_approval_requests(workspace_id, run_id, requested_at DESC);
CREATE INDEX IF NOT EXISTS idx_agent_approval_requests_workspace_state ON agent_approval_requests(workspace_id, approval_state, requested_at DESC);
CREATE UNIQUE INDEX IF NOT EXISTS uq_agent_approval_requests_idempotency ON agent_approval_requests(workspace_id, idempotency_key) WHERE idempotency_key <> '';
CREATE INDEX IF NOT EXISTS idx_agent_trace_events_workspace_id ON agent_trace_events(workspace_id);
CREATE INDEX IF NOT EXISTS idx_agent_trace_events_workspace_run ON agent_trace_events(workspace_id, run_id, created_at ASC);
CREATE INDEX IF NOT EXISTS idx_agent_trace_events_workspace_tool ON agent_trace_events(workspace_id, tool_name, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_agent_trace_events_workspace_error ON agent_trace_events(workspace_id, error_category, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_action_policy_enforcements_workspace_action ON action_policy_enforcements(workspace_id, action_name, created_at DESC);
CREATE UNIQUE INDEX IF NOT EXISTS uq_action_policy_enforcements_idempotency ON action_policy_enforcements(workspace_id, idempotency_key) WHERE idempotency_key <> '';
