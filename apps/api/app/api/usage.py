...
from app.services.ai_sales_workspace import (
...
from app.services.enrichment_queue import (
    ...
    enqueue_deep_contact_search_job,
    mark_cancelled,
    update_job_progress,
)
...
from app.services.ai_customer_finder.service import (
    result_out as first_customer_result_out,
)
from app.services.ai_customer_finder.service import (
    save_first_customer_result_to_crm,
    search_first_customer_candidates,
)
...
UsageStatus = Literal[
    "success", "partial_success", "empty", "provider_unavailable", "timeout", "error"
]
...
INTERNAL_EMAIL_SMOKE_DRAFT_ALLOWED_FIELDS = {
    "recipient_email",
    "confirmed_recipient_control",
}
...
class UsageAISalesAnalysisOut(BaseModel):
    ...
    available_versions: list[UsageAISalesAnalysisVersionOut] = Field(
...
def _parse_lead_command(
    command: str, workspace
) -> tuple[LeadFinderRequest | None, list[str]]:
    ...
    company_size = (
        f"{range_match.group(1)}-{range_match.group(2)}" if range_match else ""
    )
    ...
    if not industry:
        ...
        industry = next(
            (
                canonical
                for canonical, aliases in INDUSTRY_ALIASES.items()
                if workspace_industry == canonical
                or any(alias in workspace_industry.lower() for alias in aliases)
            ),
            workspace_industry,
        )
...
def _minimal_crm_company_out(company: Company) -> CrmCompanyOut:
    ...
    return CrmCompanyOut(
        ...
        google_rating=float(company.google_rating)
        ...
        buying_signal_score=int(metadata["buying_signal_score"])
        if str(metadata.get("buying_signal_score") or "").isdigit()
        else None,
        ...
        buying_signal_evidence=[
            item
            for item in metadata.get("buying_signal_evidence", [])
            if isinstance(item, dict)
        ]
        if isinstance(metadata.get("buying_signal_evidence"), list)
        else [],
        buying_signal_confidence=int(metadata["buying_signal_confidence"])
        if str(metadata.get("buying_signal_confidence") or "").isdigit()
        else None,
        recommended_outreach_timing=str(
            metadata.get("recommended_outreach_timing") or ""
        ),
        ...
        decision_maker_intelligence=metadata.get("decision_maker_intelligence")
        if isinstance(metadata.get("decision_maker_intelligence"), dict)
        else {},
        company_intelligence=metadata.get("company_intelligence")
        if isinstance(metadata.get("company_intelligence"), dict)
        else {},
    )
...
def _company_action_guidance(company: CrmCompanyOut) -> dict[str, Any]:
    ...
    if needs("website_analysis"):
        ...
        actions.append(
            "Run AI website analysis to fill summary, pain points, offer and outreach angle."
        )
    if needs("decision_maker"):
        missing.append("Decision maker")
        actions.append(
            "Find a decision maker automatically or add the right contact manually."
        )
    if needs("verified_email"):
        missing.append("Verified email")
        actions.append(
            "Search for a verified business email or add a known recipient manually."
        )
    if needs("ai_email"):
    ...
    if not missing:
        return {
            "missing_fields": [],
            "recommended_actions": [
                "Review the prepared opportunity and send only after confirmation."
            ],
...
def _ensure_minimal_company(
    db: Session, user_id: str, workspace, lead: Lead, metadata: dict[str, Any]
) -> Company:
    ...
    company.suggested_offer = str(
        metadata.get("suggested_offer") or company.suggested_offer or ""
    )
    company.outreach_strategy = str(
        metadata.get("outreach_strategy") or company.outreach_strategy or ""
    )
    company.sales_angle = str(metadata.get("sales_angle") or company.sales_angle or "")
    company.expected_reply_rate = str(
        metadata.get("expected_reply_rate") or company.expected_reply_rate or ""
    )
    if metadata.get("email_status"):
        ...
        company.email_status = (
            "Found" if lead.email else (company.email_status or "Not prepared")
        )
        company.crm_stage = (
            "Contact Found" if lead.email else (company.crm_stage or "New Lead")
        )
...
def _safe_company_out(
    db: Session, workspace, user_id: str, company: Company
) -> CrmCompanyOut:
    try:
        return _crm_company_out(db, workspace, user_id, company)
    except Exception as exc:
        capture_provider_exception(
            exc, provider="postgresql", endpoint="workspace_app.company_output"
        )
...
def _workspace_counts(db: Session, workspace_id: UUID) -> UsageCounts:
    ...
    companies = [
        ...
        if not _is_production_smoke_test_company(company)
    ]
    emails = [
        email
        ...
        if not _is_production_smoke_test_email(email)
    ]
    return UsageCounts(
        leads=len(leads),
...
def _activity_out(item: AuditLog) -> UsageActivityOut:
    ...
    return UsageActivityOut(
        action=item.action, created_at=item.created_at, company=company, message=message
    )
...
def _safe_provider_warning(exc: Exception) -> str:
    ...
    if isinstance(exc, ProviderConfigurationError):
        return (
            "AI is not configured yet. The company was saved and can be analyzed later."
        )
...
def _set_workflow_stage(
    ...
    stages = (
        metadata.get("workflow_stages")
        if isinstance(metadata.get("workflow_stages"), dict)
        else {}
    )
    messages = (
        metadata.get("workflow_stage_messages")
        if isinstance(metadata.get("workflow_stage_messages"), dict)
        else {}
    )
...
def _set_workflow_stages(
    lead: Lead,
    updates: dict[str, Literal["waiting", "running", "completed", "error"]],
...
def _finalize_enrichment_workflow(db: Session, workspace, lead: Lead) -> None:
    ...
    has_decision_maker = bool(
        lead.contact
        or (isinstance(selected_decision_maker, dict) and selected_decision_maker)
    )
    has_verified_email = bool(
        lead.email
        or metadata.get("hunter_verified")
        or metadata.get("email_status") == "Verified"
    )
    ...
    approval_completed = bool(
        existing_draft and existing_draft.delivery_status in {"approved", "sent"}
    )
    ...
    _set_workflow_stages(
        ...
        },
        {
            ...
            "website_analysis": _workflow_message(language, "workflow_website_done")
            if has_research
            else _workflow_message(language, "workflow_website_missing"),
            "decision_maker": _workflow_message(language, "workflow_decision_done")
            if has_decision_maker
            else _workflow_message(language, "workflow_decision_missing"),
            "verified_email": _workflow_message(language, "workflow_email_verified")
            if has_verified_email
            else _workflow_message(language, "workflow_email_missing"),
            "ai_email": _workflow_message(language, "workflow_ai_email_done")
            if has_draft
            else _workflow_message(language, "workflow_ai_email_missing"),
            "approval": _workflow_message(language, "workflow_approval_done")
            if approval_completed
            else _workflow_message(language, "workflow_approval_waiting"),
        },
    )
...
def _require_workspace_owner(
    db: Session, *, workspace: Workspace, user_id: str
) -> None:
    if not _is_workspace_owner(db, workspace=workspace, user_id=user_id):
        raise HTTPException(
            status_code=403, detail="Only the workspace owner can run this action."
        )
...
def _require_production_smoke_owner(db: Session, *, user: Any) -> None:
    try:
        ...
        raise HTTPException(
            status_code=403,
            detail="Only the system owner can run production email smoke tests.",
        ) from None


def _production_smoke_metadata(
    smoke_test_id: UUID, recipient_email: str
) -> dict[str, Any]:
    return {
...
def _internal_email_smoke_metadata(
    smoke_test_id: UUID, recipient_email: str, workspace_id: UUID
) -> dict[str, Any]:
    return {
        "source": INTERNAL_EMAIL_SMOKE_DRAFT_SOURCE,
        "is_test": True,
        "automation_disabled": True,
        "smoke_test_id": str(smoke_test_id),
        "recipient_email": recipient_email,
...
def _is_production_smoke_test_metadata(metadata: Any) -> bool:
    return (
        ...
        and metadata.get("source") == PRODUCTION_SMOKE_TEST_SOURCE
        and metadata.get("is_test") is True
    )


def _is_internal_email_smoke_draft_metadata(metadata: Any) -> bool:
    return (
        isinstance(metadata, dict)
        and metadata.get("source") == INTERNAL_EMAIL_SMOKE_DRAFT_SOURCE
        and metadata.get("is_test") is True
    )
...
def _is_production_smoke_test_company(company: Company | None) -> bool:
    return bool(
        company
        and _is_production_smoke_test_metadata(
...
def _is_internal_email_smoke_draft_lead(lead: Lead | None) -> bool:
    if not lead:
        return False
    return _is_internal_email_smoke_draft_metadata(_lead_metadata(lead))


def _is_internal_email_smoke_draft_company(company: Company | None) -> bool:
    return bool(
        company
        and _is_internal_email_smoke_draft_metadata(
            company.metadata_json if isinstance(company.metadata_json, dict) else {}
        )
    )


def _is_internal_email_smoke_draft_email(email: EmailMessage | None) -> bool:
    return bool(
        email
        and _is_internal_email_smoke_draft_metadata(
            email.tags if isinstance(email.tags, dict) else {}
...
def _internal_email_smoke_allowed_recipients() -> list[str]:
    ...
    return [
        item.strip()
        for item in settings.internal_email_smoke_draft_recipients.split(",")
        if item.strip()
    ]
...
def _validate_internal_email_smoke_recipient(recipient: str) -> None:
    if recipient not in _internal_email_smoke_allowed_recipients():
        raise HTTPException(
            status_code=400,
            detail="Use the approved controlled internal smoke alias for this smoke test.",
        )


async def _internal_email_smoke_draft_payload(
    request: Request,
) -> InternalEmailSmokeDraftCreateIn:
    ...
    try:
        parsed = json.loads(
            raw_body.decode("utf-8"), object_pairs_hook=reject_duplicate_keys
        )
    ...
    if duplicate_keys:
        raise HTTPException(
            status_code=400, detail="Duplicate JSON fields are not allowed."
        )
    ...
    if unknown_fields:
        raise HTTPException(
            status_code=422,
            detail="Unknown fields are not allowed for internal smoke draft creation.",
        )
    missing_fields = INTERNAL_EMAIL_SMOKE_DRAFT_ALLOWED_FIELDS - set(parsed)
    if missing_fields:
        raise HTTPException(
            status_code=422,
            detail="recipient_email and confirmed_recipient_control are required.",
        )
    ...
    if type(confirmed) is not bool:
        raise HTTPException(
            status_code=422, detail="confirmed_recipient_control must be a boolean."
        )
    return InternalEmailSmokeDraftCreateIn(
        recipient_email=recipient, confirmed_recipient_control=confirmed
    )
...
def _production_smoke_context(
    ...
    *,
    ...
    company_metadata = (
        ...
        if company and isinstance(company.metadata_json, dict)
        else {}
    )
    lead_metadata = _lead_metadata(lead) if lead else {}
    sender_status, _smtp_config = _outreach_sender_runtime_config(
        db, str(lead.user_id if lead else workspace.owner_user_id), workspace
    )
    recipient = (
        str(
    ...
    return ProductionEmailSmokeTestContextOut(
        ...
        sender_provider=str(
            tags.get("sender_provider") or sender_status.provider or ""
        ),
...
def _internal_email_smoke_draft_response(
    ...
    *,
    ...
    email = db.scalar(
        select(EmailMessage)
        .where(
            EmailMessage.workspace_id == workspace.id, EmailMessage.lead_id == lead.id
        )
        .order_by(EmailMessage.created_at.desc())
    )
    if (
        not company
        or not _is_internal_email_smoke_draft_company(company)
        or not email
        or not _is_internal_email_smoke_draft_email(email)
    ):
        raise HTTPException(
            status_code=409,
            detail="Internal smoke-test records are incomplete. Review existing workspace data before retrying.",
        )
    tags = email.tags if isinstance(email.tags, dict) else {}
    smoke_test_id = _valid_smoke_test_id(
        tags.get("smoke_test_id") or _lead_metadata(lead).get("smoke_test_id")
    )
    if not smoke_test_id:
        raise HTTPException(
            status_code=409,
            detail="Internal smoke-test records are missing a smoke-test ID.",
        )
    recipient = str(
        tags.get("recipient_email") or email.recipient_email or lead.email or ""
    )
    _validate_internal_email_smoke_recipient(recipient)
...
def _active_internal_email_smoke_draft_lead(
...
def _internal_email_smoke_sender_status(
    db: Session, *, user_id: str, workspace: Workspace
) -> Any:
    for _attempt in range(2):
        try:
            sender_status, _smtp_config = _outreach_sender_runtime_config(
                db, user_id, workspace
            )
...
def _active_production_smoke_context(
    db: Session, *, workspace: Workspace
) -> ProductionEmailSmokeTestContextOut | None:
    leads = [
        lead
        for lead in db.scalars(
            select(Lead)
        ...
        if _is_production_smoke_test_lead(lead)
        and _valid_smoke_test_id(_lead_metadata(lead).get("smoke_test_id"))
    ]
    for lead in leads:
        ...
        return _production_smoke_context(
            db,
            workspace=workspace,
            smoke_test_id=smoke_test_id,
            lead=lead,
    ...
    emails = [
        ...
        if _is_production_smoke_test_email(email)
        and _valid_smoke_test_id(_smoke_test_id_from_email(email))
    ]
    for email in emails:
        ...
        lead = (
            db.scalar(
                select(Lead).where(
                    Lead.id == email.lead_id, Lead.workspace_id == workspace.id
                )
            )
            if email.lead_id
            else None
...
def _production_smoke_exists_outside_workspace(
    db: Session, *, workspace_id: UUID, smoke_test_id: UUID
) -> bool:
    smoke_id = str(smoke_test_id)
    for lead in db.scalars(select(Lead).where(Lead.workspace_id != workspace_id)).all():
        if (
            _is_production_smoke_test_lead(lead)
            and str(_lead_metadata(lead).get("smoke_test_id") or "") == smoke_id
        ):
            return True
    for company in db.scalars(
        select(Company).where(Company.workspace_id != workspace_id)
        ...
        if (
            _is_production_smoke_test_company(company)
            and str((company.metadata_json or {}).get("smoke_test_id") or "")
            == smoke_id
        ):
            return True
    for email in db.scalars(
        ...
        if (
            _is_production_smoke_test_email(email)
            and _smoke_test_id_from_email(email) == smoke_id
        ):
            return True
    return False


def _smoke_test_cleanup_scope(
    db: Session, *, workspace_id: UUID, smoke_test_id: UUID
) -> tuple[list[Lead], list[Company], list[EmailMessage], list[AuditLog]]:
    smoke_id = str(smoke_test_id)
    leads = [
        lead
        for lead in db.scalars(
    ...
    companies = [
        ...
        if _is_production_smoke_test_company(company)
        and str((company.metadata_json or {}).get("smoke_test_id") or "") == smoke_id
    ]
    emails = [
        email
        for email in db.scalars(
    ...
    activities = [
        ...
        for activity in db.scalars(
            select(AuditLog).where(
                AuditLog.workspace_id == workspace_id, AuditLog.action.like("lead.%")
            )
        ).all()
...
def _smoke_test_memory_cleanup_scope(
    db: Session,
    *,
    workspace_id: UUID,
    smoke_test_id: UUID,
    draft_emails: list[EmailMessage],
) -> list[AIMemoryEntry]:
    ...
    for entry in entries:
        ...
        if draft_email_ids and (
            email_id in draft_email_ids or source_id in draft_email_ids
        ):
            scoped_entries.append(entry)
            continue
        if smoke_marker in (entry.content or "") or smoke_marker in (
            entry.summary or ""
        ):
            scoped_entries.append(entry)
...
def _company_intelligence_quality(
    lead: Lead, metadata: dict, workspace, source: str, language: str
) -> dict[str, Any]:
    has_website = bool(lead.website or metadata.get("domain"))
    has_email = bool(lead.email)
    has_contact = bool(
        lead.contact or metadata.get("contact_search_status") == "verified_email_found"
    )
    has_analysis = bool(
        metadata.get("ai_summary")
        or metadata.get("opportunity_analysis")
    ...
    technologies = (
        metadata.get("technologies")
        if isinstance(metadata.get("technologies"), list)
        else []
    )
    deep_contact = (
        metadata.get("deep_contact_search")
        if isinstance(metadata.get("deep_contact_search"), dict)
        else {}
    )
    ...
    used_sources = [
        ...
        _localized_fallback_text(language, "source_ai_research")
        if has_analysis
        else "",
        _localized_fallback_text(language, "source_verified_contact")
        if has_email
        else "",
        _localized_fallback_text(language, "source_technology")
        if technologies or deep_contact.get("technologies")
        else "",
    ]
    ...
    gaps = [
        ...
        _localized_fallback_text(language, "gap_technology")
        if not technologies and not deep_contact.get("technologies")
        else "",
    ]
    ...
    score = int(
        ...
        or (
            82
            if has_website and has_email and has_analysis
            else 68
            if has_website and has_analysis
            else 52
            if has_website
            else 38
        )
...
def _intent_evidence(
    signal: str, source_field: str, value: str, confidence: int
) -> dict[str, Any]:
    return {
        "signal": signal,
...
def _decision_evidence(
    source_field: str, value: str, confidence: int
) -> dict[str, Any]:
    return {
        "source_field": source_field,
        "value": value,
...
def _contact_decision_maker_intelligence(
    ...
    *,
    ...
    contact_metadata = (
        contact.metadata_json if isinstance(contact.metadata_json, dict) else {}
    )
    ...
    if contact.email_status == "Verified" and contact.email:
        evidence.append(
            _decision_evidence("contact.email_status", "Verified business email", 92)
        )
    if contact.linkedin:
    ...
    if reason:
        evidence.append(
            _decision_evidence(
                "deep_contact_search.selected_decision_maker.reason", reason, 82
            )
        )
    profile = (
        deep_contact.get("company_profile")
        if isinstance(deep_contact.get("company_profile"), dict)
        else {}
    )
    if profile.get("industry"):
        evidence.append(
            _decision_evidence(
                "deep_contact_search.company_profile.industry",
    ...
    if _title_contains(title, ["ceo", "founder", "owner", "president"]):
        responsibilities.extend(
            ["Company strategy", "Budget allocation", "Revenue growth decisions"]
        )
        goals.extend(
            ["Sustainable revenue growth", "Market expansion", "Operational efficiency"]
        )
        kpis.extend(["Revenue growth rate", "Gross margin", "Pipeline velocity"])
        pain_points.extend(
            ["Limited high-quality pipeline", "Unpredictable outbound performance"]
        )
        communication_style = "Executive-level, outcome-first"
        outreach_angle = "Show revenue impact and execution speed"
    elif _title_contains(
        title, ["sales", "vp sales", "cro", "head of sales", "sales director"]
    ):
        responsibilities.extend(
            ["Pipeline generation", "Quota attainment", "Team performance"]
        )
        goals.extend(
            ["Increase qualified pipeline", "Improve win rate", "Reduce sales cycle"]
        )
        kpis.extend(
            ["SQL volume", "Win rate", "Sales cycle length", "Quota attainment"]
        )
        ...
        title, ["marketing", "cmo", "head of marketing", "marketing director"]
    ):
        responsibilities.extend(
            ["Demand generation", "Campaign performance", "Funnel efficiency"]
        )
        goals.extend(
            [
    ...
    company_name = (
        company.name or (lead.company if lead else "this company") or "this company"
    )
    first_sentence = f"Noticed your role as {title or 'a key decision maker'} at {company_name} and wanted to share a quick idea to improve qualified pipeline outcomes."
    ...
    return {
        ...
        "is_verified_contact": bool(
            contact.email and contact.email_status == "Verified"
        ),
    ...
    *,
    ...
    profiles = [
        _contact_decision_maker_intelligence(
            ...
            else Company(name=lead.company or ""),
            lead=lead,
...
def _opportunity_ranking(
    *,
    ...
    buying_intent = (
        company_intelligence.get("buying_intent")
        if isinstance(company_intelligence.get("buying_intent"), dict)
        else {}
    )
    ...
    ci_fields = (
        company_intelligence.get("fields")
        if isinstance(company_intelligence.get("fields"), dict)
        else {}
    )
    ci_present = sum(
        1
        for value in ci_fields.values()
        if isinstance(value, dict) and _metadata_value_present(value.get("value"))
    )
    ...
    factor_reasons["Company Intelligence"] = [
        f"{ci_present}/{ci_total} intelligence fields populated from enrichment."
    ]

    profiles = (
        decision_maker_intelligence.get("profiles")
        if isinstance(decision_maker_intelligence.get("profiles"), list)
        else []
    )
    ...
    if technologies:
        ...
        if workspace_context and any(
            term.lower() in workspace_context for term in technologies
        ):
            tech_score = _safe_score(85)
    factor_scores["Technology Match"] = tech_score
    factor_reasons["Technology Match"] = (
        technologies[:4]
        if technologies
        else ["No technology enrichment data captured."]
    )
    ...
    factor_reasons["Company Size"] = (
        [str(size_value)]
        if _metadata_value_present(size_value)
        else ["Company size not enriched yet."]
    )
    ...
    factor_reasons["Hiring Signals"] = (
        hiring_values[:3] if hiring_values else ["No hiring evidence captured."]
    )
    ...
    factor_reasons["Growth Signals"] = (
        growth_values[:3] if growth_values else ["No growth evidence captured."]
    )

    company_country = (
        str((company.country if company else "") or lead.country or "").strip().lower()
    )
    ...
    factor_reasons["Geography"] = [
        str((company.country if company else "") or lead.country or "Unknown geography")
    ]
    ...
    factor_reasons["Website Quality"] = _dedupe_text_values(
        [
            ...
            f"Google rating: {metadata.get('google_rating')}"
            if isinstance(metadata.get("google_rating"), (int, float))
            else "",
        ]
    ...
    verified_score = (
        ...
        else _safe_score(min(100, 60 + len(verified_contacts) * 15))
    )
    ...
    raw_weights = (
        opportunity_weights
        if isinstance(opportunity_weights, dict)
        else DEFAULT_OPPORTUNITY_WEIGHTS
    )
    total_weight = sum(
        max(0.0, float(raw_weights.get(name) or 0.0))
    ...
    weighted_score = sum(
        factor_scores[name] * weight for name, weight in weights.items()
    )
    ...
    positives = [
        f"{name}: {score}" for name, score in sorted_factors[:4] if score >= 60
    ]
    negatives = [
        f"{name}: {score}"
        for name, score in sorted(factor_scores.items(), key=lambda item: item[1])[:4]
        if score <= 50
    ]
    ...
    if overall_score >= 80:
        ...
        next_action = (
            "Run additional enrichment to improve intent coverage before sending."
        )
    else:
...
def _estimated_reply_probability(
    *,
    metadata: dict[str, Any],
...
def _build_ai_outreach_strategy(
    *,
    ...
    base_company_evidence = _dedupe_evidence_items(
        [
            _decision_evidence("lead.website", str(lead.website), 85)
            if lead.website
            else {},
            _decision_evidence(
                "metadata.ai_summary", str(metadata.get("ai_summary")), 78
            )
            if metadata.get("ai_summary")
            else {},
            _decision_evidence(
                "metadata.expected_reply_rate",
                str(metadata.get("expected_reply_rate")),
                70,
            )
            if metadata.get("expected_reply_rate")
            else {},
            _decision_evidence(
                "metadata.suggested_offer", str(metadata.get("suggested_offer")), 78
            )
            if metadata.get("suggested_offer")
            else {},
            _decision_evidence(
                "metadata.recommended_cta", str(metadata.get("recommended_cta")), 80
            )
            if metadata.get("recommended_cta")
            else {},
        ]
        + buying_evidence
        + profile_evidence
    ...
    why_contact_now_signals = _dedupe_text_values(
        [
            ...
            *(
                str(item)
                for item in opportunity_ranking.get("top_positive_signals", [])
                if item
            ),
        ]
    )
    why_contact_now = (
        why_contact_now_signals[0]
        if why_contact_now_signals
        else "Insufficient intent evidence to justify immediate outreach."
    )
    ...
    best_timing_evidence = _dedupe_evidence_items(
        [
            ...
            _decision_evidence(
                "buying_intent.recommended_outreach_timing", best_timing, 80
            ),
            *buying_evidence,
        ]
    )[:4]
    ...
    has_linkedin = bool(top_profile.get("evidence_used")) or any(
        bool(contact.linkedin) for contact in contacts
    )
        ...
        [
            *[
                _decision_evidence(
                    ...
                    f"{contact.name}: {contact.email_status}",
                    90,
                )
    ...
    if not strongest_value_proposition:
        strongest_value_proposition = (
            "Insufficient offer evidence in current enrichment data."
        )
    strongest_value_proposition_evidence = _dedupe_evidence_items(
        [
            _decision_evidence(
                "metadata.suggested_offer", str(metadata.get("suggested_offer")), 82
            )
            if metadata.get("suggested_offer")
            else {},
            _decision_evidence(
                "metadata.best_outreach_angle",
                str(metadata.get("best_outreach_angle")),
                78,
            )
            if metadata.get("best_outreach_angle")
            else {},
            _decision_evidence(
                "metadata.value_proposition", str(metadata.get("value_proposition")), 76
            )
            if metadata.get("value_proposition")
            else {},
            _decision_evidence(
                "metadata.outreach_strategy", str(metadata.get("outreach_strategy")), 74
            )
            if metadata.get("outreach_strategy")
            else {},
        ]
    )[:4]
    ...
    pain_points = (
        [str(item) for item in metadata.get("pain_points", []) if str(item).strip()]
        if isinstance(metadata.get("pain_points"), list)
        else []
    )
    risks = (
        [str(item) for item in metadata.get("risks", []) if str(item).strip()]
        if isinstance(metadata.get("risks"), list)
        else []
    )
    if pain_points:
    ...
    if not strongest_pain_point:
        strongest_pain_point = (
            "Insufficient pain-point evidence in current enrichment data."
        )
    strongest_pain_point_evidence = _dedupe_evidence_items(
        [
            _decision_evidence("metadata.pain_points", pain_points[0], 78)
            if pain_points
            else {},
    ...
    first_sentence_evidence = _dedupe_evidence_items(
        [
            *profile_evidence,
            _decision_evidence(
                "decision_maker_intelligence.recommended_first_sentence",
                str(top_profile.get("recommended_first_sentence")),
                82,
            )
    ...
    weakness_items = (
        ...
        if isinstance(metadata.get("weaknesses"), list)
        else []
    )
    ...
    cta = (
        str(metadata.get("recommended_cta") or "").strip()
        or "Open to a 15-minute fit check next week?"
    )
    cta_evidence = _dedupe_evidence_items(
        [
            _decision_evidence(
                "metadata.recommended_cta", str(metadata.get("recommended_cta")), 82
            )
            if metadata.get("recommended_cta")
            else {},
            _decision_evidence(
                "metadata.suggested_offer", str(metadata.get("suggested_offer")), 76
            )
            if metadata.get("suggested_offer")
            else {},
            *base_company_evidence,
    ...
    estimated_reply_probability_evidence = _dedupe_evidence_items(
        [
            ...
            _decision_evidence(
                "opportunity_ranking.overall_score",
                str(opportunity_ranking.get("overall_score")),
                72,
            )
            if opportunity_ranking.get("overall_score") is not None
            else {},
            _decision_evidence(
                "buying_intent.buying_signal_score",
                str(buying_intent.get("buying_signal_score")),
                78,
            )
            if buying_intent.get("buying_signal_score") is not None
            else {},
            *base_company_evidence,
        ]
    )[:4]
    ...
    for profile in profiles:
        ...
        profile_first_sentence = (
            str(profile.get("recommended_first_sentence") or "").strip()
            or first_sentence
        )
...
def _build_ai_competitor_intelligence(
    *,
    ...
    report = (
        company_intelligence.get("report")
        if isinstance(company_intelligence.get("report"), dict)
        else {}
    )
    competitors = _dedupe_text_values(
        [
            *(
                ...
                if isinstance(metadata.get("competitors"), list)
                else []
            ),
            *(
                report.get("competitors", {}).get("value", [])
                if isinstance(report.get("competitors"), dict)
                and isinstance(report.get("competitors", {}).get("value"), list)
                else []
            ),
        ]
    )
    technologies = _dedupe_text_values(
        [
            ...
            *(
                report.get("technology_stack", {}).get("value", [])
                if isinstance(report.get("technology_stack"), dict)
                and isinstance(report.get("technology_stack", {}).get("value"), list)
                else []
            ),
        ]
    )
    ...
    positioning = str(
        metadata.get("value_proposition")
        or metadata.get("sales_angle")
    ...
    strengths = _dedupe_text_values(
        [
            *(
                ...
                if isinstance(metadata.get("services"), list)
                else []
            ),
            *(
                metadata.get("buying_signals")
                if isinstance(metadata.get("buying_signals"), list)
                else []
            ),
            *(
                item
    ...
    weaknesses = _dedupe_text_values(
        [
            ...
            *(
                item
                for item in opportunity_ranking.get("top_negative_signals", [])
                if isinstance(item, str)
            ),
        ]
    )[:5]
    if not weaknesses:
        weaknesses = [
            "Limited verified contact coverage",
            "Potential execution gaps in outbound consistency",
        ]
    ...
    market_gaps = _dedupe_text_values(
        [
            ...
            *(
                metadata.get("market_gaps")
                if isinstance(metadata.get("market_gaps"), list)
                else []
            ),
    ...
    if not opportunity_to_sell:
        opportunity_to_sell = (
            "Lead with a measurable pipeline-improvement offer tied to one clear KPI."
        )
...
def _build_ai_sales_timeline(
    *,
    ...
    decision_maker_strategies = (
        ai_outreach_strategy.get("decision_maker_strategies")
        if isinstance(ai_outreach_strategy.get("decision_maker_strategies"), list)
        else []
    )
    top_dm_strategy = (
        decision_maker_strategies[0]
        if decision_maker_strategies and isinstance(decision_maker_strategies[0], dict)
        else {}
    )
    profiles = (
        decision_maker_intelligence.get("profiles")
    ...
    pain_point = str(
        ...
        or (
            metadata.get("pain_points", [""])[0]
            if isinstance(metadata.get("pain_points"), list)
            and metadata.get("pain_points")
            else ""
        )
        or "Potential outbound efficiency gaps"
    ).strip()
    cta = str(
        top_dm_strategy.get("cta")
        or ai_outreach_strategy.get("cta")
        or metadata.get("recommended_cta")
        or "Open to a 15-minute fit check next week?"
    ).strip()
    best_channel = str(
        ai_outreach_strategy.get("best_communication_channel")
        or ai_outreach_strategy.get("best_channel")
        or "Email"
    ).strip()
    base_probability = _safe_score(
        ai_outreach_strategy.get("estimated_reply_probability")
        or ai_outreach_strategy.get("probability_of_reply"),
        30,
    )
    ...
    evidence = _dedupe_evidence_items(
        [
            *(
                ...
                if isinstance(
                    ai_outreach_strategy.get("why_contact_now_evidence"), list
                )
                else []
            ),
            *(
                ...
                if isinstance(
                    ai_outreach_strategy.get("best_subject_line_evidence"), list
                )
                else []
            ),
            *(
            ...
            *(
            ...
            _decision_evidence("lead.company", lead.company or "", 90),
            _decision_evidence(
                "decision_maker.title", str(top_profile.get("title") or ""), 78
            )
            if top_profile
            else {},
        ]
...
def _build_ai_risk_analyzer(
    *,
    ...
    now = datetime.utcnow()
    missing_fields = (
        company_intelligence.get("missing_fields")
        if isinstance(company_intelligence.get("missing_fields"), list)
        else []
    )
    profiles = (
        decision_maker_intelligence.get("profiles")
    ...
    personalization_bullets = (
        ...
        if isinstance(metadata.get("personalization_bullets"), list)
        else []
    )
    ...
    if str(ai_outreach_strategy.get("strongest_value_proposition") or "").startswith(
        "Insufficient"
    ):
        weak_personalization_score += 20
    weak_personalization_score = _safe_score(weak_personalization_score, 20)

    missing_data_score = _safe_score(
        15 + len(missing_fields) * 12 + (10 if not ai_summary else 0), 20
    )
    missing_decision_maker_score = _safe_score(
        88 if not profiles else (28 if profiles else 88), 35
    )
    ...
    raw_last_enriched = str(
        metadata.get("last_enriched_at")
        or company_intelligence.get("generated_at")
        or ""
    ).strip()
    enrichment_age_days = 999
    if raw_last_enriched:
        try:
            last_enriched_at = datetime.fromisoformat(
                raw_last_enriched.replace("Z", "+00:00")
            ).replace(tzinfo=None)
            enrichment_age_days = max(
                0, int((now - last_enriched_at).total_seconds() // 86400)
            )
        except ValueError:
            enrichment_age_days = 999
    stale_enrichment_score = _safe_score(
        18
        if enrichment_age_days <= 3
        else 42
        if enrichment_age_days <= 10
        else 70
        if enrichment_age_days <= 20
        else 90,
        70,
    )
    ...
    evidence_pool = _dedupe_evidence_items(
        [
            _decision_evidence(
                "company_intelligence.missing_fields",
            ...
            _decision_evidence(
                "metadata.personalization_bullets",
                ", ".join(str(item) for item in personalization_bullets[:3]),
                74,
            )
            if personalization_bullets
            else {},
            _decision_evidence("metadata.ai_summary", ai_summary, 76)
            if ai_summary
            else {},
            _decision_evidence(
                "decision_maker_intelligence.profiles", str(len(profiles)), 86
            ),
            _decision_evidence(
                "metadata.confidence_score", str(metadata.get("confidence_score")), 78
            )
            if metadata.get("confidence_score") is not None
            else {},
            _decision_evidence(
                "opportunity_ranking.confidence",
                str(opportunity_ranking.get("confidence")),
                80,
            )
            if opportunity_ranking.get("confidence") is not None
            else {},
            _decision_evidence("metadata.last_enriched_at", raw_last_enriched, 84)
            if raw_last_enriched
            else {},
            _decision_evidence(
                "metadata.risks", ", ".join(str(item) for item in risk_items[:3]), 70
            )
            if risk_items
            else {},
        ]
    ...
    if missing_data_score >= 55:
        reasons.append(
            "Missing enrichment fields reduce outreach relevance and deliverability confidence."
        )
    if weak_personalization_score >= 55:
        reasons.append(
            "Personalization depth is weak for a high-probability first response."
        )
    if missing_decision_maker_score >= 55:
        reasons.append(
            "No strong decision-maker profile is available for targeted outreach."
        )
    if low_confidence_score >= 55:
        reasons.append(
            "Overall enrichment confidence is too low for reliable messaging."
        )
    if stale_enrichment_score >= 55:
        reasons.append(
            "Enrichment may be stale and should be refreshed before outreach."
        )
    if not reasons:
    ...
    if missing_data_score >= 55:
        recommended_improvements.append(
            "Run company enrichment again to fill missing firmographic and buying-signal fields."
        )
    if weak_personalization_score >= 55:
        recommended_improvements.append(
            "Add 2-3 evidence-backed personalization bullets before sending."
        )
    if missing_decision_maker_score >= 55:
        recommended_improvements.append(
            "Find and verify at least one decision-maker contact."
        )
    if low_confidence_score >= 55:
        recommended_improvements.append(
            "Improve confidence by validating company summary, pain points, and offer fit from new sources."
        )
    if stale_enrichment_score >= 55:
        recommended_improvements.append(
            "Refresh enrichment data and regenerate outreach strategy."
        )
    if not recommended_improvements:
        recommended_improvements.append(
            "Proceed with outreach and monitor response data for optimization."
        )
    ...
    *,
    ...
    what_could_fail = _dedupe_text_values(
        [
            *(
                str(item)
                for item in ai_risk_analyzer.get("reasons", [])
                if str(item).strip()
            ),
            *(
                str(item)
                for item in metadata.get("risks", [])
                if isinstance(metadata.get("risks"), list) and str(item).strip()
            ),
        ]
    )[:5]
    if not what_could_fail:
        what_could_fail = [
            "Insufficient failure-risk evidence in current enrichment data."
        ]
    ...
    how_to_increase_reply_rate = _dedupe_text_values(
        [
            *(
                str(item)
                for item in ai_risk_analyzer.get("recommended_improvements", [])
                if str(item).strip()
            ),
    ...
    evidence = _dedupe_evidence_items(
        [
            _decision_evidence(
                "metadata.ai_summary", str(metadata.get("ai_summary")), 80
            )
            if metadata.get("ai_summary")
            else {},
            _decision_evidence(
                "metadata.opportunity_analysis",
                str(metadata.get("opportunity_analysis")),
                76,
            )
            if metadata.get("opportunity_analysis")
            else {},
            _decision_evidence(
                "ai_outreach_strategy.why_contact_now",
                str(ai_outreach_strategy.get("why_contact_now")),
                82,
            )
            if ai_outreach_strategy.get("why_contact_now")
            else {},
            _decision_evidence(
                "decision_maker_intelligence.top_profile",
                f"{decision_maker_name} {decision_maker_title}".strip(),
                84,
            )
            if decision_maker_name or decision_maker_title
            else {},
            _decision_evidence(
                "ai_risk_analyzer.risk_score",
                str(ai_risk_analyzer.get("risk_score")),
                78,
            )
            if ai_risk_analyzer.get("risk_score") is not None
            else {},
            _decision_evidence(
                "opportunity_ranking.overall_score",
                str(opportunity_ranking.get("overall_score")),
...
def _build_ai_company_predictions(
    *,
    ...
    top_profile = profiles[0] if profiles and isinstance(profiles[0], dict) else {}
    timeline_events = (
        ai_company_timeline.get("events")
    ...
    size_value = (
        metadata.get("estimated_company_size")
        or (
            (report.get("estimated_company_size") or {}).get("value")
            if isinstance(report.get("estimated_company_size"), dict)
            else None
        )
        or metadata.get("employee_count")
        or metadata.get("employees")
    ...
    funding_signals = _event_texts(metadata.get("funding_signal")) + _event_texts(
        metadata.get("funding_signals")
    )
    hiring_signals = _event_texts(metadata.get("hiring_signals")) + _event_texts(
        metadata.get("jobs_signal")
    )
    product_signals = _event_texts(metadata.get("product_launches"))
    partnership_signals = _event_texts(
        metadata.get("partnership_signals")
    ) + _event_texts(metadata.get("partnership_updates"))
    ...
    estimated_arr_score += (
        min(10, len(pricing) * 4)
        if (pricing := _event_texts(metadata.get("pricing_signals")))
        else 0
    )
    ...
    arr_evidence = _dedupe_evidence_items(
        [
            _decision_evidence("metadata.estimated_company_size", str(size_value), 78)
            if size_value
            else {},
            *[
                _decision_evidence("metadata.funding_signal", item, 82)
                for item in funding_signals[:3]
            ],
            *[
                _decision_evidence("metadata.pricing_signals", item, 72)
                for item in pricing[:3]
            ],
            *[
                _decision_evidence("metadata.hiring_signals", item, 72)
                for item in hiring_signals[:3]
            ],
        ]
    )
    arr_reasoning = (
    ...
    arr_confidence = _safe_score(
        40 + len(arr_evidence) * 12 + (8 if size_numeric is not None else 0), 30
    )
    ...
    maturity_score += (
        8 if metadata.get("value_proposition") or metadata.get("suggested_offer") else 0
    )
    ...
    maturity_evidence = _dedupe_evidence_items(
        [
            ...
            _decision_evidence(
                "metadata.ai_summary", str(metadata.get("ai_summary")), 76
            )
            if metadata.get("ai_summary")
            else {},
            _decision_evidence("metadata.icp", str(metadata.get("icp")), 72)
            if metadata.get("icp")
            else {},
            *[
                _decision_evidence("metadata.technologies", item, 74)
                for item in _event_texts(metadata.get("technologies"))[:4]
            ],
        ]
    )
    maturity_reasoning = (
    ...
    growth_evidence = _dedupe_evidence_items(
        [
            _decision_evidence(
                "company_intelligence.buying_intent.buying_signal_score",
                str(buying_signal_score),
                80,
            ),
            *[
                _decision_evidence("metadata.funding_signal", item, 82)
            ...
            *[
                _decision_evidence("metadata.hiring_signals", item, 78)
                for item in hiring_signals[:3]
            ],
            *[
                _decision_evidence("metadata.product_launches", item, 76)
                for item in product_signals[:3]
            ],
            *[
                _decision_evidence("metadata.partnership_signals", item, 74)
                for item in partnership_signals[:3]
            ],
        ]
    )
    growth_reasoning = (
    ...
    growth_confidence = _safe_score(
        40
        + len(growth_evidence) * 10
        + _safe_score(buying_intent.get("confidence"), 0) // 5,
        32,
    )
    ...
    readiness_score += (
        ...
        and ai_outreach_strategy.get("cta")
        else 0
    )
    ...
    readiness_evidence = _dedupe_evidence_items(
        [
            _decision_evidence(
                "opportunity_ranking.overall_score", str(ranking_score), 82
            ),
            _decision_evidence("ai_risk_analyzer.risk_score", str(risk_score), 80),
            _decision_evidence(
                "decision_maker_intelligence.top_profile.is_verified_contact",
                str(has_verified_decision_maker),
                88,
            ),
            _decision_evidence(
                "decision_maker_intelligence.top_profile.confidence_score",
                str(top_contact_confidence),
                80,
            ),
            _decision_evidence(
                "ai_outreach_strategy.best_subject_line",
                str(ai_outreach_strategy.get("best_subject_line") or ""),
                72,
            )
            if ai_outreach_strategy.get("best_subject_line")
            else {},
            _decision_evidence(
                "ai_outreach_strategy.cta",
                str(ai_outreach_strategy.get("cta") or ""),
                72,
            )
...
def _build_ai_specialized_agents(
    *,
    ...
    company_output = {
        "company_summary": (report.get("company_summary") or {}).get("value")
        if isinstance(report.get("company_summary"), dict)
        else "",
        "icp": (report.get("icp") or {}).get("value")
        if isinstance(report.get("icp"), dict)
        else "",
        "estimated_company_size": (report.get("estimated_company_size") or {}).get(
            "value"
        )
        if isinstance(report.get("estimated_company_size"), dict)
        else "",
        "technology_stack": (report.get("technology_stack") or {}).get("value")
        if isinstance(report.get("technology_stack"), dict)
        else [],
    }
    company_reasoning = [
    ...
    company_evidence = _dedupe_evidence_items(
        [
            _decision_evidence(
                "company_intelligence.report.company_summary",
                str(company_output.get("company_summary") or ""),
                82,
            )
            if company_output.get("company_summary")
            else {},
            _decision_evidence(
                "company_intelligence.report.icp",
                str(company_output.get("icp") or ""),
                78,
            )
            if company_output.get("icp")
            else {},
            _decision_evidence(
                "company_intelligence.report.estimated_company_size",
                str(company_output.get("estimated_company_size") or ""),
                74,
            )
    ...
    decision_evidence = _dedupe_evidence_items(
        [
            _decision_evidence(
                "decision_maker_intelligence.top_profile.name",
                str(top_profile.get("name") or ""),
                84,
            )
            if top_profile.get("name")
            else {},
            _decision_evidence(
                "decision_maker_intelligence.top_profile.title",
            ...
            _decision_evidence(
                "decision_maker_intelligence.top_profile.is_verified_contact",
                str(bool(top_profile.get("is_verified_contact"))),
                88,
            )
            if top_profile
            else {},
    ...
    buying_output = {
        ...
        "recommended_outreach_timing": str(
            buying_intent.get("recommended_outreach_timing") or ""
        ),
    }
    buying_reasoning = [
    ...
    buying_evidence = _dedupe_evidence_items(
        [
            _decision_evidence(
                "company_intelligence.buying_intent.buying_signal_score",
                str(buying_output.get("buying_signal_score") or ""),
                84,
            ),
            _decision_evidence(
                "company_intelligence.buying_intent.urgency",
                str(buying_output.get("urgency") or ""),
                80,
            )
    ...
    competitor_output = {
        "competitors": ai_competitor_intelligence.get("competitors")
        if isinstance(ai_competitor_intelligence.get("competitors"), list)
        else [],
        "market_gaps": ai_competitor_intelligence.get("market_gaps")
        if isinstance(ai_competitor_intelligence.get("market_gaps"), list)
        else [],
        "opportunity_to_sell": str(
            ai_competitor_intelligence.get("opportunity_to_sell") or ""
        ),
    }
    ...
    competitor_evidence = _dedupe_evidence_items(
        [
            *[
                _decision_evidence(
                    "ai_competitor_intelligence.competitors", str(item), 74
                )
                for item in competitor_output.get("competitors", [])[:4]
            ],
            *[
                _decision_evidence(
                    "ai_competitor_intelligence.market_gaps", str(item), 72
                )
                for item in competitor_output.get("market_gaps", [])[:4]
            ],
            _decision_evidence(
                "ai_competitor_intelligence.opportunity_to_sell",
                competitor_output.get("opportunity_to_sell", ""),
                76,
            )
    ...
    email_evidence = _dedupe_evidence_items(
        [
            *[
                ...
                for item in (
                    ai_outreach_strategy.get("best_subject_line_evidence") or []
                )
                if isinstance(item, dict)
            ],
            *[
    ...
    coach_output = {
        ...
        "what_could_fail": ai_sales_coach.get("what_could_fail")
        if isinstance(ai_sales_coach.get("what_could_fail"), list)
        else [],
        "how_to_increase_reply_rate": ai_sales_coach.get("how_to_increase_reply_rate")
        if isinstance(ai_sales_coach.get("how_to_increase_reply_rate"), list)
        else [],
    ...
    coach_evidence = _dedupe_evidence_items(
        [
            *[
            ...
            _decision_evidence(
                "ai_risk_analyzer.risk_score",
                str(coach_output.get("risk_score") or ""),
                80,
            )
    ...
    orchestrator_output = {
        ...
        "predictions": {
            "estimated_arr": ai_company_predictions.get("estimated_arr")
            if isinstance(ai_company_predictions.get("estimated_arr"), dict)
            else {},
            "company_maturity": ai_company_predictions.get("company_maturity")
            if isinstance(ai_company_predictions.get("company_maturity"), dict)
            else {},
            "growth_probability": ai_company_predictions.get("growth_probability")
            if isinstance(ai_company_predictions.get("growth_probability"), dict)
            else {},
            "sales_readiness": ai_company_predictions.get("sales_readiness")
            if isinstance(ai_company_predictions.get("sales_readiness"), dict)
            else {},
        },
        "final_recommendation": {
            "next_action": str(
                ai_sales_coach.get("alternative_strategy")
                or ai_outreach_strategy.get("cta")
                or "Insufficient evidence for next action."
            ),
    ...
    orchestrator = {
        ...
        "confidence": _safe_score(
            round(
                sum(
                    _safe_score(agent.get("confidence"), 0) for agent in agents.values()
                )
                / max(1, len(agents))
            ),
            40,
...
def _build_ai_executive_dashboard(
    *,
    ...
    orchestrator_output = (
        ai_final_orchestrator.get("output")
        if isinstance(ai_final_orchestrator.get("output"), dict)
        else {}
    )
    decision_maker = (
        orchestrator_output.get("decision_maker")
        if isinstance(orchestrator_output.get("decision_maker"), dict)
        else {}
    )
    email_plan = (
        orchestrator_output.get("email_plan")
        if isinstance(orchestrator_output.get("email_plan"), dict)
        else {}
    )
    competitor_view = (
        orchestrator_output.get("competitor_view")
        if isinstance(orchestrator_output.get("competitor_view"), dict)
        else {}
    )
    final_recommendation = (
    ...
    top_risks = (
        ai_risk_analyzer.get("reasons")
        if isinstance(ai_risk_analyzer.get("reasons"), list)
        else []
    )
    ...
    schedule = (
        ai_outreach_strategy.get("follow_up_schedule")
        if isinstance(ai_outreach_strategy.get("follow_up_schedule"), list)
        else []
    )
    if schedule:
    ...
    evidence = _dedupe_evidence_items(
        [
            ...
            if isinstance(item, dict)
            and str(item.get("source_field") or "").strip()
    ...
    return {
        ...
        "competitor_summary": {
            "competitors": competitor_view.get("competitors")
            if isinstance(competitor_view.get("competitors"), list)
            else [],
            "market_gaps": competitor_view.get("market_gaps")
            if isinstance(competitor_view.get("market_gaps"), list)
            else [],
            "opportunity_to_sell": str(
                competitor_view.get("opportunity_to_sell") or ""
            ),
        },
        "evidence": evidence,
...
def _build_ai_revenue_engine_report(
    *,
    ...
    top_pain_points = (
        [
            str(item)
            for item in report.get("possible_pain_points", {}).get("value", [])
            if str(item or "").strip()
        ]
        if isinstance(report.get("possible_pain_points"), dict)
        and isinstance(report.get("possible_pain_points", {}).get("value"), list)
        else []
    )
    if not top_pain_points:
        top_pain_points = (
            [
                str(item)
                for item in ai_outreach_strategy.get("expected_objections", [])
                if str(item or "").strip()
            ]
            if isinstance(ai_outreach_strategy.get("expected_objections"), list)
            else []
        )
    ...
    top_opportunities = (
        ...
        if isinstance(opportunity_ranking.get("top_positive_signals"), list)
        else []
    )
    opportunity_to_sell = str(
        ai_competitor_intelligence.get("opportunity_to_sell") or ""
    ...
    if not top_risks:
        top_risks = (
            ...
            if isinstance(opportunity_ranking.get("top_negative_signals"), list)
            else []
        )
    ...
    products = (
        report.get("products", {}).get("value", [])
        if isinstance(report.get("products"), dict)
        else []
    )
    technologies = (
        report.get("technology_stack", {}).get("value", [])
    ...
    technology_summary = {
        "products": [str(item) for item in products if str(item or "").strip()]
        if isinstance(products, list)
        else [],
        "technology_stack": [
            str(item) for item in technologies if str(item or "").strip()
        ]
    ...
    return {
        ...
        "competitor_position": {
            ...
            "competitors": [
                str(item)
                for item in ai_competitor_intelligence.get("competitors", [])
                if str(item or "").strip()
            ]
            if isinstance(ai_competitor_intelligence.get("competitors"), list)
            else [],
            "market_gaps": [
                str(item)
                for item in ai_competitor_intelligence.get("market_gaps", [])
                if str(item or "").strip()
            ]
            if isinstance(ai_competitor_intelligence.get("market_gaps"), list)
            else [],
        ...
        "recommended_outreach_strategy": {
            ...
            "strongest_value_proposition": str(
                ai_outreach_strategy.get("strongest_value_proposition") or ""
            ),
        },
        ...
        "recommended_follow_up_strategy": {
            "schedule": [
                str(item)
                for item in ai_outreach_strategy.get("follow_up_schedule", [])
                if str(item or "").strip()
            ]
            if isinstance(ai_outreach_strategy.get("follow_up_schedule"), list)
            else [],
...
def _build_ai_crm_summary(
    *,
    ...
    health_score = _safe_score(
        round((100 - risk_score) * 0.55 + ranking_confidence * 0.45), 0
    )
    ...
    crm_stage = str(
        (company.crm_stage if company else "") or metadata.get("crm_stage") or ""
    )
    ...
    top_opportunities = (
        ai_revenue_engine_report.get("top_opportunities")
        if isinstance(ai_revenue_engine_report.get("top_opportunities"), list)
        else []
    )
    upcoming_opportunity = (
        str(top_opportunities[0] or "").strip() if top_opportunities else ""
    )
    if not upcoming_opportunity:
        upcoming_opportunity = str(
            opportunity_ranking.get("recommended_next_action") or ""
        ).strip()
    ...
    return {
        ...
        "health": {
            ...
            "score": health_score,
            "reasoning": str(
                ai_risk_analyzer.get("reasons", [""])[0]
                if isinstance(ai_risk_analyzer.get("reasons"), list)
                and ai_risk_analyzer.get("reasons")
                else ""
            ),
        },
        "buying_intent": {
...
def _build_ai_ceo_dashboard(
    *,
    ...
    if not top_opportunities:
        top_opportunities = (
            opportunity_ranking.get("top_positive_signals")
            if isinstance(opportunity_ranking.get("top_positive_signals"), list)
            else []
        )
    todays_best_opportunities = [
        str(item) for item in top_opportunities if str(item or "").strip()
    ][:5]

    latest_changes = (
        ai_live_buying_signals.get("latest_changes")
        if isinstance(ai_live_buying_signals.get("latest_changes"), list)
        else []
    )
    new_buying_signals = [
        {
            "change_type": str(item.get("change_type") or ""),
            "added": [str(v) for v in item.get("added", []) if str(v or "").strip()]
            if isinstance(item.get("added"), list)
            else [],
    ...
    if risk_score >= 55:
        companies_at_risk.append(
            {
                "company": str(
                    (company.name if company else None) or lead.company or ""
                ),
                "risk_score": risk_score,
    ...
    sales_pipeline = {
        "crm_stage": str(
            (company.crm_stage if company else "")
            or ai_crm.get("relationship_status")
            or "New Lead"
        ),
    ...
    arr_prediction = (
        ai_company_predictions.get("estimated_arr")
        if isinstance(ai_company_predictions.get("estimated_arr"), dict)
        else {}
    )
    expected_revenue = {
        ...
        "opportunity_score": _safe_score(
            (ai_revenue_engine_report.get("overall_opportunity_score") or {}).get(
                "score"
            )
            if isinstance(
                ai_revenue_engine_report.get("overall_opportunity_score"), dict
            )
            else 0,
            0,
        ),
    ...
    ai_recommendations = _dedupe_text_values(
        [
            ...
            str(
                (
                    ai_revenue_engine_report.get("recommended_outreach_strategy") or {}
                ).get("why_contact_now")
                or ""
            ).strip()
            if isinstance(
                ai_revenue_engine_report.get("recommended_outreach_strategy"), dict
            )
            else "",
        ]
    )
    ...
    while len(top_priorities) < 3:
        top_priorities.append(
            "Continue monitoring buying signals and prioritize the safest high-confidence action."
        )
...
def _build_ai_sales_os(
    *,
    ...
    research_output = {
        ...
        "company_summary": str((report.get("company_summary") or {}).get("value") or "")
        if isinstance(report.get("company_summary"), dict)
        else "",
        "sources": [
            str(item)
            for item in company_intelligence.get("sources", [])
            if str(item or "").strip()
        ]
        if isinstance(company_intelligence.get("sources"), list)
        else [],
        "missing_fields": [
            str(item)
            for item in company_intelligence.get("missing_fields", [])
            if str(item or "").strip()
        ]
        if isinstance(company_intelligence.get("missing_fields"), list)
        else [],
    ...
    company_output = {
        "products": [
            str(item)
            for item in (report.get("products") or {}).get("value", [])
            if str(item or "").strip()
        ]
        if isinstance(report.get("products"), dict)
        and isinstance((report.get("products") or {}).get("value"), list)
        else [],
        "icp": str((report.get("icp") or {}).get("value") or "")
        if isinstance(report.get("icp"), dict)
        else "",
        "estimated_company_size": (report.get("estimated_company_size") or {}).get(
            "value"
        ...
        "technology_stack": [
            str(item)
            for item in (report.get("technology_stack") or {}).get("value", [])
            if str(item or "").strip()
        ]
        if isinstance(report.get("technology_stack"), dict)
        and isinstance((report.get("technology_stack") or {}).get("value"), list)
        else [],
    }
    buying_output = {
        "buying_signal_score": _safe_score(buying_intent.get("buying_signal_score"), 0),
    ...
    follow_up_output = {
        "strategy": [
            str(item)
            for item in (
                ai_revenue_engine_report.get("recommended_follow_up_strategy") or {}
            ).get("schedule", [])
            if str(item or "").strip()
        ]
        if isinstance(
            ai_revenue_engine_report.get("recommended_follow_up_strategy"), dict
        )
        and isinstance(
            (ai_revenue_engine_report.get("recommended_follow_up_strategy") or {}).get(
                "schedule"
            ),
            list,
        )
    ...
    crm_output = {
        "priority": ai_crm.get("priority")
        if isinstance(ai_crm.get("priority"), dict)
        else {},
    ...
    ceo_output = {
        ...
        "top_priorities": [
            str(item)
            for item in ai_ceo_dashboard.get("top_priorities", [])
            if str(item or "").strip()
        ]
        if isinstance(ai_ceo_dashboard.get("top_priorities"), list)
        else [],
        "expected_revenue": ai_ceo_dashboard.get("expected_revenue")
        if isinstance(ai_ceo_dashboard.get("expected_revenue"), dict)
        else {},
    }
    ...
    agents = {
        "research_agent": _sales_os_agent_result(
            ...
            evidence=[
                {
                    "source_field": "company_intelligence.summary",
                    "value": str(company_intelligence.get("summary") or ""),
                    "confidence": 75,
                },
            ],
            confidence=75,
        ...
        "company_agent": _sales_os_agent_result(
            ...
            output=company_output,
            reasoning=[
                "Company Agent extracts structured company profile information from enriched report fields."
            ],
            evidence=[
                {
                    "source_field": "company_intelligence.report.icp",
                    "value": str(company_output.get("icp") or ""),
                    "confidence": 74,
                },
            ],
            confidence=74,
        ...
        "buying_agent": _sales_os_agent_result(
            ...
            output=buying_output,
            reasoning=[
                "Buying Agent combines buying-intent score and detected live signal changes."
            ],
            evidence=[
                {
                    "source_field": "company_intelligence.buying_intent.buying_signal_score",
                    "value": str(buying_output.get("buying_signal_score") or ""),
                    "confidence": 82,
                },
            ],
            confidence=82,
        ...
        "decision_maker_agent": _sales_os_agent_result(
            ...
            reasoning=[
                "Decision Maker Agent ranks known contacts from decision-maker intelligence only."
            ],
            evidence=[
                {
                    "source_field": "decision_maker_intelligence.top_contact_id",
                    "value": str(decision_maker_output.get("top_contact_id") or ""),
                    "confidence": 80,
                },
            ],
            confidence=80,
        ...
        "competitor_agent": _sales_os_agent_result(
            ...
            output=competitor_output,
            reasoning=[
                "Competitor Agent summarizes competitor and market-gap enrichment artifacts."
            ],
            evidence=[
                {
                    "source_field": "ai_competitor_intelligence.opportunity_to_sell",
                    "value": str(competitor_output.get("opportunity_to_sell") or ""),
                    "confidence": 72,
                },
            ],
            confidence=72,
        ...
        "email_agent": _sales_os_agent_result(
            ...
            evidence=[
                {
                    "source_field": "ai_outreach_strategy.best_subject_line",
                    "value": str(email_output.get("subject") or ""),
                    "confidence": 76,
                },
            ],
            confidence=76,
        ...
        "follow_up_agent": _sales_os_agent_result(
            ...
            reasoning=[
                "Follow-up Agent uses existing follow-up schedules and sales timeline steps."
            ],
            evidence=[
                {
                    "source_field": "ai_sales_timeline.steps",
                    "value": str(len(follow_up_output.get("timeline_steps") or [])),
                    "confidence": 70,
                },
            ],
            confidence=70,
        ...
        "crm_agent": _sales_os_agent_result(
            ...
            output=crm_output,
            reasoning=[
                "CRM Agent reflects AI CRM summary fields that are already synchronized to company metadata."
            ],
            evidence=[
                {
                    "source_field": "ai_crm.next_action",
                    "value": str(crm_output.get("next_action") or ""),
                    "confidence": 78,
                },
            ],
            confidence=78,
        ...
        "analytics_agent": _sales_os_agent_result(
            ...
            reasoning=[
                "Analytics Agent merges ranking, risk, and predictive signals for quantitative planning."
            ],
            evidence=[
                {
                    "source_field": "opportunity_ranking.overall_score",
                    "value": str(analytics_output.get("opportunity_score") or ""),
                    "confidence": 80,
                },
            ],
            confidence=80,
        ...
        "ceo_agent": _sales_os_agent_result(
            ...
            output=ceo_output,
            reasoning=[
                "CEO Agent surfaces daily priorities and summary directly from the AI CEO dashboard."
            ],
            evidence=[
                {
                    "source_field": "ai_ceo_dashboard.daily_summary",
                    "value": str(ceo_output.get("daily_summary") or ""),
                    "confidence": 74,
                },
            ],
            confidence=74,
    ...
    intermediate_reasoning = {
        key: {
            ...
            if isinstance(value.get("reasoning"), list)
            else [],
    ...
    merged_output = {
        ...
        "ceo": ceo_output,
        "next_action": str(
            ai_crm.get("next_action") or ai_ceo_dashboard.get("daily_summary") or ""
        ),
    }
    ...
    orchestrator = {
        ...
        "confidence": _safe_score(
            round(
                sum(
                    _safe_score((agents.get(name) or {}).get("confidence"), 0)
                    for name in execution_order
                )
                / max(1, len(execution_order))
            ),
            45,
        ),
    }
    ...
    intermediate_reasoning["orchestrator"] = {
        ...
        "evidence": _dedupe_evidence_items(
            [
                {
                    "source_field": "ai_crm.next_action",
                    "value": str(ai_crm.get("next_action") or ""),
                    "confidence": 78,
                },
                {
                    "source_field": "ai_ceo_dashboard.daily_summary",
                    "value": str(ai_ceo_dashboard.get("daily_summary") or ""),
                    "confidence": 74,
                },
            ]
        ),
...
def _live_buying_signal_snapshot(
    *, lead: Lead | None, metadata: dict[str, Any], company_intelligence: dict[str, Any]
) -> dict[str, list[str]]:
    report = (
        company_intelligence.get("report")
        if isinstance(company_intelligence.get("report"), dict)
    ...
    return {key: value for key, value in categories.items() if isinstance(value, list)}
...
def _build_ai_live_buying_signals(
    *,
    ...
    for category in categories:
        current_values = [
            str(item).strip()
            for item in (current_snapshot.get(category) or [])
            if str(item or "").strip()
        ]
        previous_values = [
            str(item).strip()
            for item in (previous_snapshot.get(category) or [])
            if str(item or "").strip()
        ]
        previous_set = {item.lower() for item in previous_values}
        added_values = [
            item for item in current_values if item.lower() not in previous_set
        ]
        if not added_values:
            continue
        change = {
...
def _build_ai_company_timeline(
    *,
    ...
    funding_values = _event_texts(metadata.get("funding_signal")) + _event_texts(
        metadata.get("funding_signals")
    )
    hiring_values = _event_texts(metadata.get("hiring_signals")) + _event_texts(
        metadata.get("jobs_signal")
    )
    technology_values = _event_texts(metadata.get("technology_changes"))
    if not technology_values:
        technology_values = (
            _event_texts((report.get("technology_stack") or {}).get("value"))
            if isinstance(report.get("technology_stack"), dict)
            else _event_texts(metadata.get("technologies"))
        )
    ...
    if not leadership_values and isinstance(fields.get("ceo_founder"), dict):
        ...
        if isinstance(ceo, dict) and (ceo.get("name") or ceo.get("title")):
            leadership_values = [
                f"Leadership signal: {ceo.get('name') or ''} {ceo.get('title') or ''}".strip()
            ]
    location_values = _event_texts(metadata.get("new_locations")) + _event_texts(
        metadata.get("market_expansion")
    )
    if not location_values and (lead.city or lead.country):
        location_values = [
            f"Operating location: {', '.join(part for part in [lead.city, lead.country] if part)}"
        ]
    product_values = _event_texts(metadata.get("product_launches"))
    if not product_values:
        product_values = (
            _event_texts((report.get("products") or {}).get("value"))
            if isinstance(report.get("products"), dict)
            else _event_texts(metadata.get("products"))
        )
    partnership_values = _event_texts(
        metadata.get("partnership_signals")
    ) + _event_texts(metadata.get("partnership_updates"))
    ...
    events: list[dict[str, Any]] = []
    events.extend(
        _timeline_events_for_field(
            event_type="funding_events",
            source_field="metadata.funding_signal",
            values=funding_values,
            fallback=now - timedelta(days=90),
            provider=provider,
            enrichment_step=enrichment_step,
            model_version=model_version,
            prompt_version=prompt_version,
    ...
    events.extend(
        _timeline_events_for_field(
            event_type="hiring_events",
            source_field="metadata.hiring_signals",
            values=hiring_values,
            fallback=now - timedelta(days=60),
            provider=provider,
            enrichment_step=enrichment_step,
            model_version=model_version,
            prompt_version=prompt_version,
    ...
    events.extend(
        _timeline_events_for_field(
            event_type="technology_changes",
            source_field="metadata.technology_changes",
            values=technology_values,
            fallback=now - timedelta(days=50),
            provider=provider,
            enrichment_step=enrichment_step,
            model_version=model_version,
            prompt_version=prompt_version,
    ...
    events.extend(
        _timeline_events_for_field(
            event_type="website_changes",
            source_field="metadata.website_changes",
            values=website_values,
            fallback=now - timedelta(days=40),
            provider=provider,
            enrichment_step=enrichment_step,
            model_version=model_version,
            prompt_version=prompt_version,
    ...
    events.extend(
        _timeline_events_for_field(
            event_type="leadership_changes",
            source_field="metadata.leadership_changes",
            values=leadership_values,
    ...
    events.extend(
        _timeline_events_for_field(
            event_type="new_locations",
            source_field="metadata.new_locations",
            values=location_values,
            fallback=now - timedelta(days=20),
            provider=provider,
            enrichment_step=enrichment_step,
            model_version=model_version,
            prompt_version=prompt_version,
    ...
    events.extend(
        _timeline_events_for_field(
            ...
            source_field="metadata.product_launches",
            values=product_values,
            fallback=now - timedelta(days=15),
            provider=provider,
            enrichment_step=enrichment_step,
            model_version=model_version,
            prompt_version=prompt_version,
    ...
    events.extend(
        _timeline_events_for_field(
            event_type="partnerships",
            source_field="metadata.partnership_signals",
            values=partnership_values,
            fallback=now - timedelta(days=10),
            provider=provider,
            enrichment_step=enrichment_step,
            model_version=model_version,
            prompt_version=prompt_version,
    ...
    by_type = {
        "funding_events": [
            item for item in events if item["event_type"] == "funding_events"
        ],
        "hiring_events": [
            item for item in events if item["event_type"] == "hiring_events"
        ],
        "technology_changes": [
            item for item in events if item["event_type"] == "technology_changes"
        ],
        "website_changes": [
            item for item in events if item["event_type"] == "website_changes"
        ],
        "leadership_changes": [
            item for item in events if item["event_type"] == "leadership_changes"
        ],
        "new_locations": [
            item for item in events if item["event_type"] == "new_locations"
        ],
        "product_launches": [
            item for item in events if item["event_type"] == "product_launches"
        ],
        "partnerships": [
            item for item in events if item["event_type"] == "partnerships"
        ],
    }
    return {
    ...
    *,
    ...
    if normalized:
        for item in normalized:
            source_field = (
                str(item.get("source_field") or default_source).strip()
                or default_source
            )
            evidence_snippet = (
                str(item.get("value") or reasoning_text).strip() or reasoning_text
            )
...
def _build_ai_evidence_engine(
    *,
    ...
    _append_evidence_entries(
        ...
        reasoning=(company_intelligence.get("report") or {})
        .get("company_summary", {})
        .get("value")
        if isinstance(
            (company_intelligence.get("report") or {}).get("company_summary"), dict
        )
        else "",
        evidence_items=(company_intelligence.get("report") or {})
        .get("company_summary", {})
        .get("sources")
        if isinstance(
            (company_intelligence.get("report") or {}).get("company_summary"), dict
    ...
    for field in [
        ...
        _append_evidence_entries(
            ...
            evidence_items=[
                ...
                for item in (
                    ai_competitor_intelligence.get(field)
                    if isinstance(ai_competitor_intelligence.get(field), list)
                    else [ai_competitor_intelligence.get(field)]
                )
                if str(item or "").strip()
            ],
            provider=provider,
        ...
        for item in ai_company_timeline.get("events", []):
            ...
            _append_evidence_entries(
                ...
                evidence_items=[
                    {
                        "source_field": str(
                            item.get("source") or "ai_company_timeline"
                        ),
                        "value": str(
    ...
    for field in [
        ...
        prediction = (
            ai_company_predictions.get(field)
            if isinstance(ai_company_predictions.get(field), dict)
            else {}
        )
        _append_evidence_entries(
            entries=entries,
    ...
    if isinstance(ai_sales_timeline.get("steps"), list):
        for item in ai_sales_timeline.get("steps", []):
            if not isinstance(item, dict):
                continue
            step_name = (
                str(item.get("step") or "timeline_step")
                .strip()
                .lower()
                .replace(" ", "_")
            )
    ...
    for field in risk_fields:
        ...
        if isinstance(ai_risk_analyzer.get("factors"), dict):
            for factor in ai_risk_analyzer.get("factors", {}).values():
                if isinstance(factor, dict) and isinstance(
                    factor.get("evidence"), list
                ):
...
def _detect_buying_intent(metadata: dict[str, Any]) -> dict[str, Any]:
    ...
    for spec in signal_specs:
        ...
        if not signal_evidence:
            for source_field, candidates in fallback_sources.items():
                for text in candidates:
                    if any(
                        re.search(pattern, text, flags=re.I) for pattern in patterns
                    ):
                        signal_evidence.append(
                            _intent_evidence(signal_name, source_field, text, 65)
                        )
        ...
        signal_confidence = (
            _safe_score(
                round(sum(item["confidence"] for item in deduped) / len(deduped))
            )
            if deduped
            else 0
        )
    ...
    buying_signal_score = _safe_score(
        round(
            raw_score * 100 / max(1, sum(int(spec["weight"]) for spec in signal_specs))
        )
    )
    ...
    top_signals = [
        item["signal"].replace("_", " ") for item in per_signal if item["detected"]
    ]
    explanation = (
    ...
    confidence = (
        _safe_score(round(sum(confidence_values) / len(confidence_values)))
...
def _company_intelligence_cache_key(
    lead: Lead, metadata: dict[str, Any], company: Company | None = None
) -> str:
    ...
    place_id = str(
        (company.place_id if company else "") or metadata.get("place_id") or ""
    ).strip()
    if place_id:
...
def _fresh_company_intelligence(metadata: dict[str, Any]) -> dict[str, Any] | None:
    payload = (
        metadata.get("company_intelligence")
        if isinstance(metadata.get("company_intelligence"), dict)
        else None
    )
    if not payload:
    ...
    raw_updated = str(
        ...
        or metadata.get("company_intelligence_cached_at")
        or ""
    ).strip()
    if not raw_updated:
        return None
    try:
        updated_at = datetime.fromisoformat(raw_updated.replace("Z", "+00:00")).replace(
            tzinfo=None
        )
    except ValueError:
        return None
    ...
    return (
        payload
        if datetime.utcnow() - updated_at < timedelta(hours=cache_hours)
        else None
    )
...
def _build_company_intelligence(
    *,
    ...
    selected = (
        deep_contact.get("selected_decision_maker")
        if isinstance(deep_contact.get("selected_decision_maker"), dict)
        else {}
    )
    candidates = (
        deep_contact.get("candidates")
        if isinstance(deep_contact.get("candidates"), list)
        else []
    )
    source_types = _dedupe_text_values(
        [
            ...
            if metadata.get("place_id") or metadata.get("google_maps_url")
            else "",
            "Website analysis"
            if metadata.get("ai_summary") or metadata.get("opportunity_analysis")
            else "",
            "Contact enrichment" if contacts or selected or candidates else "",
            "Email verification"
            if lead.email
            or (company and company.email)
            or deep_contact.get("verified_email")
            else "",
            "Technographic enrichment"
            if metadata.get("technologies") or deep_contact.get("technologies")
            else "",
        ]
    )
    technologies = _dedupe_text_values(
        [
            ...
            *(
                deep_contact.get("technologies")
                if isinstance(deep_contact.get("technologies"), list)
                else []
            ),
        ]
    )
    ...
    phones = _dedupe_text_values(
        [lead.phone, company.phone if company else "", metadata.get("phone")]
    )
    ...
    official_site = (
        ...
        or (f"https://{company.domain}" if company and company.domain else "")
    )
    employee_count = (
    ...
    score += (
        10 if metadata.get("ai_summary") or metadata.get("opportunity_analysis") else 0
    )
    ...
    score = _safe_score(
        metadata.get("priority_score") or deep_contact.get("lead_score"), score
    )
    missing = [
        label
        for label, present in [
            ...
            (
                "company LinkedIn",
                metadata.get("company_linkedin") or metadata.get("linkedin_url"),
            ),
            ("technologies", technologies),
            (
                "growth signals",
                metadata.get("jobs_signal")
                or metadata.get("growth_signal")
                or metadata.get("funding_signal"),
            ),
        ]
        if not present
    ]
    ...
    ceo_founder = selected or next(
        (
            ...
            if isinstance(item, dict)
            and re.search(
                r"\b(ceo|founder|owner)\b", str(item.get("title") or ""), flags=re.I
            )
        ),
        {},
    )
    report_sources = {
        "company_summary": ["AI website analysis"]
        if metadata.get("ai_summary")
        else ["CRM profile"],
        ...
        "hiring_signals": ["Company enrichment"]
        if metadata.get("hiring_signals")
        else [],
        ...
        "possible_pain_points": ["AI website analysis"]
        if metadata.get("pain_points")
        else [],
        "best_outreach_angle": ["AI sales reasoning"]
        if metadata.get("best_outreach_angle")
        or metadata.get("suggested_offer")
        or metadata.get("sales_angle")
        else [],
        "recommended_decision_maker": ["Decision-maker ranking"]
        if selected or contacts
        else [],
        "personalization_bullets": ["AI website analysis"]
        if metadata.get("personalization_bullets")
        else [],
        "ai_confidence_score": ["AI website analysis"]
    ...
    report = {
        "company_summary": _report_field(
            ...
            confidence=82
            if metadata.get("ai_summary") or metadata.get("company_summary")
            else 60,
        ),
        ...
        "estimated_company_size": _report_field(
            ...
            confidence=72
            if employee_count or metadata.get("estimated_company_size")
            else 0,
        ),
        ...
        "hiring_signals": _report_field(
            metadata.get("hiring_signals")
            or metadata.get("jobs_signal")
            or metadata.get("growth_signal"),
        ...
        "best_outreach_angle": _report_field(
            metadata.get("best_outreach_angle")
            or metadata.get("suggested_offer")
            or metadata.get("sales_angle")
            or metadata.get("partnership_fit"),
        ...
        "recommended_decision_maker": _report_field(
            ...
            confidence=int((selected or {}).get("confidence") or 72)
    ...
    fields = {
        "official_website": _intelligence_field(
            official_site,
            source="CRM and public profile",
            confidence=90 if official_site else 0,
        ),
        "business_description": _intelligence_field(
            metadata.get("ai_summary")
            or profile.get("short_description")
            or profile.get("description"),
            source="AI website analysis",
            confidence=84 if metadata.get("ai_summary") else 62,
        ),
        "industry": _intelligence_field(
            (company.industry if company else "")
            or lead.industry
            or lead.niche
            or metadata.get("business_category"),
            source="CRM and public profile",
            confidence=78,
        ),
        "employee_count": _intelligence_field(
            employee_count, source="Company enrichment", confidence=72
        ),
        "technologies": _intelligence_field(
            technologies, source="Technographic enrichment", confidence=76
        ),
        "company_linkedin": _intelligence_field(
            metadata.get("company_linkedin")
        ...
        "key_employee_linkedin": _intelligence_field(
            contact_linkedins, source="Contact enrichment", confidence=75
        ),
        "ceo_founder": _intelligence_field(
            ceo_founder,
            source="Decision-maker ranking",
            confidence=int((ceo_founder or {}).get("confidence") or 72)
            if isinstance(ceo_founder, dict) and ceo_founder
            else 0,
        ),
        "verified_emails": _intelligence_field(
            verified_emails,
            source="Email verification",
            confidence=92 if verified_emails else 0,
        ),
        "phones": _intelligence_field(
            phones, source="Public profile and CRM", confidence=80 if phones else 0
        ),
        "social_profiles": _intelligence_field(
            social_profiles,
            source="Public profile",
            confidence=68 if social_profiles else 0,
        ),
        "buying_signals": _intelligence_field(
            buying_signals,
            source="AI and public signals",
            confidence=70 if buying_signals else 0,
        ),
        "ai_summary": _intelligence_field(
            metadata.get("ai_summary"),
            source="AI website analysis",
            confidence=82 if metadata.get("ai_summary") else 0,
        ),
        "personalized_reason": _intelligence_field(
            ai_reason, source="AI sales reasoning", confidence=78 if ai_reason else 0
        ),
    }
    return {
        "version": 1,
        ...
        "lead_score": {
            ...
            "confidence": _safe_score(
                metadata.get("confidence_score")
                or deep_contact.get("confidence_score"),
                int(fields["personalized_reason"]["confidence"] or 55),
            ),
...
def _refresh_company_intelligence(
    db: Session, user_id: str, workspace, lead: Lead, company: Company | None = None
) -> dict[str, Any]:
    db.flush()
    metadata = _lead_metadata(lead)
    if company is None:
    ...
    intelligence = _build_company_intelligence(
        lead=lead,
        metadata=metadata,
        workspace=workspace,
        company=company,
    ...
    learning_weights = continuous_learning_weights(
        settings.ai if isinstance(settings.ai, dict) else {}
    )
    learning_snapshot = continuous_learning_snapshot(
        settings.ai if isinstance(settings.ai, dict) else {}
    )
    opportunity_ranking = _opportunity_ranking(
        ...
        company_intelligence=intelligence,
        decision_maker_intelligence=decision_maker_intelligence,
        opportunity_weights=learning_weights.get("opportunity")
        if isinstance(learning_weights.get("opportunity"), dict)
        else None,
    )
    buying_intent = (
        intelligence.get("buying_intent")
        if isinstance(intelligence.get("buying_intent"), dict)
        else {}
    )
    ...
    ai_specialized_agents = (
        specialized_agents.get("agents")
        if isinstance(specialized_agents.get("agents"), dict)
        else {}
    )
    ai_agent_intermediate_reasoning = (
    ...
    ai_lead_prioritization = _lead_prioritization_from_metadata(
        company=company,
        metadata=metadata,
        prioritization_weights=learning_weights.get("prioritization")
        if isinstance(learning_weights.get("prioritization"), dict)
        else None,
    )
    ai_revenue_engine_report = _build_ai_revenue_engine_report(
        previous_report=metadata.get("ai_revenue_engine_report")
        if isinstance(metadata.get("ai_revenue_engine_report"), dict)
        else {},
        company_intelligence=intelligence,
        opportunity_ranking=opportunity_ranking,
        buying_intent=buying_intent,
        ...
        for contact in contacts:
            ...
            contact.metadata_json = {
                **(contact.metadata_json or {}),
                ...
                "decision_maker_intelligence_generated_at": decision_maker_intelligence.get(
                    "generated_at"
                ),
            }
...
def _apply_cached_company_intelligence(
    db: Session, user_id: str, workspace, lead: Lead, request_id: str
) -> bool:
    own_company = db.scalar(
        select(Company)
        .where(Company.workspace_id == workspace.id, Company.lead_id == lead.id)
        .order_by(Company.updated_at.desc())
        .limit(1)
    ...
    own_metadata = {
        **_lead_metadata(lead),
        **((own_company.metadata_json or {}) if own_company else {}),
    }
    ...
    for candidate in candidates:
        ...
        _lead_trace(
            request_id,
            "company_intelligence_cache_hit",
            lead_id=str(lead.id),
...
def _ensure_b2b_opportunity_metadata(
    lead: Lead, workspace, source: str = "fallback", language: str | None = None
) -> None:
    ...
    industry = (
        ...
        or lead.niche
        or metadata.get("business_category")
        or _localized_fallback_text(language, "market")
    ).strip()
    ...
    public_signals = [
        ...
        _localized_fallback_text(language, "listing_available")
        if metadata.get("place_id")
        else "",
        _localized_fallback_text(language, "rating_available")
        if metadata.get("google_rating")
        else "",
    ]
    ...
    summary = _localized_fallback_text(
        ...
        company=company,
        industry=industry,
        location=_localized_fallback_text(language, "location", location=location)
        if location
        else "",
    ...
    offer_focus = (
        workspace_offer
        or workspace.company
        or _localized_fallback_text(language, "offer_focus")
    )
    updates = {
        "ai_summary": _sales_metadata_value(metadata, "ai_summary", summary),
        "sales_angle": _sales_metadata_value(
            metadata,
            "sales_angle",
            _localized_fallback_text(language, "sales_angle", industry=industry),
        ),
        "suggested_offer": _sales_metadata_value(
            metadata,
            "suggested_offer",
            _localized_fallback_text(
                language, "offer", offer_focus=offer_focus, company=company
            ),
        ),
        "outreach_strategy": _sales_metadata_value(
            metadata,
            "outreach_strategy",
            _localized_fallback_text(language, "strategy"),
        ),
        "recommended_tone": metadata.get("recommended_tone")
        or workspace_tone
        or "Professional",
        "recommended_cta": _sales_metadata_value(
            metadata,
            "recommended_cta",
            workspace_cta or _localized_fallback_text(language, "cta"),
        ),
        "follow_up_strategy": _sales_metadata_value(
            metadata,
            "follow_up_strategy",
            _localized_fallback_text(language, "follow_up"),
        ),
        "expected_reply_rate": _sales_metadata_value(
            metadata,
            "expected_reply_rate",
            "8-12%"
            if has_website and has_email
            else _localized_fallback_text(language, "reply_unverified"),
        ),
        "confidence_score": metadata.get("confidence_score")
        or (72 if has_website and has_email else 58 if has_website else 42),
        "priority_score": metadata.get("priority_score")
        or (78 if has_website and has_email else 62 if has_website else 45),
        "buying_signals": _sales_metadata_value(
            metadata,
            "buying_signals",
            public_signals or [_localized_fallback_text(language, "profile_saved")],
        ),
        "risks": _sales_metadata_value(
            ...
            [
                ...
                for risk in [
                    ""
                    if has_website
                    else _localized_fallback_text(language, "risk_website"),
                    ""
                    if has_email
                    else _localized_fallback_text(language, "risk_email"),
                ]
                if risk
            ],
        ),
        "opportunity_analysis": _sales_metadata_value(
            metadata,
            "opportunity_analysis",
            _localized_fallback_text(language, "opportunity", company=company),
        ),
        "partnership_fit": _sales_metadata_value(
            metadata,
            "partnership_fit",
            _localized_fallback_text(language, "fit", offer_focus=offer_focus),
        ),
        "next_recommended_action": _sales_metadata_value(
            metadata,
            "next_recommended_action",
            _localized_fallback_text(language, "next_email")
            if has_email
            else _localized_fallback_text(language, "next_contact"),
        ),
        "research_source": metadata.get("research_source") or source,
    }
    if not metadata.get("pain_points") or _is_generic_sales_fallback(
        metadata.get("pain_points")
    ):
        updates["pain_points"] = [
    ...
    updates["intelligence_quality"] = _company_intelligence_quality(
        lead, combined_metadata, workspace, source, language
    )
    updates["company_intelligence"] = _build_company_intelligence(
        lead=lead,
        metadata={
            **combined_metadata,
            "intelligence_quality": updates["intelligence_quality"],
        },
        workspace=workspace,
    )
    updates["company_intelligence_cached_at"] = updates["company_intelligence"][
        "generated_at"
    ]
    updates["company_intelligence_cache_key"] = updates["company_intelligence"][
        "cache_key"
    ]
    buying_intent = (
        updates["company_intelligence"].get("buying_intent")
        if isinstance(updates["company_intelligence"].get("buying_intent"), dict)
        else {}
    )
    updates["buying_signal_score"] = _safe_score(
        buying_intent.get("buying_signal_score"), 0
    )
    ...
    updates["buying_signal_evidence"] = (
        buying_intent.get("evidence")
        if isinstance(buying_intent.get("evidence"), list)
        else []
    )
    updates["buying_signal_confidence"] = _safe_score(
        buying_intent.get("confidence"), 0
    )
    updates["recommended_outreach_timing"] = str(
        buying_intent.get("recommended_outreach_timing") or ""
    )
    ...
    updates["ai_lead_prioritization"] = _lead_prioritization_from_metadata(
        company=None, metadata={**combined_metadata, **updates}
    )
...
def _existing_review_draft(
    db: Session, workspace_id: UUID, lead_id: UUID
) -> EmailMessage | None:
    return db.scalar(
        select(EmailMessage)
        .where(
            EmailMessage.workspace_id == workspace_id, EmailMessage.lead_id == lead_id
        )
        .order_by(EmailMessage.created_at.desc())
        .limit(1)
    )
...
def _create_review_email_draft(
    db: Session, request: Request, user_id: str, workspace, lead: Lead
) -> EmailMessage | None:
    ...
    if existing:
        _set_workflow_stage(
            ...
            "ai_email",
            "completed",
            _workflow_message(language, "workflow_ai_email_done"),
        )
        _set_workflow_stage(
            lead,
            "approval",
            "completed"
            if existing.delivery_status in {"approved", "sent"}
            else "waiting",
            _workflow_message(language, "workflow_approval_done")
            if existing.delivery_status in {"approved", "sent"}
            else _workflow_message(language, "workflow_approval_waiting"),
        )
        return existing
    ...
    variant = personalize_email(
        PersonalizeRequest(
            ...
            offer=str(
                getattr(workspace, "offer", "")
                or metadata.get("suggested_offer")
                or workspace.company
                or _localized_fallback_text(language, "offer_focus")
            ),
            cta=str(
                _custom_workspace_cta(workspace)
                or metadata.get("recommended_cta")
                or getattr(workspace, "cta", "")
                or _localized_fallback_text(language, "cta")
            ),
            tone=str(
                getattr(workspace, "tone", "")
                or metadata.get("recommended_tone")
                or "Professional"
            ),
            language=language,
            signature="",
    ...
    lead.notes = _merge_lead_metadata(
        lead,
        {
            ...
            "email_generated_at": datetime.utcnow().isoformat(),
        },
    )
    _set_workflow_stage(
        lead,
    ...
    _set_workflow_stage(
        ...
        "approval",
        "waiting",
        _workflow_message(language, "workflow_approval_waiting"),
    )
    _add_lead_activity(
        db,
        request,
...
def _complete_turnkey_b2b_research(
    ...
    for index, lead in enumerate(leads[:MAX_TURNKEY_RESEARCH_LEADS], start=1):
        _lead_trace(
            request_id,
            "turnkey_research_started",
            lead_id=str(lead.id),
            company=lead.company,
            index=index,
        )
        public_details_updated = _complete_public_company_details(
            db, request, user_id, workspace, lead, request_id
        )
        if public_details_updated:
            _sync_lead_to_crm(db, user_id, workspace, lead)
            _lead_trace(
                request_id,
                "turnkey_public_details_completed",
                lead_id=str(lead.id),
                company=lead.company,
        ...
        if (
            not lead.email
            and hunter_key_loaded()
            and (lead.website or _lead_metadata(lead).get("domain"))
        ):
            try:
                _lead_trace(
                    request_id,
                    "turnkey_contact_refresh_started",
                    lead_id=str(lead.id),
                    company=lead.company,
                )
                enriched = _hunter_enriched_leads(
                    db, request, user_id, workspace, [_lead_out(lead)]
                )
                ...
                _lead_trace(
                    request_id,
                    "turnkey_contact_refresh_finished",
                    lead_id=str(lead.id),
                    company=lead.company,
                    has_email=bool(lead.email),
                )
                ...
                capture_provider_exception(
                    exc,
                    provider="hunter",
                    endpoint="workspace_app.turnkey_contact_refresh",
                    workspace_id=workspace.id,
                    lead_id=lead.id,
                    extra={"request_id": request_id},
                )
                warnings.append(
                    f"{lead.company}: verified contact could not be completed yet."
                )
                _lead_trace(
                    request_id,
                    "turnkey_contact_refresh_failed",
                    lead_id=str(lead.id),
                    company=lead.company,
                    reason=str(exc),
                )
        try:
            if _needs_ai_research(lead):
                ...
                _ensure_b2b_opportunity_metadata(
                    lead,
                    workspace,
                    source="turnkey_fallback",
                    language=_workspace_language(request, workspace),
                )
                metadata = _lead_metadata(lead)
                if (
                    metadata.get("ai_summary")
                    or metadata.get("suggested_offer")
                    or metadata.get("expected_reply_rate")
                ):
                    _add_lead_activity(
                        db,
                        request,
                    ...
                    warnings.append(
                        f"{lead.company}: AI research could not complete yet."
                    )
                    _lead_trace(
                        request_id,
                        "turnkey_ai_research_partial",
                        lead_id=str(lead.id),
                        company=lead.company,
                    )
            _refresh_company_intelligence(db, user_id, workspace, lead)
            ...
            capture_provider_exception(
                exc,
                provider="openai",
                endpoint="workspace_app.turnkey_research",
                workspace_id=workspace.id,
                lead_id=lead.id,
                extra={"request_id": request_id},
            )
            _ensure_b2b_opportunity_metadata(
                lead,
                workspace,
                source="turnkey_fallback_after_ai_error",
                language=_workspace_language(request, workspace),
            )
            _refresh_company_intelligence(db, user_id, workspace, lead)
            _sync_lead_to_crm(db, user_id, workspace, lead)
        ...
        try:
            ...
            capture_provider_exception(
                exc,
                provider="openai",
                endpoint="workspace_app.turnkey_email_draft",
                workspace_id=workspace.id,
                lead_id=lead.id,
                extra={"request_id": request_id},
            )
        ...
        _lead_trace(
            request_id,
            "turnkey_research_finished",
            lead_id=str(lead.id),
            company=lead.company,
        )
    if len(leads) > MAX_TURNKEY_RESEARCH_LEADS:
        warnings.append(
            f"Prepared the first {MAX_TURNKEY_RESEARCH_LEADS} opportunities. Open the rest to complete research."
        )
    return warnings
...
def _background_request(language: str) -> Any:
    return SimpleNamespace(
        headers={"x-outreachai-locale": language}, cookies={}, client=None
    )


def _enrichment_metadata_update(
    status: str, request_id: str, extra: dict[str, Any] | None = None
) -> dict[str, Any]:
    now = datetime.utcnow().isoformat()
...
def _mark_auto_enrichment_queued(
    lead: Lead, request_id: str, language: str | None = None
) -> None:
    language = language or "English"
    lead.notes = _merge_lead_metadata(
        lead, _enrichment_metadata_update("queued", request_id)
    )
    _set_workflow_stages(
        lead,
        {
        ...
        {
            "company_profile": _workflow_message(language, "workflow_company_profile"),
            "website_analysis": _localized_fallback_text(
                language, "progress_ai_analyzing"
            ),
...
def process_enrichment_job(job_id: UUID, claim_token: str | None = None) -> bool:
    ...
    try:
        ...
        if claim_token and (job.status != "running" or job.locked_by != claim_token):
            logger.warning(
                "Ignoring enrichment execution for stale claim job_id=%s claim_token=%s status=%s locked_by=%s",
                job_id,
                claim_token,
                job.status,
                job.locked_by,
        ...
        lead = db.scalar(
            select(Lead).where(
                Lead.id == job.lead_id, Lead.workspace_id == job.workspace_id
            )
        )
        ...
        if job.cancel_requested or _lead_enrichment_cancelled(lead):
            lead.notes = _merge_lead_metadata(
                ...
                _enrichment_metadata_update(
                    ...
                    request_id,
                    {
                        "enrichment_message": _localized_fallback_text(
                            language, "enrichment_stopped"
                        )
                    },
                ),
            )
            ...
            _record_action_policy_success(
                db,
                crm_policy,
                {"job_id": str(job.id), "lead_id": str(lead.id), "cancelled": True},
            )
        ...
        _record_action_policy_success(
            db,
            crm_policy,
            {"job_id": str(job.id), "lead_id": str(lead.id), "stage": "running"},
        )
        if not update_job_progress(
            db,
            job,
            stage="website_analysis",
            message=_localized_fallback_text(language, "progress_ai_analyzing"),
            percent=25,
            claim_token=claim_token,
        ):
            return False
        _lead_trace(
            request_id,
            "durable_auto_enrichment_started",
            lead_id=str(lead.id),
            job_id=str(job.id),
            company=lead.company,
        ...
        if _apply_cached_company_intelligence(
            db, job.user_id, workspace, lead, request_id
        ):
            lead.notes = _merge_lead_metadata(
                lead,
                _enrichment_metadata_update(
            ...
            completed = complete_job(
                db, job, partial=False, warnings=[], claim_token=claim_token
            )
            _lead_trace(
                request_id,
                "durable_auto_enrichment_cache_hit",
                lead_id=str(lead.id),
                job_id=str(job.id),
            )
            return completed
        ...
        warnings = _complete_turnkey_b2b_research(
            db, request, job.user_id, workspace, [lead], request_id
        )
        _record_action_policy_success(
            db,
            draft_policy,
            {"job_id": str(job.id), "lead_id": str(lead.id), "draft_only": True},
        )
        db.refresh(job)
        ...
        if lead is None:
            return mark_cancelled(
                db, job, message="Lead no longer exists.", claim_token=claim_token
            )
        ...
        lead.notes = _merge_lead_metadata(
            ...
            _enrichment_metadata_update(
                ...
                request_id,
                {
                    "enrichment_job_id": str(job.id),
                    "enrichment_message": _localized_fallback_text(
                        language, "enrichment_partial"
                    )
                    if warnings
                    else _localized_fallback_text(language, "enrichment_completed"),
        ...
        _sync_lead_to_crm(db, job.user_id, workspace, lead)
        completed = complete_job(
            db, job, partial=bool(warnings), warnings=warnings, claim_token=claim_token
        )
        _lead_trace(
            request_id,
            "durable_auto_enrichment_finished",
            lead_id=str(lead.id),
            job_id=str(job.id),
            status=status,
...
def process_deep_contact_search_job(
    job_id: UUID, claim_token: str | None = None
) -> bool:
    db = get_sessionmaker()()
    try:
        job = db.get(EnrichmentJob, job_id)
        ...
        if claim_token and (job.status != "running" or job.locked_by != claim_token):
            logger.warning(
                "Ignoring deep contact execution for stale claim job_id=%s claim_token=%s status=%s locked_by=%s",
                job_id,
                claim_token,
                job.status,
                job.locked_by,
        ...
        company = (
            db.scalar(
                select(Company).where(
                    Company.id == company_id, Company.workspace_id == job.workspace_id
                )
            )
            if company_id
            else None
        ...
        if not update_job_progress(
            ...
            message="Searching for decision makers across connected sources.",
            percent=30,
            claim_token=claim_token,
        ):
            return False
        ...
        result = run_deep_contact_search(
            domain=str(
                payload.get("domain") or company.domain or company.website or ""
            ),
        ...
        if not update_job_progress(
            ...
            message="Selecting and verifying the strongest contact email.",
            percent=70,
            claim_token=claim_token,
        ):
            return False
        ...
        _apply_deep_contact_result(
            db, request, job.user_id, workspace, company, lead, result
        )
        _record_action_policy_success(
            db,
            crm_policy,
            {
                ...
                "lead_id": str(lead.id),
                "company_id": str(company.id),
                "verified_email": bool(result.verified_email),
            },
        )
        db.commit()
        ...
        completed = complete_job(
            db, job, partial=partial, warnings=warnings, claim_token=claim_token
        )
        _lead_trace(
            request_id,
...
def mark_enrichment_job_failed(
    job_id: UUID, exc: Exception, *, final: bool = False
) -> None:
    db = get_sessionmaker()()
    try:
        job = db.get(EnrichmentJob, job_id)
        ...
        if job.job_type == "deep_contact_search":
            ...
            company = (
                db.scalar(
                    select(Company).where(
                        ...
                        Company.workspace_id == job.workspace_id,
                    )
                )
                if company_id
                else None
            ...
            if company is not None:
                _set_company_metadata_stage(
                    company,
                    "decision_maker",
                    "error",
                    "Deep contact search could not finish. Retry or add a contact manually.",
                )
                _set_company_metadata_stage(
                    company, "verified_email", "error", "No verified email was saved."
                )
                _set_company_metadata_stage(
                    company,
                    "technographics",
            ...
            _set_workflow_stage(
                lead, "verified_email", "error", "No verified email was saved."
            )
            _sync_lead_to_crm(db, job.user_id, workspace, lead)
            _record_action_policy_success(
                db,
                crm_policy,
                {"job_id": str(job.id), "lead_id": str(lead.id), "failed": True},
            )
            db.commit()
            return
        ...
        lead.notes = _merge_lead_metadata(
            ...
            _enrichment_metadata_update(
                "error",
                job.request_id or str(job.id),
                {
                    "enrichment_message": _localized_fallback_text(
                        workspace.language if workspace else job.language,
                        "enrichment_failed",
                    )
                },
            ),
...
def _lead_prioritization_from_metadata(
    *,
    ...
    now = datetime.utcnow()
    decision_intel = (
        metadata.get("decision_maker_intelligence")
        if isinstance(metadata.get("decision_maker_intelligence"), dict)
        else {}
    )
    profiles = (
        decision_intel.get("profiles")
        if isinstance(decision_intel.get("profiles"), list)
        else []
    )
    top_profile = profiles[0] if profiles and isinstance(profiles[0], dict) else {}
    ...
    website_activity_score = _safe_score(
        25 + min(70, len(website_activity_signals) * 20), 20
    )

    last_enriched = _coerce_dt(
        metadata.get("last_enriched_at")
        or metadata.get("company_intelligence_cached_at")
    )
    ...
    age_days = (
        max(0, int((now - last_enriched).total_seconds() // 86400))
        if last_enriched
        else 999
    )
    freshness = _safe_score(
        95
        if age_days <= 1
        else 82
        if age_days <= 3
    ...
    raw_weights = (
        ...
        if isinstance(prioritization_weights, dict)
        else DEFAULT_PRIORITIZATION_WEIGHTS
    )
    total_weight = sum(
        max(0.0, float(raw_weights.get(name) or 0.0))
    ...
    missing_ratio = sum(1 for flag in missing_data_flags if flag) / max(
        1, len(missing_data_flags)
    )
    ...
    *, workspace_id: UUID | None = None, initiated_by_user_id: str = ""
    ...
    try:
        ...
        for company in companies:
            ...
            lead = (
                db.scalar(
                    select(Lead).where(
                        ...
                        Lead.workspace_id == company.workspace_id,
                    )
                )
                if company.lead_id
                else None
            ...
            latest_changes = [
                item
                for item in monitoring.get("latest_changes", [])
                if isinstance(item, dict)
            ]
            ...
            workspace = (
                db.get(Workspace, company.workspace_id)
            ...
            from app.services.revenue_intelligence import (
                build_and_store_revenue_intelligence,
                create_revenue_notification_if_needed,
            )

            workspace_companies = [
                item for item in companies if item.workspace_id == company.workspace_id
            ]
            revenue_snapshot = build_and_store_revenue_intelligence(
                db, company=company, companies=workspace_companies
            )
            create_revenue_notification_if_needed(
                db,
                company=company,
            ...
            changed.append(
                {
                    ...
                    "detected_at": str(
                        monitoring.get("generated_at") or datetime.utcnow().isoformat()
                    ),
...
def run_company_monitoring(
    user: WorkspaceUserContext, db: Session = Depends(get_db)
) -> UsageMonitoringOut:
    workspace = _current_workspace(db, user.user_id, user.email)
    monitored_count = int(
        db.scalar(
            select(func.count(Company.id)).where(Company.workspace_id == workspace.id)
        )
        or 0
    )
...
def run_nightly_lead_prioritization_once() -> int:
    ...
    try:
        companies = list(
            db.scalars(
                select(Company).order_by(Company.updated_at.desc()).limit(2000)
            ).all()
        )
                ...
                if workspace_key not in weights_by_workspace:
                    settings = db.scalar(
                        select(AppSettings).where(
                            AppSettings.workspace_id == company.workspace_id
                        )
                    )
                    if settings is not None:
                        learning = continuous_learning_weights(
                            settings.ai if isinstance(settings.ai, dict) else {}
                        )
                        weights_by_workspace[workspace_key] = (
                            learning.get("prioritization")
                            if isinstance(learning.get("prioritization"), dict)
                            else DEFAULT_PRIORITIZATION_WEIGHTS.copy()
                        )
                    else:
                        weights_by_workspace[workspace_key] = (
                            DEFAULT_PRIORITIZATION_WEIGHTS.copy()
                        )
...
def _enqueue_auto_enrichment(
    ...
    *,
    ...
    if (
        ...
        or os.getenv("OUTREACHAI_SYNC_ENRICHMENT") == "true"
    ):
        for job in jobs:
...
def _complete_public_company_details(
    db: Session, request: Request, user_id: str, workspace, lead: Lead, request_id: str
) -> bool:
    metadata = _lead_metadata(lead)
    ...
    if (
        ...
        and metadata.get("google_maps_url")
    ):
        return False
    try:
        _lead_trace(
            request_id,
            "google_place_details_started",
            lead_id=str(lead.id),
            company=lead.company,
            place_id=place_id,
        )
        ...
        capture_provider_exception(
            ...
            endpoint="workspace_app.place_details",
            workspace_id=workspace.id,
            lead_id=lead.id,
            extra={"request_id": request_id, "place_id": place_id},
        )
        _lead_trace(
            request_id,
    ...
    updates = {
        key: value for key, value in details.items() if _metadata_value_present(value)
    }
    if not updates:
        return False
    ...
    updates["domain"] = (
        str(
            updates.get("domain")
            or _domain_from_website(lead.website)
            or metadata.get("domain")
            or ""
        )
        or None
    ...
    _add_lead_activity(
        ...
        user_id,
        workspace,
        "company.public_details_completed",
        lead,
        {"source": "turnkey_lead_research"},
    )
    return True
...
def _complete_public_details_for_search_results(
    leads: list[LeadOut], request_id: str
) -> list[LeadOut]:
    ...
    for lead in leads:
        ...
        updates = {
            ...
            if _metadata_value_present(value)
        }
        if not updates:
            completed.append(lead)
            continue
        completed.append(
            lead.model_copy(
                update={
                    ...
                    or updates.get("business_category"),
...
def _preserve_search_public_details(
    original: list[LeadOut], enriched: list[LeadOut]
) -> list[LeadOut]:
        ...
        preserved.append(
            lead.model_copy(
                update={
                    ...
                    "business_category": lead.business_category
                    or source_lead.business_category,
                    "notes": _merge_metadata_notes_for_lead_out(
                        lead, {**public_metadata, **_lead_metadata(lead)}
                    ),
                }
            )
        )
    ...
    lead.notes = _merge_lead_metadata(
        lead,
        {
            **metadata,
            ...
            "contact_search_status": "verified_email_found"
            if enriched.email
            else metadata.get("hunter_status", "no_verified_email"),
            "contact_search_message": "Verified contact saved to CRM."
            if enriched.email
            else "No verified business email was found yet. Add a decision maker manually or continue with research.",
            "decision_maker_roles_searched": list(DECISION_MAKER_TITLES),
            "hunter_status": enriched.hunter_status
            or metadata.get("hunter_status")
            or ("verified" if enriched.email else "no_verified_email"),
            "hunter_verified": bool(
                enriched.hunter_verified or metadata.get("hunter_verified")
            ),
            "email_status": "Verified"
            if enriched.hunter_verified or metadata.get("hunter_verified")
            else ("Found" if enriched.email or lead.email else "No verified email"),
            "contact_found_at": datetime.utcnow().isoformat()
            if enriched.email or lead.email
            else metadata.get("contact_found_at"),
        },
    )


def _set_company_metadata_stage(
    company: Company, key: str, status: str, message: str
) -> None:
...
def _apply_deep_contact_result(
    db: Session,
    request: Request,
    user_id: str,
    workspace,
    ...
    metadata["contact_search_status"] = (
        "verified_email_found" if result.verified_email else "no_verified_email"
    )
    ...
    company.domain = company.domain or normalize_domain(
        str(profile.get("domain") or company.website or "")
    )
    company.website = company.website or str(
        profile.get("website")
        or (f"https://{company.domain}" if company.domain else "")
    )
    ...
    if selected:
        contact = _upsert_deep_contact(
            ...
            lead,
            selected,
            selected.email == result.verified_email and bool(result.verified_email),
        )
        if result.verified_email:
    ...
    for candidate in result.candidates:
        if selected and (candidate.email or candidate.linkedin or candidate.name) == (
            selected.email or selected.linkedin or selected.name
        ):
            continue
    ...
    if lead:
        lead.notes = _merge_lead_metadata(
            lead,
            {
                **metadata,
                ...
                "contact_found_at": now.isoformat()
                if result.verified_email
                else _lead_metadata(lead).get("contact_found_at"),
        ...
        _set_workflow_stage(
            lead,
            "decision_maker",
            "completed" if selected else "error",
            "Decision maker selected."
            if selected
            else "Choose or add a decision maker manually.",
        )
        _set_workflow_stage(
            lead,
            "verified_email",
            "completed" if result.verified_email else "error",
            "Verified business email saved."
            if result.verified_email
            else "No verified email was found.",
        )
        _add_lead_activity(
            db,
            request,
            ...
            "contact.deep_search",
            lead,
            {"status": result.status, "verified": bool(result.verified_email)},
        )
        _sync_lead_to_crm(db, user_id, workspace, lead)
    ...
    _set_company_metadata_stage(
        company,
        "technographics",
        "completed" if result.technologies else "error",
        "Technology stack saved."
        if result.technologies
        else "Technology stack unavailable.",
    )
    if lead:
        _refresh_company_intelligence(db, user_id, workspace, lead, company)


def _upsert_deep_contact(
    db: Session,
    user_id: str,
    workspace,
    company: Company,
    ...
    if candidate.email:
        contact = db.scalar(
            select(Contact)
            .where(
                Contact.workspace_id == workspace.id, Contact.email == candidate.email
            )
            .order_by(Contact.updated_at.desc())
            .limit(1)
        )
    ...
    contact.email_status = (
        "Verified"
        if selected and candidate.email
        else ("Unverified" if candidate.email else "Unknown")
    )
    contact.metadata_json = {
        **(contact.metadata_json or {}),
...
def _integration_status(
    key: str, label: str, configured: bool, ready_message: str, missing_message: str
...
def _scoped_memory_entry(
    db: Session, workspace_id: UUID, memory_id: UUID
) -> AIMemoryEntry:
    entry = db.scalar(
        select(AIMemoryEntry).where(
            AIMemoryEntry.id == memory_id, AIMemoryEntry.workspace_id == workspace_id
        )
    )
    if entry is None:
...
def _memory_entry_out(entry: AIMemoryEntry) -> dict[str, Any]:
    return {
        ...
        if isinstance(entry.memory_type, AIMemoryType)
...
def _latest_website_analysis_for_lead(
    db: Session, workspace_id: UUID, lead_id: UUID | None
    ...
    return db.scalar(
        ...
        .where(
            WebsiteAnalysis.workspace_id == workspace_id,
            WebsiteAnalysis.lead_id == lead_id,
        )
...
def _save_ai_sales_analysis_snapshot(
    ...
    *,
    ...
    try:
        ...
        logger.warning(
            "AI sales snapshot storage unavailable; skipping table persistence",
            exc_info=exc,
        )
        db.rollback()
        capture_provider_exception(
            exc,
            provider="database",
            endpoint="workspace_app.ai_sales_analysis.snapshot_write",
        )
        return None
    if snapshot is None:
    ...
    snapshot.evidence_json = [
        item for item in analysis_payload.get("evidence", []) if isinstance(item, dict)
    ]
...
def _metadata_ai_sales_analysis_by_version(
    company: Company, version: int
...
def _metadata_ai_sales_analysis_versions(
    company: Company,
) -> list[UsageAISalesAnalysisVersionOut]:
    return [
...
def _store_ai_sales_analysis_metadata(
    company: Company, analysis: dict[str, Any]
) -> None:
    metadata = company.metadata_json if isinstance(company.metadata_json, dict) else {}
    history = [
        item
    ...
    company.metadata_json = {
        **metadata,
        ...
        "ai_sales_workspace_updated_at": _analysis_visible_timestamp(analysis)
...
def _analysis_version_summary(
    snapshot: AISalesWorkspaceAnalysis,
) -> UsageAISalesAnalysisVersionOut:
    analysis = (
        snapshot.analysis_json if isinstance(snapshot.analysis_json, dict) else {}
    )
...
def _list_ai_sales_snapshot_versions(
    db: Session, workspace_id: UUID, company_id: UUID
) -> list[UsageAISalesAnalysisVersionOut]:
    try:
        snapshots = list(
            db.scalars(
                ...
                .order_by(
                    AISalesWorkspaceAnalysis.version_number.desc(),
                    AISalesWorkspaceAnalysis.updated_at.desc(),
                )
            ).all()
        )
    except SQLAlchemyError as exc:
        logger.warning(
            "AI sales snapshot version list failed; using metadata cache", exc_info=exc
        )
        db.rollback()
        capture_provider_exception(
            exc,
            provider="database",
            endpoint="workspace_app.ai_sales_analysis.snapshot_list",
        )
        return []
    return [_analysis_version_summary(snapshot) for snapshot in snapshots]


def _available_ai_sales_versions(
    db: Session, workspace_id: UUID, company: Company
) -> list[UsageAISalesAnalysisVersionOut]:
    ...
    *,
    ...
    if force:
        ...
        if not str(stamped.get("generated_at") or "").strip():
            stamped["generated_at"] = now
        stamped.setdefault(
            "analysis_timestamp", str(stamped.get("generated_at") or now)
        )
...
def _analysis_content_key(analysis_payload: dict[str, Any]) -> dict[str, Any]:
    normalized = (
        read_cached_analysis({"ai_sales_workspace": analysis_payload})
...
def _fallback_ai_sales_analysis(
    *,
    ...
    decision_maker = {
        ...
        "title": (
            top_contact.title if top_contact and top_contact.title else ""
        ).strip(),
        "email": (
            top_contact.email
            if top_contact and top_contact.email
            else (company.email or "")
        ).strip(),
    }
    ...
    if decision_maker["email"]:
        evidence.append(
            {
                "source": "decision_maker.email",
                "source_field": "decision_maker.email",
                "value": decision_maker["email"],
                "confidence": 94,
                "evidence_type": "verified_source",
                "verified": True,
            }
    ...
    return {
        ...
        "business_model": (company.industry or "").strip()
        or "Business model needs additional enrichment.",
        "target_customers": (
            lead.company if lead and lead.company else company.industry or ""
        ).strip(),
        "opportunity_score": opportunity_score,
        "buying_intent_score": buying_intent_score,
        "confidence_score": confidence_score,
        "decision_maker": decision_maker,
        "recommended_decision_maker_role": decision_maker["title"]
        or "Founder or revenue owner",
        "outreach_angle": (company.sales_angle or "").strip()
        or "Start with a short intro tied to one measurable business outcome.",
...
def _analysis_recommendation_defaults(analysis: dict[str, Any]) -> dict[str, Any]:
    decision_maker = (
        analysis.get("decision_maker")
        if isinstance(analysis.get("decision_maker"), dict)
        else {}
    )
    ...
    evidence = (
        analysis.get("evidence") if isinstance(analysis.get("evidence"), list) else []
    )
    ...
    return {
        "decision_maker": _entry(
            "Best decision maker",
            {
                ...
                "recommended_role": str(
                    analysis.get("recommended_decision_maker_role") or ""
                ),
            },
        ),
        "first_message": _entry(
            ...
            str(
                analysis.get("recommended_first_message")
                or analysis.get("personalized_opening_line")
                or ""
            ),
        ),
        "follow_up_sequence": _entry(
            "Follow-up sequence",
            analysis.get("personalized_follow_up_sequence")
            if isinstance(analysis.get("personalized_follow_up_sequence"), list)
            else [],
        ),
        "best_channel": _entry(
            "Best outreach channel",
            str(analysis.get("best_communication_channel") or ""),
        ),
        "reply_probability": _entry(
            "Reply probability",
            max(0, min(100, int(analysis.get("estimated_reply_probability") or 0))),
        ),
        "deal_success_probability": _entry(
            "Deal success probability",
            max(0, min(100, int(analysis.get("buying_probability") or 0))),
        ),
        "priority_score": _entry(
            "Priority score",
            max(0, min(100, int(analysis.get("lead_priority_score") or 0))),
        ),
        "next_best_action": _entry(
            "Next best action",
            str(
                analysis.get("recommended_next_action")
...
def _ensure_analysis_recommendation_structures(
    analysis: dict[str, Any],
) -> dict[str, Any]:
    normalized = read_cached_analysis({"ai_sales_workspace": analysis})
    recommendation_actions = (
        normalized.get("recommendation_actions")
        if isinstance(normalized.get("recommendation_actions"), dict)
        else {}
    )
    ...
    for key, value in defaults.items():
        ...
        merged_actions[key] = (
            {**value, **existing} if isinstance(existing, dict) else value
        )
    normalized["recommendation_actions"] = merged_actions

    copilot = (
        normalized.get("ai_copilot_panel")
        if isinstance(normalized.get("ai_copilot_panel"), dict)
        else {}
    )
    recommendations = []
        ...
        recommendations.append(
            {
                ...
                "reasoning": str(
                    item.get("reasoning") or normalized.get("score_explanation") or ""
                ),
    ...
    normalized["ai_copilot_panel"] = {
        **copilot,
        "generated_at": str(
            copilot.get("generated_at") or datetime.utcnow().isoformat()
        ),
        "summary": str(
            copilot.get("summary")
            or normalized.get("summary")
            or normalized.get("company_summary")
            or ""
        ),
        "confidence": max(
            1,
            min(
                100,
                int(
                    copilot.get("confidence")
                    or normalized.get("confidence_score")
                    or 70
                ),
            ),
        ),
        "reasoning": copilot.get("reasoning")
        if isinstance(copilot.get("reasoning"), list)
        else (
            normalized.get("reasoning")
            if isinstance(normalized.get("reasoning"), list)
            else []
        ),
        "recommendations": recommendations,
        "evidence": copilot.get("evidence")
        if isinstance(copilot.get("evidence"), list)
        else (
        ...
        "policy": str(
            copilot.get("policy")
            or "Every recommendation is evidence-backed, confidence-scored, editable, and auditable."
        ),
    }
    normalized["recommendation_audit_log"] = (
        normalized.get("recommendation_audit_log")
        if isinstance(normalized.get("recommendation_audit_log"), list)
        else []
    )
    return normalized
...
def _apply_recommendation_action(
    ...
    *,
    ...
    actions = (
        updated.get("recommendation_actions")
        if isinstance(updated.get("recommendation_actions"), dict)
        else {}
    )
    ...
    if action == "edit":
        ...
        if isinstance(current_value, list):
            ...
            next_value = (
                ...
                if text
                    ...
                    updated.get("recommended_next_action")
        ...
        raise HTTPException(
            status_code=400, detail="Unsupported recommendation action."
        )
    ...
    if key == "decision_maker":
        if isinstance(next_value, dict):
            updated["decision_maker"] = {
                "name": str(
                    next_value.get("name")
                    or updated.get("decision_maker", {}).get("name")
                    if isinstance(updated.get("decision_maker"), dict)
                    else ""
                ),
                "title": str(
                    next_value.get("title")
                    or updated.get("decision_maker", {}).get("title")
                    if isinstance(updated.get("decision_maker"), dict)
                    else ""
                ),
                "email": str(
                    next_value.get("email")
                    or updated.get("decision_maker", {}).get("email")
                    if isinstance(updated.get("decision_maker"), dict)
                    else ""
                ),
            }
            updated["recommended_decision_maker_role"] = str(
                next_value.get("recommended_role")
                or next_value.get("title")
                or updated.get("recommended_decision_maker_role")
                or ""
            )
        else:
            updated["recommended_decision_maker_role"] = str(
                next_value or updated.get("recommended_decision_maker_role") or ""
            )
    elif key == "first_message":
        updated["recommended_first_message"] = str(next_value or "")
    elif key == "follow_up_sequence":
        updated["personalized_follow_up_sequence"] = (
            next_value
            if isinstance(next_value, list)
            else ([str(next_value)] if str(next_value).strip() else [])
        )
        ...
        updated["lead_priority_tier"] = (
            "Hot"
            if updated["lead_priority_score"] >= 75
            else ("Warm" if updated["lead_priority_score"] >= 45 else "Cold")
        )
    ...
    audit_log = (
        updated.get("recommendation_audit_log")
        if isinstance(updated.get("recommendation_audit_log"), list)
        else []
    )
    audit_log.append(
        {
    ...
    copilot = (
        updated.get("ai_copilot_panel")
        if isinstance(updated.get("ai_copilot_panel"), dict)
        else {}
    )
    ...
    *,
    ...
    contacts = list(
        db.scalars(
            select(Contact)
            .where(
                Contact.workspace_id == workspace.id, Contact.company_id == company.id
            )
            .order_by(Contact.updated_at.desc())
            .limit(20)
        ).all()
    )
    website_analysis = _latest_website_analysis_for_lead(
        db, workspace.id, company.lead_id
    )
    ...
    try:
        memory = retrieve_memory(
            ...
            query=" ".join(
                [
                    ...
                    company.industry or "",
                    company.country or "",
                    company.ai_summary or "",
                    company.outreach_strategy or "",
                ]
            ),
            company_id=company.id,
            lead_id=lead.id if lead else None,
        ...
        capture_provider_exception(
            exc,
            provider="ai_memory",
            endpoint="workspace_app.ai_sales_analysis.auto_refresh_memory",
            workspace_id=workspace.id,
            lead_id=company.lead_id,
        )
        memory_context = memory_context_none(
            f"AI Memory retrieval failed: {type(exc).__name__}."
        )
    ...
    if previous_analysis and _analysis_content_key(
        previous_analysis
    ) == _analysis_content_key(analysis):
        _store_ai_sales_analysis_metadata(company, previous_analysis)
        return previous_analysis

    stamped = _stamp_analysis_payload(
        analysis, force=bool(previous_analysis), previous_analysis=previous_analysis
    )
...
def _latest_ai_sales_snapshot(
    ...
    try:
        ...
        logger.warning(
            "AI sales snapshot table query failed; using metadata cache", exc_info=exc
        )
        db.rollback()
        capture_provider_exception(
            exc,
            provider="database",
            endpoint="workspace_app.ai_sales_analysis.snapshot_read",
        )
        return None


def _ai_sales_snapshot_by_version(
    db: Session, workspace_id: UUID, company_id: UUID, version: int
) -> Optional[AISalesWorkspaceAnalysis]:
    try:
        return db.scalar(
            select(AISalesWorkspaceAnalysis)
        ...
        logger.warning(
            "AI sales snapshot version query failed; using metadata cache", exc_info=exc
        )
        db.rollback()
        capture_provider_exception(
            exc,
            provider="database",
            endpoint="workspace_app.ai_sales_analysis.snapshot_read_version",
        )
        return None
...
def _email_analysis_context(analysis: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "company_summary": str(
            analysis.get("company_summary") or analysis.get("summary") or ""
        ),
        "business_model": str(analysis.get("business_model") or ""),
        "icp_fit_score": int(
            analysis.get("icp_fit_score") or analysis.get("ai_lead_score") or 0
        ),
        "buying_probability": int(
            analysis.get("buying_probability")
            or analysis.get("buying_intent_score")
            or 0
        ),
        ...
        "pain_points": [
            ...
            for item in analysis.get(
                "pain_points", analysis.get("likely_business_pains", [])
            )
            if str(item or "").strip()
        ]
        if isinstance(
            analysis.get("pain_points", analysis.get("likely_business_pains", [])), list
        )
        else [],
        "decision_makers": [
            item
            for item in analysis.get("decision_makers", [])
            if isinstance(item, dict)
        ]
        if isinstance(analysis.get("decision_makers"), list)
        else (
            [analysis.get("decision_maker")]
            if isinstance(analysis.get("decision_maker"), dict)
            else []
        ...
        "outreach_angle": str(
            analysis.get("best_outreach_angle") or analysis.get("outreach_angle") or ""
        ),
        "value_proposition": str(analysis.get("value_proposition") or ""),
        "best_communication_channel": str(
            analysis.get("best_communication_channel") or ""
        ),
        "personalization_variables": [
            str(item)
            for item in analysis.get("personalization_variables", [])
            if str(item or "").strip()
        ]
        if isinstance(analysis.get("personalization_variables"), list)
        else [],
        "predicted_objections": [
            str(item)
            for item in analysis.get("predicted_objections", [])
            if str(item or "").strip()
        ]
        if isinstance(analysis.get("predicted_objections"), list)
        else [],
        "next_recommended_action": str(
            analysis.get("recommended_next_action") or analysis.get("next_action") or ""
        ),
        ...
        "suggested_cta": str(
            analysis.get("suggested_cta") or analysis.get("best_cta") or ""
        ),
    }
...
def _find_existing_company(
    db: Session, workspace_id: UUID, payload: UsageCompanyCreateIn
) -> Company | None:
    clauses: list[Any] = []
    ...
    stmt = (
        select(Company)
        .where(Company.workspace_id == workspace_id, or_(*clauses))
    ...
    if payload.name and payload.city:
        for item in candidates:
            if (
                item.name.lower() == payload.name.lower()
                and (item.city or "").lower() == payload.city.lower()
            ):
                return item
...
def get_ai_memory_settings(
    ...
    return {
        ...
        "counts_by_type": {
            str(
                memory_type.value
                if isinstance(memory_type, AIMemoryType)
                else memory_type
            ): int(count)
            for memory_type, count in by_type_rows
        },
    }


@router.patch("/ai-memory/settings")
def update_ai_memory_settings(
    payload: AIMemorySettingsIn,
    request: Request,
...
def list_ai_memory_entries(
    ...
    if lead_id and not db.scalar(
        select(Lead.id).where(Lead.id == lead_id, Lead.workspace_id == workspace.id)
    ):
...
def add_ai_memory_entry(
    payload: AIMemoryEntryIn,
    request: Request,
    user: WorkspaceUserContext,
    db: Session = Depends(get_db),
) -> dict[str, Any]:
    ...
    if payload.lead_id and not db.scalar(
    ...
    if payload.memory_type == AIMemoryType.verified_fact.value and not payload.verified:
        raise HTTPException(
            status_code=400, detail="verified_fact requires verified=true."
        )
    try:
...
def confirm_ai_memory_preference(
    payload: AIMemoryPreferenceIn,
    request: Request,
    user: WorkspaceUserContext,
    db: Session = Depends(get_db),
) -> dict[str, Any]:
...
def correct_ai_memory_entry(
    memory_id: UUID,
    payload: AIMemoryCorrectionIn,
    request: Request,
    user: WorkspaceUserContext,
    db: Session = Depends(get_db),
) -> dict[str, Any]:
    ...
    if entry.deleted_at is not None:
        raise HTTPException(
            status_code=409, detail="Deleted memory cannot be corrected."
        )
    try:
        correct_memory_entry(
            db,
        ...
        if str(exc) == "duplicate_memory":
            raise HTTPException(
                status_code=409,
                detail="Corrected memory duplicates an existing active entry.",
            ) from exc
        raise HTTPException(status_code=400, detail=str(exc)) from exc
    db.commit()
...
def clear_ai_memory(
    request: Request, user: WorkspaceUserContext, db: Session = Depends(get_db)
) -> dict[str, Any]:
    _require_ai_memory_schema_ready(db)
    workspace = _current_workspace(db, user.user_id, user.email)
    entries = list(
...
def explain_ai_memory_decision(
    company_id: UUID,
    user: WorkspaceUserContext,
    ...
    snapshot = (
        _ai_sales_snapshot_by_version(db, workspace.id, company.id, version)
        if version
        else _latest_ai_sales_snapshot(db, workspace.id, company.id)
    )
    analysis = (
        snapshot.analysis_json
        if snapshot and isinstance(snapshot.analysis_json, dict)
        else _cached_ai_sales_analysis_from_company(company)
    )
    return explain_memory_context(analysis)


@router.get("/bootstrap", response_model=UsageBootstrapOut)
def bootstrap_workspace_app(
    user: WorkspaceUserContext, db: Session = Depends(get_db)
) -> UsageBootstrapOut:
    workspace = _current_workspace(db, user.user_id, user.email)
    ...
    recent = [
        item
        for item in recent
        if not _is_production_smoke_test_company(item)
        and _is_customer_visible_company(item)
    ][:5]
    activity = list(
        db.scalars(
            select(AuditLog)
    ...
    return UsageBootstrapOut(
        ...
        recent_companies=[
            _crm_company_out(db, workspace, user.user_id, item) for item in recent
        ],
        ...
        integrations=[
            ...
            _integration_status(
                ...
                bool(
                    ...
                    and settings.stripe_agency_price_id
                ),
...
def create_production_email_smoke_test(
    payload: ProductionEmailSmokeTestCreateIn,
    request: Request,
    user: WorkspaceUserContext,
    db: Session = Depends(get_db),
) -> ProductionEmailSmokeTestOut:
    ...
    if not payload.confirmed_recipient_control:
        raise HTTPException(
            status_code=409,
            detail="Confirm that you control this recipient email before creating test records.",
        )
    if _is_placeholder_recipient(recipient):
        raise HTTPException(
            status_code=400,
            detail="Use a real recipient email that you control, not a placeholder address.",
        )
    active_smoke = [
        lead
        for lead in db.scalars(
    ...
    if active_smoke:
        raise HTTPException(
            status_code=409,
            detail="Cleanup the existing production smoke-test record before creating another one.",
        )
    existing_real = [
        lead
        for lead in db.scalars(
            select(Lead).where(
                Lead.workspace_id == workspace.id, func.lower(Lead.email) == recipient
            )
        ).all()
        if not _is_production_smoke_test_lead(lead)
    ]
    if existing_real:
        raise HTTPException(
            status_code=409,
            detail="This recipient already belongs to a real CRM lead. Enter a separate owner-controlled test recipient.",
        )
    ...
    smoke_test_id = uuid4()
    smoke_policy = _enforce_action_policy(
        db,
        workspace=workspace,
        user_id=user.user_id,
        action_name="email.draft.create",
        ...
        input_payload={
            "route": "workspace_app.production_smoke.create",
            "smoke_test_id": str(smoke_test_id),
            "recipient_email": recipient,
    ...
    _add_lead_activity(
        ...
        user.user_id,
        workspace,
        "lead.production_smoke_test_created",
        lead,
        {"email_id": str(draft.id), **metadata},
    )
    db.add(
        AuditLog(
            user_id=user.user_id,
...
@router.get(
    "/production-email-smoke-test/active", response_model=ProductionEmailSmokeTestOut
)
def get_active_production_email_smoke_test(
    user: WorkspaceUserContext, db: Session = Depends(get_db)
) -> ProductionEmailSmokeTestOut:
    _require_production_smoke_owner(db, user=user)
    workspace = _current_workspace(db, user.user_id, user.email)
    ...
    return ProductionEmailSmokeTestOut(
        status="success",
        message="Active production smoke-test state loaded."
        if active
        else "No active production smoke-test records for this workspace.",
        smoke_test=active,
    )


@router.post(
    "/production-email-smoke-test/cleanup", response_model=ProductionEmailSmokeTestOut
)
def cleanup_production_email_smoke_test(
    payload: ProductionEmailSmokeTestCleanupIn,
    request: Request,
    user: WorkspaceUserContext,
    db: Session = Depends(get_db),
) -> ProductionEmailSmokeTestOut:
    _require_production_smoke_owner(db, user=user)
    workspace = _current_workspace(db, user.user_id, user.email)
    leads, companies, emails, activities = _smoke_test_cleanup_scope(
        db, workspace_id=workspace.id, smoke_test_id=payload.smoke_test_id
    )
    draft_emails = [
        email
        for email in emails
        if email.sent_at is None and not email.provider_message_id
    ]
    memories = _smoke_test_memory_cleanup_scope(
        db,
        workspace_id=workspace.id,
    ...
    if not leads and not companies and not emails and not activities and not memories:
        if _production_smoke_exists_outside_workspace(
            db, workspace_id=workspace.id, smoke_test_id=payload.smoke_test_id
        ):
            raise HTTPException(
                status_code=404,
                detail="Production smoke-test record not found in this workspace.",
            )
        return ProductionEmailSmokeTestOut(
            status="success",
    ...
    cleanup_policy = _enforce_action_policy(
        db,
        workspace=workspace,
        user_id=user.user_id,
        action_name="crm.write",
        input_payload={
            "route": "workspace_app.production_smoke.cleanup",
            "smoke_test_id": str(payload.smoke_test_id),
            "draft_count": len(draft_emails),
            "lead_count": len(leads),
            "company_count": len(companies),
        },
        resource_id=payload.smoke_test_id,
    )
    now = datetime.utcnow()
    for entry in memories:
        ...
        log_memory_event(
            ...
            entry_id=entry.id,
            metadata={
                "source": "production_smoke_test.cleanup",
                "smoke_test_id": str(payload.smoke_test_id),
            },
        )
        deleted["memories"] += 1
    ...
    db.add(
        AuditLog(
            ...
            metadata_json={
                "smoke_test_id": str(payload.smoke_test_id),
                "deleted": deleted,
                "preserved_sent_email_records": max(0, len(emails) - len(draft_emails)),
            },
        )
    )
    _record_action_policy_success(
        db,
        cleanup_policy,
        {"smoke_test_id": str(payload.smoke_test_id), "deleted": deleted},
    )
    db.commit()
    return ProductionEmailSmokeTestOut(
        status="success",
...
@router.get(
    "/internal-email-smoke-draft/config",
    response_model=InternalEmailSmokeDraftConfigOut,
)
def get_internal_email_smoke_draft_config(
    user: WorkspaceUserContext, db: Session = Depends(get_db)
) -> InternalEmailSmokeDraftConfigOut:
    _current_workspace(db, user.user_id, user.email)
    allowed_recipients = _internal_email_smoke_allowed_recipients()
    return InternalEmailSmokeDraftConfigOut(
        recipient_email=allowed_recipients[0] if len(allowed_recipients) == 1 else ""
    )


@router.post("/internal-email-smoke-draft", response_model=ProductionEmailSmokeTestOut)
def create_internal_email_smoke_draft(
    request: Request,
    user: WorkspaceUserContext,
    payload: InternalEmailSmokeDraftCreateIn = Depends(
        _internal_email_smoke_draft_payload
    ),
    db: Session = Depends(get_db),
) -> ProductionEmailSmokeTestOut:
    workspace = _current_workspace(db, user.user_id, user.email)
    ...
    if not payload.confirmed_recipient_control:
        raise HTTPException(
            status_code=409,
            detail="Confirm that you control this recipient email before creating internal test records.",
        )
    _validate_internal_email_smoke_recipient(recipient)

    active_smoke = _active_internal_email_smoke_draft_lead(
    ...
    existing_real = [
        ...
        if not _is_internal_email_smoke_draft_lead(lead)
        and not _is_production_smoke_test_lead(lead)
    ]
    if existing_real:
        raise HTTPException(
            status_code=409,
            detail="This recipient already belongs to a CRM lead. Use only the isolated controlled smoke recipient.",
        )

    sender_status = _internal_email_smoke_sender_status(
        db, user_id=user.user_id, workspace=workspace
    )
    smoke_test_id = uuid4()
    smoke_policy = _enforce_action_policy(
        db,
        ...
        required_permissions=("crm:write", "email:draft"),
        input_payload={
            "route": "workspace_app.internal_smoke_draft.create",
            "smoke_test_id": str(smoke_test_id),
            "recipient_email": recipient,
            "sender_email": sender_status.sender_email,
        },
    ...
    lead = Lead(
        ...
        campaign_id=None,
        sales_employee_id=None,
        company=f"Internal email smoke draft {smoke_test_id}",
        website=None,
        industry="Internal QA",
        country=workspace.target_country or "",
        city="",
        contact="Controlled internal smoke recipient",
        email=recipient,
        status=LeadStatus.email_generated,
        notes=json.dumps(
            {
                **metadata,
                ...
                "workflow_stage_messages": {
                    "company_profile": "Internal smoke-test profile for the authenticated workspace.",
                    "website_analysis": "Skipped for isolated internal smoke-test record.",
                    "decision_maker": "Controlled internal recipient supplied explicitly.",
                    "verified_email": "Recipient matched the current controlled internal smoke alias.",
                    "ai_email": "Internal smoke-test draft prepared.",
                    "approval": "Manual approval and separate final send confirmation are required.",
                },
            },
            sort_keys=True,
        ),
    ...
    try:
        ...
        raise HTTPException(
            status_code=409,
            detail="An internal email smoke draft already exists or is being created. Retry after refreshing the workspace.",
        ) from None
    company = Company(
        user_id=user.user_id,
        workspace_id=workspace.id,
        ...
        industry="Internal QA",
        country=workspace.target_country or "",
        ai_summary="Internal non-owner smoke-test record. Not a real customer or discovered contact.",
        suggested_offer="Internal test only.",
        outreach_strategy="Manual draft review, manual approval, and separate final send confirmation.",
        sales_angle="Internal manual approval and reply tracking verification only.",
        expected_reply_rate="Not counted",
        email_status="Draft Ready",
        crm_stage="Internal Test",
        metadata_json=metadata,
    ...
    draft = EmailMessage(
        ...
        direction="outbound",
        recipient_email=recipient,
        subject=f"[OutreachAI Internal Smoke Draft] {smoke_test_id}",
        preview="Internal non-owner smoke-test draft. This is not customer outreach.",
        body=(
            "Internal OutreachAI non-owner smoke-test draft.\n\n"
            f"Smoke test ID: {smoke_test_id}\n"
            f"Workspace: {workspace.name}\n"
            f"Sender: {sender_status.sender_email or 'not configured'} via {sender_status.provider}\n"
            f"Recipient: {recipient}\n\n"
            "This draft is created only for manual approval, explicit send confirmation, and reply tracking verification."
        ),
        cta="Internal test only",
        tags={
            **metadata,
    ...
    _add_lead_activity(
        ...
        user.user_id,
        workspace,
        "lead.internal_email_smoke_draft_created",
        lead,
        {"email_id": str(draft.id), **metadata},
    )
    db.add(
        AuditLog(
            user_id=user.user_id,
            workspace_id=workspace.id,
            action="internal_email_smoke_draft.created",
            metadata_json={
                "smoke_test_id": str(smoke_test_id),
                "lead_id": str(lead.id),
                "company_id": str(company.id),
...
def create_company(
    payload: UsageCompanyCreateIn,
    request: Request,
    user: WorkspaceUserContext,
    db: Session = Depends(get_db),
) -> UsageCompanyCreateOut:
    workspace = _current_workspace(db, user.user_id, user.email)
    crm_policy = _enforce_action_policy(
        ...
        action_name="crm.write",
        input_payload={
            "route": "workspace_app.companies.create",
    ...
    if existing_company:
        if existing_company.lead_id:
            existing_lead = db.scalar(
                select(Lead).where(
                    Lead.id == existing_company.lead_id,
            ...
            if existing_lead:
                _ensure_b2b_opportunity_metadata(
                    existing_lead,
                    workspace,
                    source="manual_company_reused_fallback",
                    language=_workspace_language(request, workspace),
                )
                existing_company = _sync_lead_to_crm(
                    db, user.user_id, workspace, existing_lead
                )
                _record_action_policy_success(
                    db,
                    crm_policy,
    ...
    if existing_lead:
        ...
        company = _ensure_minimal_company(
            db, user.user_id, workspace, existing_lead, metadata
        )
        try:
            ...
            capture_provider_exception(
                exc,
                provider="postgresql",
                endpoint="workspace_app.company_sync_existing",
            )
        _record_action_policy_success(
            db,
            crm_policy,
    ...
    _ensure_b2b_opportunity_metadata(
        lead,
        workspace,
        source="manual_company_fallback",
        language=_workspace_language(request, workspace),
    )
    ...
    try:
        ...
        capture_provider_exception(
            exc, provider="postgresql", endpoint="workspace_app.company_sync"
        )
    if payload.address:
        company.address = payload.address

    request_id = request.headers.get("x-request-id") or str(uuid4())
    _mark_auto_enrichment_queued(
        lead, request_id, _workspace_language(request, workspace)
    )
    enqueue_company_enrichment_job(
        db,
        user_id=user.user_id,
        ...
        language=_workspace_language(request, workspace),
        max_attempts=max(
            1, min(5, int(get_settings().enrichment_max_retries or 2) + 1)
        ),
    )
...
def search_first_customers(
    payload: FirstCustomerSearchIn,
    request: Request,
    user: WorkspaceUserContext,
    db: Session = Depends(get_db),
) -> CustomerFinderJobOut:
    ...
    try:
        ...
        capture_provider_exception(
            exc,
            provider="google_maps",
            endpoint="workspace_app.leads.first_customers.search",
            workspace_id=workspace.id,
            extra={"request_id": request_id},
        )
        raise HTTPException(
            status_code=503,
            detail="First-customer search is temporarily unavailable. Add the search key or try again later.",
        ) from exc
    except ValueError as exc:
        db.rollback()
        raise HTTPException(status_code=400, detail=str(exc)) from exc
...
@router.post(
    "/leads/first-customers/results/{result_id}/save",
    response_model=CustomerFinderResultActionOut,
)
def save_first_customer_result(
    result_id: UUID, user: WorkspaceUserContext, db: Session = Depends(get_db)
) -> CustomerFinderResultActionOut:
    workspace = _current_workspace(db, user.user_id, user.email)
    crm_policy = _enforce_action_policy(
        db,
        ...
        action_name="crm.write",
        input_payload={
            "route": "workspace_app.first_customer_result.save",
    ...
    try:
        result = save_first_customer_result_to_crm(
            db, workspace_id=workspace.id, result_id=result_id
        )
    except ValueError as exc:
    ...
    _record_action_policy_success(
        ...
        crm_policy,
        {
            ...
            "lead_id": str(result.lead_id) if result.lead_id else "",
            "company_id": str(result.company_id) if result.company_id else "",
        },
    )
    db.commit()
    return CustomerFinderResultActionOut(
        ...
        result=first_customer_result_out(result),
    )
...
def _search_leads_impl(
    ...
    *,
    ...
    try:
        _lead_trace(
            request_id,
            "google_places_started",
            timeout_seconds=LEAD_PROVIDER_TIMEOUT_SECONDS,
        )
        result = _run_provider_with_deadline(
            request_id,
            "google_places",
        ...
        logger.warning(
            "Workspace app lead search provider failed request_id=%s reason=%s",
            request_id,
            exc,
        )
        capture_provider_exception(
            exc,
            provider="google_maps",
            endpoint="workspace_app.leads.search",
            workspace_id=workspace.id,
            extra={"request_id": request_id},
        )
    ...
    try:
        ...
        _lead_trace(
            ...
            "hunter_finished",
            leads=len(enriched),
            verified=sum(1 for item in enriched if item.hunter_verified),
        )
    except Exception as exc:
        capture_provider_exception(
            exc,
            provider="hunter",
            endpoint="workspace_app.hunter.enrichment",
            workspace_id=workspace.id,
            extra={"request_id": request_id},
        )
        warnings.append(
            "Contacts could not be verified right now. Companies were saved without verified emails."
        )
        enriched = found
    ...
    duplicates_skipped = sum(
        ...
        if _existing_duplicate_lead(db, workspace, user.user_id, item) is not None
    )
    ...
    if saved:
        ...
        _lead_trace(
            request_id,
            "auto_enrichment_batch_queued",
            leads=len(queued_for_enrichment),
            skipped_recent=len(saved) - len(queued_for_enrichment),
            run_turnkey_research=run_turnkey_research,
        )
    ...
    for lead in saved:
        try:
            ...
            capture_provider_exception(
                exc,
                provider="postgresql",
                endpoint="workspace_app.company_output_after_search",
                workspace_id=workspace.id,
                lead_id=lead.id,
                extra={"request_id": request_id},
            )
            _lead_trace(
                request_id,
                "company_output_after_search_failed",
                lead_id=str(lead.id),
                company=lead.company,
                reason=str(exc),
            ...
            try:
                ...
                pass
            fallback_company = _ensure_minimal_company(
                db, user.user_id, workspace, lead, _lead_metadata(lead)
            )
            companies.append(_minimal_crm_company_out(fallback_company))

    db.commit()
    enrichment_ran_inline = (
        _enqueue_auto_enrichment(
            db, request, user.user_id, workspace, queued_for_enrichment, request_id
        )
    ...
    if enrichment_ran_inline:
        ...
        for lead in saved:
            refreshed = db.scalar(
                select(Lead).where(
                    Lead.id == lead.id, Lead.workspace_id == workspace.id
                )
            )
    ...
    _lead_trace(
        ...
        companies=len(companies),
        ai_ready=sum(
            1 for company in companies if company.ai_summary and company.suggested_offer
        ),
...
def search_leads_from_command(
    payload: UsageLeadCommandIn,
    request: Request,
    user: WorkspaceUserContext,
    db: Session = Depends(get_db),
) -> UsageLeadCommandOut:
    ...
    return UsageLeadCommandOut(
        **result.model_dump(), filters=filters, interpreted_query=payload.command
    )
...
def list_companies(
    ...
    companies = [
        company
        for company in db.scalars(
            stmt.order_by(Company.updated_at.desc()).limit(200)
        ).all()
        if not _is_production_smoke_test_company(company)
        and (company.lead_id is not None or _is_customer_visible_company(company))
    ][:100]
    batch_context = _crm_company_batch_context(db, workspace, user.user_id, companies)
    return [
        _crm_company_out(db, workspace, user.user_id, company, batch_context)
        for company in companies
    ]
...
def get_company(
    company_id: UUID, user: WorkspaceUserContext, db: Session = Depends(get_db)
) -> CrmCompanyOut:
    workspace = _current_workspace(db, user.user_id, user.email)
    company = _scoped_company(db, workspace.id, company_id)
...
@router.get(
    "/companies/{company_id}/ai-sales-analysis", response_model=UsageAISalesAnalysisOut
)
def get_ai_sales_analysis(
    company_id: UUID,
    user: WorkspaceUserContext,
    version: Optional[int] = Query(default=None, ge=1),
    db: Session = Depends(get_db),
    ...
    if version:
        if (
            snapshot
            and isinstance(snapshot.analysis_json, dict)
            and snapshot.analysis_json
        ):
        ...
        if not analysis:
            raise HTTPException(
                status_code=404, detail="AI sales analysis version not found."
            )
    else:
        analysis = (
            snapshot.analysis_json
    ...
    generated_at = _analysis_visible_timestamp(analysis)
    latest_version = (
        available_versions[0].version
        if available_versions
        else (_version_from_analysis(cached) if cached else None)
    )
    if not analysis:
...
def generate_ai_sales_analysis(
    ...
    if not payload.force:
        if cached:
            ...
            return UsageAISalesAnalysisOut(
                ...
                if available_versions
                else _version_from_analysis(cached),
                available_versions=available_versions,
            )
    ...
    lead = (
        db.scalar(
            select(Lead).where(
                Lead.id == company.lead_id, Lead.workspace_id == workspace.id
            )
        )
        if company.lead_id
        else None
    ...
    try:
        ...
        capture_provider_exception(
            exc,
            provider="ai_memory",
            endpoint="workspace_app.ai_sales_analysis.memory",
            workspace_id=workspace.id,
            lead_id=company.lead_id,
        )
        memory_context = memory_context_none(
    ...
    try:
        ...
        analysis = _stamp_analysis_payload(
            analysis, force=payload.force, previous_analysis=previous_analysis
        )
        ...
        capture_provider_exception(
            exc,
            provider="openai",
            endpoint="workspace_app.ai_sales_analysis",
            workspace_id=workspace.id,
            lead_id=company.lead_id,
        )
        if payload.force and previous_analysis:
            analysis = _stamp_analysis_payload(
                previous_analysis, force=True, previous_analysis=previous_analysis
            )
            ...
            return UsageAISalesAnalysisOut(
                ...
                latest_version=available_versions[0].version
                if available_versions
                else _version_from_analysis(analysis),
                available_versions=available_versions,
            )
        if payload.force:
            analysis = _stamp_analysis_payload(
        ...
        capture_provider_exception(
            exc,
            provider="openai",
            endpoint="workspace_app.ai_sales_analysis.unexpected",
            workspace_id=workspace.id,
            lead_id=company.lead_id,
        )
        if payload.force and previous_analysis:
    ...
    missing_data = (
        ...
        if isinstance(analysis.get("missing_data"), list)
        else []
    )
    available_versions = _available_ai_sales_versions(db, workspace.id, company)
    return UsageAISalesAnalysisOut(
        status="partial_success" if missing_data else "success",
        message="AI sales analysis generated with missing data warnings."
        if missing_data
        else "AI sales analysis generated.",
        company_id=company.id,
        analysis=analysis,
        generated_at=_analysis_visible_timestamp(analysis),
        cached=False,
...
@router.post(
    "/companies/{company_id}/ai-sales-analysis/recommendations",
    response_model=UsageAISalesAnalysisOut,
)
def update_ai_sales_recommendation(
    ...
    stamped = _stamp_analysis_payload(
        updated, force=True, previous_analysis=previous_analysis
    )
    ...
    return UsageAISalesAnalysisOut(
        ...
        latest_version=available_versions[0].version
        if available_versions
        else _version_from_analysis(stamped),
        available_versions=available_versions,
    )
...
def analyze_company(
    ...
    if not company.lead_id:
        return UsageActionOut(
            status="error",
            message="This company needs a saved lead before analysis.",
            company=_crm_company_out(db, workspace, user.user_id, company),
        )
    lead = db.scalar(
        select(Lead).where(
    ...
    try:
        _complete_public_company_details(
            db,
            request,
            user.user_id,
            workspace,
        ...
        return UsageActionOut(
            status="success",
            message="Website analysis saved.",
            company=_crm_company_out(db, workspace, user.user_id, company),
        )
    except Exception as exc:
        capture_provider_exception(
            exc,
            provider="openai",
            endpoint="workspace_app.company.analyze",
            workspace_id=workspace.id,
            lead_id=lead.id,
        )
        return UsageActionOut(
...
@router.post(
    "/companies/{company_id}/deep-contact-search",
    response_model=UsageActionOut,
    status_code=202,
)
def deep_search_company_contacts(
    ...
    domain = normalize_domain(
        company.domain or company.website or (lead.website if lead else "") or ""
    )
    if not domain:
        return UsageActionOut(
            status="error",
    ...
    crm_policy = _enforce_action_policy(
        ...
        action_name="crm.write",
        input_payload={
            "route": "workspace_app.deep_contact_search",
            "company_id": str(company.id),
    ...
    _set_company_metadata_stage(
        ...
        "decision_maker",
        "running",
        "Searching for decision makers across connected sources.",
    )
    _set_company_metadata_stage(
        company,
        "verified_email",
        "running",
        "Finding and verifying a usable business email.",
    )
    _set_company_metadata_stage(
        company, "technographics", "running", "Detecting the website technology stack."
    )
    if lead is None:
        _set_company_metadata_stage(
            company,
            "decision_maker",
            "error",
            "This company needs a saved lead before deep contact search can run.",
        )
        _set_company_metadata_stage(
            company, "verified_email", "error", "No verified email was saved."
        )
        ...
        _record_action_policy_success(
            db,
            crm_policy,
            {"company_id": str(company.id), "status": "blocked_missing_lead"},
        )
        db.commit()
        raise HTTPException(
            status_code=400,
            detail="Save this company to CRM before running deep contact search.",
        )
    ...
    _record_action_policy_success(
        db,
        crm_policy,
        {"company_id": str(company.id), "job_id": str(job.id)},
    )
    db.commit()
...
@router.get(
    "/companies/{company_id}/deep-contact-search/jobs/{job_id}",
    response_model=UsageJobStatusOut,
)
def deep_contact_search_job_status(
    company_id: UUID,
    job_id: UUID,
    user: WorkspaceUserContext,
    db: Session = Depends(get_db),
    ...
    if not job:
        raise HTTPException(
            status_code=404, detail="Deep contact search job not found."
        )
...
def discover_company_contacts(
    ...
    if not company.lead_id:
        return UsageActionOut(
            status="error",
            message="Save this company as a lead before finding contacts.",
            company=_crm_company_out(db, workspace, user.user_id, company),
        )
    lead = db.scalar(
        select(Lead).where(
    ...
    if not hunter_key_loaded():
        return UsageActionOut(
            status="provider_unavailable",
            message="Contact discovery needs setup. You can add a contact manually and continue.",
            company=_crm_company_out(db, workspace, user.user_id, company),
        )
    ...
    crm_policy = _enforce_action_policy(
        ...
        action_name="crm.write",
        input_payload={
            "route": "workspace_app.company.contacts",
            "company_id": str(company.id),
            "lead_id": str(lead.id),
        },
        resource_workspace_id=company.workspace_id,
    ...
    try:
        enriched = _hunter_enriched_leads(
            db, request, user.user_id, workspace, [_lead_out(lead)]
        )
    except Exception as exc:
        capture_provider_exception(
            exc,
            provider="hunter",
            endpoint="workspace_app.company.contacts",
            workspace_id=workspace.id,
            lead_id=lead.id,
        )
        return UsageActionOut(
            status="provider_unavailable",
            message="Contact discovery is temporarily unavailable. You can add a contact manually and continue.",
            company=_crm_company_out(db, workspace, user.user_id, company),
        )
    ...
    contact_search_status = (
        "verified_email_found" if has_verified_contact else "no_verified_email"
    )
    ...
    lead.notes = _merge_lead_metadata(
        ...
        {
            **metadata,
            "contact_found_at": contact_search_checked_at
            if has_verified_contact
            else _lead_metadata(lead).get("contact_found_at"),
            ...
            "decision_maker_roles_searched": list(DECISION_MAKER_TITLES),
            "hunter_status": enriched_lead.hunter_status
            or metadata.get("hunter_status")
            or ("verified" if enriched_lead.email else "no_verified_email"),
            "hunter_verified": bool(
                enriched_lead.hunter_verified or metadata.get("hunter_verified")
            ),
            "email_status": "Verified"
            if enriched_lead.hunter_verified or metadata.get("hunter_verified")
            else (
                "Found" if enriched_lead.email or lead.email else "No verified email"
            ),
        },
    )
    _ensure_b2b_opportunity_metadata(
        lead,
        workspace,
        source="workspace_contact_discovery",
        language=_workspace_language(request, workspace),
    )
    company = _sync_lead_to_crm(db, user.user_id, workspace, lead)
    if lead.email and lead.email != before_email:
        ...
        _add_lead_activity(
            ...
            {
                "source": "contact_discovery",
                "roles_searched": list(DECISION_MAKER_TITLES),
            },
        )
    _record_action_policy_success(
        db,
        crm_policy,
        {
            "company_id": str(company.id),
            "lead_id": str(lead.id),
            "verified_contact": bool(lead.email),
        },
    )
    db.commit()
    db.refresh(company)
...
def add_manual_company_contact(
    company_id: UUID,
    payload: UsageContactCreateIn,
    request: Request,
    user: WorkspaceUserContext,
    db: Session = Depends(get_db),
) -> UsageActionOut:
    ...
    crm_policy = _enforce_action_policy(
        ...
        action_name="crm.write",
        input_payload={
            "route": "workspace_app.company.contact_manual",
            "company_id": str(company.id),
            "lead_id": str(lead.id) if lead else "",
            "has_email": bool(clean_email),
            "has_phone": bool(clean_phone),
            "has_linkedin": bool(clean_linkedin),
        },
        resource_workspace_id=company.workspace_id,
        resource_id=company.id,
    )
    ...
    if clean_email:
        contact = db.scalar(
            select(Contact)
            .where(Contact.workspace_id == workspace.id, Contact.email == clean_email)
            .order_by(Contact.updated_at.desc())
        )
    ...
    contact.metadata_json = {
        **(contact.metadata_json or {}),
        "source": "manual",
        "updated_from": "company_workspace",
    }
    contact.updated_at = datetime.utcnow()
    ...
    if lead:
        ...
        lead.notes = _merge_lead_metadata(
            lead,
            {
                ...
                "email_status": "Found"
                if contact.email
                else _lead_metadata(lead).get("email_status", "No verified email"),
        ...
        _add_lead_activity(
            ...
            "contact.found",
            lead,
            {"source": "manual", "email_status": contact.email_status},
        )
        company = _sync_lead_to_crm(db, user.user_id, workspace, lead)
    db.flush()
    _record_action_policy_success(
        db,
        crm_policy,
        {"company_id": str(company.id), "contact_id": str(contact.id)},
    )
    db.commit()
    db.refresh(company)
    return UsageActionOut(
...
def generate_email_draft(
    ...
    if not company.lead_id:
        return UsageActionOut(
            status="error",
            message="This company needs a saved lead before email generation.",
            company=_crm_company_out(db, workspace, user.user_id, company),
        )
    lead = db.scalar(
        select(Lead).where(
    ...
    analysis = (
        analysis_snapshot.analysis_json
        if analysis_snapshot
        and isinstance(analysis_snapshot.analysis_json, dict)
        and analysis_snapshot.analysis_json
        else _cached_ai_sales_analysis_from_company(company)
    )
    ...
    try:
        ...
        capture_provider_exception(
            exc,
            provider="ai_memory",
            endpoint="workspace_app.email_draft.memory",
            workspace_id=workspace.id,
            lead_id=lead.id,
        )
        memory_context = memory_context_none(
    ...
    draft_policy = _enforce_action_policy(
        ...
        input_payload={
            "route": "workspace_app.company.email_draft",
            "company_id": str(company.id),
            "lead_id": str(lead.id),
    ...
    try:
        ...
        variant = personalize_email(
            PersonalizeRequest(
                ...
                niche=lead.industry or lead.niche or "",
                website_summary=(
                    str(analysis_context.get("company_summary") or "")
                    + "\n"
                    + str(
                        company.ai_summary
                        or _lead_metadata(lead).get("ai_summary")
                        or ""
                    )
                ).strip(),
                offer=str(
                    getattr(workspace, "offer", "")
                    or analysis_context.get("value_proposition")
                    or company.suggested_offer
                    or workspace.company
                    or "AI-powered lead generation and outbound growth"
                ),
                cta=str(
                    _custom_workspace_cta(workspace)
                    or analysis_context.get("suggested_cta")
                    or getattr(workspace, "cta", "")
        ...
        capture_provider_exception(
            exc,
            provider="openai",
            endpoint="workspace_app.email_draft",
            workspace_id=workspace.id,
            lead_id=lead.id,
        )
        return UsageActionOut(
    ...
    record_email_memory(
        ...
        company=company,
        event="draft",
        extra={"memory_context": memory_context},
    )
    _record_action_policy_success(
        db,
        draft_policy,
        {"email_id": str(email.id), "lead_id": str(lead.id), "draft_only": True},
    )
    db.commit()
    db.refresh(email)
    return UsageActionOut(
...
@router.post(
    "/companies/{company_id}/complete-opportunity", response_model=UsageActionOut
)
def complete_company_opportunity(
    company_id: UUID,
    request: Request,
    user: WorkspaceUserContext,
    db: Session = Depends(get_db),
    ...
    opportunity_policy = _enforce_action_policy(
        db,
        workspace=workspace,
        user_id=user.user_id,
        action_name="email.draft.create",
        required_permissions=("crm:write", "email:draft"),
        input_payload={
            "route": "workspace_app.complete_opportunity",
            "company_id": str(company.id),
            "lead_id": str(lead.id),
        },
        resource_workspace_id=company.workspace_id,
    ...
    _lead_trace(
        request_id,
        "complete_opportunity_profile_started",
        lead_id=str(lead.id),
        company=lead.company,
    )
    try:
        _complete_public_company_details(
            db, request, user.user_id, workspace, lead, request_id
        )
        _set_workflow_stage(
            lead,
            "company_profile",
            "completed",
            _workflow_message(language, "workflow_company_profile"),
        )
        completed_steps.append("Company profile checked")
        _lead_trace(
            request_id,
            "complete_opportunity_profile_finished",
            lead_id=str(lead.id),
            company=lead.company,
        )
    except Exception as exc:
        ...
        warnings.append(
            "Company profile could not be refreshed. Saved CRM data was kept."
        )
        _set_workflow_stage(
            lead,
            "company_profile",
        ...
        _lead_trace(
            request_id,
            "complete_opportunity_profile_failed",
            lead_id=str(lead.id),
            company=lead.company,
            reason=str(exc),
        )
    ...
    _lead_trace(
        request_id,
        "complete_opportunity_analysis_started",
        lead_id=str(lead.id),
        company=lead.company,
    )
    try:
        _set_workflow_stage(
            ...
            "website_analysis",
            "running",
            _localized_fallback_text(language, "progress_ai_analyzing"),
        )
        metadata = _lead_metadata(lead)
        has_completed_research = bool(metadata.get("website_analyzed_at"))
        if (lead.website or metadata.get("domain")) and (
            _needs_ai_research(lead) or not has_completed_research
        ):
            _analyze_lead_if_possible(
                db,
                user.user_id,
        ...
        if not any(
            ...
            warnings.append(
                "AI research could not complete yet. The company is saved and can be retried."
            )
            _set_workflow_stage(
                lead,
                "website_analysis",
                "error",
                _workflow_message(language, "workflow_website_missing"),
            )
        else:
            _set_workflow_stage(
                lead,
                "website_analysis",
                "completed",
                _workflow_message(language, "workflow_website_done"),
            )
            _add_lead_activity(
                db,
                request,
        ...
        _lead_trace(
            request_id,
            "complete_opportunity_analysis_finished",
            lead_id=str(lead.id),
            company=lead.company,
        )
    except Exception as exc:
        ...
        _ensure_b2b_opportunity_metadata(
            lead,
            workspace,
            source="complete_opportunity_ai_fallback",
            language=_workspace_language(request, workspace),
        )
        warnings.append(_safe_provider_warning(exc))
        _set_workflow_stage(
            lead, "website_analysis", "error", _safe_provider_warning(exc)
        )
        completed_steps.append("Website analysis checked")
        _lead_trace(
            request_id,
            "complete_opportunity_analysis_failed",
            lead_id=str(lead.id),
            company=lead.company,
            reason=str(exc),
        )
    ...
    _lead_trace(
        request_id,
        "complete_opportunity_contacts_started",
        lead_id=str(lead.id),
        company=lead.company,
    )
    try:
        _set_workflow_stage(
            ...
            "decision_maker",
            "running",
            _workflow_message(language, "workflow_decision_missing"),
        )
        _set_workflow_stage(
            lead,
            "verified_email",
            "running",
            _workflow_message(language, "workflow_email_missing"),
        )
        if not lead.email:
            metadata = _lead_metadata(lead)
            if hunter_key_loaded() and (lead.website or metadata.get("domain")):
                ...
                if not lead.email:
                    warnings.append(
                        "No verified business email was found yet. Add a decision maker manually or continue with research."
                    )
                    _set_workflow_stage(
                        lead,
                        "decision_maker",
                ...
                lead.notes = _merge_lead_metadata(
                    lead,
                    {
                        ...
                        "decision_maker_roles_searched": list(DECISION_MAKER_TITLES),
                        "email_status": _lead_metadata(lead).get(
                            "email_status", "No verified email"
                        ),
                    },
                )
                warnings.append(
                    "Contact discovery needs setup. Add a decision maker email manually and continue."
                )
                _set_workflow_stage(
                    lead,
                    "decision_maker",
        ...
        if lead.email:
            _set_workflow_stage(
                ...
                "decision_maker",
                "completed",
                _workflow_message(language, "workflow_decision_done"),
            )
            _set_workflow_stage(
                lead,
                "verified_email",
                "completed",
                _workflow_message(language, "workflow_email_verified"),
            )
        completed_steps.append("Contact search checked")
        _lead_trace(
            request_id,
            "complete_opportunity_contacts_finished",
            lead_id=str(lead.id),
            company=lead.company,
            has_email=bool(lead.email),
        )
        ...
        warnings.append(
            "Contact search is temporarily unavailable. Add a contact manually and continue."
        )
        _set_workflow_stage(
            lead,
            "decision_maker",
        ...
        _lead_trace(
            request_id,
            "complete_opportunity_contacts_failed",
            lead_id=str(lead.id),
            company=lead.company,
            reason=str(exc),
        )
    ...
    _lead_trace(
        request_id,
        "complete_opportunity_email_started",
        lead_id=str(lead.id),
        company=lead.company,
    )
    try:
        _set_workflow_stage(
            ...
            "ai_email",
            "running",
            _workflow_message(language, "workflow_ai_email_missing"),
        )
        _ensure_b2b_opportunity_metadata(
            lead,
            workspace,
            source="complete_opportunity_before_draft",
            language=_workspace_language(request, workspace),
        )
        ...
        _lead_trace(
            request_id,
            "complete_opportunity_email_finished",
            lead_id=str(lead.id),
            company=lead.company,
            has_email_draft=bool(email),
        )
    except Exception as exc:
        capture_provider_exception(
            exc,
        ...
        _lead_trace(
            request_id,
            "complete_opportunity_email_failed",
            lead_id=str(lead.id),
            company=lead.company,
            reason=str(exc),
        )
    ...
    _lead_trace(
        request_id,
        "complete_opportunity_database_save_started",
        lead_id=str(lead.id),
        company=lead.company,
    )
    company = _sync_lead_to_crm(db, user.user_id, workspace, lead)
    ...
    _record_action_policy_success(
        ...
        opportunity_policy,
        {
            "company_id": str(company.id),
            "lead_id": str(lead.id),
            "email_id": str(email.id) if email else "",
            "draft_only": True,
        },
    )
    db.commit()
    ...
    _lead_trace(
        request_id,
        "complete_opportunity_database_save_finished",
        lead_id=str(lead.id),
        company=lead.company,
        company_id=str(company.id),
        has_email_draft=bool(email),
...
@router.post(
    "/companies/{company_id}/enrichment/restart", response_model=UsageActionOut
)
def restart_company_auto_enrichment(
    company_id: UUID,
    request: Request,
    user: WorkspaceUserContext,
    db: Session = Depends(get_db),
    ...
    try:
        ...
        if not company.lead_id:
            return UsageActionOut(
                status="error",
                message="This company needs a saved lead before AI enrichment.",
                company=_crm_company_out(db, workspace, user.user_id, company),
            )
        lead = db.scalar(
            select(Lead).where(
        ...
        crm_policy = _enforce_action_policy(
            ...
            action_name="crm.write",
            input_payload={
                "route": "workspace_app.enrichment_restart",
                "company_id": str(company.id),
                "lead_id": str(lead.id),
                "request_id": request_id,
            },
        ...
        try:
            ...
            _record_action_policy_success(
                db,
                crm_policy,
                {"company_id": str(company.id), "lead_id": str(lead.id)},
            )
            db.commit()
        except Exception as exc:
            db.rollback()
            ...
            warnings.append(
                "AI enrichment restart setup is temporarily unavailable. Continuing with queue restart."
            )

        ran_inline = False
        try:
            ran_inline = _enqueue_auto_enrichment(
                db, request, user.user_id, workspace, [lead], request_id, force=True
            )
        except Exception as exc:
            db.rollback()
        ...
        try:
            ...
            logger.exception(
                "Workspace enrichment restart response serialization failed"
            )
            capture_provider_exception(
                out_exc,
                provider="workspace_app",
            ...
            warnings.append(
                "Company card update is temporarily unavailable. Reload the page to continue."
            )
        return UsageActionOut(
            status="partial_success" if warnings else "success",
            message=(
        ...
        if workspace is not None:
            try:
                fallback_company = company or _scoped_company(
                    db, workspace.id, company_id
                )
                fallback_company_out = _crm_company_out(
                    db, workspace, user.user_id, fallback_company
                )
            except Exception as out_exc:
                logger.exception(
                    "Workspace enrichment restart unhandled fallback serialization failed"
                )
                capture_provider_exception(
                    out_exc,
                    provider="workspace_app",
        ...
        return UsageActionOut(
            ...
            workflow_stages=fallback_company_out.workflow_stages
            if fallback_company_out
            else {},
...
def cancel_company_auto_enrichment(
    company_id: UUID,
    request: Request,
    user: WorkspaceUserContext,
    db: Session = Depends(get_db),
    ...
    if not company.lead_id:
        return UsageActionOut(
            status="error",
            message="This company does not have a running AI enrichment job.",
            company=_crm_company_out(db, workspace, user.user_id, company),
        )
    lead = db.scalar(
        select(Lead).where(
    ...
    crm_policy = _enforce_action_policy(
        ...
        action_name="crm.write",
        input_payload={
            "route": "workspace_app.enrichment_cancel",
            "company_id": str(company.id),
            "lead_id": str(lead.id),
            "request_id": request_id,
        },
    ...
    cancel_jobs_for_lead(
        ...
        workspace_id=workspace.id,
        lead_id=lead.id,
        reason="AI enrichment was stopped by the user.",
    )
    lead.notes = _merge_lead_metadata(
        lead,
        _enrichment_metadata_update(
...
def _sync_ai_customer_finder_result_email_state(
    db: Session, email: EmailMessage
) -> None:
    tags = email.tags if isinstance(email.tags, dict) else {}
    ...
    simple_status = (
        ...
        else "Письмо подтверждено"
        if email.delivery_status == "approved"
        else "Письмо подготовлено"
    )
    result.metadata_json = {
        **metadata,
        "simple_customer_finder": {
            **(
                metadata.get("simple_customer_finder")
                if isinstance(metadata.get("simple_customer_finder"), dict)
                else {}
            ),
...
def _email_send_confirmation_snapshot(
    *,
    sender_email: str,
    recipient_email: str,
    subject: str,
...
def _current_email_send_confirmation_snapshot(
    ...
    *,
    ...
    recipient_email = _normalized_email(
        email.recipient_email or (lead.email if lead else "")
    )
...
def approve_email(
    ...
    request: Request,
    user: WorkspaceUserContext,
    payload: Optional[EmailApproveIn] = None,
    db: Session = Depends(get_db),
) -> UsageActionOut:
    workspace = _current_workspace(db, user.user_id, user.email)
    email = db.scalar(
        select(EmailMessage).where(
            EmailMessage.id == email_id, EmailMessage.workspace_id == workspace.id
        )
    )
    if not email:
    ...
    if email.direction != "outbound":
        raise HTTPException(
            status_code=409, detail="Only outbound email drafts can be approved."
        )
    if email.delivery_status in {
        "sent",
        "delivered",
        "opened",
        "replied",
    ...
    if email.delivery_status not in {"draft", "approved"}:
        raise HTTPException(
            status_code=409, detail="Only outbound draft emails can be approved."
        )
    tags = email.tags if isinstance(email.tags, dict) else {}
    ...
    if payload and payload.confirmed_exact_draft:
        if (
            ...
            raise HTTPException(
                status_code=422,
                detail="Sender, recipient, subject, and body are required for final send confirmation.",
            )
        ...
        if submitted_snapshot["fingerprint"] != current_snapshot["fingerprint"]:
            raise HTTPException(
                status_code=409,
                detail="The displayed sender, recipient, subject, or body no longer matches this draft. Refresh and confirm again.",
            )
    ...
    approval_policy = _enforce_action_policy(
        ...
        input_payload={
            "email_id": str(email.id),
            "delivery_status": email.delivery_status,
            "confirmed_exact_draft": bool(payload and payload.confirmed_exact_draft),
            "approval_fingerprint": draft_approval_fingerprint,
            "confirmation_fingerprint": (
                confirmation_snapshot.get("fingerprint")
    ...
    if lead:
        lead.notes = _merge_lead_metadata(
            lead,
            {
                ...
                "email_approved_at": datetime.utcnow().isoformat(),
            },
        )
        _set_workflow_stage(
            lead,
            "approval",
            "completed",
            "Human review completed. The email is ready to send.",
        )
        _add_lead_activity(
            db,
            request,
    ...
    _record_action_policy_success(
        db,
        approval_policy,
        {"email_id": str(email.id), "delivery_status": email.delivery_status},
    )
    db.commit()
    db.refresh(email)
    return UsageActionOut(
...
def update_email_draft(
    ...
    if email.direction != "outbound":
        raise HTTPException(
            status_code=409, detail="Inbound emails and captured replies are read-only."
        )
    if (
        email.delivery_status in provider_statuses
        or email.provider_message_id
        or email.sent_at
        or email.delivered_at
        or email.opened_at
        or email.replied_at
        or email.bounced_at
    ):
        raise HTTPException(
            status_code=409,
            detail="Sent, delivered, opened, replied, bounced, and failed provider records cannot be edited.",
        )
    if email.delivery_status not in {"draft", "approved"}:
        raise HTTPException(
            status_code=409, detail="Only outbound email drafts can be edited."
        )

    updates = payload.model_dump(exclude_unset=True)
    if not updates:
        raise HTTPException(
            status_code=422, detail="Provide at least one editable draft field."
        )
    previous_status = email.delivery_status
    if "recipient_email" in updates and previous_status != "draft":
        raise HTTPException(
            status_code=409,
            detail="Recipient email can only be changed while the email is a draft.",
        )
    status_transition = ""
    ...
    if result.rowcount != 1:
        ...
        raise HTTPException(
            status_code=409,
            detail="This email is already being sent or has a provider record and cannot be edited.",
        )
    email = db.scalar(
        select(EmailMessage).where(
            EmailMessage.id == email_id, EmailMessage.workspace_id == workspace.id
    ...
    company = (
        db.scalar(
            select(Company)
            .where(Company.lead_id == lead.id, Company.workspace_id == workspace.id)
            .order_by(Company.updated_at.desc())
        )
        if lead
        else None
    ...
    if lead:
        if status_transition:
            ...
            _set_workflow_stage(
                ...
                "approval",
                "waiting",
                "Email was edited after approval. Review and approve it again before sending.",
            )
            company = _sync_lead_to_crm(db, user.user_id, workspace, lead)
        _add_lead_activity(
            db,
...
def _email_send_idempotency_key(
    workspace_id: UUID, email_id: UUID, approval_version: int
) -> str:
...
def _recover_stale_email_send_claim(
    db: Session, *, workspace_id: UUID, email_id: UUID, now: datetime | None = None
) -> bool:
    now = now or datetime.utcnow()
    current = db.scalar(
        select(EmailMessage).where(
            EmailMessage.id == email_id, EmailMessage.workspace_id == workspace_id
        )
    )
    if (
        not current
        or current.delivery_status != "sending"
        or current.provider_message_id
        or current.sent_at
    ):
        return False
    ...
    provider_supports_idempotency = bool(
        tags.get("provider_idempotency_supported") is True
        or provider in EMAIL_SEND_PROVIDER_IDEMPOTENCY_SUPPORTED
    )
    restored_status = (
        "approved"
        if provider_supports_idempotency or not provider
        else EMAIL_SEND_CONFIRMATION_PENDING_STATUS
    )
    restored_tags = {
        **tags,
        ...
        else "send_confirmation_required_after_interruption",
    }
    result = db.execute(
        update(EmailMessage)
        .where(
    ...
    if result.rowcount == 1:
        db.add(
            AuditLog(
                ...
                if restored_status == "approved"
                else "email.send_confirmation_pending",
                metadata_json={
                    "email_id": str(email_id),
                    "reason": restored_tags["last_send_error"],
                    "sender_provider": provider or None,
                },
            )
        )
        db.commit()
...
def _claim_approved_email_for_send(
    db: Session, *, workspace_id: UUID, email_id: UUID, idempotency_key: str
) -> EmailMessage:
    now = datetime.utcnow()
    _recover_stale_email_send_claim(
        db, workspace_id=workspace_id, email_id=email_id, now=now
    )
    current_tags = db.scalar(
        select(EmailMessage.tags).where(
            EmailMessage.id == email_id, EmailMessage.workspace_id == workspace_id
        )
    )
    send_attempt = (
        int(
            (current_tags if isinstance(current_tags, dict) else {}).get("send_attempt")
            or 0
        )
        + 1
    )
    next_tags = {
        **(current_tags if isinstance(current_tags, dict) else {}),
        ...
        "send_claim_expires_at": (
            now + timedelta(seconds=EMAIL_SEND_CLAIM_LEASE_SECONDS)
        ).isoformat(),
    }
    result = db.execute(
    ...
    if result.rowcount != 1:
        ...
        if current.delivery_status == "sent":
            raise HTTPException(
                status_code=409, detail="This email has already been sent."
            )
        if current.delivery_status == "sending":
            raise HTTPException(
                status_code=409,
                detail="This email is already being sent. Wait for the current send lease to expire before retrying.",
            )
...
def _approved_email_send_claim_update(
    *, workspace_id: UUID, email_id: UUID, values: dict[str, Any]
):
    return (
        update(EmailMessage)
        .where(
...
def _restore_email_send_retry_state(
    db: Session,
    *,
    request: Request,
    user_id: str,
    ...
    if (
        email
        and email.delivery_status == "sending"
        and not email.provider_message_id
        and not email.sent_at
    ):
        email.delivery_status = "approved"
        email.tags = {
            **(email.tags if isinstance(email.tags, dict) else {}),
    ...
    if lead:
        _add_lead_activity(
            ...
            "email.send_failed",
            lead,
            {"email_id": str(email_id), "reason": reason, "retryable": True},
        )
    db.add(
        AuditLog(
            user_id=user_id,
...
def _mark_email_send_confirmation_pending(
    db: Session,
    *,
    request: Request,
    user_id: str,
    ...
    if lead:
        _add_lead_activity(
            ...
            user_id,
            workspace,
            "email.send_confirmation_pending",
            lead,
            {
                "email_id": str(email_id),
                "reason": reason,
    ...
    db.add(
        AuditLog(
            ...
            metadata_json={
                ...
                "retryable": False,
                "sender_provider": sender_provider,
                "status_after": EMAIL_SEND_CONFIRMATION_PENDING_STATUS,
            },
        )
    )
    db.commit()
...
def _record_email_send_provider_context(
    db: Session,
    *,
    workspace_id: UUID,
    email_id: UUID,
    ...
    if (
        not email
        or email.delivery_status != "sending"
        or email.provider_message_id
        or email.sent_at
    ):
        raise HTTPException(
            status_code=409,
            detail="This email is already being sent or has a provider record.",
        )
    email.tags = {
        **(email.tags if isinstance(email.tags, dict) else {}),
        "sender_provider": sender_provider,
        "sender_email": sender_email,
        "provider_idempotency_supported": sender_provider
        in EMAIL_SEND_PROVIDER_IDEMPOTENCY_SUPPORTED,
    }
    db.commit()


def _record_smoke_send_confirmation_once(
    db: Session,
    *,
    workspace_id: UUID,
    user_id: str,
...
def recover_email_send(
    email_id: UUID,
    payload: RecoverEmailSendIn,
    request: Request,
    user: WorkspaceUserContext,
    db: Session = Depends(get_db),
) -> UsageActionOut:
    ...
    if email.provider_message_id or email.sent_at:
        raise HTTPException(
            status_code=409,
            detail="Provider email records cannot be recovered to approved.",
        )
    tags = email.tags if isinstance(email.tags, dict) else {}
    now = datetime.utcnow()
    expires_at = _parse_iso_datetime(tags.get("send_claim_expires_at"))
    if email.delivery_status == "sending" and expires_at and expires_at > now:
        raise HTTPException(
            status_code=409,
            detail="This email is still being sent. Wait for the current send lease to expire before recovery.",
        )
    if email.delivery_status not in {"sending", EMAIL_SEND_CONFIRMATION_PENDING_STATUS}:
        raise HTTPException(
            status_code=409, detail="Only interrupted sends can be recovered."
        )
    if not payload.confirmed_not_delivered:
        raise HTTPException(
            status_code=409,
            detail="Confirm that the email is not in Gmail or SMTP Sent before recovering it for retry.",
        )

    recover_policy = _enforce_action_policy(
        db,
        workspace=workspace,
        user_id=user.user_id,
        action_name="email.draft.update",
        input_payload={
            "email_id": str(email.id),
            "status_before": email.delivery_status,
            "status_after": "approved",
            "confirmed_not_delivered": payload.confirmed_not_delivered,
        },
        resource_workspace_id=email.workspace_id,
        resource_id=email.id,
    )
    ...
    if lead:
        _add_lead_activity(
            ...
            user.user_id,
            workspace,
            "email.send_recovered",
            lead,
            {
                "email_id": str(email.id),
                "reason": "manual_send_recovery",
                "user_id": user.user_id,
    ...
    _record_action_policy_success(
        db,
        recover_policy,
        {"email_id": str(email.id), "status_after": "approved"},
    )
    db.commit()
    db.refresh(email)
    company = (
...
def send_approved_email(
    ...
    request: Request,
    user: WorkspaceUserContext,
    payload: Optional[ProductionEmailSmokeTestSendConfirmIn] = None,
    db: Session = Depends(get_db),
) -> UsageActionOut:
    workspace = _current_workspace(db, user.user_id, user.email)
    initial_email = db.scalar(
        select(EmailMessage).where(
            EmailMessage.id == email_id, EmailMessage.workspace_id == workspace.id
        )
    )
    ...
    initial_lead = (
        db.scalar(
            select(Lead).where(
                Lead.id == initial_email.lead_id, Lead.workspace_id == workspace.id
            )
        )
        if initial_email.lead_id
        else None
    )
    is_production_smoke_test = _is_production_smoke_test_email(
        initial_email
    ) or _is_production_smoke_test_lead(initial_lead)
    is_internal_email_smoke_draft = _is_internal_email_smoke_draft_email(
        initial_email
    ) or _is_internal_email_smoke_draft_lead(initial_lead)
    if initial_email.delivery_status == "sending":
        if _recover_stale_email_send_claim(
            db,
            workspace_id=workspace.id,
            email_id=email_id,
        ):
            db.refresh(initial_email)
    if initial_email.delivery_status == "sent":
    ...
    if initial_email.delivery_status != "approved":
        raise HTTPException(status_code=409, detail="Approve the email before sending.")
    ...
    if is_production_smoke_test:
        ...
        smoke_recipient = (
            str(
                tags.get("recipient_email")
                or initial_email.recipient_email
                or (initial_lead.email if initial_lead else "")
                or ""
            )
            .strip()
            .lower()
        ...
        if not payload or not payload.confirmed_send:
            raise HTTPException(
                status_code=409,
                detail="Final send confirmation is required for production smoke-test email.",
            )
        if str(payload.smoke_test_id or "") != smoke_test_id:
            raise HTTPException(
                status_code=409,
                detail="Smoke-test ID confirmation does not match this draft.",
            )
        if str(payload.recipient_email or "").strip().lower() != smoke_recipient:
            raise HTTPException(
                status_code=409,
                detail="Recipient confirmation does not match this smoke-test draft.",
            )
        if initial_email.campaign_id or (initial_lead and initial_lead.campaign_id):
            raise HTTPException(
                status_code=409,
                detail="Production smoke-test records cannot be attached to campaigns or automations.",
            )
        smoke_confirmation_audits.append(
            ("production_smoke_test.send_confirmed", smoke_test_id, smoke_recipient)
        )
    if is_internal_email_smoke_draft:
        ...
        _validate_internal_email_smoke_recipient(smoke_recipient)
        if not payload or not payload.confirmed_send:
            raise HTTPException(
                status_code=409,
                detail="Final send confirmation is required for internal smoke-test email.",
            )
        if str(payload.smoke_test_id or "") != smoke_test_id:
            raise HTTPException(
                status_code=409,
        ...
        if initial_email.campaign_id or (initial_lead and initial_lead.campaign_id):
            raise HTTPException(
                status_code=409,
                detail="Internal smoke-test records cannot be attached to campaigns or automations.",
            )
        smoke_confirmation_audits.append(
            (
    ...
    idempotency_key = _email_send_idempotency_key(
        workspace.id, email_id, _email_approval_version(initial_email)
    )
    lead = initial_lead
    recipient_email = (
        str(initial_email.recipient_email or (lead.email if lead else "") or "")
        .strip()
        .lower()
    )
    if not lead or not recipient_email:
        raise HTTPException(
            status_code=409, detail="Add a verified recipient email before sending."
        )
    if _is_placeholder_recipient(recipient_email):
        raise HTTPException(
            status_code=400, detail="Use a real recipient email before sending."
        )
    tags = _email_tags(initial_email)
    confirmed_snapshot = (
        tags.get("send_confirmation_snapshot")
        if isinstance(tags.get("send_confirmation_snapshot"), dict)
        else None
    ...
    current_snapshot = _current_email_send_confirmation_snapshot(
        ...
        email=initial_email,
        lead=lead,
        approval_version=_email_approval_version(initial_email),
    )
    server_confirmed_snapshot = (
        current_snapshot
        if (
            (is_production_smoke_test or is_internal_email_smoke_draft)
            and payload is not None
            and payload.confirmed_send
        )
        else None
    )
    if (
        not confirmed_snapshot
        or confirmed_snapshot.get("fingerprint") != current_snapshot["fingerprint"]
    ):
        if server_confirmed_snapshot is None:
            raise HTTPException(
                status_code=409,
                detail="Confirm the exact sender, recipient, subject, and body before sending.",
            )
        confirmed_snapshot = server_confirmed_snapshot
        initial_email.tags = {
            **_email_tags(initial_email),
            "send_confirmation_snapshot": confirmed_snapshot,
            "send_confirmation_confirmed_at": datetime.utcnow().isoformat(),
            "send_confirmation_user_id": user.user_id,
        }
    send_policy = _enforce_action_policy(
        ...
        input_payload={
            "email_id": str(initial_email.id),
            "lead_id": str(lead.id),
            "recipient_email": recipient_email,
            "sender_email": current_snapshot["sender_email"],
            "subject": initial_email.subject,
            "body": initial_email.body,
            "approval_version": _email_approval_version(initial_email),
            "confirmation_fingerprint": current_snapshot["fingerprint"],
        },
        ...
        idempotency_key=idempotency_key,
        resource_workspace_id=initial_email.workspace_id,
        resource_id=initial_email.id,
        required_approval_fingerprint=current_snapshot["fingerprint"],
    )
    if send_policy.replay and initial_email.delivery_status == "sent":
        company = (
            db.scalar(
                select(Company)
                .where(Company.lead_id == lead.id, Company.workspace_id == workspace.id)
        ...
        return UsageActionOut(
            status="success",
            message="Approved email was already sent. CRM stage is unchanged.",
            company=_crm_company_out(db, workspace, user.user_id, company)
            if company
            else None,
            email=EmailOut.model_validate(initial_email),
        )
    email = _claim_approved_email_for_send(
        db,
        workspace_id=workspace.id,
        email_id=email_id,
        idempotency_key=idempotency_key,
    ...
    for action, smoke_test_id, smoke_recipient in smoke_confirmation_audits:
        _record_smoke_send_confirmation_once(
            db,
            workspace_id=workspace.id,
            user_id=user.user_id,
            action=action,
            smoke_test_id=smoke_test_id,
            email_id=email.id,
            recipient_email=smoke_recipient,
        )
    ...
    try:
        ...
        db.commit()
        _record_action_policy_failure(db, send_policy, str(exc.detail))
        db.commit()
        ...
        raise HTTPException(
            status_code=exc.status_code if exc.status_code >= 400 else 409,
        ...
        capture_provider_exception(
            exc,
            provider="email",
            endpoint="workspace_app.email.send",
            workspace_id=workspace.id,
            lead_id=lead.id,
        )
        raise HTTPException(
            status_code=409,
            detail="Email sending needs setup. The approved draft is still saved.",
        ) from exc
    except EmailProviderRequestError as exc:
        sender_provider = (
            str(locals().get("sender_status").provider)
            if "sender_status" in locals()
            else ""
        )
        if (
            sender_provider
        ...
        raise HTTPException(
            status_code=502,
            detail="Email sending is temporarily unavailable. The approved draft is still saved.",
        ) from exc
    except Exception as exc:
        sender_provider = (
            str(locals().get("sender_status").provider)
        ...
        raise HTTPException(
            status_code=500,
            detail="Email sending failed before provider confirmation. The approved draft is still saved.",
        ) from exc
    ...
    lead.notes = _merge_lead_metadata(
        lead, {"email_status": "Sent", "email_sent_at": email.sent_at.isoformat()}
    )
    _add_lead_activity(
        db,
        request,
        ...
        "email.sent",
        lead,
        {"email_id": str(email.id), "provider_message_id": email.provider_message_id},
    )
    company = _sync_lead_to_crm(db, user.user_id, workspace, lead)
    record_email_memory(
        db,
    ...
    return UsageActionOut(
        status="success",
        message="Approved email was sent. CRM stage updated.",
        company=_crm_company_out(db, workspace, user.user_id, company),
        email=EmailOut.model_validate(email),
    )
