...
def personalize_email(payload: PersonalizeRequest) -> EmailVariantOut:
    client = _client()
    if client is None:
        cold = f"Subject: Quick idea for {payload.company}\n\nHi, I reviewed {payload.company} and noticed an opportunity around {payload.website_summary}. We help {payload.niche} teams turn that into qualified meetings with {payload.offer}. Worth a quick look?"
