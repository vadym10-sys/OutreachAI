...
os.environ["INTERNAL_EMAIL_SMOKE_DRAFT_RECIPIENTS"] = (
    "romaniukvadym10+client-smoke-20260812-1@gmail.com"
)
...
from app.api.usage import (
    _approved_email_send_claim_update,
    _parse_lead_command,
    _require_workspace_owner,
)  # noqa: E402
from app.api.routes import (
    _audit_log_lead_id_clause,
    _lead_ai_payload,
    _plan_for_workspace,
...
from app.services.deep_contact_search import (
    DeepContactCandidate,
    DeepContactSearchResult,
    deep_contact_cache_is_fresh,
    normalize_domain,
...
from app.services.ai_memory import (
    MODE_KEYWORD,
    MODE_OPENAI_EMBEDDING,
    _openai_embedding,
    _pgvector_retrieval_sql,
    record_email_memory,
    retrieve_memory,
    upsert_memory_entry,
)  # noqa: E402
from app.services.website import (
    ...
    WebsiteFetchError,
    WebsiteSnapshot,
    WebsiteTemporaryUnavailableError,
...
from app.services.billing import (
    ...
    plan_from_price_id,
    require_plan_for_price_id,
    subscription_diagnostics_for_customer,
)  # noqa: E402
...
USER_A_AUTH = {
    ...
    "X-Test-User-Email": "tenant-a@example.com",
}
USER_B_AUTH = {
    "Authorization": "Bearer dev",
    "X-Test-User-Email": "tenant-b@example.com",
}
OWNER_AUTH = {
    "Authorization": "Bearer dev",
    "X-Test-User-Email": "romaniukvadym10@gmail.com",
}
NON_OWNER_AUTH = {
    "Authorization": "Bearer dev",
    "X-Test-User-Email": "not-owner@example.com",
}
security.limiter.limit = 10000


def _grant_subscription_for_test(
    workspace_id: str,
    ...
    with get_sessionmaker()() as db:
        ...
        subscription = db.scalar(
            select(Subscription).where(
                ...
                == f"sub_test_{workspace_id}_{plan}",
            )
        )
        if subscription is None:
            subscription = Subscription(
                ...
                stripe_customer_id=f"cus_test_{workspace_id}",
                stripe_subscription_id=f"sub_test_{workspace_id}_{plan}",
            )
            db.add(subscription)
        ...
        subscription.trial_end = (
            datetime.utcnow() + timedelta(days=14) if status == "trialing" else None
        )
...
def _create_campaigns_for_test(
    workspace_id: str, user_id: str, count: int
) -> list[UUID]:
    with get_sessionmaker()() as db:
        campaigns = [
            Campaign(
...
def _assert_url_components(
    value: str, *, scheme: str, hostname: str, path: str
) -> None:
...
def _enable_ai_memory(headers: dict[str, str]) -> dict[str, Any]:
    response = client.patch(
        "/api/workspace-app/ai-memory/settings", headers=headers, json={"enabled": True}
    )
...
def test_sentry_debug_endpoint_throws_only_when_debug_enabled(monkeypatch) -> None:
    ...
    with pytest.raises(
        RuntimeError, match="OutreachAI backend development Sentry test error"
    ):
    ...
    event = _before_send(
        {
            ...
            "contexts": {
                "outreachai": {
                    "body": "private email content",
                    "endpoint": "/api/workspace-app/emails",
                }
            },
...
def test_website_url_normalization_adds_https_and_rejects_invalid_domains() -> None:
    ...
    assert (
        normalize_website_url("https://Example.COM/path?q=1")
    ...
    class FakeClient:
        ...
        def get(self, url):  # type: ignore[no-untyped-def]
            ...
            return httpx.Response(
                ...
                text="<title>Recovered</title><p>Recovered website content.</p>",
                request=request,
            )

    monkeypatch.setattr(website_module.httpx, "Client", FakeClient)
    monkeypatch.setattr(
        website_module.time, "sleep", lambda value: sleeps.append(value)
    )
...
def test_deep_contact_selects_revenue_decision_maker() -> None:
    ...
    selected = select_best_decision_maker(
        ...
        industry="SaaS",
        product_context="Outbound sales",
    )
...
def test_ai_customer_finder_scoring_and_dedupe_require_public_evidence() -> None:
    from app.services.ai_customer_finder.dedupe import (
        company_dedupe_key,
        signal_fingerprint,
    )
    ...
    assert (
        score.lead_intelligence["components"]["buying_intent"]
    ...
    assert signal_fingerprint(
        ...
        evidence="Manual spreadsheet workflow",
        company_name="Example",
    )
...
def test_ai_customer_finder_evidence_dedupe_uses_canonical_source_and_signal() -> None:
    ...
    assert (
        canonical_url("https://www.Example.com/path/?utm_source=x")
        == "https://example.com/path"
    )
    first = signal_fingerprint(
        source_url="https://www.Example.com/path/?utm_source=x",
        signal_type="manual_workaround",
        evidence="Manual spreadsheet workflow for outbound sales",
        company_name="Example Inc.",
    )
    second = signal_fingerprint(
...
def test_ai_customer_finder_scoring_handles_quality_edge_cases() -> None:
    ...
    assert (
        strong_signal_poor_icp.buying_intent_score
        > strong_signal_poor_icp.icp_fit_score
    )
    ...
    assert (
        stale_signal.factors["signal_recency"] < multi_source.factors["signal_recency"]
    )
    assert unknown_date.penalties["stale_or_unknown_publication_date"] > 0
    assert (
        multi_source.factors["source_diversity"]
        > unknown_date.factors["source_diversity"]
    )
...
def test_ai_customer_finder_lead_intelligence_uses_verified_contact_and_growth() -> (
    None
):
    from app.services.ai_customer_finder.schemas import CustomerFinderCriteria
    from app.services.ai_customer_finder.scoring import score_candidate
    ...
    assert (
        strong_public_profile.lead_reasoning["why_selected"] != "Недостаточно данных."
    )
    ...
    assert (
        strong_public_profile.lead_reasoning["Confidence"]["score"]
        == strong_public_profile.ai_confidence_score
    )
    assert strong_public_profile.lead_reasoning["Recommended Action"]
    assert (
        strong_public_profile.lead_reasoning["Reason Summary"] != "Недостаточно данных."
    )
    ...
    assert (
        research["Buying Intent"]["score"] == strong_public_profile.buying_intent_score
    )
    assert (
        research["Growth Signals"]["score"] == strong_public_profile.growth_signal_score
    )
    assert (
        research["Recommended Outreach Strategy"]["Mention in email"]["value"]
        != "Недостаточно данных."
    )
    ...
    assert (
        "public_work_contact"
        not in strong_public_profile.lead_intelligence["insufficient_data"]
    )
...
def test_ai_customer_finder_lead_intelligence_v2_rejects_low_quality_false_positive() -> (
    None
):
    from app.services.ai_customer_finder.schemas import CustomerFinderCriteria
    from app.services.ai_customer_finder.scoring import score_candidate
    ...
    assert (
        profile["Opportunity Detection"]["Sales opportunity now"]["value"]
        == "Недостаточно данных."
    )
    ...
    assert (
        strategy["recommended_cta"] == "Manually verify the evidence before outreach."
    )
...
def test_ai_customer_finder_explainable_ai_separates_facts_and_probabilistic_conclusions() -> (
    None
):
    from app.services.ai_customer_finder.schemas import CustomerFinderCriteria
    from app.services.ai_customer_finder.scoring import score_candidate
    ...
    assert profile["Recommended Outreach Strategy"]["Do not write"]["value"].startswith(
        "Do not claim private knowledge"
    )
    assert (
        profile["Opportunity Detection"]["May increase reply probability"]["value"]
        != "Недостаточно данных."
    )
    strategy = score.outreach_strategy
...
def test_ai_customer_finder_outreach_strategy_does_not_invent_missing_decision_maker_or_cta() -> (
    None
):
    from app.services.ai_customer_finder.schemas import CustomerFinderCriteria
    from app.services.ai_customer_finder.scoring import score_candidate
...
def test_ai_customer_finder_google_places_uses_natural_language_search_terms(
    monkeypatch,
) -> None:
    from app.services.ai_customer_finder.providers import (
    ...
    def fake_search(payload):  # type: ignore[no-untyped-def]
        ...
        return GooglePlacesSearchResult(
            leads=[
                LeadOut(
                    company="Warsaw Balloons",
                    website="https://warsaw-balloons.example",
                    industry="Balloons",
                    country="Poland",
                )
            ],
    ...
    monkeypatch.setattr(
        "app.services.ai_customer_finder.providers.search_google_places", fake_search
    )
    criteria = CustomerFinderCriteria(
...
def test_ai_customer_finder_google_places_broadens_query_and_deduplicates(
    monkeypatch,
) -> None:
    from app.services.ai_customer_finder.providers import (
        GooglePlacesCustomerSearchProvider,
    ...
    def fake_search(payload):  # type: ignore[no-untyped-def]
        ...
        return GooglePlacesSearchResult(
            leads=[
                LeadOut(
                    company="Warsaw Balloonmakers",
                    website="https://balloons.example",
                    industry="Balloons",
                    country="Poland",
                ),
                LeadOut(
                    company="Warsaw Balloonmakers Duplicate",
                    website="https://balloons.example",
                    industry="Balloons",
                    country="Poland",
                ),
                LeadOut(
                    company="Helium Party Warsaw",
                    website="https://party.example",
                    industry="Balloons",
                    country="Poland",
                ),
            ],
    ...
    candidates = GooglePlacesCustomerSearchProvider().search(
        criteria, max_candidates=10
    )

    assert len(queries) >= 2
    assert "balony z helem" in queries
    assert len(candidates) == 2
    assert {candidate.website for candidate in candidates} == {
        "https://balloons.example",
        "https://party.example",
    }
...
def _queue_deep_contact_search(company_id: str) -> dict[str, object]:
    response = client.post(
        f"/api/workspace-app/companies/{company_id}/deep-contact-search",
        headers=USER_A_AUTH,
        json={},
    )
...
def _process_deep_contact_search_job(job_id: str) -> None:
    ...
    assert (
        process_deep_contact_search_job(UUID(job_id), claim_token=claim_token) is True
    )


def test_deep_contact_search_endpoint_saves_verified_decision_maker(
    monkeypatch,
) -> None:
    ...
    response = client.post(
        ...
        headers=USER_A_AUTH,
        json={
            "name": "Deep Contact Co",
            "website": "https://deepcontact.example",
            "industry": "SaaS",
            "country": "Germany",
        },
    )
    ...
    assert (
        job_payload["company"]["deep_contact_search"]["verified_email"]
        == "jane@deepcontact.example"
    )
...
def test_ai_customer_finder_search_requires_manual_crm_save(monkeypatch) -> None:
    ...
    headers = {
        "Authorization": "Bearer dev",
        "X-Test-User-Email": "customer-finder@example.com",
    }
    ...
    db = get_sessionmaker()()
    try:
        claimed = claim_next_ai_customer_finder_job(
            db, worker_id="test-worker:customer-finder"
        )
        assert claimed is not None
        assert str(claimed.id) == job_id
        claim_token = claimed.locked_by
    ...
    refreshed = client.get(
        f"/api/workspace-app/ai-customer-finder/searches/{job_id}", headers=headers
    )
    assert refreshed.status_code == 200, refreshed.text
    ...
    assert (
        payload["results"][0]["canonical_source_url"]
        == "https://verified-finder.example"
    )
    ...
    assert (
        payload["results"][0]["score_penalties"]["stale_or_unknown_publication_date"]
        > 0
    )
    assert (
        payload["results"][0]["overall_lead_score"]
        == payload["results"][0]["ai_relevance_score"]
    )
    assert (
        payload["results"][0]["buying_intent_score"]
        >= payload["results"][0]["overall_lead_score"]
    )
    assert (
        payload["results"][0]["lead_intelligence"]["components"]["outreach_readiness"]
        > 0
    )
    assert (
        payload["results"][0]["ai_research_profile"]["Company Summary"]["value"]
        != "Недостаточно данных."
    )
    assert payload["results"][0]["ai_research_profile"][
        "Recommended Outreach Strategy"
    ]["Best first-contact angle"]["facts"]
    assert payload["results"][0]["ai_research_profile"]["Risk Analysis"][
        "Manual checks"
    ]["value"]
    ...
    assert (
        payload["results"][0]["outreach_strategy"]["recommended_cta"]
        == "Open to a quick fit review?"
    )
    assert payload["results"][0]["first_line_opener"]
    assert payload["results"][0]["draft_email"]
    assert (
        payload["results"][0]["public_work_contact"] == "sales@verified-finder.example"
    )
    ...
    crm = client.get(
        "/api/workspace-app/companies?search=Verified%20Finder", headers=headers
    )
    ...
    try:
        assert (
            db.scalar(
                select(func.count())
                .select_from(Lead)
                .where(Lead.email == "sales@verified-finder.example")
            )
            == 0
        )
        assert (
            db.scalar(
                select(func.count())
                .select_from(Company)
                .where(Company.website == "https://verified-finder.example")
            )
            == 0
        )
        assert (
            db.scalar(
                ...
                .where(
                    EmailMessage.tags["result_id"].as_string()
    ...
    save = client.post(
        f"/api/workspace-app/leads/first-customers/results/{payload['results'][0]['id']}/save",
        headers=headers,
    )
    ...
    assert (
        companies[0]["lead_intelligence"]["components"]["outreach_readiness"]
        == saved["lead_intelligence"]["components"]["outreach_readiness"]
    )
    db = get_sessionmaker()()
    try:
        ...
        assert (
            metadata["simple_customer_finder"]["source_url"]
            == "https://verified-finder.example"
        )
        assert (
            metadata["simple_customer_finder"]["simple_status"] == "Письмо подготовлено"
        )
        assert (
            metadata["simple_customer_finder"]["ai_research_profile"][
                "Company Summary"
            ]["value"]
            != "Недостаточно данных."
        )
        assert (
            metadata["simple_customer_finder"]["outreach_strategy"][
                "should_contact_now"
            ]
            is True
        )
        assert (
            metadata["ai_research_profile"]["Overall Lead Score"]["score"]
            == saved["overall_lead_score"]
        )
        assert metadata["outreach_strategy"]["recommended_channel"] == "Email"
        email = db.scalar(
            select(EmailMessage).where(EmailMessage.lead_id == UUID(saved["lead_id"]))
        )
...
def test_ai_customer_finder_job_ranks_by_outreach_success_probability(
    monkeypatch,
) -> None:
    import app.services.ai_customer_finder.service as finder_service
    from app.services.ai_customer_finder.schemas import PublicCustomerCandidate
    ...
    headers = {
        "Authorization": "Bearer dev",
        "X-Test-User-Email": "customer-finder-ranking@example.com",
    }
    ...
    try:
        claimed = claim_next_ai_customer_finder_job(
            db, worker_id="test-worker:customer-finder-ranking"
        )
        assert claimed is not None
        claim_token = claimed.locked_by
    finally:
    ...
    assert (
        payload["results"][0]["lead_intelligence"]["components"]["company_momentum"] > 0
    )
...
def test_ai_customer_finder_partial_provider_failure_keeps_verified_results(
    monkeypatch,
) -> None:
    import app.services.ai_customer_finder.service as finder_service
    from app.services.ai_customer_finder.schemas import PublicCustomerCandidate
    ...
    headers = {
        "Authorization": "Bearer dev",
        "X-Test-User-Email": "customer-finder-partial@example.com",
    }
    ...
    class FakeProvider:
        ...
        def search(self, criteria, *, max_candidates):  # type: ignore[no-untyped-def]
            return [
                PublicCustomerCandidate(
                    company_name="Partial Good Co",
                    website="https://partial-good.example",
                    industry=criteria.target_industry,
                    country=criteria.target_country,
                    source_provider=self.key,
                ),
                PublicCustomerCandidate(
                    company_name="Partial Broken Co",
                    website="https://partial-broken.example",
                    industry=criteria.target_industry,
                    country=criteria.target_country,
                    source_provider=self.key,
                ),
    ...
    try:
        claimed = claim_next_ai_customer_finder_job(
            db, worker_id="test-worker:customer-finder-partial"
        )
        assert claimed is not None
        claim_token = claimed.locked_by
    finally:
...
def test_ai_customer_finder_retains_relevant_candidate_without_buying_signal(
    monkeypatch,
) -> None:
    import app.services.ai_customer_finder.service as finder_service
    from app.services.ai_customer_finder.schemas import PublicCustomerCandidate
    ...
    headers = {
        "Authorization": "Bearer dev",
        "X-Test-User-Email": "customer-finder-fit@example.com",
    }
    ...
    class FakeProvider:
        ...
        def search(self, criteria, *, max_candidates):  # type: ignore[no-untyped-def]
            return [
                PublicCustomerCandidate(
                    company_name="Weak ICP Only Co",
                    website="https://weak-icp.example",
                    industry=criteria.target_industry,
                    country=criteria.target_country,
                    source_provider=self.key,
                )
    ...
    try:
        claimed = claim_next_ai_customer_finder_job(
            db, worker_id="test-worker:customer-finder-fit"
        )
        assert claimed is not None
        claim_token = claimed.locked_by
    finally:
    ...
    assert payload["results"][0]["result_tier"] in {
        "Relevant match",
        "Weak / needs review",
    }
...
def test_ai_customer_finder_website_429_remains_eligible_with_warning(
    monkeypatch,
) -> None:
    import app.services.ai_customer_finder.service as finder_service
    from app.services.ai_customer_finder.schemas import PublicCustomerCandidate
    ...
    headers = {
        "Authorization": "Bearer dev",
        "X-Test-User-Email": "customer-finder-429@example.com",
    }
            ...
            return [
                PublicCustomerCandidate(
                    ...
                    source_payload={
                        ...
                        "business_category": "Balloon store",
                    },
                )
            ]
    ...
    def fake_collect_website(url: str) -> WebsiteSnapshot:
        raise WebsiteTemporaryUnavailableError(
            "Website could not be reached. HTTP status: 429.",
            status_code=429,
    ...
    try:
        claimed = claim_next_ai_customer_finder_job(
            db, worker_id="test-worker:customer-finder-429"
        )
        assert claimed is not None
        claim_token = claimed.locked_by
    finally:
...
def test_ai_customer_finder_repeat_search_deduplicates_company_lead_and_draft(
    monkeypatch,
) -> None:
    import app.services.ai_customer_finder.service as finder_service
    from app.services.ai_customer_finder.schemas import PublicCustomerCandidate
    ...
    headers = {
        "Authorization": "Bearer dev",
        "X-Test-User-Email": "customer-finder-dedupe@example.com",
    }
    ...
    class FakeProvider:
        ...
        def search(self, criteria, *, max_candidates):  # type: ignore[no-untyped-def]
            return [
                PublicCustomerCandidate(
                    company_name="Repeat Finder Co",
                    website="https://repeat-finder.example",
                    industry=criteria.target_industry,
                    country=criteria.target_country,
                    source_provider=self.key,
                )
    ...
    for run_index in range(2):
        ...
        try:
            claimed = claim_next_ai_customer_finder_job(
                db, worker_id=f"test-worker:intent-growth:{run_index}"
            )
            assert claimed is not None
            claim_token = claimed.locked_by
        finally:
            db.close()
        assert (
            process_ai_customer_finder_job(UUID(job_id), claim_token=claim_token)
            is True
        )
        refreshed = client.get(
            f"/api/workspace-app/ai-customer-finder/searches/{job_id}", headers=headers
        ...
        saved = client.post(
            f"/api/workspace-app/leads/first-customers/results/{result_id}/save",
            headers=headers,
        )
    ...
    crm = client.get(
        "/api/workspace-app/companies?search=Repeat%20Finder", headers=headers
    )
    assert crm.status_code == 200, crm.text
    ...
    try:
        leads = list(
            db.scalars(
                select(Lead).where(Lead.email == "sales@repeat-finder.example")
            ).all()
        )
        assert len(leads) == 1
        emails = list(
            db.scalars(
                select(EmailMessage).where(EmailMessage.lead_id == leads[0].id)
            ).all()
        )
        assert len(emails) == 1
    finally:
...
def test_ai_customer_finder_draft_action_keeps_email_unsent(monkeypatch) -> None:
    ...
    headers = {
        "Authorization": "Bearer dev",
        "X-Test-User-Email": "customer-finder-draft-action@example.com",
    }
    ...
    class FakeProvider:
        ...
        def search(self, criteria, *, max_candidates):  # type: ignore[no-untyped-def]
            return [
                PublicCustomerCandidate(
                    company_name="Draft Action Co",
                    website="https://draft-action.example",
                    industry=criteria.target_industry,
                    country=criteria.target_country,
                    source_provider=self.key,
                )
    ...
    try:
        claimed = claim_next_ai_customer_finder_job(
            db, worker_id="test-worker:draft-action"
        )
        assert claimed is not None
        claim_token = claimed.locked_by
    finally:
    ...
    result = refreshed.json()["results"][0]
    blocked = client.post(
        f"/api/workspace-app/ai-customer-finder/results/{result['id']}/draft",
        headers=headers,
    )
...
def test_lead_finder_first_customers_requires_manual_crm_save_and_keeps_outreach_draft_only(
    monkeypatch,
) -> None:
    import app.services.ai_customer_finder.service as finder_service
    from app.services.ai_customer_finder.schemas import PublicCustomerCandidate
    ...
    headers = {
        "Authorization": "Bearer dev",
        "X-Test-User-Email": "first-customers-mode@example.com",
    }
    ...
    try:
        assert (
            db.scalar(
                select(func.count())
                .select_from(Lead)
                .where(Lead.email == "sales@first-signal.example")
            )
            == 0
        )
        assert (
            db.scalar(
                select(func.count())
                .select_from(Company)
                .where(Company.website == "https://first-signal.example")
            )
            == 0
        )
        assert (
            db.scalar(
                select(func.count())
                .select_from(EmailMessage)
                .where(EmailMessage.tags["result_id"].as_string() == result["id"])
            )
            == 0
        )
        source = db.scalar(
            select(AICustomerFinderSource).where(
                AICustomerFinderSource.source_url == "https://first-signal.example"
            )
        )
    ...
    approved = client.post(
        f"/api/workspace-app/emails/{saved['email_id']}/approve", headers=headers
    )
    assert approved.status_code == 200, approved.text
    refreshed_after_approve = client.get(
        f"/api/workspace-app/ai-customer-finder/searches/{payload['id']}",
        headers=headers,
    )
    ...
    try:
        leads = list(
            db.scalars(
                select(Lead).where(Lead.email == "sales@first-signal.example")
            ).all()
        )
        assert len(leads) == 1
...
def test_revenue_intelligence_feed_scores_watchlist_and_tenant_isolation() -> None:
    headers = {
        "Authorization": "Bearer dev",
        "X-Test-User-Email": "revenue-intelligence@example.com",
    }
    workspace = client.get("/api/workspace/me", headers=headers).json()
    _grant_subscription_for_test(
        workspace["id"],
        user_id="revenue-intelligence@example.com",
        plan="Pro",
        status="active",
    )
    company_response = client.post(
        ...
        headers=headers,
        json={
            "name": "Revenue Intel Co",
            "website": "https://revenue-intel.example",
    ...
    peer_response = client.post(
        ...
        headers=headers,
        json={
            "name": "Revenue Peer Co",
            "website": "https://revenue-peer.example",
            "country": "United States",
            "industry": "B2B SaaS",
        },
    )
        ...
        company.metadata_json = {
            **(company.metadata_json or {}),
            ...
            "buying_signal_evidence": [
                {
                    "source_url": "https://news.example/revenue-intel-funding",
                    "value": "Funding announced",
                },
                {
                    "source_url": "https://jobs.example/revenue-intel-sdr",
                    "value": "Hiring SDRs",
                },
            ],
            "confidence_score": 88,
            ...
            "ai_live_buying_signals": {
                ...
                "latest_changes": [
                    {
                        "change_type": "new_funding",
                        "detected_at": datetime.utcnow().isoformat(),
                        "added": ["Funding announced"],
                    }
                ],
                "change_timeline": [
                    {
                        "change_type": "new_hiring",
                        "detected_at": (
    ...
    watch = client.post(
        f"/api/workspace-app/revenue-intelligence/companies/{company_id}/watchlist",
        headers=headers,
        json={"watchlisted": True},
    )
    ...
    _grant_subscription_for_test(
        ...
        user_id="tenant-b@example.com",
        plan="Pro",
        status="active",
    )
    isolated = client.get(
        "/api/workspace-app/revenue-intelligence", headers=USER_B_AUTH
    )
    assert isolated.status_code == 200
    assert all(
        item["company"] != "Revenue Intel Co"
        for item in isolated.json()["top_opportunities"]
    )
...
def test_deep_contact_search_endpoint_downgrades_crm_apply_failure(monkeypatch) -> None:
    ...
    response = client.post(
        ...
        headers=USER_A_AUTH,
        json={
            "name": "Deep Contact Apply Co",
            "website": "https://deepcontact-apply.example",
            "industry": "SaaS",
            "country": "Germany",
        },
    )
    ...
    def fake_deep_search(**_: object) -> DeepContactSearchResult:
        return DeepContactSearchResult(
            status="success",
            company_profile={
                "domain": "deepcontact-apply.example",
                "industry": "SaaS",
                "employee_count": 42,
            },
            candidates=[
...
def test_deep_contact_search_endpoint_handles_deep_contact_search_error(
    monkeypatch,
) -> None:
    import app.api.usage as usage_module
    ...
    response = client.post(
        ...
        headers=USER_A_AUTH,
        json={
            "name": "Deep Contact Error Co",
            "website": "https://deepcontact-error.example",
            "industry": "SaaS",
            "country": "Germany",
        },
    )
    ...
    monkeypatch.setattr(
        usage_module,
        "run_deep_contact_search",
        lambda **_: (_ for _ in ()).throw(
            usage_module.DeepContactSearchError("provider unavailable")
        ),
    )
...
def test_deep_contact_search_endpoint_handles_unexpected_exception(monkeypatch) -> None:
    ...
    response = client.post(
        ...
        headers=USER_A_AUTH,
        json={
            "name": "Deep Contact Unexpected Co",
            "website": "https://deepcontact-unexpected.example",
            "industry": "SaaS",
            "country": "Germany",
        },
    )
...
def test_deep_contact_search_endpoint_handles_provider_timeout(monkeypatch) -> None:
    ...
    response = client.post(
        ...
        headers=USER_A_AUTH,
        json={
            "name": "Deep Contact Timeout Co",
            "website": "https://deepcontact-timeout.example",
            "industry": "SaaS",
            "country": "Germany",
        },
    )
    ...
    monkeypatch.setattr(
        ...
        "run_deep_contact_search",
        lambda **_: (_ for _ in ()).throw(
            usage_module.DeepContactSearchError("Provider timeout")
        ),
    )
...
def test_deep_contact_search_endpoint_handles_provider_unavailable(monkeypatch) -> None:
    ...
    response = client.post(
        ...
        headers=USER_A_AUTH,
        json={
            "name": "Deep Contact Provider Co",
            "website": "https://deepcontact-provider.example",
            "industry": "SaaS",
            "country": "Germany",
        },
    )
    ...
    monkeypatch.setattr(
        ...
        "run_deep_contact_search",
        lambda **_: (_ for _ in ()).throw(
            usage_module.DeepContactSearchError("Apollo is not connected.")
        ),
    )
...
def test_deep_contact_search_endpoint_handles_company_serialization_failure(
    monkeypatch,
) -> None:
    import app.api.usage as usage_module
    ...
    response = client.post(
        ...
        headers=USER_A_AUTH,
        json={
            "name": "Deep Contact Serialization Co",
            "website": "https://deepcontact-serialization.example",
            "industry": "SaaS",
            "country": "Germany",
        },
    )
    ...
    monkeypatch.setattr(
        ...
        lambda *_args, **_kwargs: (_ for _ in ()).throw(
            RuntimeError("crm serialization failed")
        ),
    )
...
def _stripe_product_object(plan: str) -> SimpleNamespace:
    return SimpleNamespace(
        id=f"prod_{plan.lower()}_test",
        name=PLAN_CATALOG[plan].display_name,
        metadata={"plan": plan, "brand": "OutreachAI"},
    )


def _stripe_price_object(
    price_id: str,
    *,
    amount: int = 14900,
    currency: str = "eur",
    ...
    active: bool = True,
    lookup_key: str = "outreachai_pro_monthly",
    ...
    return SimpleNamespace(
        ...
        else _stripe_product_object(_plan_for_test_price(price_id)),
    )
            ...
            "data": [
                {
                    ...
                    "price": {
                        "id": price_id,
                        "product": {
                            "id": f"prod_{_plan_for_test_price(price_id).lower()}_test",
                            "metadata": {
                                "plan": _plan_for_test_price(price_id),
                                "brand": "OutreachAI",
                            },
                        },
                    },
...
def _pending_checkout_session_for_test(
    workspace_id: str,
    user_id: str,
    *,
    plan: str = "Pro",
    customer_id: str = "cus_checkout_test",
    session_id: str = "cs_checkout_test",
) -> None:
    with get_sessionmaker()() as db:
        db.add(
            BillingCheckoutSession(
...
class _FakePostgresConnection:
    ...
    def execute(
        ...
        if "FROM information_schema.tables" in sql:
            rows = [
                (name,) for name in sorted(self.state.tables - {"schema_migrations"})
            ]
            return _FakeScalarResult(rows=rows)
        if "SELECT version FROM schema_migrations" in sql:
            return _FakeScalarResult(
                rows=[(version,) for version in sorted(self.state.applied_versions)]
            )
        ...
        if "to_regclass" in sql:
            table_name = (
                str(params["name"]).split(".")[-1] if params else "schema_migrations"
            )
...
def test_postgres_migration_runner_applies_011_to_existing_database_idempotently(
    ...
    for version in database_module.REQUIRED_POSTGRES_MIGRATIONS:
        ...
        migration_path.write_text(
            (database_module.PACKAGED_MIGRATIONS_DIR / f"{version}.sql").read_text(),
            encoding="utf-8",
        )
    ...
    assert {"ai_memory_settings", "ai_memory_entries", "ai_memory_audit_logs"}.issubset(
        state.tables
    )
...
def test_postgres_migration_runner_serializes_parallel_instances(
    tmp_path, monkeypatch
) -> None:
    import threading
    import app.core.database as database_module
...
def test_postgres_migration_runner_drops_invalid_concurrent_index_before_retry(
    tmp_path, monkeypatch
) -> None:
    import app.core.database as database_module
    ...
    migration_path.write_text(
        (
            database_module.PACKAGED_MIGRATIONS_DIR
            / "013_production_hardening_read_paths.sql"
        ).read_text(),
        encoding="utf-8",
    )
    ...
    drop_positions = [
        ...
        and "idx_audit_logs_workspace_lead_created_id" in statement
    ]
    create_positions = [
        index
        for index, statement in enumerate(state.statements)
        if "CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_audit_logs_workspace_lead_created_id"
        in statement
    ]
...
def test_postgres_migration_runner_repairs_real_invalid_concurrent_index(
    monkeypatch,
) -> None:
    database_url = os.getenv("POSTGRES_TEST_DATABASE_URL")
    if not database_url:
        pytest.skip(
            "POSTGRES_TEST_DATABASE_URL is required for the real PostgreSQL invalid-index smoke test"
        )
    ...
    try:
        ...
        pytest.skip(
            f"PostgreSQL invalid-index smoke requires local connectivity and CREATE DATABASE privileges: {exc}"
        )
    ...
    try:
        ...
        with temp_engine.begin() as connection:
            ...
            connection.execute(
                text(
                    "INSERT INTO schema_migrations (version) VALUES ('011_ai_memory'), ('012_crm_inbox_read_indexes')"
                )
            )
            connection.execute(
                text("CREATE TABLE ai_memory_settings (id uuid PRIMARY KEY)")
            )
            connection.execute(
                text("CREATE TABLE ai_memory_entries (id uuid PRIMARY KEY)")
            )
            connection.execute(
                text("CREATE TABLE ai_memory_audit_logs (id uuid PRIMARY KEY)")
            )
            connection.execute(
                text(
                    "CREATE TABLE email_messages (id uuid PRIMARY KEY, workspace_id uuid, direction varchar(16), lead_id uuid, created_at timestamp)"
                )
            )
            connection.execute(
                text(
                    "CREATE TABLE contacts (id uuid PRIMARY KEY, workspace_id uuid, company_id uuid, created_at timestamp)"
                )
            )
            connection.execute(
                text(
                    "CREATE TABLE deals (id uuid PRIMARY KEY, workspace_id uuid, company_id uuid, created_at timestamp)"
                )
            )
            connection.execute(
                text(
                    "CREATE TABLE notes (id uuid PRIMARY KEY, workspace_id uuid, company_id uuid, created_at timestamp)"
                )
            )
            connection.execute(
                text(
                    "CREATE TABLE audit_logs (id uuid PRIMARY KEY, workspace_id uuid, metadata_json jsonb, created_at timestamp)"
                )
            )
            connection.execute(
                text(
        ...
        monkeypatch.setattr(
            ...
            lambda: [
                ...
                / "db"
                / "migrations"
                / "014_email_message_recipient_email.sql",
            ],
        )
        ...
        with temp_engine.connect() as connection:
            assert (
                connection.execute(
                    text(
                        "SELECT version FROM schema_migrations WHERE version = '013_production_hardening_read_paths'"
                    )
                ).scalar()
            ...
            assert (
                connection.execute(
                    text(
                        "SELECT version FROM schema_migrations WHERE version = '014_email_message_recipient_email'"
                    )
                ).scalar()
                == "014_email_message_recipient_email"
            )
...
def test_postgres_migration_failure_sets_negative_schema_status(
    tmp_path, monkeypatch
) -> None:
    import app.core.database as database_module
    ...
    migration_path.write_text(
        (REPO_ROOT / "db" / "migrations" / "011_ai_memory.sql").read_text(),
        encoding="utf-8",
    )
    monkeypatch.setattr(database_module, "_migration_paths", lambda: [migration_path])
    ...
    assert status.pending_migrations == list(
        database_module.REQUIRED_POSTGRES_MIGRATIONS
    )
    assert set(status.missing_tables) == {
        "ai_memory_settings",
        "ai_memory_entries",
        "ai_memory_audit_logs",
        "agent_runs",
...
def test_ai_memory_migration_assets_are_packaged_with_api_image() -> None:
    root_migration = (REPO_ROOT / "db" / "migrations" / "011_ai_memory.sql").read_text(
        encoding="utf-8"
    )
    packaged_migration = (
        REPO_ROOT / "apps" / "api" / "app" / "db" / "migrations" / "011_ai_memory.sql"
    ).read_text(encoding="utf-8")
    root_read_indexes = (
        REPO_ROOT / "db" / "migrations" / "012_crm_inbox_read_indexes.sql"
    ).read_text(encoding="utf-8")
    packaged_read_indexes = (
        REPO_ROOT
        / "apps"
        / "api"
        / "app"
        / "db"
        / "migrations"
        / "012_crm_inbox_read_indexes.sql"
    ).read_text(encoding="utf-8")
    root_hardening = (
        REPO_ROOT / "db" / "migrations" / "013_production_hardening_read_paths.sql"
    ).read_text(encoding="utf-8")
    packaged_hardening = (
        REPO_ROOT
        / "apps"
    ...
    root_recipient = (
        REPO_ROOT / "db" / "migrations" / "014_email_message_recipient_email.sql"
    ).read_text(encoding="utf-8")
    packaged_recipient = (
        REPO_ROOT
        / "apps"
    ...
    root_workspace_profile = (
        REPO_ROOT / "db" / "migrations" / "016_workspace_profile_send_confirmation.sql"
    ).read_text(encoding="utf-8")
    packaged_workspace_profile = (
        REPO_ROOT
        / "apps"
        / "api"
        / "app"
        / "db"
        / "migrations"
        / "016_workspace_profile_send_confirmation.sql"
    ).read_text(encoding="utf-8")
    root_canonical_billing = (
        REPO_ROOT / "db" / "migrations" / "019_canonical_subscription_resolver.sql"
    ).read_text(encoding="utf-8")
    packaged_canonical_billing = (
        REPO_ROOT
        / "apps"
        / "api"
        / "app"
    ...
    assert (
        ...
        in root_workspace_profile
    )
    assert "stripe_event_created_at" in root_canonical_billing
    assert (
        "duplicate nonempty stripe_subscription_id values exist"
        in root_canonical_billing
    )
...
def test_canonical_billing_migration_preflights_duplicate_stripe_subscription_ids() -> (
    None
):
    sql = (
        REPO_ROOT / "db" / "migrations" / "019_canonical_subscription_resolver.sql"
    ).read_text(encoding="utf-8")
    preflight_index = sql.index(
        "duplicate nonempty stripe_subscription_id values exist"
    )
    unique_index = sql.index(
...
def test_canonical_billing_migration_duplicate_preflight_query_counts_zero_and_duplicates() -> (
    None
):
    ...
    try:
        ...
        connection.execute(
            "INSERT INTO subscriptions (stripe_subscription_id) VALUES ('sub_two')"
        )
...
def test_api_railway_watch_patterns_include_database_migrations() -> None:
    railway_config = (REPO_ROOT / "apps" / "api" / "railway.toml").read_text(
        encoding="utf-8"
    )
...
def test_ai_memory_migration_has_no_destructive_runtime_sql() -> None:
    sql = (REPO_ROOT / "db" / "migrations" / "011_ai_memory.sql").read_text(
        encoding="utf-8"
    )
    runtime_sql = "\n".join(
...
def test_validate_required_environment_fails_fast_in_production(monkeypatch) -> None:
    ...
    settings = Settings(
        ...
        required_runtime_envs="DATABASE_URL,CLERK_SECRET_KEY",
    )
...
def test_validate_required_environment_rejects_placeholder_values_in_production(
    monkeypatch,
) -> None:
    monkeypatch.setenv("APP_ENV", "production")
    ...
    settings = Settings(
        app_env="production",
        strict_startup_env_validation=False,
        required_runtime_envs="DATABASE_URL,CLERK_SECRET_KEY,CLERK_JWT_ISSUER",
    )

    with pytest.raises(
        RuntimeError, match="DATABASE_URL|CLERK_SECRET_KEY|CLERK_JWT_ISSUER"
    ):
        validate_required_environment(settings)


def test_validate_database_connectivity_requires_postgresql_in_production(
    monkeypatch,
) -> None:
    monkeypatch.setenv("APP_ENV", "production")
    ...
    settings = Settings(
        app_env="production",
        strict_startup_env_validation=False,
        database_url="sqlite:///./outreachai.db",
    )
...
def test_serve_main_routes_worker_role_to_worker_entrypoint(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        serve_module, "_start_worker_health_server", lambda *args, **kwargs: None
    )
...
def test_readiness_returns_503_when_postgresql_is_unavailable_in_production(
    monkeypatch,
) -> None:
    import app.main as main_module
...
def test_readiness_returns_503_when_required_environment_is_missing_in_production(
    monkeypatch,
) -> None:
    import app.main as main_module
    ...
    monkeypatch.setattr(
        ...
        Settings(
            ...
            strict_startup_env_validation=False,
            required_runtime_envs="DATABASE_URL,CLERK_SECRET_KEY",
            database_url="postgresql+psycopg://db.example/outreachai",
        ),
    )
    monkeypatch.delenv("CLERK_SECRET_KEY", raising=False)
    monkeypatch.setattr(
        main_module, "validate_database_connectivity", lambda settings: None
    )
    ...
    assert any(
        "CLERK_SECRET_KEY" in failure for failure in payload["critical_failures"]
    )


def test_readiness_returns_503_when_database_backups_are_missing_in_production(
    monkeypatch,
) -> None:
    import app.main as main_module
    ...
    monkeypatch.setattr(
        main_module, "database_backups_operational", lambda db, settings: False
    )
...
def test_liveness_survives_schema_drift_but_readiness_fails(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        main_module, "validate_runtime_schema", lambda engine: drift_status
    )
    monkeypatch.setattr(
        main_module, "database_backups_operational", lambda db, settings: True
    )
    ...
    assert set(payload["schema"]["missing_tables"]) == {
        "ai_memory_settings",
        "ai_memory_entries",
        "ai_memory_audit_logs",
    }
    assert any(
        "Database schema is not ready" in failure
        for failure in payload["critical_failures"]
    )
...
def test_database_backup_readiness_requires_strict_true() -> None:
    ...
    assert (
        database_backup_configured(Settings(database_backups_enabled="false")) is False
    )
...
def test_postgres_schema_assets_include_billing_checkout_sessions() -> None:
    ...
    packaged_migration = (
        ...
        / "db"
        / "migrations"
        / "018_billing_checkout_idempotency.sql"
    ).read_text(encoding="utf-8")
    root_migration = (
...
def test_postgres_schema_assets_include_billing_subscription_transitions() -> None:
    ...
    packaged_migration = (
        ...
        / "db"
        / "migrations"
        / "020_billing_subscription_transitions.sql"
    ).read_text(encoding="utf-8")
    root_migration = (
        Path(__file__).resolve().parents[3]
        / "db"
...
def test_startup_logs_validation_steps_and_fails_fast_on_database_error(
    ...
    monkeypatch.setattr(
        main_module, "validate_required_environment", lambda settings: []
    )
    monkeypatch.setattr(
        main_module,
        "validate_database_connectivity",
        lambda settings: (_ for _ in ()).throw(
            RuntimeError("Production startup requires PostgreSQL")
        ),
    )
    ...
    monkeypatch.setattr(
        "app.jobs.worker.start_embedded_enrichment_worker", lambda: None
    )
...
def test_cloud_backup_retention_respects_age_and_count() -> None:
    ...
    assert (
        _is_past_retention(
            settings, index=0, last_modified=now - timedelta(days=1), now=now
        )
        is False
    )
    assert (
        _is_past_retention(
            settings, index=2, last_modified=now - timedelta(days=1), now=now
        )
        is True
    )
    assert (
        _is_past_retention(
            settings, index=0, last_modified=now - timedelta(days=31), now=now
        )
        is True
    )


def test_restore_verification_resets_restore_database_before_import(
    ...
    monkeypatch.setattr(
        "app.services.backups.shutil.which",
    ...
    assert (
        commands[0][-1] == "DROP SCHEMA IF EXISTS public CASCADE; CREATE SCHEMA public;"
    )
    assert commands[1][1] == "postgresql://restore.example/outreachai_restore_test"


def test_restore_verification_refuses_source_database(
    monkeypatch, tmp_path: Path
) -> None:
    archive = tmp_path / "backup.sql.gz"
    import gzip
    ...
    with get_sessionmaker()() as db:
        ...
        audit = next(
            (
                item
                for item in audits
                if item.metadata_json.get("request_id") == "audit-request-123"
            ),
            None,
        )
...
def test_profile_language_updates_private_workspace_language() -> None:
    headers = {
        "Authorization": "Bearer dev",
        "X-Test-User-Email": "profile-language@example.com",
    }
    profile = client.put(
        "/api/profile",
        headers=headers,
...
def test_owner_can_update_feature_flags() -> None:
    denied = client.patch(
        "/api/owner/feature-flags", headers=NON_OWNER_AUTH, json={"ai_ceo_voice": True}
    )
    assert denied.status_code == 403
    ...
    response = client.patch(
        ...
        headers=OWNER_AUTH,
        json={"ai_ceo_voice": True, "analytics_nav": True},
    )
    assert response.status_code == 200
    data = response.json()
...
def test_workspace_is_created_once_for_repeated_first_login() -> None:
    ...
    with get_sessionmaker()() as db:
        workspaces = list(
            db.scalars(select(Workspace).where(Workspace.owner_user_id == email)).all()
        )
        members = list(
            db.scalars(
                select(WorkspaceMember).where(WorkspaceMember.user_id == email)
            ).all()
        )
...
def test_gmail_alias_customer_does_not_inherit_system_owner_access() -> None:
    headers = {
        "Authorization": "Bearer dev",
        "X-Test-User-Email": "romaniukvadym10+client@gmail.com",
    }
...
def test_admin_queue_health_is_owner_only_and_reports_metrics() -> None:
    ...
    try:
        ...
        pending_lead = Lead(
            ...
            company="Queue Pending Co",
        )
        running_lead = Lead(
            user_id="dev_user",
            workspace_id=workspace_id,
            campaign_id=campaign.id,
            company="Queue Running Co",
        )
        dead_lead = Lead(
            user_id="dev_user",
            workspace_id=workspace_id,
            campaign_id=campaign.id,
            company="Queue Dead Co",
        )
...
def test_workspace_data_is_private_between_users(monkeypatch) -> None:
    monkeypatch.setattr(
        "app.api.routes._hunter_enriched_leads",
        lambda db, request, user_id, workspace, leads: leads,
    )
    monkeypatch.setattr(
        "app.api.routes._analyze_lead_if_possible",
        lambda db, user_id, workspace, lead: None,
    )
    ...
    user_a_leads = client.get(
        "/api/leads?search=Tenant%20A%20Berlin%20Builders", headers=USER_A_AUTH
    )
    user_b_leads = client.get(
        "/api/leads?search=Tenant%20A%20Berlin%20Builders", headers=USER_B_AUTH
    )
    ...
    user_a_companies = client.get(
        "/api/crm/companies?search=Tenant%20A%20Berlin%20Builders", headers=USER_A_AUTH
    )
    user_b_companies = client.get(
        "/api/crm/companies?search=Tenant%20A%20Berlin%20Builders", headers=USER_B_AUTH
    )
    ...
    forbidden_stage_update = client.patch(
        f"/api/crm/companies/{company_id}/stage",
        headers=USER_B_AUTH,
    ...
    campaign_response = client.post(
        "/api/campaigns", headers=USER_A_AUTH, json=campaign_payload
    )
    ...
    forbidden_campaign_update = client.put(
        f"/api/campaigns/{campaign_id}",
        headers=USER_B_AUTH,
        json={**campaign_payload, "name": "Hijacked"},
    )
...
def test_workspace_drafts_and_gmail_settings_are_private_between_users(
    monkeypatch,
) -> None:
    ...
    lead = client.post(
        ...
        headers=user_a_headers,
        json={
            "company": "Draft Isolation Co",
            "website": "https://draft-isolation.example",
            "industry": "Construction",
            "email": "buyer@draft-isolation.example",
        },
    )
    assert lead.status_code == 200
    draft = client.post(
        f"/api/leads/{lead.json()['id']}/draft-email", headers=user_a_headers
    )
    ...
    forbidden_edit = client.patch(
        f"/api/emails/{draft_id}", headers=user_b_headers, json={"subject": "Hijacked"}
    )
        ...
        assert (
            db.scalar(
                select(EmailMessage).where(
                    EmailMessage.id == UUID(draft_id),
                    EmailMessage.workspace_id == UUID(workspace_b["id"]),
                )
            )
            is None
        )
...
def test_ai_memory_tenant_isolation_delete_clear_and_secret_redaction() -> None:
    ...
    user_b_list = client.get(
        "/api/workspace-app/ai-memory/entries", headers=USER_B_AUTH
    )
    assert user_b_list.status_code == 200
    assert all(item["id"] != entry["id"] for item in user_b_list.json()["entries"])

    forbidden_delete = client.delete(
        f"/api/workspace-app/ai-memory/entries/{entry['id']}", headers=USER_B_AUTH
    )
    assert forbidden_delete.status_code == 404

    deleted = client.delete(
        f"/api/workspace-app/ai-memory/entries/{entry['id']}", headers=USER_A_AUTH
    )
    assert deleted.status_code == 200
    after_delete = client.get(
        "/api/workspace-app/ai-memory/entries", headers=USER_A_AUTH
    ).json()
    assert all(item["id"] != entry["id"] for item in after_delete["entries"])

    client.post(
        "/api/workspace-app/ai-memory/preferences",
        headers=USER_A_AUTH,
        json={"content": "Use one CTA only."},
    )
...
def test_ai_memory_new_workspace_defaults_disabled_and_can_be_enabled() -> None:
    headers = {
        "Authorization": "Bearer dev",
        "X-Test-User-Email": f"memory-disabled-{uuid4()}@example.com",
    }
...
def test_ai_memory_disabled_does_not_call_embedding_provider(monkeypatch) -> None:
    ...
    headers = {
        "Authorization": "Bearer dev",
        "X-Test-User-Email": f"memory-no-embedding-{uuid4()}@example.com",
    }
    workspace = client.get("/api/workspace/me", headers=headers).json()
    settings = client.get(
        "/api/workspace-app/ai-memory/settings", headers=headers
    ).json()
    ...
    created = client.post(
        ...
        json={
            "memory_type": "verified_fact",
            "content": "Product: disabled memory should not embed",
            "source": "test",
            "verified": True,
        },
    )
    ...
    with get_sessionmaker()() as db:
        ...
        retrieval = retrieve_memory(
            ...
            query="disabled memory",
            purpose="disabled_test",
        )
...
def test_ai_memory_retrieval_works_after_explicit_enable() -> None:
    headers = {
        "Authorization": "Bearer dev",
        "X-Test-User-Email": f"memory-enabled-{uuid4()}@example.com",
    }
    workspace = client.get("/api/workspace/me", headers=headers).json()
    disabled_created = client.post(
        ...
        json={
            "memory_type": "verified_fact",
            "content": "Product: staged rollout clinic workflow",
            "source": "test",
            "verified": True,
        },
    )
    ...
    with get_sessionmaker()() as db:
        ...
        retrieval = retrieve_memory(
            ...
            workspace=ws,
            user_id=headers["X-Test-User-Email"],
            query="clinic workflow",
            purpose="enabled_test",
        )
        assert retrieval.context["enabled"] is True
        assert retrieval.context["retrieval_mode"] == MODE_KEYWORD
        assert any(
            "staged rollout clinic workflow" in item["content"]
            for item in retrieval.context["items"]
        )


def test_ai_memory_preference_requires_confirmation_and_inference_is_not_verified() -> (
    None
):
    ...
    inference = client.post(
        ...
        headers=AUTH,
        json={
            "memory_type": "ai_inference",
            "content": "They may need RevOps help.",
            "source": "test",
            "verified": True,
        },
    )
...
def test_ai_memory_retrieval_filters_workspace_deleted_expired_and_prompt_injection() -> (
    None
):
    ...
    with get_sessionmaker()() as db:
        ...
        deleted = upsert_memory_entry(
            db,
            workspace=ws_a,
            user_id="tenant-a@example.com",
            memory_type="outcome",
            content="Deleted success outcome dental",
            source="test",
        )
        expired = upsert_memory_entry(
            db,
            workspace=ws_a,
            user_id="tenant-a@example.com",
            memory_type="outcome",
            content="Expired success outcome dental",
            source="test",
        )
        other = upsert_memory_entry(
            db,
            workspace=ws_b,
            user_id="tenant-b@example.com",
            memory_type="verified_fact",
            content="Other tenant dental context",
            source="test",
            verified=True,
        )
...
def test_ai_memory_hash_fallback_reports_keyword(monkeypatch) -> None:
    ...
    headers = {
        "Authorization": "Bearer dev",
        "X-Test-User-Email": f"memory-keyword-{uuid4()}@example.com",
    }
    _enable_ai_memory(headers)
    workspace = client.get("/api/workspace/me", headers=headers).json()
    with get_sessionmaker()() as db:
        ...
        upsert_memory_entry(
            ...
            user_id=headers["X-Test-User-Email"],
            memory_type="verified_fact",
            content="Product: keyword-only clinic scheduling workflow",
            source="test",
            verified=True,
        )
        db.commit()
        retrieval = retrieve_memory(
            ...
            workspace=ws,
            user_id=headers["X-Test-User-Email"],
            query="clinic scheduling workflow",
            purpose="test",
        )
        assert retrieval.context["retrieval_mode"] == MODE_KEYWORD
...
def test_ai_memory_openai_embedding_without_pgvector_does_not_report_pgvector(
    monkeypatch,
) -> None:
    embedding = [0.01] * 1536
    monkeypatch.setattr(
        "app.services.ai_memory._openai_embedding", lambda value: embedding
    )
    monkeypatch.setattr(
        "app.services.ai_memory._pgvector_column_available", lambda db: False
    )
    headers = {
        "Authorization": "Bearer dev",
        "X-Test-User-Email": f"memory-openai-{uuid4()}@example.com",
    }
    _enable_ai_memory(headers)
    workspace = client.get("/api/workspace/me", headers=headers).json()
    with get_sessionmaker()() as db:
        ...
        entry = upsert_memory_entry(
            ...
            user_id=headers["X-Test-User-Email"],
            memory_type="verified_fact",
            content="Product: OpenAI embedding clinic scheduling workflow",
            source="test",
            verified=True,
        )
...
def test_ai_memory_migration_optional_vector_permission_failure_is_non_blocking() -> (
    None
):
    ...
    ) > migration.index("optional pgvector setup skipped")
    assert "DROP INDEX IF EXISTS idx_ai_memory_entries_embedding" in migration


def test_ai_memory_context_is_added_to_ai_sales_analysis_and_explain(
    monkeypatch,
) -> None:
    headers = {
        "Authorization": "Bearer dev",
        "X-Test-User-Email": f"memory-analysis-{uuid4()}@example.com",
    }
    _enable_ai_memory(headers)
    workspace = client.get("/api/workspace/me", headers=headers).json()
    with get_sessionmaker()() as db:
        lead = Lead(
            user_id=headers["X-Test-User-Email"],
            workspace_id=UUID(workspace["id"]),
            company="Memory Fit Co",
            website="https://memory-fit.example",
            industry="Dental SaaS",
            email="buyer@memory-fit.example",
        )
        db.add(lead)
        db.flush()
        company = Company(
            ...
            ai_summary="Dental SaaS platform",
        )
        db.add(company)
        upsert_memory_entry(
            db,
            ...
            user_id=headers["X-Test-User-Email"],
            memory_type="verified_fact",
            content="Product: AI appointment follow-up for clinics",
            source="test",
            verified=True,
        )
        db.commit()
    ...
    def fake_analysis(**kwargs: Any) -> dict[str, Any]:
        ...
        return {
            ...
            "evidence": [
                {
                    "source_field": "company.website",
                    "value": "https://memory-fit.example",
                    "verified": True,
                    "confidence": 95,
                }
            ],
    ...
    monkeypatch.setattr(
        "app.api.usage.build_ai_sales_workspace_analysis", fake_analysis
    )
    response = client.post(
        f"/api/workspace-app/companies/{company_id}/ai-sales-analysis",
        headers=headers,
        json={"force": True},
    )
    assert response.status_code == 200
    ...
    explain = client.get(
        f"/api/workspace-app/ai-memory/decisions/{company_id}/explain", headers=headers
    )
...
def test_ai_memory_feedback_outcome_and_approve_before_send(monkeypatch) -> None:
    headers = {
        "Authorization": "Bearer dev",
        "X-Test-User-Email": f"memory-feedback-{uuid4()}@example.com",
    }
    _enable_ai_memory(headers)
    workspace = client.get("/api/workspace/me", headers=headers).json()
    with get_sessionmaker()() as db:
        lead = Lead(
            ...
            workspace_id=UUID(workspace["id"]),
            company="Outcome Co",
            website="https://outcome.example",
            industry="SaaS",
            email="buyer@outcome.example",
        )
        db.add(lead)
        db.flush()
        company = Company(
    ...
    monkeypatch.setattr(
        "app.api.usage.send_email", lambda **kwargs: {"id": "memory-provider-message"}
    )
    sent = client.post(f"/api/workspace-app/emails/{email_id}/send", headers=headers)
    assert sent.status_code == 200
    entries = client.get(
        "/api/workspace-app/ai-memory/entries?memory_type=outcome", headers=headers
    )
...
def test_openai_sdk_is_compatible_with_pinned_httpx_for_ai_memory_embeddings() -> None:
    ...
    openai_version = tuple(
        int(part) for part in importlib_metadata.version("openai").split(".")[:3]
    )
...
def _embedding_response(
    request: httpx.Request, embedding: Optional[list[float]] = None
) -> httpx.Response:
    return httpx.Response(
        200,
        json={
            ...
            "data": [
                {
                    "object": "embedding",
                    "index": 0,
                    "embedding": embedding or [0.01] * 1536,
                }
            ],
...
def test_openai_chat_completion_and_embedding_response_parsing_with_mock_transport() -> (
    None
):
    ...
    chat = sdk.chat.completions.create(
        model="gpt-test", messages=[{"role": "user", "content": "Return JSON."}]
    )
...
def test_openai_empty_embeddings_response_parsing_with_mock_transport() -> None:
    sdk = _openai_mock_client(
        lambda request: httpx.Response(
            200,
            json={"object": "list", "data": [], "model": "text-embedding-3-small"},
            request=request,
        )
    )
...
def test_ai_memory_empty_embeddings_response_falls_back_without_error(
    monkeypatch,
) -> None:
    sdk = _openai_mock_client(
        lambda request: httpx.Response(
...
def test_openai_sdk_http_errors_do_not_return_embeddings(
...
def test_openai_retry_after_120_is_not_slept_when_max_retries_zero(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        openai_base_client.time, "sleep", lambda seconds: sleeps.append(seconds)
    )
    sdk = _openai_mock_client(handler, max_retries=0)
...
def test_openai_retry_after_120_is_observed_with_max_retries_one_without_real_sleep(
    monkeypatch,
) -> None:
    import openai._base_client as openai_base_client
...
def test_ai_memory_embedding_client_uses_bounded_no_retry_policy(monkeypatch) -> None:
    ...
    client_kwargs = next(
        call["client_kwargs"] for call in calls if "client_kwargs" in call
    )
...
def test_approve_email_uses_ai_memory_embedding_path_without_openai_httpx_client_error(
    monkeypatch,
) -> None:
    headers = {
        "Authorization": "Bearer dev",
        "X-Test-User-Email": f"memory-approve-{uuid4()}@example.com",
    }
    calls: list[dict[str, Any]] = []
    ...
    monkeypatch.setattr("app.services.ai_memory.OpenAI", FakeOpenAI)
    monkeypatch.setattr(
        "app.services.ai_memory._write_pgvector_embedding",
        lambda db, entry_id, embedding: None,
    )
    monkeypatch.setattr(
        "app.services.ai_memory._clear_pgvector_embedding", lambda db, entry_id: None
    )
    _enable_ai_memory(headers)
    email = _workspace_app_test_draft(
        headers, monkeypatch, company_name="Memory Approve Embedding Co"
    )
    service_settings = SimpleNamespace(
        app_env="production",
        openai_api_key="openai_test",
    ...
    monkeypatch.setattr("app.services.ai_memory.get_settings", lambda: service_settings)
    approved = client.post(
        f"/api/workspace-app/emails/{email['id']}/approve", headers=headers
    )

    assert approved.status_code == 200
    assert approved.json()["email"]["delivery_status"] == "approved"
    assert any(
        call.get("model") == service_settings.openai_embedding_model for call in calls
    )
    assert any("client_kwargs" in call for call in calls)


def test_approve_email_embedding_failure_is_safe_and_does_not_duplicate_memory(
    monkeypatch,
) -> None:
    headers = {
        "Authorization": "Bearer dev",
        "X-Test-User-Email": f"memory-approve-failure-{uuid4()}@example.com",
    }
    ...
    _enable_ai_memory(headers)
    email = _workspace_app_test_draft(
        headers, monkeypatch, company_name="Memory Approve Failure Co"
    )
    service_settings = SimpleNamespace(
        app_env="production",
        openai_api_key="openai_test",
    ...
    with get_sessionmaker()() as db:
        rows = db.scalars(
            select(AIMemoryEntry).where(
                AIMemoryEntry.email_id == UUID(email["id"]),
                AIMemoryEntry.source == "email.approved",
            )
        ).all()
        assert len(rows) == 1
...
def test_ai_memory_correction_recomputes_keywords_embedding_and_blocks_cross_workspace(
    monkeypatch,
) -> None:
    embedding = [0.02] * 1536
    monkeypatch.setattr(
    ...
    headers = {
        "Authorization": "Bearer dev",
        "X-Test-User-Email": f"memory-correct-{uuid4()}@example.com",
    }
    _enable_ai_memory(headers)
    created = client.post(
        "/api/workspace-app/ai-memory/entries",
        ...
        json={
            "memory_type": "verified_fact",
            "content": "Old product: spreadsheet cleanup",
            "source": "test",
            "verified": True,
        },
    )
    ...
    blocked = client.patch(
        ...
        headers=USER_B_AUTH,
        json={"content": "Cross workspace edit"},
    )
    ...
    corrected = client.patch(
        ...
        headers=headers,
        json={
            "content": "New product: clinic appointment workflow. api_key=secret-value"
        },
    )
...
def test_ai_memory_email_generator_receives_memory_context_before_generation(
    monkeypatch,
) -> None:
    ...
    headers = {
        "Authorization": "Bearer dev",
        "X-Test-User-Email": f"memory-email-{uuid4()}@example.com",
    }
    _enable_ai_memory(headers)
    workspace = client.get("/api/workspace/me", headers=headers).json()
    with get_sessionmaker()() as db:
        ...
        lead = Lead(
            user_id=headers["X-Test-User-Email"],
            workspace_id=ws.id,
            company="Memory Email Co",
            website="https://memory-email.example",
            industry="Clinic SaaS",
            email="buyer@memory-email.example",
        )
        db.add(lead)
        db.commit()
        lead_id = str(lead.id)
    ...
    assert any(
        "AI appointment follow-up" in item["content"]
...
def test_ai_memory_disabled_email_generator_gets_disabled_context(monkeypatch) -> None:
    ...
    headers = {
        "Authorization": "Bearer dev",
        "X-Test-User-Email": f"memory-disabled-email-{uuid4()}@example.com",
    }
    workspace = client.get("/api/workspace/me", headers=headers).json()
    assert (
        client.get("/api/workspace-app/ai-memory/settings", headers=headers).json()[
            "enabled"
        ]
        is False
    )
    with get_sessionmaker()() as db:
        ...
        lead = Lead(
            user_id=headers["X-Test-User-Email"],
            workspace_id=ws.id,
            company="Disabled Memory Email Co",
            website="https://disabled-memory-email.example",
            industry="Clinic SaaS",
            email="buyer@disabled-memory-email.example",
        )
        db.add(lead)
        db.commit()
        lead_id = str(lead.id)
...
def test_workspace_me_creates_private_workspace_with_owner_email() -> None:
    response = client.get(
        ...
        headers={
            "Authorization": "Bearer dev",
            "X-Test-User-Email": "new-owner@example.com",
        },
    )
    assert response.status_code == 200
    data = response.json()
...
def test_workspace_business_profile_offer_tone_cta_persist_after_refresh() -> None:
    headers = {
        "Authorization": "Bearer dev",
        "X-Test-User-Email": f"profile-owner-{uuid4()}@example.com",
    }
    ...
    with get_sessionmaker()() as db:
        ...
        member = db.scalar(
            select(WorkspaceMember).where(
                ...
                WorkspaceMember.user_id == user_email,
            )
        )
...
def test_new_private_workspace_gets_fourteen_day_trial_status() -> None:
    headers = {
        "Authorization": "Bearer dev",
        "X-Test-User-Email": "trial-owner@example.com",
    }
...
def test_existing_workspace_without_authoritative_subscription_is_inactive_in_production_ai_gate(
    monkeypatch,
) -> None:
    headers = {
        "Authorization": "Bearer dev",
        "X-Test-User-Email": "legacy-trial-owner@example.com",
    }
    workspace_response = client.get("/api/workspace/me", headers=headers)
    assert workspace_response.status_code == 200
    ...
    with SessionLocal() as db:
        ...
        assert workspace is not None
        settings = db.scalar(
            select(AppSettings).where(AppSettings.workspace_id == UUID(workspace_id))
        )
        if settings is None:
            settings = AppSettings(
                user_id="legacy-trial-owner@example.com", workspace_id=workspace.id
            )
...
def test_legacy_inactive_subscription_is_inactive_in_production_ai_gate(
    monkeypatch,
) -> None:
    headers = {
        "Authorization": "Bearer dev",
        "X-Test-User-Email": "legacy-inactive-subscription@example.com",
    }
    workspace_response = client.get("/api/workspace/me", headers=headers)
    assert workspace_response.status_code == 200
    ...
    with SessionLocal() as db:
        ...
        assert workspace is not None
        settings = db.scalar(
            select(AppSettings).where(AppSettings.workspace_id == workspace_id)
        )
        if settings is None:
            settings = AppSettings(
                user_id=workspace.owner_user_id, workspace_id=workspace.id
            )
        settings.billing = {"plan": "Starter", "renewal": "monthly"}
        user = User(
            clerk_user_id="legacy-inactive-subscription",
            email="legacy-inactive-subscription@example.com",
        )
        db.add(user)
        db.flush()
        db.add(
...
def test_inactive_stripe_metadata_is_inactive_in_production_ai_gate(
    monkeypatch,
) -> None:
    headers = {
        "Authorization": "Bearer dev",
        "X-Test-User-Email": "legacy-inactive-stripe@example.com",
    }
    workspace_response = client.get("/api/workspace/me", headers=headers)
    assert workspace_response.status_code == 200
    workspace_id = UUID(workspace_response.json()["id"])
    ...
    with SessionLocal() as db:
        ...
        user = User(
            clerk_user_id="legacy-inactive-stripe",
            email="legacy-inactive-stripe@example.com",
        )
        db.add(user)
        db.flush()
        db.add(
...
def test_billing_plan_matrix_keeps_expected_feature_progression() -> None:
    ...
    for lower, higher in zip(plan_order, plan_order[1:]):
        for metric in numeric_limits:
            ...
            assert (
                higher_limit == 0 or higher_limit >= lower_limit
            ), f"{higher}.{metric} is lower than {lower}.{metric}"
        for flag in feature_flags:
            if PLAN_LIMITS[lower][flag]:
                assert (
                    PLAN_LIMITS[higher][flag] is True
                ), f"{higher}.{flag} disables a feature available on {lower}"
...
def test_checkout_rejects_annual_and_unknown_billing_periods() -> None:
    headers = {
        "Authorization": "Bearer dev",
        "X-Test-User-Email": f"period-{uuid4()}@example.com",
    }
    annual = client.post(
        "/api/billing/checkout",
        headers=headers,
        json={"plan": "Starter", "billing_period": "annual"},
    )
    unknown = client.post(
        "/api/billing/checkout",
        headers=headers,
        json={"plan": "Starter", "billing_period": "weekly"},
    )
...
def test_stripe_price_mapping_is_monthly_allowlist_only() -> None:
    ...
    assert all(
        spec.stripe_monthly.interval == "month" for spec in PLAN_CATALOG.values()
    )
...
@pytest.mark.parametrize(
    ...
    [
        (
            _stripe_price_object(
                "price_pro_test", amount=14800, lookup_key="outreachai_pro_monthly"
            ),
            None,
        ),
        (
            _stripe_price_object(
                "price_pro_test", currency="usd", lookup_key="outreachai_pro_monthly"
            ),
            None,
        ),
        (
            _stripe_price_object(
                "price_pro_test", interval="year", lookup_key="outreachai_pro_monthly"
            ),
            None,
        ),
        (
            _stripe_price_object(
                "price_pro_test", interval_count=2, lookup_key="outreachai_pro_monthly"
            ),
            None,
        ),
        (
            _stripe_price_object(
                "price_pro_test", active=False, lookup_key="outreachai_pro_monthly"
            ),
            None,
        ),
        (
            _stripe_price_object(
                "price_unconfigured_reused_lookup",
                amount=14900,
        ...
        (
            _stripe_price_object(
                "price_unconfigured_reused_lookup_invalid",
                amount=4900,
                lookup_key="outreachai_pro_monthly",
            ),
            None,
...
def test_stripe_price_lookup_key_fallback_validates_exact_monthly_price(
    monkeypatch, price: SimpleNamespace, expected: tuple[str, str] | None
) -> None:
    settings = get_settings()
    monkeypatch.setattr(settings, "stripe_secret_key", "sk_test_lookup_validation")
    monkeypatch.setattr(
        "app.services.billing.stripe.Price.retrieve", lambda price_id: price
    )
...
def test_usage_limit_boundaries_fail_closed_for_every_plan() -> None:
    for plan, limits in PLAN_LIMITS.items():
        for metric in ("leads", "ai_generations", "email_sends"):
            ...
            workspace = client.get("/api/workspace/me", headers=headers).json()
            _grant_subscription_for_test(
                workspace["id"], user_id=user_id, plan=plan, status="active"
            )
...
def test_plan_usage_check_then_record_boundaries_and_failed_operation_no_charge() -> (
    None
):
    ...
    with get_sessionmaker()() as db:
        workspace_row = db.get(Workspace, UUID(workspace["id"]))
        assert workspace_row is not None
        routes_module._check_usage_available(
            db, user_id, workspace_row, "ai_generations"
        )
        db.commit()
    ...
    with get_sessionmaker()() as db:
        workspace_row = db.get(Workspace, UUID(workspace["id"]))
        assert workspace_row is not None
        routes_module._record_usage_after_success(
            db, user_id, workspace_row, "ai_generations"
        )
        db.commit()
...
def test_plan_usage_concurrent_records_cannot_exceed_limit() -> None:
    ...
    def record_once() -> int:
        with get_sessionmaker()() as db:
            ...
            try:
                routes_module._record_usage_after_success(
                    db, user_id, workspace_row, "email_sends"
                )
                db.commit()
                return 200
...
def test_plan_usage_is_workspace_isolated_and_owner_does_not_bypass_customer_limit() -> (
    ...
    _grant_subscription_for_test(
        owner_workspace["id"],
        user_id="romaniukvadym10@gmail.com",
        plan="Starter",
        status="active",
    )
    _grant_subscription_for_test(
        other_workspace["id"], user_id=other_email, plan="Starter", status="active"
    )
    ...
    with get_sessionmaker()() as db:
        ...
        with pytest.raises(HTTPException):
            routes_module._check_usage_available(
                db, "romaniukvadym10@gmail.com", owner_row, metric
            )
...
def test_plan_usage_follows_subscription_status_for_active_gate(
    monkeypatch, status: str, allowed: bool
) -> None:
    user_id = f"usage-status-{status}-{uuid4()}@example.com"
    headers = {"Authorization": "Bearer dev", "X-Test-User-Email": user_id}
    workspace = client.get("/api/workspace/me", headers=headers).json()
    _grant_subscription_for_test(
        workspace["id"], user_id=user_id, plan="Starter", status=status
    )
...
def test_campaign_duplicate_enforces_finite_plan_limit_boundaries(plan: str) -> None:
    ...
    other_workspace = client.get("/api/workspace/me", headers=other_headers).json()
    _grant_subscription_for_test(
        other_workspace["id"], user_id=other_user_id, plan=plan, status="active"
    )
    _create_campaigns_for_test(other_workspace["id"], other_user_id, limit)

    allowed = client.post(
        f"/api/campaigns/{campaign_ids[0]}/duplicate", headers=headers
    )
    ...
    with get_sessionmaker()() as db:
        count = db.scalar(
            ...
            .where(
                Campaign.workspace_id == UUID(workspace["id"]),
...
def test_campaign_duplicate_concurrent_requests_do_not_exceed_limit() -> None:
    ...
    campaign_ids = _create_campaigns_for_test(
        workspace["id"], user_id, int(PLAN_LIMITS[plan]["campaigns"]) - 1
    )
...
def test_customer_settings_update_rejects_billing_security_credentials_and_mass_assignment(
    monkeypatch,
) -> None:
    ...
    protected_payloads = [
        ...
        {"security": {"gmail_oauth_states": {"nonce": "forged"}, "api_key": "forged"}},
        {
            "email": {
                "sender": {
                    "oauth": {"refresh_token_encrypted": "forged"},
                    "provider": "gmail",
                }
            }
        },
    ...
    allowed = client.put(
        ...
        json={
            ...
            "email": {"signature": "Regards", "reply_tracking_enabled": True},
        },
    )
    assert allowed.status_code == 200
    ...
    with get_sessionmaker()() as db:
        settings = db.scalar(
            select(AppSettings).where(AppSettings.workspace_id == UUID(workspace["id"]))
        )
    ...
    try:
        with get_sessionmaker()() as db:
            ...
            assert workspace_row is not None
            assert (
                _subscription_status_for_workspace(db, workspace_row, target_email)
                == "inactive"
            )
...
def test_autopilot_worker_ignores_forged_settings_billing_entitlements(
    monkeypatch,
) -> None:
    ...
    with get_sessionmaker()() as db:
        ...
        job = EnrichmentJob(
            ...
            user_id=target_email,
            lead_id=lead.id,
            job_type="autopilot_email",
            request_id=f"worker-forge-{uuid4()}",
        )
        db.add(job)
        db.commit()
        job_id = job.id
    ...
    try:
        with get_sessionmaker()() as db:
            ...
            assert (
                entitlement.limits["email_sends"]
                == PLAN_LIMITS["Starter"]["email_sends"]
            )
    finally:
        monkeypatch.setattr(app_settings, "app_env", original_env)
...
def test_owner_test_entitlement_grant_revoke_and_checkout_skip(monkeypatch) -> None:
    ...
    target_headers = {
        "Authorization": "Bearer dev",
        "X-Test-User-Email": target_user_id,
    }
    ...
    other_headers = {
        "Authorization": "Bearer dev",
        "X-Test-User-Email": f"other-entitled-{uuid4()}@example.com",
    }
    other_workspace = client.get("/api/workspace/me", headers=other_headers).json()
    cross_workspace = client.post(
        ...
        json={
            "workspace_id": workspace["id"],
            "user_id": other_workspace["owner_user_id"]
            if "owner_user_id" in other_workspace
            else other_headers["X-Test-User-Email"],
            "plan": "Starter",
            "expires_at": expires_at,
    ...
    grant = client.post(
        ...
        json={
            ...
            "plan": "Pro",
            "expires_at": expires_at,
            "reason": "controlled production onboarding smoke",
        },
    )
    ...
    def fake_checkout(
        ...
        return {
            ...
            "customer_id": customer_id or "cus_should_not_create",
        }
    ...
    try:
        ...
        checkout = client.post(
            "/api/billing/checkout", headers=target_headers, json={"plan": "Starter"}
        )
        ...
        assert (
            owner_console.json()["subscriptions"].get("total", 0)
            == subscriptions_before
        )

        revoke_forbidden = client.post(
            f"/api/owner/test-entitlements/{entitlement['id']}/revoke",
            headers=target_headers,
            json={"reason": "customer revoke"},
        )
        assert revoke_forbidden.status_code == 403
        revoke = client.post(
            f"/api/owner/test-entitlements/{entitlement['id']}/revoke",
            headers=OWNER_AUTH,
            json={"reason": "controlled smoke finished"},
        )
        ...
        with get_sessionmaker()() as db:
            ...
            assert workspace_row is not None
            assert (
                _subscription_status_for_workspace(db, workspace_row, target_user_id)
                == "inactive"
            )
            with pytest.raises(HTTPException):
...
def test_owner_test_entitlement_expiry_and_concurrent_replace(monkeypatch) -> None:
    ...
    with ThreadPoolExecutor(max_workers=2) as executor:
        responses = list(
            executor.map(grant, ["concurrent grant one", "concurrent grant two"])
        )
    ...
    with get_sessionmaker()() as db:
        active_count = db.scalar(
            ...
            .where(
                BillingTestEntitlement.workspace_id == workspace_id,
                BillingTestEntitlement.user_id == target_user_id,
                BillingTestEntitlement.revoked_at.is_(None),
                BillingTestEntitlement.expires_at > datetime.utcnow(),
            )
        )
        assert active_count == 1


def test_stripe_subscription_state_remains_authoritative_over_test_entitlement(
    monkeypatch,
) -> None:
...
def test_canonical_billing_prefers_active_with_null_period_over_newer_canceled_and_stale_settings(
    monkeypatch,
) -> None:
    ...
    with get_sessionmaker()() as db:
        ...
        settings.billing = {
            "plan": "Starter",
            "status": "canceled",
            "stripeSubscriptionId": "sub_canceled_newer",
...
def test_canonical_billing_trialing_wins_over_later_canceled_duplicate_and_inactive_never_grants(
    monkeypatch,
) -> None:
    ...
    with get_sessionmaker()() as db:
        ...
        settings.billing = {
            "plan": "Agency",
            "status": "active",
            "stripeSubscriptionId": "sub_forged_active",
        }
        _create_billing_subscription(
            db,
            workspace_id=workspace_id,
    ...
    with get_sessionmaker()() as db:
        db.query(Subscription).filter(
            Subscription.workspace_id == workspace_id, Subscription.status == "trialing"
        ).delete()
        db.commit()
...
def test_canonical_billing_duplicate_active_degrades_and_is_workspace_customer_isolated() -> (
    None
):
...
def test_billing_cache_reconciliation_is_owner_only_dry_run_and_idempotent() -> None:
    ...
    forbidden = client.post(
        "/api/owner/billing/reconcile-cache", headers=headers, json=payload
    )
    assert forbidden.status_code == 403
    dry_run = client.post(
        "/api/owner/billing/reconcile-cache", headers=OWNER_AUTH, json=payload
    )
    ...
    apply = client.post(
        "/api/owner/billing/reconcile-cache",
        headers=OWNER_AUTH,
...
def test_stripe_canceled_webhook_replay_and_out_of_order_do_not_override_canonical_active() -> (
    None
):
...
def test_workspace_me_prefers_owned_private_workspace_over_old_membership() -> None:
    ...
    assert any(
        member["email"] == user_email and member["role"].lower() == "owner"
...
def test_workspace_me_ignores_shared_membership_without_owned_workspace() -> None:
    ...
    with SessionLocal() as db:
        shared = Workspace(
            owner_user_id="another-owner", name="Shared Legacy Workspace"
        )
        db.add(shared)
        db.flush()
        db.add(
...
def test_workspace_member_invites_are_disabled_for_private_accounts(
    monkeypatch,
) -> None:
    SessionLocal = get_sessionmaker()
    app_settings = get_settings()
    ...
    with SessionLocal() as db:
        member = db.scalar(
            select(WorkspaceMember).where(
                WorkspaceMember.email == "teammate@example.com"
            )
        )
        assert member is None


@pytest.mark.parametrize("plan", ["Starter", "Pro", "Agency"])
def test_workspace_member_invites_use_current_private_workspace_limits(
    plan: str,
) -> None:
    user_id = f"workspace-current-limits-{plan.lower()}-{uuid4()}@example.com"
    headers = {"Authorization": "Bearer dev", "X-Test-User-Email": user_id}
    workspace = client.get("/api/workspace/me", headers=headers).json()
    _grant_subscription_for_test(
        workspace["id"], user_id=user_id, plan=plan, status="active"
    ...
    assert PLAN_LIMITS[plan]["workspaces"] == 1
    assert PLAN_LIMITS[plan]["team_members"] == 1

    response = client.post(
        "/api/workspace/members",
        headers=headers,
        json={"email": f"{plan.lower()}-teammate@example.com", "role": "Member"},
    )
    ...
    with get_sessionmaker()() as db:
        members = list(
            db.scalars(
                select(WorkspaceMember).where(
                    WorkspaceMember.workspace_id == UUID(workspace["id"])
                )
            ).all()
        )
        assert len(members) == 1
...
def test_current_user_context_without_email_claim_uses_clerk_lookup(
    monkeypatch,
) -> None:
    def fake_verify(_: str) -> dict:
    ...
    monkeypatch.setattr(security, "_verify_clerk_token", fake_verify)
    monkeypatch.setattr(
        security, "_fetch_clerk_user_email", lambda _: "lookup@example.com"
    )
...
def test_fetch_clerk_user_email_uses_only_verified_addresses(monkeypatch) -> None:
    ...
    assert (
        security._fetch_clerk_user_email("user_verified_secondary")
        == "secondary-verified@example.com"
    )
...
def test_fetch_clerk_user_email_prefers_verified_primary(monkeypatch) -> None:
    ...
    assert (
        security._fetch_clerk_user_email("user_verified_primary")
        == "primary@example.com"
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        {
            "primary_email_address_id": "malformed",
            "email_addresses": [
                {"id": "malformed", "email_address": "broken@example.com"}
            ],
        },
...
def test_fetch_clerk_user_email_returns_empty_without_verified_address(
...
def test_workspace_user_context_without_email_claim_uses_clerk_lookup(
    monkeypatch,
) -> None:
    def fake_verify(_: str) -> dict:
    ...
    monkeypatch.setattr(security, "_verify_clerk_token", fake_verify)
    monkeypatch.setattr(
        security, "_fetch_clerk_user_email", lambda _: "workspace-lookup@example.com"
    )
...
def test_workspace_user_context_without_verified_email_fails_closed(
...
@pytest.mark.parametrize(
    "exc_type", [ValueError, httpx.TimeoutException, httpx.HTTPError]
)
def test_workspace_user_context_lookup_error_fails_closed(
...
def _assert_no_workspace_identity_rows(user_id: str) -> None:
    with get_sessionmaker()() as db:
        ...
        assert (
            db.scalar(select(Workspace).where(Workspace.owner_user_id == user_id))
            is None
        )
        assert (
            db.scalar(select(WorkspaceMember).where(WorkspaceMember.user_id == user_id))
            is None
        )


@pytest.mark.parametrize(("method", "path", "payload"), WORKSPACE_ROUTE_CASES)
def test_workspace_routes_reject_unverified_jwt_email_addresses_without_writes(
...
@pytest.mark.parametrize(
    ...
    [
        ...
        {"email_address": "scalar@example.com"},
        {"primary_email_address": "scalar@example.com"},
    ],
)
def test_workspace_routes_reject_scalar_jwt_email_without_verified_evidence(
    ...
    monkeypatch.setattr(
        security, "_verify_clerk_token", lambda _: {"sub": user_id, **email_claim}
    )
    monkeypatch.setattr(security, "_fetch_clerk_user_email", lambda _: "")
...
def test_workspace_routes_reject_malformed_ambiguous_or_unverifiable_jwt_email_claims_without_writes(
    ...
    monkeypatch.setattr(
        security, "_verify_clerk_token", lambda _: {"sub": user_id, **email_claims}
    )
    monkeypatch.setattr(security, "_fetch_clerk_user_email", lambda _: "")
...
@pytest.mark.parametrize(("method", "path", "payload"), WORKSPACE_ROUTE_CASES)
def test_workspace_routes_accept_verified_primary_jwt_email_idempotently_and_isolated(
    monkeypatch, method: str, path: str, payload: dict | None
) -> None:
    ...
    with get_sessionmaker()() as db:
        other_workspace = Workspace(
            owner_user_id=other_user_id, name="Other verified JWT workspace"
        )
    ...
    monkeypatch.setattr(
        ...
        lambda _: {
            ...
            "email_addresses": [
                ...
                {
                    "id": "primary_verified",
                    "email_address": verified_email.upper(),
                    "verification": {"status": "verified"},
                },
            ],
        },
...
def test_workspace_verified_primary_jwt_email_concurrent_requests_are_idempotent(
    monkeypatch,
) -> None:
    ...
    with ThreadPoolExecutor(max_workers=2) as executor:
        first = executor.submit(
            client.get, "/api/workspace/me", headers={"Authorization": "Bearer token"}
        )
    ...
    with get_sessionmaker()() as db:
        assert (
            len(
                list(
                    db.scalars(select(User).where(User.clerk_user_id == user_id)).all()
                )
            )
            == 1
        )
...
@pytest.mark.parametrize(("method", "path", "payload"), WORKSPACE_ROUTE_CASES)
def test_workspace_routes_accept_verified_clerk_fallback_idempotently_and_isolated(
    monkeypatch, method: str, path: str, payload: dict | None
) -> None:
    ...
    with get_sessionmaker()() as db:
        other_workspace = Workspace(
            owner_user_id=other_user_id, name="Other verified fallback workspace"
        )
        db.add(other_workspace)
        db.flush()
        db.add(
...
def test_workspace_request_without_verified_fallback_does_not_persist_binding(
    monkeypatch,
) -> None:
    ...
    response = client.get(
        "/api/workspace/me", headers={"Authorization": "Bearer token"}
    )
    assert response.status_code == 401
...
@pytest.mark.parametrize("fallback", ["", None])
def test_legacy_workspace_request_without_verified_fallback_fails_closed_without_writes(
    monkeypatch, fallback
) -> None:
...
def test_legacy_workspace_request_lookup_error_fails_closed_without_writes(
    monkeypatch, exc_type
) -> None:
...
def test_verified_clerk_fallback_workspace_resolution_is_idempotent_and_isolated(
    monkeypatch,
) -> None:
    ...
    with get_sessionmaker()() as db:
        other_workspace = Workspace(
            owner_user_id=other_user_id, name="Other customer workspace"
        )
        db.add(other_workspace)
        db.flush()
        db.add(
...
def test_verified_clerk_fallback_legacy_workspace_resolution_is_idempotent_and_isolated(
    monkeypatch,
) -> None:
    ...
    with get_sessionmaker()() as db:
        other_workspace = Workspace(
            owner_user_id=other_user_id, name="Other legacy customer workspace"
        )
        db.add(other_workspace)
        db.flush()
        db.add(
...
def test_current_user_context_lookup_failure_returns_authenticated_user(
    monkeypatch,
) -> None:
    def fake_verify(_: str) -> dict:
...
def test_require_owner_allows_owner_user_id_when_email_is_unavailable() -> None:
    ...
    with SessionLocal() as db:
        db.add(
            User(
                clerk_user_id="owner_user_id_only",
...
def test_require_owner_rejects_non_owner_user_id_without_email() -> None:
    ...
    with SessionLocal() as db:
        db.add(
            User(
                clerk_user_id="not_owner_user_id_only",
...
def test_require_owner_allows_workspace_owner_mapping_when_user_row_missing() -> None:
    ...
    with SessionLocal() as db:
        workspace = Workspace(
            owner_user_id="owner_workspace_only", name="Owner Workspace"
        )
        db.add(workspace)
        db.flush()
        db.add(
...
def test_require_queue_health_access_allows_workspace_owner_without_email() -> None:
    ...
    with SessionLocal() as db:
        workspace = Workspace(
            owner_user_id="queue_owner_workspace", name="Queue Owner Workspace"
        )
        db.add(workspace)
        db.flush()
        db.add(
...
def test_workspace_app_bootstrap_creates_private_workspace() -> None:
    response = client.get(
        ...
        headers={
            "Authorization": "Bearer dev",
            "X-Test-User-Email": "usage-owner@example.com",
        },
    )
    assert response.status_code == 200
    data = response.json()
...
def test_customer_facing_workspace_hides_internal_qa_records() -> None:
    headers = {
        "Authorization": "Bearer dev",
        "X-Test-User-Email": "qa-cleanup-owner@example.com",
    }
    workspace_response = client.get("/api/workspace/me", headers=headers)
    assert workspace_response.status_code == 200
    workspace_id = UUID(workspace_response.json()["id"])
    ...
    assert [item["name"] for item in workspace_companies.json()] == [
        "Real Berlin Construction GmbH"
    ]
...
def test_workspace_app_manual_company_save_persists_and_dedupes() -> None:
    headers = {
        "Authorization": "Bearer dev",
        "X-Test-User-Email": "usage-company@example.com",
    }
    payload = {
    ...
    refreshed = client.get(
        "/api/workspace-app/companies?search=Usage%20Berlin", headers=headers
    )
    ...
    filtered = client.get(
        "/api/workspace-app/companies?city=Berlin&industry=Construction&email_status=Found",
        headers=headers,
    )
...
def test_workspace_app_companies_batch_load_avoids_per_company_queries() -> None:
    headers = {
        "Authorization": "Bearer dev",
        "X-Test-User-Email": "usage-company-perf@example.com",
    }
    SessionLocal = get_sessionmaker()
    with SessionLocal() as db:
        workspace = Workspace(
            owner_user_id="usage-company-perf@example.com", name="Usage company perf"
        )
        db.add(workspace)
        db.flush()
        db.add(
            WorkspaceMember(
                ...
                email="usage-company-perf@example.com",
                role=WorkspaceRole.owner,
                status="active",
            )
        )
        ...
        for index in range(30):
            ...
            db.add(
                AuditLog(
                    ...
                    workspace_id=workspace.id,
                    action="lead.found",
                    metadata_json={"lead_id": str(lead.id)},
                )
            )
            db.add(
                AuditLog(
...
def test_inbox_defaults_to_bounded_pages() -> None:
    headers = {
        "Authorization": "Bearer dev",
        "X-Test-User-Email": "usage-inbox-perf@example.com",
    }
    SessionLocal = get_sessionmaker()
    with SessionLocal() as db:
        workspace = Workspace(
            owner_user_id="usage-inbox-perf@example.com", name="Usage inbox perf"
        )
        db.add(workspace)
        db.flush()
        db.add(
            WorkspaceMember(
                ...
                email="usage-inbox-perf@example.com",
                role=WorkspaceRole.owner,
                status="active",
            )
        )
    ...
    second_page = client.get(
        f"/api/inbox?page_size=75&cursor={first_page.headers['x-next-cursor']}",
        headers=headers,
    )
...
def test_inbox_cursor_pagination_has_no_duplicates_or_skips_when_new_email_arrives() -> (
    None
):
    headers = {
        "Authorization": "Bearer dev",
        "X-Test-User-Email": "usage-inbox-cursor@example.com",
    }
    SessionLocal = get_sessionmaker()
    expected_existing_ids: list[str] = []
    with SessionLocal() as db:
        workspace = Workspace(
            owner_user_id="usage-inbox-cursor@example.com", name="Usage inbox cursor"
        )
        db.add(workspace)
        db.flush()
        db.add(
            WorkspaceMember(
                ...
                email="usage-inbox-cursor@example.com",
                role=WorkspaceRole.owner,
                status="active",
            )
        )
    ...
    with SessionLocal() as db:
        workspace = db.scalar(
            select(Workspace).where(
                Workspace.owner_user_id == "usage-inbox-cursor@example.com"
            )
        )
        assert workspace is not None
        db.add(
    ...
    second_page = client.get(
        f"/api/inbox?page_size=50&cursor={cursor}", headers=headers
    )
    assert second_page.status_code == 200
...
def test_workspace_app_manual_company_save_survives_crm_sync_failure(
    monkeypatch,
) -> None:
    headers = {
        "Authorization": "Bearer dev",
        "X-Test-User-Email": "usage-degraded@example.com",
    }
    ...
    refreshed = client.get(
        "/api/workspace-app/companies?search=Usage%20Degraded", headers=headers
    )
    assert refreshed.status_code == 200
    assert len(refreshed.json()) == 1
...
def test_workspace_app_company_data_is_private_between_users() -> None:
    ...
    response = client.post(
        ...
        headers=user_a,
        json={
            "name": "Private Usage Build",
            "website": "https://private-usage-build.example",
            "country": "Germany",
            "city": "Berlin",
            "industry": "Construction",
        },
    ...
    user_a_company = client.get(
        f"/api/workspace-app/companies/{company_id}", headers=user_a
    )
    user_b_company = client.get(
        f"/api/workspace-app/companies/{company_id}", headers=user_b
    )
    assert user_a_company.status_code == 200
    assert user_b_company.status_code == 404

    user_b_list = client.get(
        "/api/workspace-app/companies?search=Private%20Usage", headers=user_b
    )
    assert user_b_list.status_code == 200
...
def test_workspace_app_lead_search_success_saves_to_crm(monkeypatch) -> None:
    headers = {
        "Authorization": "Bearer dev",
        "X-Test-User-Email": "usage-search@example.com",
    }
    monkeypatch.setattr(
        "app.api.usage._hunter_enriched_leads",
        lambda db, request, user_id, workspace, leads: leads,
    )
    ...
    assert (
        data["companies"][0]["ai_summary"]
        == "Usage Search helps Berlin construction buyers evaluate new partners."
    )
    ...
    assert (
        data["companies"][0]["value_proposition"] == "Reviewed local partner pipeline"
    )
    assert (
        data["companies"][0]["suggested_offer"]
        == "Offer a reviewed B2B partnership pipeline."
    )
    assert data["companies"][0]["recommended_cta"] == "Book a fit review"
    assert (
        data["companies"][0]["follow_up_strategy"]
        == "Follow up with one local proof point."
    )
    assert data["companies"][0]["expected_reply_rate"] == "8-12%"
    assert data["companies"][0]["generated_emails"][0]["delivery_status"] == "draft"

    persisted = client.get(
        "/api/workspace-app/companies?search=Usage%20Search", headers=headers
    )
...
def test_workspace_app_ai_lead_command_parses_sales_search() -> None:
    ...
    filters, missing = _parse_lead_command(
        "Find 25 construction companies in Berlin with 20-100 employees", workspace
    )
    assert missing == []
    ...
    ru_filters, ru_missing = _parse_lead_command(
        "Найди 10 строительных компаний в Берлине", workspace
    )
    assert ru_missing == []
    ...
    beauty_filters, beauty_missing = _parse_lead_command(
        "Хочу найти клиентов для своей продукции по косметике", beauty_workspace
    )
...
def test_workspace_app_ai_lead_command_uses_fast_search_path(monkeypatch) -> None:
    headers = {
        "Authorization": "Bearer dev",
        "X-Test-User-Email": "usage-command-fast@example.com",
    }
    ...
    def fail_if_called(*args, **kwargs):
        raise AssertionError(
            "AI command should not run full turnkey research before the response is saved"
        )
    ...
    monkeypatch.setattr(
        ...
        leads,
        request_id,
        **kwargs: queued.extend([str(lead.id) for lead in leads]) or False,
    )
    monkeypatch.setattr(
        "app.api.usage.search_google_places",
        lambda payload: GooglePlacesSearchResult(
...
def test_workspace_app_lead_search_reports_reused_duplicates(monkeypatch) -> None:
    headers = {
        "Authorization": "Bearer dev",
        "X-Test-User-Email": "usage-search-duplicates@example.com",
    }
    monkeypatch.setattr(
        "app.api.usage._hunter_enriched_leads",
        lambda db, request, user_id, workspace, leads: leads,
...
def test_workspace_app_turnkey_research_completes_public_details_before_ai_and_contacts(
    monkeypatch,
) -> None:
    headers = {
        "Authorization": "Bearer dev",
        "X-Test-User-Email": "usage-turnkey-details@example.com",
    }
    monkeypatch.setattr(
        "app.api.usage.search_google_places",
        lambda payload: GooglePlacesSearchResult(
    ...
    assert (
        company["ai_summary"]
        == "Usage Details serves Berlin construction buyers with specialist services."
    )
...
def test_workspace_app_lead_search_provider_error_returns_structured_status(
    monkeypatch,
) -> None:
    headers = {
        "Authorization": "Bearer dev",
        "X-Test-User-Email": "usage-provider-error@example.com",
    }
    monkeypatch.setattr(
        "app.api.usage.search_google_places",
        lambda payload: (_ for _ in ()).throw(
            GoogleMapsRequestError("provider outage")
        ),
    )
    response = client.post(
        "/api/workspace-app/leads/search",
...
def test_workspace_app_integration_status_is_private_and_actionable() -> None:
    headers = {
        "Authorization": "Bearer dev",
        "X-Test-User-Email": "usage-integrations@example.com",
    }
    ...
    assert all(
        item["status"] in {"connected", "missing_key", "needs_setup", "error"}
        for item in items
    )
...
def test_workspace_app_contact_discovery_email_approval_and_send(monkeypatch) -> None:
    headers = {
        "Authorization": "Bearer dev",
        "X-Test-User-Email": "usage-email@example.com",
    }
    company_response = client.post(
        "/api/workspace-app/companies",
        headers=headers,
        json={
            "name": "Usage Email Build",
            "website": "https://usage-email.example",
            "country": "Germany",
            "city": "Berlin",
            "industry": "Construction",
        },
    ...
    contacts = client.post(
        f"/api/workspace-app/companies/{company_id}/contacts", headers=headers
    )
    assert contacts.status_code == 200
    ...
    draft = client.post(
        f"/api/workspace-app/companies/{company_id}/email-draft", headers=headers
    )
    assert draft.status_code == 200
    ...
    cross_workspace_edit = client.patch(
        ...
        headers={
            "Authorization": "Bearer dev",
            "X-Test-User-Email": "other-usage-email@example.com",
        },
    ...
    assert (
        edited.json()["email"]["recipient_email"]
        == "safe.recipient+draft@recipient-safety-mail.com"
    )
    ...
    with get_sessionmaker()() as db:
        ...
        assert (
            sent_payload["idempotency_key"]
            == f"workspace-app-email-send:{saved_email.workspace_id}:{email['id']}:v1"
        )
        ...
        assert (
            db.scalar(
                ...
                .where(
                    AuditLog.action.in_(
                        (
                            "internal_email_smoke_draft.send_confirmed",
                            "production_smoke_test.send_confirmed",
                        )
                    ),
...
def _workspace_app_test_draft(
    headers: dict[str, str], monkeypatch, *, company_name: str = "Patch Safety Co"
) -> dict[str, Any]:
    company_response = client.post(
        "/api/workspace-app/companies",
        headers=headers,
    ...
    contact = client.post(
        ...
        json={
            "name": "Patch Owner",
            "title": "Founder",
            "email": f"owner-{uuid4().hex[:8]}@patch-safety.example",
        },
    )
...
def _exact_email_approval_payload(
    email: dict[str, Any], sender_email: str
) -> dict[str, Any]:
    return {
        "confirmed_exact_draft": True,
...
def _internal_smoke_draft_payload(
    recipient: Any = INTERNAL_SMOKE_DRAFT_RECIPIENT, confirmed: bool = True
) -> dict[str, Any]:
    return {"recipient_email": recipient, "confirmed_recipient_control": confirmed}


def test_workspace_app_internal_smoke_draft_rejects_unsafe_raw_recipient_forms() -> (
    None
):
    rejected_cases: list[tuple[str, Any, int]] = [
        ...
        (
            "unicode confusable",
            "rоmaniukvadym10+client-smoke-20260812-1@gmail.com",
            400,
        ),
        (
            "url encoded plus",
            "romaniukvadym10%2Bclient-smoke-20260812-1@gmail.com",
            400,
        ),
        ...
        (
            "semicolon separated",
            f"{INTERNAL_SMOKE_DRAFT_RECIPIENT};other@example.com",
            400,
        ),
        ("display name", f"Smoke <{INTERNAL_SMOKE_DRAFT_RECIPIENT}>", 400),
        (
            "crlf injection",
            f"{INTERNAL_SMOKE_DRAFT_RECIPIENT}\r\nBcc: victim@example.com",
            400,
        ),
    ]
    for label, value, expected_status in rejected_cases:
        headers = {
            "Authorization": "Bearer dev",
            "X-Test-User-Email": f"internal-smoke-reject-{label.replace(' ', '-')}-{uuid4()}@example.com",
        }
        response = client.post(
            "/api/workspace-app/internal-email-smoke-draft",
            headers=headers,
            json=_internal_smoke_draft_payload(value),
        )
        assert response.status_code == expected_status, label

    headers = {
        "Authorization": "Bearer dev",
        "X-Test-User-Email": f"internal-smoke-extra-{uuid4()}@example.com",
    }
    ...
    duplicate_key = client.post(
        ...
        headers={
            "Authorization": "Bearer dev",
            "X-Test-User-Email": f"internal-smoke-duplicate-{uuid4()}@example.com",
...
def test_workspace_app_internal_smoke_draft_is_normal_user_scoped_and_no_send_by_default(
    monkeypatch,
) -> None:
    headers = {
        "Authorization": "Bearer dev",
        "X-Test-User-Email": f"client-smoke-{uuid4()}@example.com",
    }
    other_headers = {
        "Authorization": "Bearer dev",
        "X-Test-User-Email": f"client-smoke-other-{uuid4()}@example.com",
    }
    ...
    cross_workspace_company = client.get(
        f"/api/workspace-app/companies/{company['id']}", headers=other_headers
    )
    assert cross_workspace_company.status_code == 404
    cross_workspace_send = client.post(
        f"/api/workspace-app/emails/{email['id']}/send", headers=other_headers
    )
    ...
    monkeypatch.setattr(
        ...
        lambda **kwargs: provider_calls.append(kwargs) or {"id": "must-not-send"},
    )
    send_before_approval = client.post(
    ...
    approved = client.post(
        f"/api/workspace-app/emails/{email['id']}/approve",
        headers=headers,
        json=_exact_email_approval_payload(email, "sender@internal-smoke.example"),
    )
    assert approved.status_code == 200
    assert approved.json()["email"]["delivery_status"] == "approved"

    missing_final_confirmation = client.post(
        f"/api/workspace-app/emails/{email['id']}/send", headers=headers
    )
    assert missing_final_confirmation.status_code == 409
    assert (
        "Final send confirmation is required"
        in missing_final_confirmation.json()["detail"]
    )
    assert provider_calls == []
    ...
    wrong_recipient_confirmation = client.post(
        ...
        json={
            "confirmed_send": True,
            "smoke_test_id": email["tags"]["smoke_test_id"],
            "recipient_email": "romaniukvadym10+client-smoke-20260812-2@gmail.com",
        },
    )
    assert wrong_recipient_confirmation.status_code == 409
    assert (
        "Recipient confirmation does not match"
        in wrong_recipient_confirmation.json()["detail"]
    )
    assert provider_calls == []
    ...
    with get_sessionmaker()() as db:
        send_confirmed_count = db.scalar(
            ...
            .where(
                ...
                AuditLog.metadata_json["smoke_test_id"].as_string()
                == email["tags"]["smoke_test_id"],
                AuditLog.metadata_json["email_id"].as_string() == email["id"],
            )
        )
    ...
    with get_sessionmaker()() as db:
        ...
        assert (
            saved_email.tags["send_idempotency_key"]
            == f"workspace-app-email-send:{saved_email.workspace_id}:{email['id']}:v1"
        )
...
def test_workspace_app_internal_smoke_send_confirmation_audit_is_single_on_uncertain_retry(
    monkeypatch,
) -> None:
    headers = {
        "Authorization": "Bearer dev",
        "X-Test-User-Email": f"client-smoke-uncertain-{uuid4()}@example.com",
    }
    recipient = INTERNAL_SMOKE_DRAFT_RECIPIENT
    created = client.post(
        "/api/workspace-app/internal-email-smoke-draft",
    ...
    monkeypatch.setattr(
        ...
        lambda db, user_id, workspace: (
            ...
            {
                "host": "smtp.internal-smoke.example",
                "port": 587,
                "username": "sender",
                "password": "secret",
                "use_tls": True,
...
def test_workspace_app_internal_smoke_draft_concurrent_create_is_bounded_and_single_record() -> (
    None
):
    headers = {
        "Authorization": "Bearer dev",
        "X-Test-User-Email": f"client-smoke-race-{uuid4()}@example.com",
    }
    ...
    with get_sessionmaker()() as db:
        workspace = db.scalar(
            select(Workspace).where(
                Workspace.owner_user_id == headers["X-Test-User-Email"]
            )
        )
        assert workspace is not None
        internal_leads = [
            ...
            if lead.email == INTERNAL_SMOKE_DRAFT_RECIPIENT
            and lead.campaign_id is None
            and lead.user_id == headers["X-Test-User-Email"]
        ]
        ...
        assert (
            db.scalar(
                ...
                .where(
                    ...
                    Company.lead_id == internal_leads[0].id,
                )
            )
            == 1
        )
        assert (
            db.scalar(
                ...
                .where(
                    ...
                    EmailMessage.lead_id == internal_leads[0].id,
                )
            )
            == 1
        )
...
def test_workspace_app_exact_send_confirmation_invalidates_after_draft_change(
    monkeypatch,
) -> None:
    headers = {
        "Authorization": "Bearer dev",
        "X-Test-User-Email": f"exact-confirmation-{uuid4()}@example.com",
    }
    email = _workspace_app_test_draft(
        headers, monkeypatch, company_name="Exact Confirmation Co"
    )
    sender_setup = client.put(
        "/api/outreach/sender",
        headers=headers,
    ...
    approved = client.post(
        f"/api/workspace-app/emails/{email['id']}/approve",
        headers=headers,
        json=_exact_email_approval_payload(email, "sender@exact-confirmation.example"),
    )
    assert approved.status_code == 200
    ...
    edited = client.patch(
        f"/api/workspace-app/emails/{email['id']}",
        headers=headers,
        json={"subject": "Changed after exact confirmation"},
    )
    assert edited.status_code == 200
    ...
    reapproved = client.post(
        f"/api/workspace-app/emails/{email['id']}/approve",
        headers=headers,
        json=_exact_email_approval_payload(
            changed_email, "sender@exact-confirmation.example"
        ),
    )
    ...
    changed_sender_send = client.post(
        f"/api/workspace-app/emails/{email['id']}/send", headers=headers
    )
...
def test_outbound_provider_kill_switch_blocks_provider_boundaries(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        ...
        lambda **kwargs: provider_calls.append("resend") or {"id": "resend"},
    )
    monkeypatch.setattr(
        "app.services.emailer._send_gmail_email",
        lambda **kwargs: provider_calls.append("gmail") or {"id": "gmail"},
    )
    monkeypatch.setattr(
        "app.services.emailer._send_smtp_email",
        lambda **kwargs: provider_calls.append("smtp") or {"id": "smtp"},
    )
    monkeypatch.setattr(
        "smtplib.SMTP", lambda *args, **kwargs: provider_calls.append("smtp-verify")
    )
    ...
    for provider in ["resend", "gmail", "smtp"]:
        with pytest.raises(
            ...
            emailer.send_email(
                to_email="buyer@safe-mail.example",
                subject="Subject",
                body="Body",
                provider=provider,
                smtp_config={
                    "host": "smtp.safe-mail.example",
                    "username": "user",
                    "password": "pass",
                },
            )
    ...
    with pytest.raises(
        ...
        emailer.verify_smtp_connection(
            host="smtp.safe-mail.example", port=587, username="user", password="pass"
        )

    assert provider_calls == []


def test_workspace_app_outbound_kill_switch_blocks_send_but_allows_draft_and_approve(
    monkeypatch,
) -> None:
    headers = {
        "Authorization": "Bearer dev",
        "X-Test-User-Email": f"kill-switch-send-{uuid4()}@example.com",
    }
    email = _workspace_app_test_draft(
        headers, monkeypatch, company_name="Kill Switch Send Co"
    )
    edited = client.patch(
        f"/api/workspace-app/emails/{email['id']}",
        headers=headers,
        json={"subject": "Reviewed kill switch draft", "body": "Reviewed body"},
    )
    assert edited.status_code == 200
    ...
    approved = client.post(
        f"/api/workspace-app/emails/{email['id']}/approve",
        headers=headers,
        json=_exact_email_approval_payload(email, "sender@kill-switch.example"),
    )
    assert approved.status_code == 200
    assert approved.json()["email"]["delivery_status"] == "approved"
    ...
    monkeypatch.setattr(
        "app.services.emailer._send_resend_email",
        lambda **kwargs: provider_calls.append(kwargs) or {"id": "guard-provider-id"},
    )
    monkeypatch.setattr(get_settings(), "outbound_provider_sends_disabled", True)
    blocked = client.post(
        f"/api/workspace-app/emails/{email['id']}/send", headers=headers
    ...
    assert (
        blocked.json()["detail"] == "Outbound sending is disabled in this environment."
    )
    assert provider_calls == []
...
def test_workspace_app_email_patch_state_machine_and_audit_regressions(
    monkeypatch,
) -> None:
    headers = {
        "Authorization": "Bearer dev",
        "X-Test-User-Email": f"patch-safety-{uuid4()}@example.com",
    }
    ...
    cross_workspace = client.patch(
        ...
        headers={
            "Authorization": "Bearer dev",
            "X-Test-User-Email": f"patch-other-{uuid4()}@example.com",
        },
    ...
    invalid_recipient = client.patch(
        f"/api/workspace-app/emails/{email['id']}",
        headers=headers,
    ...
    draft_edit = client.patch(
        ...
        headers=headers,
        json={
            "recipient_email": "Reviewed.Recipient@Patch-Safety.Example",
    ...
    assert (
        draft_edit.json()["email"]["recipient_email"]
        == "reviewed.recipient@patch-safety.example"
    )
    ...
    assert (
        "Recipient email can only be changed while the email is a draft"
        in approved_recipient_edit.json()["detail"]
    )
    with get_sessionmaker()() as db:
        ...
        assert (
            unchanged_email.recipient_email == "reviewed.recipient@patch-safety.example"
        )
    ...
    send_without_reapproval = client.post(
        f"/api/workspace-app/emails/{email['id']}/send", headers=headers
    )
    assert send_without_reapproval.status_code == 409
    assert (
        "Approve the email before sending" in send_without_reapproval.json()["detail"]
    )
    ...
    reapproved = client.post(
        ...
        headers=headers,
        json=_exact_email_approval_payload(
            approved_edit.json()["email"], "sender@patch-safety.example"
        ),
    )
    assert reapproved.status_code == 200
    ...
    monkeypatch.setattr(
        "app.api.usage.send_email",
        lambda **kwargs: {"id": "patch-provider-id", "thread_id": "patch-thread-id"},
    )
    sent = client.post(f"/api/workspace-app/emails/{email['id']}/send", headers=headers)
    assert sent.status_code == 200
    assert sent.json()["email"]["delivery_status"] == "sent"
    ...
    with get_sessionmaker()() as db:
        ...
        inbound = EmailMessage(
            ...
            body="Inbound body",
            delivery_status="received",
            reply_body="Sensitive reply",
        )
        captured_reply = EmailMessage(
            user_id=user_id,
            workspace_id=workspace_id,
            lead_id=lead_id,
            direction="outbound",
    ...
    inbound_edit = client.patch(
        f"/api/workspace-app/emails/{inbound_id}",
        headers=headers,
        json={"subject": "Inbound changed"},
    )
    assert inbound_edit.status_code == 409
    for provider_id in [*provider_ids, captured_reply_id]:
        rejected = client.patch(
            f"/api/workspace-app/emails/{provider_id}",
            headers=headers,
            json={
                "recipient_email": "provider-change@patch-safety.example",
                "body": "Provider record changed",
            },
        )
    ...
    with get_sessionmaker()() as db:
        audit = db.scalar(
            select(AuditLog)
            .where(
                AuditLog.action == "email.edited", AuditLog.workspace_id == workspace_id
            )
            .order_by(AuditLog.created_at.desc())
        )
        assert audit is not None
...
def test_workspace_app_email_provider_failure_uses_non_200_http_status(
    monkeypatch,
) -> None:
    headers = {
        "Authorization": "Bearer dev",
        "X-Test-User-Email": "usage-provider-send-error@example.com",
    }
    company_response = client.post(
        "/api/workspace-app/companies",
        headers=headers,
        json={
            "name": "Provider Error Build",
            "website": "https://provider-error.example",
            "country": "Germany",
            "city": "Berlin",
            "industry": "Construction",
            "email": "buyer@provider-error.example",
        },
    )
    assert company_response.status_code == 200
    company_id = company_response.json()["company"]["id"]
    ...
    approved = client.post(
        f"/api/workspace-app/emails/{email['id']}/approve",
        headers=headers,
        json=_exact_email_approval_payload(email, "sales@provider-error.example"),
    )
    assert approved.status_code == 200
    ...
    monkeypatch.setattr(
        ...
        lambda **kwargs: (_ for _ in ()).throw(
            EmailProviderRequestError("provider unavailable")
        ),
    )
    sent = client.post(f"/api/workspace-app/emails/{email['id']}/send", headers=headers)
    ...
    refreshed = client.get(
        f"/api/workspace-app/companies/{company_id}", headers=headers
    )
    assert refreshed.status_code == 200
...
def test_workspace_app_email_unexpected_exception_restores_approved_with_audit(
    ...
    headers = {
        "Authorization": "Bearer dev",
        "X-Test-User-Email": f"usage-unexpected-send-{uuid4()}@example.com",
    }
    email = _workspace_app_test_draft(
        headers, monkeypatch, company_name="Unexpected Send Co"
    )
    sender_setup = client.put(
        "/api/outreach/sender",
        headers=headers,
    ...
    approved = client.post(
        f"/api/workspace-app/emails/{email['id']}/approve",
        headers=headers,
        json=_exact_email_approval_payload(email, "sender@unexpected-send.example"),
    )
    assert approved.status_code == 200
    ...
    monkeypatch.setattr(
        "app.api.usage.send_email",
        lambda **kwargs: (_ for _ in ()).throw(
            RuntimeError("unexpected provider client bug")
        ),
    )
    response = client.post(
        f"/api/workspace-app/emails/{email['id']}/send", headers=headers
...
def test_workspace_app_non_idempotent_provider_error_requires_delivery_confirmation(
    monkeypatch,
) -> None:
    test_user_id = f"usage-smtp-confirmation-{uuid4()}@example.com"
    headers = {"Authorization": "Bearer dev", "X-Test-User-Email": test_user_id}
    email = _workspace_app_test_draft(
        headers, monkeypatch, company_name="SMTP Confirmation Co"
    )
    ...
    monkeypatch.setattr(
        ...
        lambda db, user_id, workspace: (
            SimpleNamespace(
                provider="smtp",
                sender_email="sender@smtp-confirmation.example",
                sender_name="SMTP Sender",
                reply_to="reply@smtp-confirmation.example",
            ),
            {
                "host": "smtp.confirmation.example",
                "port": 587,
                "username": "sender",
                "password": "secret",
                "use_tls": True,
    ...
    approved = client.post(
        f"/api/workspace-app/emails/{email['id']}/approve",
        headers=headers,
        json=_exact_email_approval_payload(email, "sender@smtp-confirmation.example"),
    )
    assert approved.status_code == 200
    calls: list[dict[str, Any]] = []
    ...
    missing_confirmation = client.post(
        f"/api/workspace-app/emails/{email['id']}/recover", headers=headers
    )
    assert missing_confirmation.status_code == 422
    false_confirmation = client.post(
        f"/api/workspace-app/emails/{email['id']}/recover",
        headers=headers,
        json={"confirmed_not_delivered": False},
    )
    assert false_confirmation.status_code == 409
    assert (
        "Confirm that the email is not in Gmail or SMTP Sent"
        in false_confirmation.json()["detail"]
    )
    ...
    assert (
        recovered.json()["message"]
        == "Interrupted send recovered for retry. Nothing was sent automatically."
    )
    ...
    with get_sessionmaker()() as db:
        ...
        audit = db.scalar(
            select(AuditLog)
            .where(
                AuditLog.action == "email.send_recovered",
...
def test_workspace_app_email_workspace_isolation_for_approve_send_patch_and_recover(
    monkeypatch,
) -> None:
    owner_headers = {
        "Authorization": "Bearer dev",
        "X-Test-User-Email": f"usage-owner-{uuid4()}@example.com",
    }
    other_headers = {
        "Authorization": "Bearer dev",
        "X-Test-User-Email": f"usage-other-{uuid4()}@example.com",
    }
    email = _workspace_app_test_draft(
        owner_headers, monkeypatch, company_name="Isolation Email Co"
    )

    approve_other = client.post(
        f"/api/workspace-app/emails/{email['id']}/approve", headers=other_headers
    )
    send_other = client.post(
        f"/api/workspace-app/emails/{email['id']}/send", headers=other_headers
    )
    patch_other = client.patch(
        f"/api/workspace-app/emails/{email['id']}",
        headers=other_headers,
        json={"body": "Cross-workspace body"},
    )
...
def test_workspace_owner_cannot_run_system_production_email_smoke_test() -> None:
    headers = {
        "Authorization": "Bearer dev",
        "X-Test-User-Email": f"workspace-owner-{uuid4()}@example.com",
    }

    active = client.get(
        "/api/workspace-app/production-email-smoke-test/active", headers=headers
    )
    ...
    created = client.post(
        ...
        headers=headers,
        json={
            "recipient_email": "owner@smoke-safety-mail.com",
            "confirmed_recipient_control": True,
        },
    )
...
def test_workspace_app_owner_production_email_smoke_test_safety(monkeypatch) -> None:
    ...
    missing_recipient = client.post(
        "/api/workspace-app/production-email-smoke-test",
        headers=headers,
        json={"confirmed_recipient_control": True},
    )
    ...
    no_control = client.post(
        ...
        json={
            "recipient_email": "owner@smoke-safety-mail.com",
            "confirmed_recipient_control": False,
        },
    )
    ...
    assert (
        active.json()["smoke_test"]["recipient_email"] == "owner@smoke-safety-mail.com"
    )
    ...
    duplicate = client.post(
        ...
        headers=headers,
        json={
            "recipient_email": "owner2@smoke-safety-mail.com",
            "confirmed_recipient_control": True,
        },
    )
    ...
    with get_sessionmaker()() as db:
        ...
        with pytest.raises(
            ValueError, match="production_smoke_test_automation_disabled"
        ):
    ...
    approved = client.post(
        f"/api/workspace-app/emails/{email_id}/approve", headers=headers
    )
    assert approved.status_code == 200
    assert approved.json()["email"]["delivery_status"] == "approved"
    assert provider_calls == []

    no_final_confirm = client.post(
        f"/api/workspace-app/emails/{email_id}/send", headers=headers
    )
    ...
    wrong_workspace = client.post(
        ...
        headers={
            "Authorization": "Bearer dev",
            "X-Test-User-Email": f"other-smoke-{uuid4()}@example.com",
        },
        json={
            "confirmed_send": True,
            "smoke_test_id": smoke_test_id,
    ...
    wrong_recipient = client.post(
        ...
        json={
            "confirmed_send": True,
            "smoke_test_id": smoke_test_id,
            "recipient_email": "wrong@smoke-safety-mail.com",
        },
    )
    ...
    with get_sessionmaker()() as db:
        ...
        real_lead = Lead(
            user_id=owner_email,
            workspace_id=workspace_id,
            company="Real Cleanup Co",
            email="real@cleanup-mail.com",
            notes=json.dumps({"source": "manual"}),
        )
        db.add(real_lead)
        db.flush()
        real_company = Company(
    ...
    other_workspace_cleanup = client.post(
        ...
        headers={
            "Authorization": "Bearer dev",
            "X-Test-User-Email": f"other-cleanup-{uuid4()}@example.com",
        },
    ...
    assert repeated_cleanup.json()["smoke_test"]["cleanup_deleted"] == {
        "leads": 0,
        "companies": 0,
        "drafts": 0,
        "activities": 0,
...
def test_workspace_app_smoke_send_obeys_outbound_provider_kill_switch(
    monkeypatch,
) -> None:
    headers = OWNER_AUTH
    sender_setup = client.put(
    ...
    with get_sessionmaker()() as db:
        ...
        assert saved_email is not None
        assert saved_email.workspace_id == UUID(
            created.json()["smoke_test"]["workspace_id"]
        )
        assert saved_email.delivery_status == "approved"
        assert saved_email.provider_message_id is None
...
def test_workspace_app_production_email_smoke_cleanup_soft_deletes_ai_memory_without_sending(
    monkeypatch,
) -> None:
    headers = OWNER_AUTH
    owner_email = OWNER_AUTH["X-Test-User-Email"]
    ...
    monkeypatch.setattr(
        "app.api.usage.send_email",
        lambda **kwargs: pytest.fail("cleanup must not send email"),
    )
    _enable_ai_memory(headers)
    workspace = client.get("/api/workspace/me", headers=headers).json()
    ...
    created = client.post(
        ...
        headers=headers,
        json={
            "recipient_email": "owner@smoke-memory-mail.com",
            "confirmed_recipient_control": True,
        },
    )
    assert created.status_code == 200
    ...
    with get_sessionmaker()() as db:
        memory = db.scalar(
            select(AIMemoryEntry).where(
                AIMemoryEntry.workspace_id == UUID(workspace["id"]),
                AIMemoryEntry.source == "email.approved",
                AIMemoryEntry.source_id == email_id,
            )
        )
    ...
    assert cleanup.status_code == 200
    assert cleanup.json()["smoke_test"]["cleanup_deleted"] == {
        "leads": 1,
        "companies": 1,
        "drafts": 1,
        "activities": 1,
    ...
    assert repeated.json()["smoke_test"]["cleanup_deleted"] == {
        "leads": 0,
        "companies": 0,
        "drafts": 0,
        "activities": 0,
...
def test_workspace_app_production_email_smoke_cleanup_recovers_orphan_ai_memory(
    monkeypatch,
) -> None:
    headers = OWNER_AUTH
    monkeypatch.setattr("app.services.ai_memory._openai_embedding", lambda value: [])
    ...
    created = client.post(
        ...
        headers=headers,
        json={
            "recipient_email": "owner@smoke-orphan-memory-mail.com",
            "confirmed_recipient_control": True,
        },
    )
    assert created.status_code == 200
...
def test_workspace_app_production_email_smoke_test_reload_recovery_and_idempotent_cleanup(
    monkeypatch,
) -> None:
    owner_email = OWNER_AUTH["X-Test-User-Email"]
    headers = OWNER_AUTH
    ...
    created = client.post(
        ...
        headers=headers,
        json={
            "recipient_email": "owner@smoke-reload-mail.com",
            "confirmed_recipient_control": True,
        },
    )
    assert created.status_code == 200
    ...
    assert (
        active.json()["smoke_test"]["recipient_email"] == "owner@smoke-reload-mail.com"
    )
    ...
    with get_sessionmaker()() as db:
        ...
        real_lead = Lead(
            user_id=owner_email,
            workspace_id=workspace_id,
            company="Real Not Smoke",
            email="real@not-smoke-mail.com",
            notes=json.dumps({"source": "manual"}),
        )
        db.add(real_lead)
        db.commit()
    ...
    active_after_cleanup = client.get(
        "/api/workspace-app/production-email-smoke-test/active", headers=headers
    )
    assert active_after_cleanup.status_code == 200
    active_after_cleanup_context = active_after_cleanup.json()["smoke_test"]
    assert (
        active_after_cleanup_context is None
        or active_after_cleanup_context["smoke_test_id"] != smoke_test_id
    )
...
def test_workspace_app_production_email_smoke_test_recovers_orphan_lead_without_deleting_real_company(
    monkeypatch,
) -> None:
    headers = OWNER_AUTH
    client.put(
    ...
    created = client.post(
        ...
        headers=headers,
        json={
            "recipient_email": "owner@smoke-orphan-mail.com",
            "confirmed_recipient_control": True,
        },
    )
    assert created.status_code == 200
    ...
    assert active.json()["smoke_test"]["smoke_test_id"] == smoke_test_id
    assert (
        active.json()["smoke_test"]["recipient_email"] == "owner@smoke-orphan-mail.com"
    )
...
def test_workspace_app_production_email_smoke_test_rejects_workspace_member(
    monkeypatch,
) -> None:
    ...
    with get_sessionmaker()() as db:
        workspace = Workspace(
            owner_user_id=owner_user_id, name="Smoke Shared Workspace"
        )
        db.add(workspace)
        db.flush()
        db.add(
    ...
    response = client.post(
        ...
        json={
            "recipient_email": "member@smoke-safety-mail.com",
            "confirmed_recipient_control": True,
        },
    )
    assert response.status_code == 403
...
def test_workspace_app_parallel_send_claims_approved_email_once(monkeypatch) -> None:
    headers = {
        "Authorization": "Bearer dev",
        "X-Test-User-Email": f"usage-concurrent-send-{uuid4()}@example.com",
    }
    email = _workspace_app_test_draft(
        headers, monkeypatch, company_name="Concurrent Send Co"
    )
    ...
    approved = client.post(
        f"/api/workspace-app/emails/{email['id']}/approve",
        headers=headers,
        json=_exact_email_approval_payload(email, "sender@concurrent-send.example"),
    )
    assert approved.status_code == 200
...
def test_workspace_app_email_send_recovers_stale_sending_claim_after_interruption(
    monkeypatch,
) -> None:
    headers = {
        "Authorization": "Bearer dev",
        "X-Test-User-Email": f"usage-stale-send-{uuid4()}@example.com",
    }
    email = _workspace_app_test_draft(
        headers, monkeypatch, company_name="Stale Send Co"
    )
    ...
    approved = client.post(
        f"/api/workspace-app/emails/{email['id']}/approve",
        headers=headers,
        json=_exact_email_approval_payload(email, "sender@stale-send.example"),
    )
    assert approved.status_code == 200
    ...
    monkeypatch.setattr(
        "app.api.usage.send_email",
        lambda **kwargs: calls.append(kwargs) or {"id": "stale-provider-id"},
    )
    sent = client.post(f"/api/workspace-app/emails/{email['id']}/send", headers=headers)
    assert sent.status_code == 200
    assert len(calls) == 1
...
def test_workspace_app_email_patch_cannot_change_payload_after_send_claim(
    monkeypatch,
) -> None:
    headers = {
        "Authorization": "Bearer dev",
        "X-Test-User-Email": f"usage-patch-send-race-{uuid4()}@example.com",
    }
    email = _workspace_app_test_draft(
        headers, monkeypatch, company_name="Patch Send Race Co"
    )
    ...
    approved = client.post(
        f"/api/workspace-app/emails/{email['id']}/approve",
        headers=headers,
        json=_exact_email_approval_payload(email, "sender@patch-send-race.example"),
    )
    assert approved.status_code == 200
    ...
    with ThreadPoolExecutor(max_workers=2) as executor:
        ...
        patch_response = client.patch(
            f"/api/workspace-app/emails/{email['id']}",
            headers=headers,
            json={"body": "Changed after send claim"},
        )
        release_provider.set()
...
def test_workspace_app_company_creation_queues_enrichment_job() -> None:
    headers = {
        "Authorization": "Bearer dev",
        "X-Test-User-Email": "usage-company-enrichment-queue@example.com",
    }
    company_response = client.post(
        "/api/workspace-app/companies",
        headers=headers,
        json={
            "name": "Usage Enrichment Queue",
            "website": "https://usage-enrichment-queue.example",
            "country": "Germany",
            "city": "Berlin",
            "industry": "Construction",
        },
    ...
    try:
        ...
        job = db.scalar(
            select(EnrichmentJob).where(
                EnrichmentJob.workspace_id == lead.workspace_id,
...
def test_workspace_app_company_enrichment_restart_and_cancel(monkeypatch) -> None:
    headers = {
        "Authorization": "Bearer dev",
        "X-Test-User-Email": "usage-enrichment-controls@example.com",
    }
    company_response = client.post(
        "/api/workspace-app/companies",
        headers=headers,
        json={
            "name": "Usage Enrichment Control",
            "website": "https://usage-enrichment.example",
            "country": "Germany",
            "city": "Berlin",
            "industry": "Construction",
        },
    ...
    restarted = client.post(
        f"/api/workspace-app/companies/{company_id}/enrichment/restart", headers=headers
    )
    ...
    assert len(queued) == 1
    assert (
        restart_payload["company"]["workflow_stages"]["website_analysis"] == "running"
    )
    assert restart_payload["company"]["workflow_stages"]["decision_maker"] == "running"

    cancelled = client.post(
        f"/api/workspace-app/companies/{company_id}/enrichment/cancel", headers=headers
    )
...
def test_workspace_app_company_enrichment_restart_handles_enqueue_failure(
    monkeypatch,
) -> None:
    headers = {
        "Authorization": "Bearer dev",
        "X-Test-User-Email": "usage-enrichment-restart-failure@example.com",
    }
    company_response = client.post(
        "/api/workspace-app/companies",
        headers=headers,
        json={
            "name": "Usage Enrichment Restart Failure",
            "website": "https://usage-enrichment-failure.example",
            "country": "Germany",
            "city": "Berlin",
            "industry": "Construction",
        },
...
def test_workspace_app_company_enrichment_restart_handles_sync_failure(
    monkeypatch,
) -> None:
    headers = {
        "Authorization": "Bearer dev",
        "X-Test-User-Email": "usage-enrichment-restart-sync-failure@example.com",
    }
    company_response = client.post(
        "/api/workspace-app/companies",
        headers=headers,
        json={
            "name": "Usage Enrichment Restart Sync Failure",
            "website": "https://usage-enrichment-sync-failure.example",
            "country": "Germany",
            "city": "Berlin",
            "industry": "Construction",
        },
...
def test_workspace_app_company_enrichment_restart_handles_company_out_failure(
    monkeypatch,
) -> None:
    import app.api.usage as usage_module
    ...
    headers = {
        "Authorization": "Bearer dev",
        "X-Test-User-Email": "usage-enrichment-restart-out-failure@example.com",
    }
    company_response = client.post(
        "/api/workspace-app/companies",
        headers=headers,
        json={
            "name": "Usage Enrichment Restart Out Failure",
            "website": "https://usage-enrichment-out-failure.example",
            "country": "Germany",
            "city": "Berlin",
            "industry": "Construction",
        },
    ...
    monkeypatch.setattr(
        "app.api.usage._enqueue_auto_enrichment", lambda *args, **kwargs: False
    )
...
def test_workspace_app_company_enrichment_restart_handles_unexpected_failure(
    monkeypatch,
) -> None:
    headers = {
        "Authorization": "Bearer dev",
        "X-Test-User-Email": "usage-enrichment-restart-unhandled-failure@example.com",
    }
    company_response = client.post(
        "/api/workspace-app/companies",
        headers=headers,
        json={
            "name": "Usage Enrichment Restart Unhandled Failure",
            "website": "https://usage-enrichment-unhandled-failure.example",
            "country": "Germany",
            "city": "Berlin",
            "industry": "Construction",
        },
    ...
    monkeypatch.setattr(
        "app.api.usage._current_workspace",
...
def test_workspace_app_company_enrichment_restart_continues_enqueue_when_setup_and_serialization_fail(
    monkeypatch,
) -> None:
    import app.api.usage as usage_module
    ...
    headers = {
        "Authorization": "Bearer dev",
        "X-Test-User-Email": "usage-enrichment-restart-continue-enqueue@example.com",
    }
    company_response = client.post(
        "/api/workspace-app/companies",
        headers=headers,
        json={
            "name": "Usage Enrichment Continue Queue",
            "website": "https://usage-enrichment-continue-queue.example",
            "country": "Germany",
            "city": "Berlin",
            "industry": "Construction",
        },
...
def test_workspace_app_company_enrichment_restart_creates_queue_job_and_worker_claims() -> (
    None
):
    from app.services.enrichment_queue import claim_next_enrichment_job
    ...
    headers = {
        "Authorization": "Bearer dev",
        "X-Test-User-Email": "usage-enrichment-restart-queue-claim@example.com",
    }
    company_response = client.post(
        "/api/workspace-app/companies",
        headers=headers,
        json={
            "name": "Usage Enrichment Restart Queue Claim",
            "website": "https://usage-enrichment-restart-queue-claim.example",
            "country": "Germany",
            "city": "Berlin",
            "industry": "Construction",
        },
    ...
    try:
        ...
        queued_job = db.scalar(
            ...
            .where(
                EnrichmentJob.workspace_id == company.workspace_id,
...
def test_workspace_app_company_enrichment_restart_downgrades_dependency_runtime_error() -> (
    None
):
    ...
    app.dependency_overrides[security.get_current_workspace_user_context] = (
        broken_workspace_user_context
    )
    try:
        response = local_client.post(
            f"/api/workspace-app/companies/{company_id}/enrichment/restart",
            headers=AUTH,
        )
    finally:
...
def test_workspace_app_company_enrichment_restart_downgrades_dependency_http_500() -> (
    None
):
...
def test_workspace_app_monitoring_returns_only_changes_and_regenerates_report(
    monkeypatch,
) -> None:
    headers = {
        "Authorization": "Bearer dev",
        "X-Test-User-Email": "usage-monitoring@example.com",
    }
    company_response = client.post(
        "/api/workspace-app/companies",
        headers=headers,
        json={
            "name": "Usage Monitor Co",
            "website": "https://usage-monitor.example",
            "country": "Germany",
            "city": "Berlin",
            "industry": "Construction",
        },
    ...
    monitored = next(
        item for item in payload["changes"] if item["company_id"] == str(company_id)
    )
    ...
    try:
        ...
        revenue_snapshot = (refreshed_company.metadata_json or {}).get(
...
def test_workspace_app_enrichment_queue_persists_and_cancels_job() -> None:
    from app.services.enrichment_queue import (
        cancel_jobs_for_lead,
        enqueue_company_enrichment_job,
    )

    headers = {
        "Authorization": "Bearer dev",
        "X-Test-User-Email": "usage-enrichment-queue@example.com",
    }
    company_response = client.post(
        "/api/workspace-app/companies",
        headers=headers,
        json={
            "name": "Usage Durable Queue",
            "website": "https://usage-queue.example",
            "country": "Germany",
            "city": "Berlin",
            "industry": "Construction",
        },
    ...
    try:
        ...
        cancelled = cancel_jobs_for_lead(
            db, workspace_id=workspace.id, lead_id=lead.id, reason="Test cancellation."
        )
        assert cancelled == 1
        db.refresh(stored)
...
def test_enrichment_queue_reuses_active_job_for_duplicate_enqueue() -> None:
    ...
    headers = {
        "Authorization": "Bearer dev",
        "X-Test-User-Email": "usage-enrichment-idempotency@example.com",
    }
    company_response = client.post(
        "/api/workspace-app/companies",
        headers=headers,
        json={
            "name": "Usage Queue Idempotency",
            "website": "https://usage-queue-idempotency.example",
            "country": "Germany",
            "city": "Berlin",
            "industry": "Construction",
        },
    ...
    try:
        ...
        jobs = db.scalars(
            select(EnrichmentJob).where(EnrichmentJob.lead_id == lead.id)
        ).all()
        assert len(jobs) == 1
...
def test_enrichment_queue_reclaims_stale_job_and_blocks_old_claim_completion() -> None:
    ...
    headers = {
        "Authorization": "Bearer dev",
        "X-Test-User-Email": "usage-enrichment-reclaim@example.com",
    }
    company_response = client.post(
        "/api/workspace-app/companies",
        headers=headers,
        json={
            "name": "Usage Queue Reclaim",
            "website": "https://usage-queue-reclaim.example",
            "country": "Germany",
            "city": "Berlin",
            "industry": "Construction",
        },
    ...
    try:
        ...
        first_claim = claim_next_enrichment_job(
            db, worker_id=first_claim_token, stale_after_seconds=900
        )
        ...
        reclaimed = claim_next_enrichment_job(
            db, worker_id=second_claim_token, stale_after_seconds=900
        )
        ...
        assert (
            heartbeat_job_lock(db, job_id=reclaimed.id, claim_token=second_claim_token)
            is True
        )
...
def test_enrichment_queue_retry_uses_exponential_backoff_and_dead_letters() -> None:
    ...
    headers = {
        "Authorization": "Bearer dev",
        "X-Test-User-Email": "usage-enrichment-retry@example.com",
    }
    company_response = client.post(
        "/api/workspace-app/companies",
        headers=headers,
        json={
            "name": "Usage Queue Retry",
            "website": "https://usage-queue-retry.example",
            "country": "Germany",
            "city": "Berlin",
            "industry": "Construction",
        },
    ...
    try:
        ...
        assert (
            fail_or_retry_job(
                db,
                queued,
                RuntimeError("temporary failure again"),
                retry_delay_seconds=60,
        ...
        assert (
            fail_or_retry_job(
                db,
                queued,
                RuntimeError("poison job"),
                retry_delay_seconds=60,
                claim_token=third_claim_token,
            )
            is True
        )
        db.refresh(queued)
...
def test_worker_restart_recovers_stale_job_without_duplicate_execution(
    monkeypatch,
) -> None:
    ...
    headers = {
        "Authorization": "Bearer dev",
        "X-Test-User-Email": "usage-worker-restart@example.com",
    }
    company_response = client.post(
        "/api/workspace-app/companies",
        headers=headers,
        json={
            "name": "Usage Worker Restart",
            "website": "https://usage-worker-restart.example",
            "country": "Germany",
            "city": "Berlin",
            "industry": "Construction",
        },
    ...
    monkeypatch.setattr(
        worker_module, "claim_next_enrichment_job", claim_only_target_job
    )
    ...
    try:
        ...
        assert (
            complete_job(db, recovered, claim_token="worker-crashed:claim-1") is False
        )
...
def test_workspace_app_complete_opportunity_prepares_research_contact_and_review_draft(
    monkeypatch,
) -> None:
    headers = {
        "Authorization": "Bearer dev",
        "X-Test-User-Email": "usage-complete-opportunity@example.com",
    }
    company_response = client.post(
        "/api/workspace-app/companies",
        headers=headers,
        json={
            "name": "Usage Complete Build",
            "website": "https://usage-complete.example",
            "country": "Germany",
            "city": "Berlin",
            "industry": "Construction",
        },
        ...
        metadata.update(
            {
                ...
                "pricing_signals": [
                    "Introduced new pricing plans for enterprise customers"
                ],
                "blog_news_activity": [
                    "Published product update blog posts this month"
                ],
    ...
    response = client.post(
        f"/api/workspace-app/companies/{company_id}/complete-opportunity",
        headers=headers,
    )
    assert response.status_code == 200
    data = response.json()
    ...
    assert data["completed_steps"] == [
        ...
        "Website analysis checked",
        "Contact search checked",
        "Email draft checked",
    ]
    assert data["company"]["email"] == "eva@usage-complete.example"
    assert (
        data["company"]["ai_summary"]
        == "Usage Complete builds construction services for Berlin B2B buyers."
    )
    assert (
        data["company"]["opportunity_analysis"]
        == "Strong fit because the company can benefit from partner discovery."
    )
    ...
    assert intelligence["report"]["products"]["value"] == [
        "Construction services",
        "B2B partnership sourcing",
    ]
    assert (
        intelligence["report"]["icp"]["value"]
        == "Construction firms needing partner discovery in Berlin."
    )
    assert (
        intelligence["report"]["estimated_company_size"]["value"] == "50-100 employees"
    )
    assert intelligence["report"]["buying_signals"]["value"] == [
        "Public B2B footprint",
        "Clear local market",
    ]
    assert intelligence["report"]["hiring_signals"]["value"] == [
        "Hiring for sales roles"
    ]
    assert intelligence["report"]["technology_stack"]["value"] == [
        "WordPress",
        "HubSpot",
    ]
    assert intelligence["report"]["competitors"]["value"] == [
        "Local construction brokers"
    ]
    assert intelligence["report"]["possible_pain_points"]["value"] == [
        "Needs partner discovery"
    ]
    assert (
        intelligence["report"]["best_outreach_angle"]["value"]
        == "Offer qualified B2B partnership leads."
    )
    assert (
        intelligence["report"]["recommended_decision_maker"]["value"]
    ...
    assert all(
        item.get("source_field") and item.get("value")
        for item in buying_intent["evidence"]
    )
    assert (
        data["company"]["buying_signal_score"] == buying_intent["buying_signal_score"]
    )
    ...
    assert (
        data["company"]["recommended_outreach_timing"]
        == buying_intent["recommended_outreach_timing"]
    )
    ...
    assert all(
        item.get("source_field") and item.get("value")
        for item in top_profile["evidence_used"]
    )
    assert decision_intel["top_contact_id"] == top_profile["contact_id"]
    assert data["company"]["contacts"]
    matching_contact = next(
        item for item in data["company"]["contacts"] if item["name"] == "Eva Founder"
    )
    assert (
        matching_contact["decision_maker_intelligence"]["contact_id"]
        == top_profile["contact_id"]
    )
    assert (
        matching_contact["decision_maker_intelligence"]["confidence_score"]
        == top_profile["confidence_score"]
    )
    ...
    assert (
        data["company"]["recommended_next_action"] == ranking["recommended_next_action"]
    )
    ...
    assert outreach_strategy["best_communication_channel"] in {
        "Email",
        "LinkedIn",
        "Phone",
    }
    ...
    assert all(
        item.get("source_field") and item.get("value")
        for item in outreach_strategy["why_contact_now_evidence"]
    )
    assert all(
        item.get("source_field") and item.get("value")
        for item in outreach_strategy["best_subject_line_evidence"]
    )
    assert all(
        item.get("source_field") and item.get("value")
        for item in outreach_strategy["first_sentence_evidence"]
    )
    assert all(
        item.get("source_field") and item.get("value")
        for item in outreach_strategy["strongest_value_proposition_evidence"]
    )
    assert all(
        item.get("source_field") and item.get("value")
        for item in outreach_strategy["strongest_pain_point_evidence"]
    )
    ...
    assert all(
        item.get("source_field") and item.get("value")
        for item in dm_strategy["evidence"]
    )
    ...
    assert all(
        item.get("source_field") and item.get("value")
        for item in revenue_report.get("evidence", [])
    )
    ...
    cached_company = client.get(
        "/api/crm/companies?search=Usage%20Complete", headers=headers
    ).json()[0]
    cached_revenue_report = cached_company["ai_revenue_engine_report"]
    assert cached_revenue_report.get("source_fingerprint") == revenue_report.get(
        "source_fingerprint"
    )
    assert cached_revenue_report.get("generated_at") == revenue_report.get(
        "generated_at"
    )
    ...
    assert cached_company["ai_workflow_engine"].get("current_state") in {
        "needs_manual_review",
        "workflow_completed",
    }
    assert (
        cached_company["ai_workflow_engine"]
        .get("states", {})
        .get("needs_email", {})
        .get("status")
        == "completed"
    ...
    for step in sales_timeline["steps"]:
        ...
        assert all(
            item.get("source_field") and item.get("value") for item in step["evidence"]
        )
    ...
    assert all(
        item.get("source_field") and item.get("value")
        for item in risk_analyzer["factors"]["missing_data"]["evidence"]
    )
    assert all(
        item.get("source_field") and item.get("value")
        for item in risk_analyzer["factors"]["weak_personalization"]["evidence"]
    )
    assert all(
        item.get("source_field") and item.get("value")
        for item in risk_analyzer["factors"]["missing_decision_maker"]["evidence"]
    )
    assert all(
        item.get("source_field") and item.get("value")
        for item in risk_analyzer["factors"]["low_confidence"]["evidence"]
    )
    assert all(
        item.get("source_field") and item.get("value")
        for item in risk_analyzer["factors"]["stale_enrichment"]["evidence"]
    )
    ...
    assert all(
        item.get("source_field") and item.get("value")
        for item in sales_coach["evidence"]
    )
    ...
    assert (
        intelligence["fields"]["official_website"]["value"]
        == "https://usage-complete.example"
    )
    assert intelligence["fields"]["verified_emails"]["value"] == [
        "eva@usage-complete.example"
    ]
    ...
    assert data["recommended_actions"] == [
        "Review and approve the draft before anything is sent."
    ]
    assert (
        data["next_action"] == "Review and approve the draft before anything is sent."
    )
...
def test_workspace_app_email_draft_uses_current_ui_locale(monkeypatch) -> None:
    headers = {
        "Authorization": "Bearer dev",
        "X-Test-User-Email": "usage-locale@example.com",
    ...
    company_response = client.post(
        ...
        headers=headers,
        json={
            "name": "Usage Locale Build",
            "website": "https://usage-locale.example",
            "country": "Germany",
            "city": "Berlin",
            "industry": "Construction",
        },
...
def test_workspace_app_manual_company_gets_fallback_intelligence_and_review_draft(
    monkeypatch,
) -> None:
    headers = {
        "Authorization": "Bearer dev",
        "X-Test-User-Email": "usage-fallback-intelligence@example.com",
    }
    company_response = client.post(
        "/api/workspace-app/companies",
        headers=headers,
        json={
            "name": "Fallback Partner Build",
            "country": "Poland",
            "city": "Warsaw",
    ...
    monkeypatch.setattr("app.api.usage.personalize_email", fake_personalize)
    draft = client.post(
        f"/api/workspace-app/companies/{company['id']}/email-draft", headers=headers
    )
    assert draft.status_code == 200
    data = draft.json()
    assert data["status"] == "success"
...
def test_workspace_app_email_draft_uses_structured_ai_sales_analysis(
    monkeypatch,
) -> None:
    headers = {
        "Authorization": "Bearer dev",
        "X-Test-User-Email": "usage-analysis-context@example.com",
    }
    company_response = client.post(
        "/api/workspace-app/companies",
        headers=headers,
        json={
            "name": "Analysis Context Build",
            "website": "https://analysis-context.example",
            "country": "Germany",
            "city": "Berlin",
            "industry": "B2B SaaS",
        },
    ...
    contact = client.post(
        ...
        json={
            "name": "Avery Founder",
            "title": "Founder",
            "email": "avery@analysis-context.example",
        },
    )
    assert contact.status_code == 200
    ...
    seeded_analysis = {
        ...
        "personalization_variables": [
            ...
            "Recent hiring for sales operations",
        ],
        "predicted_objections": [
            "Timing is unclear",
            "Current workflow may feel good enough",
        ],
        "personalized_opening_line": "Hi Avery, I noticed your team is still scaling outbound operations manually.",
        "strongest_sales_arguments": [
            "Manual work can be removed",
            "Reply quality can improve",
        ],
        ...
        "missing_data": [],
        "evidence": [
            {"source_field": "company.industry", "value": "B2B SaaS", "confidence": 90}
        ],
    ...
    with get_sessionmaker()() as db:
        ...
        company.metadata_json = {
            **(company.metadata_json or {}),
            "ai_sales_workspace": seeded_analysis,
            "ai_sales_workspace_updated_at": seeded_analysis["generated_at"],
        }
        db.commit()
    ...
    assert (
        captured["offer"]
        == "Reduce manual outbound work while improving qualified conversations."
    )
    assert captured["cta"] == "Book a 15-minute workflow review"
    assert (
        "Strong fit because the team still relies on manual outbound."
        in captured["website_summary"]
    )
    ...
    assert (
        captured["analysis_context"]["decision_makers"][0]["email"]
        == "avery@analysis-context.example"
    )
...
def test_workspace_app_manual_company_fallback_uses_requested_locale() -> None:
    ...
    company_response = client.post(
        ...
        headers=headers,
        json={
            "name": "Русский Партнер",
            "country": "Poland",
            "city": "Warsaw",
            "industry": "B2B партнерства",
        },
    )
    assert company_response.status_code == 200
    company = company_response.json()["company"]
    assert (
        "Публичный профиль" in company["ai_summary"]
        or "Проверенные публичные сигналы" in company["ai_summary"]
    )
...
def test_workspace_app_relocalizes_previous_generic_sales_fallback() -> None:
    ...
    assert (
        "Verified public signals" in first_company["ai_summary"]
        or "Public profile is saved" in first_company["ai_summary"]
    )
    ...
    assert (
        "Публичный профиль" in russian_company["ai_summary"]
        or "Проверенные публичные сигналы" in russian_company["ai_summary"]
    )
...
def test_workspace_app_contact_discovery_empty_persists_search_state(
    monkeypatch,
) -> None:
    headers = {
        "Authorization": "Bearer dev",
        "X-Test-User-Email": "usage-contact-empty@example.com",
    }
    company_response = client.post(
        "/api/workspace-app/companies",
        headers=headers,
        json={
            "name": "Usage Contact Empty",
            "website": "https://usage-contact-empty.example",
            "country": "Germany",
            "city": "Berlin",
            "industry": "Construction",
        },
        ...
        return [
            leads[0].model_copy(
                update={
                    ...
                    "notes": '{"hunter_status":"no_verified_email"}',
                }
            )
        ]
...
def test_workspace_app_blocks_placeholder_recipient_before_send(monkeypatch) -> None:
    headers = {
        "Authorization": "Bearer dev",
        "X-Test-User-Email": "usage-placeholder-send@example.com",
    }
    company_response = client.post(
        "/api/workspace-app/companies",
        headers=headers,
        json={
            "name": "Placeholder Send Build",
            "website": "https://placeholder-send.example",
            "country": "Germany",
            "city": "Berlin",
            "industry": "Construction",
        },
    ...
    def fail_send(**kwargs):
        raise AssertionError(
            "Placeholder recipient should not reach the email provider"
        )
...
def test_ai_sales_analysis_generate_success(monkeypatch) -> None:
    ...
    headers = {
        "Authorization": "Bearer dev",
        "X-Test-User-Email": "analysis-success@example.com",
    }
    created = client.post(
        "/api/workspace-app/companies",
        headers=headers,
        json={
            "name": "Analysis Success Co",
            "website": "https://analysis-success.example",
            "industry": "SaaS",
            "country": "Germany",
            "city": "Berlin",
        },
    ...
    client.post(
        ...
        json={
            "name": "Alex Founder",
            "title": "Founder",
            "email": "alex@analysis-success.example",
        },
    )
    ...
    monkeypatch.setattr(
        ...
        lambda **_: {
            ...
            "personalized_follow_up_sequence": [
                "Day 3: share one relevant customer result",
                "Day 7: offer a short teardown",
            ],
            ...
            "evidence": [
                {
                    "source_field": "company.website",
                    "value": "analysis-success.example",
                    "confidence": 95,
                }
            ],
            "version": 1,
...
def test_ai_sales_analysis_partial_when_missing_data(monkeypatch) -> None:
    ...
    headers = {
        "Authorization": "Bearer dev",
        "X-Test-User-Email": "analysis-partial@example.com",
    }
    created = client.post(
        "/api/workspace-app/companies",
        headers=headers,
        json={
            "name": "Analysis Partial Co",
            "website": "https://analysis-partial.example",
            "industry": "SaaS",
            "country": "Germany",
        },
    )
    ...
    monkeypatch.setattr(
        ...
        lambda **_: {
            ...
            "evidence": [
                {"source_field": "company.industry", "value": "SaaS", "confidence": 88}
            ],
            "version": 1,
        },
    )
...
def test_ai_sales_analysis_provider_failure_returns_cached(monkeypatch) -> None:
    ...
    headers = {
        "Authorization": "Bearer dev",
        "X-Test-User-Email": "analysis-failure@example.com",
    }
    created = client.post(
        "/api/workspace-app/companies",
        headers=headers,
        json={
            "name": "Analysis Failure Co",
            "website": "https://analysis-failure.example",
            "industry": "SaaS",
            "country": "Germany",
        },
    )
    ...
    with get_sessionmaker()() as db:
        ...
        company.metadata_json = {
            **(company.metadata_json or {}),
            "ai_sales_workspace": seed,
            "ai_sales_workspace_updated_at": seed["generated_at"],
        }
        db.commit()
    ...
    monkeypatch.setattr(
        usage_module,
        "build_ai_sales_workspace_analysis",
        lambda **_: (_ for _ in ()).throw(ProviderRequestError("provider down")),
    )
    failed = client.post(
        f"/api/workspace-app/companies/{company_id}/ai-sales-analysis",
        headers=headers,
...
def test_ai_sales_analysis_force_regeneration_updates_snapshot_and_avoids_duplicates(
    monkeypatch,
) -> None:
    import app.api.usage as usage_module
    ...
    headers = {
        "Authorization": "Bearer dev",
        "X-Test-User-Email": "analysis-regen-observable@example.com",
    }
    created = client.post(
        "/api/workspace-app/companies",
        headers=headers,
        json={
            "name": "Analysis Regen Co",
            "website": "https://analysis-regen.example",
            "industry": "SaaS",
            "country": "Germany",
        },
    )
    ...
    loaded = client.get(
        f"/api/workspace-app/companies/{company_id}/ai-sales-analysis", headers=headers
    )
    assert loaded.status_code == 200
    loaded_payload = loaded.json()
    assert (
        loaded_payload["analysis"]["version"] == second_payload["analysis"]["version"]
    )
    ...
    assert [item["version"] for item in loaded_payload["available_versions"]] == [
        second_payload["analysis"]["version"],
        first_version,
    ]

    historical = client.get(
        f"/api/workspace-app/companies/{company_id}/ai-sales-analysis?version={first_version}",
        headers=headers,
    )
    ...
    with get_sessionmaker()() as db:
        ...
        assert [snapshot.version_number for snapshot in snapshots] == [
            first_version,
            second_payload["analysis"]["version"],
        ]
        assert (
            snapshots[-1].analysis_json.get("version")
            == second_payload["analysis"]["version"]
        )


def test_ai_sales_analysis_recommendation_actions_are_versioned_and_audited(
    monkeypatch,
) -> None:
    import app.api.usage as usage_module
    ...
    headers = {
        "Authorization": "Bearer dev",
        "X-Test-User-Email": "analysis-recommendations@example.com",
    }
    created = client.post(
        "/api/workspace-app/companies",
        headers=headers,
        json={
            "name": "Analysis Recommendations Co",
            "website": "https://analysis-recommendations.example",
            "industry": "SaaS",
            "country": "Germany",
        },
    )
    ...
    monkeypatch.setattr(
        ...
        lambda **_: {
            ...
            "decision_makers": [
                {
                    "name": "Alex",
                    "title": "Founder",
                    "email": "alex@analysis-recommendations.example",
                }
            ],
            ...
            "personalized_follow_up_sequence": [
                "Day 3: customer proof",
                "Day 7: low-friction CTA",
            ],
            ...
            "evidence": [
                {
                    "source_field": "company.website",
                    "value": "analysis-recommendations.example",
                    "confidence": 95,
                }
            ],
    ...
    edited = client.post(
        ...
        json={
            "key": "first_message",
            "action": "edit",
            "value": "Hi Alex, quick tailored idea for your outbound pipeline.",
    ...
    assert (
        edited_payload["analysis"]["recommended_first_message"]
        == "Hi Alex, quick tailored idea for your outbound pipeline."
    )
    assert (
        edited_payload["analysis"]["recommendation_actions"]["first_message"]["edited"]
        is True
    )
    ...
    assert (
        approved_payload["analysis"]["recommendation_actions"]["next_best_action"][
            "approved"
        ]
        is True
    )
    assert any(
        item.get("event") == "recommendation_approve"
        for item in approved_payload["analysis"].get("recommendation_audit_log", [])
    )
...
def test_ai_sales_analysis_cache_load_and_refresh(monkeypatch) -> None:
    ...
    headers = {
        "Authorization": "Bearer dev",
        "X-Test-User-Email": "analysis-cache@example.com",
    }
    created = client.post(
        "/api/workspace-app/companies",
        headers=headers,
        json={
            "name": "Analysis Cache Co",
            "website": "https://analysis-cache.example",
            "industry": "SaaS",
            "country": "Germany",
        },
    )
    ...
    with get_sessionmaker()() as db:
        ...
        company.metadata_json = {
            **(company.metadata_json or {}),
            "ai_sales_workspace": initial,
            "ai_sales_workspace_updated_at": initial["generated_at"],
        }
        db.commit()
...
def test_ai_sales_analysis_auto_refresh_reuses_versions_until_content_changes(
    monkeypatch,
) -> None:
    import app.api.usage as usage_module
    ...
    headers = {
        "Authorization": "Bearer dev",
        "X-Test-User-Email": "analysis-auto-refresh@example.com",
    }
    created = client.post(
        "/api/workspace-app/companies",
        headers=headers,
        json={
            "name": "Analysis Auto Refresh Co",
            "website": "https://analysis-auto-refresh.example",
            "industry": "SaaS",
            "country": "Germany",
            "city": "Berlin",
        },
    ...
    client.post(
        ...
        json={
            "name": "Jordan Revenue",
            "title": "VP Sales",
            "email": "jordan@analysis-auto-refresh.example",
        },
    )
    ...
    def payload(summary: str, priority: int) -> dict:
        return {
            ...
            "evidence": [
                {
                    "source_field": "company.website",
                    "value": "analysis-auto-refresh.example",
                    "confidence": 95,
                }
            ],
            "opportunity_score": 82,
    ...
    payloads = [
        payload("Auto refresh v1", 88),
        payload("Auto refresh v1", 88),
        payload("Auto refresh v2", 91),
    ]
    monkeypatch.setattr(
        usage_module, "build_ai_sales_workspace_analysis", lambda **_: payloads.pop(0)
    )
    ...
    with get_sessionmaker()() as db:
        ...
        first = usage_module._refresh_cached_ai_sales_workspace_analysis(
            db,
            workspace=workspace,
            user_id=str(company.user_id or ""),
            company=company,
            lead=lead,
        )
        db.commit()
        ...
        second = usage_module._refresh_cached_ai_sales_workspace_analysis(
            db,
            workspace=workspace,
            user_id=str(company.user_id or ""),
            company=company,
        ...
        assert [
            item["version"]
            for item in company.metadata_json["ai_sales_workspace_history"]
        ] == [1]

        third = usage_module._refresh_cached_ai_sales_workspace_analysis(
            db,
            workspace=workspace,
            user_id=str(company.user_id or ""),
            company=company,
        ...
        assert (
            company.metadata_json["ai_sales_workspace"]["summary"] == "Auto refresh v2"
        )
        assert [
            item["version"]
            for item in company.metadata_json["ai_sales_workspace_history"]
...
def test_ai_sales_analysis_isolation_between_workspaces() -> None:
    created = client.post(
        ...
        headers=USER_A_AUTH,
        json={
            "name": "Tenant A Analysis Co",
            "website": "https://tenant-a-analysis.example",
            "industry": "SaaS",
            "country": "Germany",
        },
    )
...
def test_ai_sales_analysis_endpoints_do_not_500_when_snapshot_table_unavailable(
    monkeypatch,
) -> None:
    import app.api.usage as usage_module
    ...
    headers = {
        "Authorization": "Bearer dev",
        "X-Test-User-Email": "analysis-db-fallback@example.com",
    }
    created = client.post(
        "/api/workspace-app/companies",
        headers=headers,
        json={
            "name": "Analysis DB Fallback Co",
            "website": "https://analysis-db-fallback.example",
            "industry": "SaaS",
            "country": "Germany",
        },
    )
    ...
    with get_sessionmaker()() as db:
        ...
        company.metadata_json = {
            **(company.metadata_json or {}),
            "ai_sales_workspace": cached,
            "ai_sales_workspace_updated_at": cached["generated_at"],
        }
        db.commit()
    ...
    assert (
        generated.json()["analysis"]["summary"]
        == "Generated despite snapshot table outage"
    )
    assert [item["version"] for item in generated.json()["available_versions"]] == [
        2,
        1,
    ]

    historical = client.get(
        f"/api/workspace-app/companies/{company_id}/ai-sales-analysis?version=1",
        headers=headers,
    )
    assert historical.status_code == 200
    ...
    missing = client.get(
        f"/api/workspace-app/companies/{company_id}/ai-sales-analysis?version=999",
        headers=headers,
    )
...
def test_ai_sales_analysis_unexpected_generation_error_returns_fallback(
    monkeypatch,
) -> None:
    import app.api.usage as usage_module
    ...
    headers = {
        "Authorization": "Bearer dev",
        "X-Test-User-Email": "analysis-unexpected-error@example.com",
    }
    created = client.post(
        "/api/workspace-app/companies",
        headers=headers,
        json={
            "name": "Analysis Unexpected Error Co",
            "website": "https://analysis-unexpected-error.example",
            "industry": "SaaS",
            "country": "Germany",
        },
    )
...
def test_legacy_null_workspace_records_are_not_returned_to_authenticated_workspace() -> (
    None
):
    SessionLocal = get_sessionmaker()
    with SessionLocal() as db:
    ...
    response = client.get(
        "/api/leads?search=Legacy%20Shared%20Lead", headers=USER_A_AUTH
    )
    assert response.status_code == 200
...
def test_quality_console_requires_owner_and_creates_repair_tasks() -> None:
    ...
    assert any(
        check["module"] == "AI Data Consistency Checker" for check in data["checks"]
    )

    open_bugs = data["open_bugs"]
    assert open_bugs
    task = client.post(
        "/api/admin/quality/tasks",
        headers=OWNER_AUTH,
        json={"fingerprint": open_bugs[0]["fingerprint"]},
    )
...
def test_production_auth_rejects_unsigned_clerk_token(monkeypatch) -> None:
    ...
    payload = (
        base64.urlsafe_b64encode(
            json.dumps({"iss": "https://clerk.test", "sub": "forged"}).encode()
        )
        .rstrip(b"=")
        .decode()
    )
...
def test_production_auth_accepts_standard_clerk_session_jwt_without_audience_when_not_configured(
    monkeypatch,
) -> None:
    issuer = "https://clerk.test"
    private_pem, jwks = _auth_test_keypair()
...
def test_production_auth_accepts_custom_domain_clerk_issuer_fallback(
    monkeypatch,
) -> None:
...
def test_production_owner_context_uses_verified_clerk_user_email(monkeypatch) -> None:
    ...
    monkeypatch.setattr(security, "_fetch_clerk_jwks", lambda _: jwks)
    monkeypatch.setattr(
        security, "_fetch_clerk_user_email", lambda user_id: "romaniukvadym10@gmail.com"
    )
    get_settings.cache_clear()
...
def test_lead_finder_returns_before_inline_website_analysis(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "app.api.routes._analyze_lead_if_possible",
        lambda *args, **kwargs: (_ for _ in ()).throw(
            RuntimeError("inline analysis should not run")
        ),
    )
    monkeypatch.setattr(
        "app.api.routes.search_google_places",
    ...
    response = client.post(
        "/api/leads/find",
        headers=AUTH,
        json={"industry": "Construction", "country": "Germany", "city": "Berlin"},
    )
    assert response.status_code == 200
    lead = response.json()[0]
...
def test_google_maps_missing_key_blocks_lead_finder(monkeypatch) -> None:
    ...
    assert (
        response.json()["detail"]
        == "This connection is not ready. Please contact the workspace owner."
    )
    get_settings.cache_clear()
...
def test_google_maps_key_alias_enables_lead_search(monkeypatch) -> None:
    ...
    assert response.status_code == 200
    lead_search = next(
        item for item in response.json()["integrations"] if item["key"] == "lead_search"
    )
...
def test_google_maps_timeout_returns_user_safe_error(monkeypatch) -> None:
    monkeypatch.setattr(
        ...
        lambda payload: (_ for _ in ()).throw(
            GoogleMapsRequestError(
                "Google Maps is temporarily unavailable after retries."
            )
        ),
    )
    ...
    assert (
        response.json()["detail"]
        == "This connection is temporarily unavailable. Please try again later."
    )


def test_lead_finder_uses_apollo_fallback_when_google_request_fails(
    monkeypatch,
) -> None:
    monkeypatch.setattr("app.api.routes.enrich_leads_with_hunter", lambda leads: leads)
    monkeypatch.setattr(
        "app.api.routes.search_google_places",
        lambda payload: (_ for _ in ()).throw(
            GoogleMapsRequestError("Google Maps timed out.")
        ),
    )
    ...
    monkeypatch.setattr(
        ...
        lambda payload: ApolloSearchResult(
            leads=[fallback_lead], raw_count=1, duration_ms=7
        ),
    )
    response = client.post(
        "/api/leads/find",
        headers=AUTH,
        json={"industry": "Construction", "country": "Germany", "city": "Munich"},
    )
    assert response.status_code == 200
...
def test_lead_finder_returns_partial_results_when_hunter_times_out(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "app.api.routes.search_google_places",
        lambda payload: GooglePlacesSearchResult(
            leads=[lead], raw_count=1, duration_ms=5
        ),
    )
    ...
    response = client.post(
        "/api/leads/find",
        headers=AUTH,
        json={"industry": "Construction", "country": "Germany", "city": "Hamburg"},
    )
    assert response.status_code == 200
    payload = response.json()
    ...
    with get_sessionmaker()() as db:
        lead_record = db.scalar(
            select(Lead).where(Lead.company == "Partial Hunter Timeout GmbH")
        )
    assert lead_record is not None


def test_lead_finder_does_not_run_inline_website_analysis_before_response(
    monkeypatch,
) -> None:
    monkeypatch.setattr("app.api.routes.enrich_leads_with_hunter", lambda leads: leads)
    lead = LeadOut(
    ...
    response = client.post(
        "/api/leads/find",
        headers=AUTH,
        json={"industry": "Construction", "country": "Germany", "city": "Cologne"},
    )
    assert response.status_code == 200
...
def test_google_maps_duplicate_prevention_by_place_id(monkeypatch) -> None:
    ...
    with get_sessionmaker()() as db:
        count = db.scalar(
            select(func.count())
            .select_from(Lead)
            .where(Lead.company == "Duplicate Google Maps GmbH")
        )
    assert count == 1
...
def test_apollo_invalid_key_reports_safe_error(monkeypatch) -> None:
    ...
    get_settings.cache_clear()
    monkeypatch.setattr(
        "app.api.routes.test_apollo_connection",
        lambda: (_ for _ in ()).throw(
            ApolloRequestError(
                "Apollo rejected the backend API key. Verify the live Apollo key and account access."
            )
        ),
    )
...
def test_hunter_invalid_key_reports_safe_error(monkeypatch) -> None:
    ...
    get_settings.cache_clear()
    monkeypatch.setattr(
        "app.api.routes.test_hunter_connection",
        lambda: (_ for _ in ()).throw(
            HunterRequestError(
                "Hunter rejected the backend API key. Verify the live Hunter key and account access."
            )
        ),
    )
...
def test_apollo_company_search_enriches_with_hunter(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "app.api.routes.search_apollo_companies",
        lambda payload: ApolloSearchResult(
            leads=[apollo_lead], raw_count=1, duration_ms=5
        ),
    )
    monkeypatch.setattr(
        "app.api.routes.enrich_leads_with_hunter", lambda leads: [hunter_lead]
    )
    response = client.post(
...
def test_hunter_no_verified_email_is_friendly(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "app.api.routes.enrich_leads_with_hunter", lambda leads: [enriched]
    )
    response = client.post(
        "/api/apollo/search-companies",
        headers=AUTH,
...
def test_apollo_timeout_returns_user_safe_error(monkeypatch) -> None:
    monkeypatch.setattr(
        ...
        lambda payload: (_ for _ in ()).throw(
            ApolloRequestError(
                "Apollo is temporarily unavailable. Please try again in a few minutes."
            )
        ),
    )
    response = client.post(
...
def test_apollo_empty_results_are_safe(monkeypatch) -> None:
    monkeypatch.setattr(
        "app.api.routes.search_apollo_companies",
        lambda payload: ApolloSearchResult(leads=[], raw_count=0, duration_ms=5),
    )
    response = client.post(
        "/api/apollo/search-companies",
        headers=AUTH,
...
def test_apollo_duplicate_prevention(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "app.api.routes.search_apollo_companies",
        lambda payload: ApolloSearchResult(leads=[lead], raw_count=1, duration_ms=5),
    )
    payload = {"industry": "Construction", "country": "Germany", "city": "Berlin"}
    ...
    with get_sessionmaker()() as db:
        count = db.scalar(
            select(func.count())
            .select_from(Lead)
            .where(Lead.company == "Duplicate Apollo GmbH")
        )
    assert count == 1
...
def test_update_lead_returns_409_for_duplicate_email() -> None:
    ...
    assert (
        update.json()["detail"]
        == "A lead with this email already exists. Use a different email."
    )
...
def test_manual_lead_creation_enriches_with_hunter_and_ai(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "app.api.routes._hunter_enriched_leads",
        lambda db, request, user_id, workspace, leads: enriched(leads),
    )
    monkeypatch.setattr(
        ...
        lambda website: type(
            ...
            (),
            {
                "url": website,
                "title": "Manual Build",
                "meta_description": "Construction company",
                "text": "Construction services contact us case studies",
    ...
    monkeypatch.setattr(
        ...
        lambda **kwargs: AnalysisOut(
            ...
            buying_signals=[
                ...
                "Local construction market focus",
            ],
    ...
    response = client.post(
        ...
        json={
            "company": "Manual Build GmbH",
            "website": "https://manual-build.example",
            "country": "Germany",
            "city": "Berlin",
            "industry": "Construction",
        },
    ...
    assert (
        lead["ai_summary"]
        == "Manual Build serves commercial construction buyers in Berlin."
    )
    assert (
        lead["suggested_offer"]
        == "Offer a reviewed outreach campaign for project leads."
    )
    ...
    assert (
        ...
        == "Strong B2B partnership opportunity for project lead generation."
    )
    ...
    assert (
        company["next_recommended_action"]
        == "Review and approve the first outreach draft."
    )
...
def test_manual_lead_creation_survives_hunter_no_email(monkeypatch) -> None:
        ...
        return [
            lead.model_copy(
                update={
                    "hunter_status": "no_verified_email",
                    "source": "manual",
                    "notes": '{"source":"manual","hunter_status":"no_verified_email"}',
                }
            )
        ]
    ...
    response = client.post(
        ...
        headers=AUTH,
        json={
            "company": "Manual No Email Build",
            "website": "https://manual-no-email-build.example",
            "country": "Germany",
            "city": "Berlin",
            "industry": "Construction",
        },
...
def test_ai_analyze_skips_unreachable_website_without_failing_saved_lead(
    monkeypatch,
) -> None:
    monkeypatch.setattr(
        "app.api.routes._hunter_enriched_leads",
    ...
    lead_response = client.post(
        ...
        headers=AUTH,
        json={
            "company": "Unreachable Build",
            "website": "unreachable-build.example",
            "country": "Germany",
            "city": "Berlin",
            "industry": "Construction",
        },
    ...
    crm_response = client.get(
        "/api/crm/companies?search=Unreachable%20Build", headers=AUTH
    )
    assert crm_response.status_code == 200
    ...
    with SessionLocal() as db:
        analysis = db.scalar(
            select(WebsiteAnalysis).where(
                WebsiteAnalysis.company == "Long AI Fields GmbH"
            )
        )
...
def test_manual_lead_draft_email_does_not_send(monkeypatch) -> None:
    ...
    lead_response = client.post(
        ...
        headers=AUTH,
        json={
            "company": "Manual Draft Build",
            "website": "https://manual-draft.example",
            "industry": "Construction",
            "email": "founder@manual-draft.example",
        },
    )
    assert lead_response.status_code == 200
    lead = lead_response.json()
...
def _gmail_reply_sync_headers(fixture: dict[str, str]) -> dict[str, str]:
    return {
        "Authorization": "Bearer dev",
...
def _assert_gmail_reply_sync_unchanged(
    fixture: dict[str, str], message_id: str
) -> None:
    db = get_sessionmaker()()
    try:
        ...
        assert _gmail_reply_sync_counts(fixture["workspace_id"], message_id) == (0, 0)
    finally:
        db.close()


def test_gmail_reply_sync_matches_base_address_reply_to_plus_alias(
    monkeypatch,
) -> None:
    run_id = uuid4().hex
    recipient = f"romaniukvadym10+client-smoke-{run_id}@gmail.com"
    reply_message_id = f"reply-gmail-plus-{run_id}"
    fixture = _create_gmail_sync_fixture(
        user_email=f"gmail-plus-alias-{run_id}@example.com",
        lead_email=recipient,
        recipient_email=recipient,
        thread_id=f"thread-gmail-plus-{run_id}",
        outbound_message_id=f"outbound-gmail-plus-{run_id}",
    )
    _install_gmail_sync_mocks(
        monkeypatch,
        {
            f"thread-gmail-plus-{run_id}": [
                _gmail_sync_message(
                    message_id=reply_message_id,
                    thread_id=f"thread-gmail-plus-{run_id}",
                    from_email="romaniukvadym10@gmail.com",
                    snippet="Reply from the base Gmail account.",
    ...
    response = client.post(
        ...
        headers=_gmail_reply_sync_headers(fixture),
    )

    assert response.status_code == 200
    assert response.json()["synced"] == 1
    assert _gmail_reply_sync_counts(fixture["workspace_id"], reply_message_id) == (
        1,
        1,
    )


def test_gmail_reply_sync_keeps_exact_gmail_address_match(monkeypatch) -> None:
    run_id = uuid4().hex
    recipient = f"exact-gmail-{run_id}@gmail.com"
    reply_message_id = f"reply-exact-gmail-{run_id}"
    fixture = _create_gmail_sync_fixture(
        user_email=f"gmail-exact-{run_id}@example.com",
        lead_email=recipient,
        recipient_email=recipient,
        thread_id=f"thread-exact-gmail-{run_id}",
        outbound_message_id=f"outbound-exact-gmail-{run_id}",
    )
    _install_gmail_sync_mocks(
        monkeypatch,
        {
            f"thread-exact-gmail-{run_id}": [
                _gmail_sync_message(
                    message_id=reply_message_id,
                    thread_id=f"thread-exact-gmail-{run_id}",
                    from_email=recipient.upper(),
                    snippet="Exact normalized Gmail reply.",
                    internal_date=datetime.utcnow(),
                )
            ]
        },
...
@pytest.mark.parametrize(
    ("recipient_domain", "from_domain"),
    [("googlemail.com", "gmail.com"), ("gmail.com", "googlemail.com")],
)
def test_gmail_reply_sync_matches_googlemail_and_gmail_domains(
    monkeypatch, recipient_domain: str, from_domain: str
) -> None:
    run_id = uuid4().hex
    local_part = f"googlemail-match-{run_id}"
    recipient = f"{local_part}+client@{recipient_domain}"
    reply_message_id = f"reply-googlemail-{recipient_domain}-{run_id}"
    fixture = _create_gmail_sync_fixture(
        user_email=f"gmail-googlemail-{recipient_domain}-{run_id}@example.com",
        lead_email=recipient,
        recipient_email=recipient,
        thread_id=f"thread-googlemail-{recipient_domain}-{run_id}",
        outbound_message_id=f"outbound-googlemail-{recipient_domain}-{run_id}",
    )
    _install_gmail_sync_mocks(
        monkeypatch,
        {
            f"thread-googlemail-{recipient_domain}-{run_id}": [
                _gmail_sync_message(
                    message_id=reply_message_id,
                    thread_id=f"thread-googlemail-{recipient_domain}-{run_id}",
                    from_email=f"{local_part}@{from_domain}",
                    snippet="Googlemail and Gmail domains match.",
                    internal_date=datetime.utcnow(),
                )
            ]
        },
...
def test_gmail_reply_sync_rejects_different_gmail_local_part(monkeypatch) -> None:
    run_id = uuid4().hex
    recipient = f"buyer-{run_id}+client@gmail.com"
    reply_message_id = f"reply-wrong-gmail-local-{run_id}"
    fixture = _create_gmail_sync_fixture(
        user_email=f"gmail-wrong-local-{run_id}@example.com",
        lead_email=recipient,
        recipient_email=recipient,
        thread_id=f"thread-wrong-gmail-local-{run_id}",
        outbound_message_id=f"outbound-wrong-gmail-local-{run_id}",
    )
    _install_gmail_sync_mocks(
        monkeypatch,
        {
            f"thread-wrong-gmail-local-{run_id}": [
                _gmail_sync_message(
                    message_id=reply_message_id,
                    thread_id=f"thread-wrong-gmail-local-{run_id}",
                    from_email=f"other-{run_id}@gmail.com",
                    snippet="Wrong Gmail local part.",
                    internal_date=datetime.utcnow(),
                )
            ]
        },
    ...
    _assert_gmail_reply_sync_unchanged(fixture, reply_message_id)


def test_gmail_reply_sync_rejects_plus_alias_for_arbitrary_domain(
    monkeypatch,
) -> None:
    run_id = uuid4().hex
    recipient = f"buyer-{run_id}+client@customer.example"
    reply_message_id = f"reply-arbitrary-plus-{run_id}"
    fixture = _create_gmail_sync_fixture(
        user_email=f"gmail-arbitrary-plus-{run_id}@example.com",
        lead_email=recipient,
        recipient_email=recipient,
        thread_id=f"thread-arbitrary-plus-{run_id}",
        outbound_message_id=f"outbound-arbitrary-plus-{run_id}",
    )
    _install_gmail_sync_mocks(
        monkeypatch,
        {
            f"thread-arbitrary-plus-{run_id}": [
                _gmail_sync_message(
                    message_id=reply_message_id,
                    thread_id=f"thread-arbitrary-plus-{run_id}",
                    from_email=f"buyer-{run_id}@customer.example",
                    snippet="Non-Gmail plus aliases must not collapse.",
                    internal_date=datetime.utcnow(),
                )
            ]
        },
...
def test_gmail_plus_alias_reply_sync_does_not_cross_workspace(monkeypatch) -> None:
    run_id = uuid4().hex
    recipient = f"workspace-{run_id}+client@gmail.com"
    reply_message_id = f"reply-plus-workspace-{run_id}"
    current = _create_gmail_sync_fixture(
        user_email=f"gmail-plus-current-{run_id}@example.com",
        lead_email=recipient,
        recipient_email=recipient,
        thread_id=f"thread-plus-workspace-{run_id}",
        outbound_message_id=f"outbound-plus-current-{run_id}",
    )
    other = _create_gmail_sync_fixture(
        user_email=f"gmail-plus-other-{run_id}@example.com",
        lead_email=recipient,
        recipient_email=recipient,
        thread_id=f"thread-plus-workspace-{run_id}",
        outbound_message_id=f"outbound-plus-other-{run_id}",
    )
    _install_gmail_sync_mocks(
        monkeypatch,
        {
            f"thread-plus-workspace-{run_id}": [
                _gmail_sync_message(
                    message_id=reply_message_id,
                    thread_id=f"thread-plus-workspace-{run_id}",
                    from_email=f"workspace-{run_id}@gmail.com",
                    snippet="Workspace-scoped plus alias reply.",
                    internal_date=datetime.utcnow(),
                )
            ]
        },
    ...
    response = client.post(
        "/api/outreach/oauth/gmail/sync",
        headers=_gmail_reply_sync_headers(current),
    )

    assert response.status_code == 200
    assert response.json()["synced"] == 1
    assert _gmail_reply_sync_counts(current["workspace_id"], reply_message_id) == (
        1,
        1,
    )
    _assert_gmail_reply_sync_unchanged(other, reply_message_id)


def test_gmail_plus_alias_reply_sync_repeated_run_does_not_duplicate(
    monkeypatch,
) -> None:
    run_id = uuid4().hex
    recipient = f"repeat-plus-{run_id}+client@gmail.com"
    reply_message_id = f"reply-repeat-plus-{run_id}"
    fixture = _create_gmail_sync_fixture(
        user_email=f"gmail-repeat-plus-{run_id}@example.com",
        lead_email=recipient,
        recipient_email=recipient,
        thread_id=f"thread-repeat-plus-{run_id}",
        outbound_message_id=f"outbound-repeat-plus-{run_id}",
    )
    _install_gmail_sync_mocks(
        monkeypatch,
        {
            f"thread-repeat-plus-{run_id}": [
                _gmail_sync_message(
                    message_id=reply_message_id,
                    thread_id=f"thread-repeat-plus-{run_id}",
                    from_email=f"repeat-plus-{run_id}@gmail.com",
                    snippet="Repeated plus alias sync should not duplicate.",
                    internal_date=datetime.utcnow(),
                )
            ]
        },
...
def test_gmail_plus_alias_reply_sync_ignores_outbound_message_in_thread(
    monkeypatch,
) -> None:
    run_id = uuid4().hex
    recipient = f"outbound-plus-{run_id}+client@gmail.com"
    reply_message_id = f"outbound-plus-followup-{run_id}"
    fixture = _create_gmail_sync_fixture(
        user_email=f"gmail-outbound-plus-{run_id}@example.com",
        lead_email=recipient,
        recipient_email=recipient,
        thread_id=f"thread-outbound-plus-{run_id}",
        outbound_message_id=f"outbound-plus-primary-{run_id}",
    )
    _install_gmail_sync_mocks(
        monkeypatch,
        {
            f"thread-outbound-plus-{run_id}": [
                _gmail_sync_message(
                    message_id=reply_message_id,
                    thread_id=f"thread-outbound-plus-{run_id}",
                    from_email="qa.sender@testmail.local",
                    snippet="Follow-up sent from our Gmail account.",
                    internal_date=datetime.utcnow(),
                    labels=["SENT"],
                )
            ]
...
def test_gmail_reply_sync_updates_crm_and_is_idempotent(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        ...
        lambda **kwargs: {"id": "gmail-outbound-1", "thread_id": "gmail-thread-1"},
    )
    ...
    lead_response = client.post(
        ...
        json={
            ...
            "website": f"https://reply-crm-sync-{run_id}.com",
            "industry": "SaaS",
            "email": lead_email,
        },
    )
    ...
    draft_response = client.post(
        f"/api/leads/{lead['id']}/draft-email", headers=headers
    )
    assert draft_response.status_code == 200
    draft = draft_response.json()
    ...
    try:
        workspace = db.scalar(
            select(Workspace).where(Workspace.owner_user_id == user_email)
        )
        app_settings = db.scalar(
            select(AppSettings).where(AppSettings.workspace_id == workspace.id)
        )
            ...
            "sender": {
                ...
                "oauth": {
                    ...
                    "refresh_token_encrypted": encrypt_secret(
                        "refresh-token", settings.encryption_key
                    ),
    ...
    approved_response = client.post(
        f"/api/emails/{draft['id']}/approve", headers=headers
    )
    assert approved_response.status_code == 200
                        ...
                        "messages": [
                            {
                                ...
                                "payload": {
                                    "headers": [
                                        {
                                            "name": "From",
                                            "value": "QA Sender <qa.sender@testmail.local>",
                                        }
                                    ]
                                },
                            },
    ...
    crm_after_reply = client.get(
        f"/api/crm/companies?search={company_name.replace(' ', '%20')}", headers=headers
    ).json()[0]
    assert crm_after_reply["crm_stage"] == "Replied"
    assert crm_after_reply["email_status"] == "Replied"
    assert crm_after_reply["replied_at"]
    assert any(
        item["action"] == "outreach.gmail.reply_synced"
        for item in crm_after_reply["activity"]
    )
    ...
    try:
        ...
        assert (
            ...
            .filter(
                AuditLog.action == "outreach.gmail.reply_synced",
                AuditLog.metadata_json["email_id"].as_string() == draft["id"],
            )
            .count()
            == 1
        )
        usage_before_second_sync = db.scalar(
            select(UsageCounter.email_sends).where(
                UsageCounter.workspace_id == email.workspace_id
            )
        )
    finally:
        db.close()
...
def test_outreach_sender_status_and_update() -> None:
    headers = {
        "Authorization": "Bearer dev",
        "X-Test-User-Email": "sender-settings@example.com",
    }
...
def test_gmail_oauth_start_reports_missing_secret_without_treating_resend_as_oauth(
    monkeypatch,
) -> None:
    headers = {
        "Authorization": "Bearer dev",
        "X-Test-User-Email": "gmail-oauth-missing-secret@example.com",
    }
    settings = get_settings()
    monkeypatch.setattr(settings, "google_oauth_client_id", "google-client")
    ...
    monkeypatch.setattr(
        ...
        "https://api.example.test/api/outreach/oauth/gmail/callback",
    )
    monkeypatch.setattr(
        settings,
        "google_oauth_allowed_test_users",
        "gmail-oauth-missing-secret@example.com",
    )
...
def test_gmail_oauth_start_uses_encrypted_workspace_state_and_explicit_account_selection(
    monkeypatch,
) -> None:
    ...
    state_payload = json.loads(
        decrypt_secret(params["state"][0], settings.encryption_key)
    )
...
def test_gmail_oauth_state_and_handoff_lookup_keys_use_keyed_digest(
    monkeypatch,
) -> None:
    ...
    callback = client.get(
        "/api/outreach/oauth/gmail/callback",
    ...
    handoff_lookup_key = routes_module._oauth_nonce_hash(
        handoff, settings.encryption_key
    )
...
def _prepare_gmail_oauth_handoff(
    monkeypatch,
    *,
    email: str | None = None,
    mailbox: str = "client.mailbox@example.com",
    code: str = "valid-code",
):
    ...
    assert location.startswith(
        "https://preview.example.test/dashboard/settings/gmail-oauth/complete?handoff="
    )
    handoff = parse_qs(urlparse(location).query)["handoff"][0]
    return headers, workspace, handoff, location


def _fake_google_client(
...
def test_gmail_oauth_callback_binds_mailbox_to_state_workspace_only(
    monkeypatch,
) -> None:
    ...
    monkeypatch.setattr(
        settings, "google_oauth_allowed_test_users", "client.mailbox@example.com"
    )
    monkeypatch.setattr(settings, "encryption_key", "test-encryption-key")
    ...
    client_status_before_finalize = client.get(
        "/api/outreach/sender/status", headers=client_headers
    )
    ...
    finalize = client.post(
        "/api/outreach/oauth/gmail/finalize",
    ...
    assert provider_calls == [
        ("post", routes_module.GOOGLE_OAUTH_TOKEN_URL),
        ("get", routes_module.GOOGLE_USERINFO_URL),
    ]
    ...
    with get_sessionmaker()() as db:
        client_settings = db.scalar(
            select(AppSettings).where(AppSettings.user_id == client_email)
        )
        other_settings = db.scalar(
            select(AppSettings).where(AppSettings.user_id == other_email)
        )
        ...
        assert (
            decrypt_secret(
                client_sender["oauth"]["refresh_token_encrypted"],
...
def test_gmail_oauth_callback_rejects_expired_state_before_provider_exchange(
    monkeypatch,
) -> None:
    ...
    created_at = datetime.utcnow() - timedelta(
        seconds=routes_module.GMAIL_OAUTH_STATE_TTL_SECONDS + 5
    )
    ...
    with get_sessionmaker()() as db:
        workspace_row = db.get(Workspace, UUID(workspace["id"]))
        app_settings = routes_module._settings_for_workspace(
            db, client_email, workspace_row
        )
        routes_module._store_gmail_oauth_state(
            db,
    ...
    class UnexpectedGoogleClient:
        def __init__(self, *args, **kwargs) -> None:
            raise AssertionError(
                "Expired OAuth state must not be exchanged with Google"
            )
    ...
    assert (
        ...
        == "https://preview.example.test/dashboard/settings?mail=state_expired"
    )


def test_gmail_oauth_callback_rejects_replayed_state_before_provider_exchange(
    monkeypatch,
) -> None:
    ...
    assert (
        ...
        == "https://preview.example.test/dashboard/settings?mail=state_replayed"
    )


def test_gmail_oauth_callback_rejects_state_for_different_workspace_before_provider_exchange(
    monkeypatch,
) -> None:
    ...
    class UnexpectedGoogleClient:
        def __init__(self, *args, **kwargs) -> None:
            raise AssertionError(
                "Invalid OAuth state must not be exchanged with Google"
            )
    ...
    callback = client.get(
        "/api/outreach/oauth/gmail/callback",
        params={"code": "valid-code", "state": invalid_state},
        follow_redirects=False,
    )
    assert callback.status_code == 307
    assert (
        callback.headers["location"]
        == "https://preview.example.test/dashboard/settings?mail=workspace_error"
    )


def test_gmail_oauth_finalize_rejects_clerk_user_mismatch_without_connecting(
    monkeypatch,
) -> None:
    owner_headers, _workspace, handoff, _location = _prepare_gmail_oauth_handoff(
        monkeypatch, mailbox="mismatch.mailbox@example.com"
    )
    other_headers = {
        "Authorization": "Bearer dev",
        "X-Test-User-Email": f"gmail-mismatch-{uuid4()}@example.com",
    }
    ...
    assert (
        client.get("/api/outreach/sender/status", headers=owner_headers).json()[
            "oauth_connected"
        ]
        is False
    )
...
def test_gmail_oauth_finalize_rejects_expired_handoff_without_connecting(
    monkeypatch,
) -> None:
    headers, workspace, handoff, _location = _prepare_gmail_oauth_handoff(
        monkeypatch, mailbox="expired-handoff@example.com"
    )
    handoff_key = routes_module._oauth_nonce_hash(
        handoff, get_settings().encryption_key
    )
    old = datetime.utcnow() - timedelta(
        seconds=routes_module.GMAIL_OAUTH_HANDOFF_TTL_SECONDS + 5
    )
    with get_sessionmaker()() as db:
        settings_row = db.scalar(
            select(AppSettings).where(AppSettings.workspace_id == UUID(workspace["id"]))
        ...
        settings_row.security = {
            **settings_row.security,
            "gmail_oauth_pending_handoffs": handoffs,
        }
    ...
    assert (
        client.get("/api/outreach/sender/status", headers=headers).json()[
            "oauth_connected"
        ]
        is False
    )
...
def test_gmail_oauth_finalize_replay_does_not_call_provider_twice(monkeypatch) -> None:
    headers, _workspace, handoff, _location = _prepare_gmail_oauth_handoff(
        monkeypatch, mailbox="replay.mailbox@example.com"
    )
    provider_calls: list[tuple[str, str]] = []
    monkeypatch.setattr(
        routes_module.httpx,
        "Client",
        _fake_google_client(provider_calls, mailbox="replay.mailbox@example.com"),
    )
...
def test_gmail_oauth_finalize_concurrent_requests_claim_handoff_once(
    monkeypatch,
) -> None:
    headers, _workspace, handoff, _location = _prepare_gmail_oauth_handoff(
        monkeypatch, mailbox="concurrent.mailbox@example.com"
    )
    provider_calls: list[tuple[str, str]] = []
    provider_lock = threading.Lock()

    class SlowGoogleClient(
        _fake_google_client(provider_calls, mailbox="concurrent.mailbox@example.com")
    ):
        def post(self, url: str, data: dict[str, str]):
            with provider_lock:
                time.sleep(0.05)
...
def test_gmail_oauth_finalize_rejects_cross_workspace_handoff_without_provider_call(
    monkeypatch,
) -> None:
    headers, workspace, handoff, _location = _prepare_gmail_oauth_handoff(
        monkeypatch, mailbox="cross-workspace@example.com"
    )
    other_headers = {
        "Authorization": "Bearer dev",
        "X-Test-User-Email": f"gmail-cross-{uuid4()}@example.com",
    }
    other_workspace = client.get("/api/workspace/me", headers=other_headers).json()
    handoff_key = routes_module._oauth_nonce_hash(
        handoff, get_settings().encryption_key
...
def test_gmail_oauth_finalize_token_exchange_failure_does_not_connect(
    monkeypatch,
) -> None:
    headers, _workspace, handoff, _location = _prepare_gmail_oauth_handoff(
        monkeypatch, mailbox="token-failure@example.com"
    )
...
def test_gmail_oauth_finalize_persistence_failure_does_not_return_secrets_or_connect(
    monkeypatch,
) -> None:
    headers, _workspace, handoff, _location = _prepare_gmail_oauth_handoff(
        monkeypatch, mailbox="persist-failure@example.com"
    )
    provider_calls: list[tuple[str, str]] = []
    monkeypatch.setattr(
        routes_module.httpx,
        ...
        _fake_google_client(
            provider_calls,
            mailbox="persist-failure@example.com",
...
def test_approved_email_uses_workspace_sender(monkeypatch) -> None:
    headers = {
        "Authorization": "Bearer dev",
        "X-Test-User-Email": "sender-send@example.com",
    }
    monkeypatch.setattr(
        "app.api.routes._hunter_enriched_leads",
        lambda db, request, user_id, workspace, leads: leads,
    ...
    lead = client.post(
        ...
        headers=headers,
        json={
            "company": "Sender Send Co",
            "website": "https://sender-send.example",
            "industry": "Construction",
            "email": "buyer@sender-send.example",
        },
    ).json()
...
def test_smtp_sender_requires_custom_encryption_key() -> None:
    headers = {
        "Authorization": "Bearer dev",
        "X-Test-User-Email": "smtp-no-key@example.com",
    }
    ...
    assert (
        response.json()["detail"]
        == "Connect email sending or adjust the daily sending limit before sending."
    )
...
def test_smtp_sender_send_uses_decrypted_workspace_config(monkeypatch) -> None:
    headers = {
        "Authorization": "Bearer dev",
        "X-Test-User-Email": "smtp-send@example.com",
    }
    monkeypatch.setenv("ENCRYPTION_KEY", "test-custom-encryption-key")
    get_settings.cache_clear()
    monkeypatch.setattr("app.api.routes.verify_smtp_connection", lambda **kwargs: None)
    ...
    try:
        ...
        lead = client.post(
            ...
            headers=headers,
            json={
                "company": "SMTP Send Co",
                "website": "https://smtp-send.example",
                "industry": "Construction",
                "email": "buyer@smtp-send.example",
            },
        ).json()
        draft = client.post(
            f"/api/leads/{lead['id']}/draft-email", headers=headers
...
def test_smtp_sender_save_rejects_unverified_mailbox(monkeypatch) -> None:
    headers = {
        "Authorization": "Bearer dev",
        "X-Test-User-Email": "smtp-unverified@example.com",
    }
    monkeypatch.setenv("ENCRYPTION_KEY", "test-custom-encryption-key")
    get_settings.cache_clear()
    ...
    def fail_verify(**kwargs):
        raise EmailProviderRequestError(
            "SMTP connection could not be verified. Check host, port, username and app password."
        )
...
def test_disabled_outreach_sender_blocks_send(monkeypatch) -> None:
    headers = {
        "Authorization": "Bearer dev",
        "X-Test-User-Email": "sender-disabled@example.com",
    }
    monkeypatch.setattr(
        "app.api.routes._hunter_enriched_leads",
        lambda db, request, user_id, workspace, leads: leads,
    ...
    lead = client.post(
        ...
        headers=headers,
        json={
            "company": "Sender Disabled Co",
            "website": "https://sender-disabled.example",
            "industry": "Construction",
            "email": "buyer@sender-disabled.example",
        },
    ).json()
    draft = client.post(f"/api/leads/{lead['id']}/draft-email", headers=headers).json()
    approved = client.post(f"/api/emails/{draft['id']}/approve", headers=headers)
...
def test_crm_duplicate_prevention_reuses_manual_company(monkeypatch) -> None:
    ...
    payload = {
        "company": "CRM Duplicate Build",
        "website": "https://crm-duplicate.example",
        "country": "Germany",
        "city": "Berlin",
        "industry": "Construction",
    }
...
def test_crm_stage_move_and_note_are_persisted(monkeypatch) -> None:
    ...
    lead_response = client.post(
        ...
        headers=AUTH,
        json={
            "company": "CRM Action Build",
            "website": "https://crm-action.example",
            "country": "Germany",
            "city": "Berlin",
            "industry": "Construction",
        },
    ...
    assert lead_response.status_code == 200
    company = client.get("/api/crm/companies?search=CRM%20Action", headers=AUTH).json()[
        0
    ]

    moved = client.patch(
        f"/api/crm/companies/{company['id']}/stage",
        headers=AUTH,
        json={"stage": "Meeting Scheduled"},
    )
    ...
    note = client.post(
        f"/api/crm/companies/{company['id']}/notes",
        headers=AUTH,
        json={"body": "Customer asked to review next week."},
    )
    assert note.status_code == 200
    assert note.json()["body"] == "Customer asked to review next week."

    refreshed = client.get(
        "/api/crm/companies?search=CRM%20Action", headers=AUTH
    ).json()[0]
    ...
    try:
        settings = (
            ...
            .filter(AppSettings.workspace_id == UUID(workspace["id"]))
            .first()
        )
        assert settings is not None
        ai = settings.ai if isinstance(settings.ai, dict) else {}
        continuous_learning = (
            ai.get("continuous_learning")
            if isinstance(ai.get("continuous_learning"), dict)
            else {}
        )
        outcomes = (
            continuous_learning.get("outcomes")
            if isinstance(continuous_learning.get("outcomes"), dict)
            else {}
        )
...
def test_crm_batch_loader_returns_older_visible_contacts_and_emails(
    monkeypatch,
) -> None:
    monkeypatch.setattr(
        "app.api.routes._hunter_enriched_leads",
    ...
    lead_response = client.post(
        ...
        headers=AUTH,
        json={
            "company": "Visible History Build",
            "website": "https://visible-history.example",
            "country": "Germany",
            "city": "Berlin",
            "industry": "Construction",
        },
    ...
    assert [contact["name"] for contact in company_out["contacts"]] == [
        f"Visible Contact {index}" for index in range(10)
    ]
    assert [email["subject"] for email in company_out["generated_emails"]] == [
        f"Visible email {index}" for index in range(10)
    ]
...
def test_crm_pipeline_returns_company_cards_with_activity_timeline(monkeypatch) -> None:
    ...
    lead_response = client.post(
        ...
        headers=AUTH,
        json={
            "company": "Pipeline Activity Build",
            "website": "https://pipeline-activity.example",
            "country": "Germany",
            "city": "Berlin",
            "industry": "Construction",
        },
    ...
    try:
        db.add(
            AuditLog(
                user_id="dev_user",
                workspace_id=UUID(workspace["id"]),
                action="lead.pipeline_activity_test",
                metadata_json={"lead_id": lead["id"], "source": "test"},
            )
        )
        db.commit()
    finally:
    ...
    assert response.status_code == 200
    company = next(
        item for item in response.json()["companies"] if item["lead_id"] == lead["id"]
    )
    assert company["name"] == "Pipeline Activity Build"
    assert any(
        item["action"] == "lead.pipeline_activity_test" for item in company["activity"]
    )
...
def test_crm_company_and_pipeline_default_sort_by_overall_score(monkeypatch) -> None:
    ...
    first = client.post(
        ...
        headers=AUTH,
        json={
            "company": "Ranking Lower",
            "website": "https://ranking-lower.example",
            "country": "Germany",
            "city": "Berlin",
            "industry": "Construction",
        },
    ...
    second = client.post(
        ...
        headers=AUTH,
        json={
            "company": "Ranking Higher",
            "website": "https://ranking-higher.example",
            "country": "Germany",
            "city": "Berlin",
            "industry": "Construction",
        },
    ...
    companies_response = client.get(
        "/api/crm/companies?search=Ranking%20", headers=AUTH
    )
    assert companies_response.status_code == 200
    companies = [
        item
        for item in companies_response.json()
        if item["name"] in {"Ranking Lower", "Ranking Higher"}
    ]
    ...
    ranked = [
        item
        for item in pipeline_response.json()["companies"]
        if item["name"] in {"Ranking Lower", "Ranking Higher"}
    ]
    assert len(ranked) == 2
...
def test_ai_sales_copilot_endpoints(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        ...
        lambda url: type(
            "Snapshot",
            (),
            {"text": "Contact us for services", "technologies": ["Next.js"]},
        )(),
    )
    monkeypatch.setattr(
        "app.api.routes.meeting_preparation",
        lambda payload: MeetingPrepOut(
            company_summary="Commercial builder",
            decision_maker_profile="Owner-led team",
            likely_objections=["Timing"],
            suggested_questions=["What is your lead target?"],
            sales_strategy="Lead with booked meetings.",
        ),
    )
    monkeypatch.setattr(
        "app.api.routes.adaptive_follow_ups",
        lambda payload: FollowUpSequenceOut(
            no_open=["Bump"],
            opened=["Saw you had a look"],
            clicked=["Worth discussing?"],
            replied=["Thanks for the reply"],
        ),
    )
    monkeypatch.setattr(
        "app.api.routes.campaign_analytics",
        lambda payload: CampaignAnalyticsOut(
            campaign_id=payload["campaign_id"],
            campaign_success=68,
            predicted_reply_rate=12.5,
            predicted_conversion_rate=3.2,
            suggested_improvements=["Tighten ICP"],
        ),
    )
    ...
    campaign = client.post(
        "/api/campaigns",
        headers=AUTH,
        json={"name": "Copilot Campaign", "industry": "Construction"},
    ).json()
    lead = client.post(
        "/api/leads",
        headers=AUTH,
        json={
            "company": "Copilot Build Co",
            "website": "https://example.com",
            "industry": "Construction",
    ...
    try:
        db.query(Subscription).filter(
            Subscription.workspace_id == UUID(workspace["id"])
        ).delete()
        db.commit()
    finally:
        db.close()
    ...
    analytics = client.post(
        f"/api/campaigns/{campaign['id']}/ai-analytics", headers=AUTH
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("неизвестно", None),
        (
            "Revenue depends on contract size and cannot be estimated from the current data.",
            None,
        ),
    ],
)
def test_llm_number_parser_handles_sales_copilot_revenue_shapes(
    value, expected
) -> None:
...
def test_sales_copilot_moves_textual_revenue_into_reason(monkeypatch) -> None:
    ...
    result = sales_copilot(
        {"response_language": "Russian", "lead": {"company": "Safe Revenue Co"}}
    )
    ...
    assert (
        result.fit_reason
        == "The company matches the target segment and has visible conversion gaps."
    )
...
def test_lead_ai_payload_normalizes_language_fallback(
    workspace_language, expected
) -> None:
    lead = Lead(
        company="Language Fallback Co",
        website="https://example.com",
        industry="SaaS",
        country="Poland",
        city="Warsaw",
...
def test_resend_webhook_updates_delivery_metrics() -> None:
    ...
    delivered = client.post(
        "/webhooks/resend",
        json={"type": "email.delivered", "data": {"email_id": "resend-msg-1"}},
    )
    ...
    opened = client.post(
        "/webhooks/resend",
        json={"type": "email.opened", "data": {"email_id": "resend-msg-1"}},
    )
    ...
    try:
        settings = (
            db.query(AppSettings)
            .filter(AppSettings.workspace_id == workspace_id)
            .first()
        )
        assert settings is not None
        ai = settings.ai if isinstance(settings.ai, dict) else {}
...
def test_resend_webhook_handles_bounce_complaint_and_reply(monkeypatch) -> None:
        ...
        lambda payload: type(
            ...
            (),
            {
                "model_dump": lambda self: {
                    "next_step": "Book meeting",
                    "suggested_response": "Let's lock in a time",
    ...
    bounced = client.post(
        "/webhooks/resend",
        json={"type": "email.bounced", "data": {"email_id": "resend-msg-2"}},
    )
    assert bounced.status_code == 200
    complained = client.post(
        "/webhooks/resend",
        json={"type": "email.complained", "data": {"email_id": "resend-msg-2"}},
    )
    ...
    replied = client.post(
        ...
        json={
            "type": "email.received",
            "data": {"email_id": "resend-msg-2", "text": "Interested."},
        },
    )
    ...
    try:
        saved = (
            db.query(EmailMessage)
            .filter(EmailMessage.provider_message_id == "resend-msg-2")
            .one()
        )
        ...
        sales_inbox = (
            saved.reply_assistant.get("sales_inbox")
            if isinstance(saved.reply_assistant, dict)
            else None
        )
        ...
        inbound = (
            db.query(EmailMessage)
            .filter(EmailMessage.provider_message_id == "reply:resend-msg-2")
            .one()
        )
        ...
        company = (
            db.query(Company)
            .filter(Company.lead_id == saved.lead_id)
        ...
        assert (
            metadata["ai_sales_inbox_latest"].get("classified_as")
            == "Meeting Requested"
        )
        ...
        settings = (
            db.query(AppSettings).filter(AppSettings.user_id == "dev_user").first()
        )
        assert settings is not None
        ai = settings.ai if isinstance(settings.ai, dict) else {}
        continuous_learning = (
            ...
            "sequence": [
                {
                    "step_order": 1,
                    "name": "Email #1",
                    "subject": "Seller appointment idea",
                    "body": "Intro",
                    "delay_days": 0,
                },
                {
...
def test_stripe_webhook_activates_subscription_with_verified_allowlisted_price(
    monkeypatch,
) -> None:
    future = int(time.time()) + 14 * 24 * 60 * 60
    ...
    _pending_checkout_session_for_test(
        workspace["id"],
        user_id,
        plan="Pro",
    ...
    monkeypatch.setattr(
        "app.api.webhooks.stripe.Subscription.retrieve",
        lambda subscription_id: _stripe_subscription_object(
            subscription_id,
            customer_id=stripe_customer_id,
            workspace_id=workspace["id"],
            user_id=user_id,
    ...
    def fake_price_retrieve(price_id: str, **_kwargs: object) -> SimpleNamespace:
        prices = {
            "price_pro_test": _stripe_price_object(
                price_id, amount=14900, lookup_key="outreachai_pro_monthly"
            ),
            "price_agency_test": _stripe_price_object(
                price_id, amount=49900, lookup_key="outreachai_agency_monthly"
            ),
        }
        return prices[price_id]

    monkeypatch.setattr(
        "app.services.billing.stripe.Price.retrieve", fake_price_retrieve
    )
    payload = {
    ...
    try:
        ...
        settings = (
            db.query(AppSettings)
            .filter(AppSettings.workspace_id == subscription.workspace_id)
            .one()
        )
...
def test_checkout_completed_metadata_plan_without_subscription_price_does_not_grant_access(
    monkeypatch,
) -> None:
    ...
    monkeypatch.setattr(
        ...
        lambda subscription_id: {
            ...
            "customer": customer_id,
            "status": "active",
            "metadata": {"user_id": user_id, "workspace_id": workspace["id"]},
            "items": {"data": []},
        },
    )

    raw, signature = stripe_signature(
        _checkout_completed_payload(
            event_id=f"evt_no_price_{uuid4().hex}",
            session_id=session_id,
            customer_id=customer_id,
...
def test_checkout_completed_uses_verified_price_over_metadata_plan(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "app.services.billing.stripe.Price.retrieve",
        lambda price_id: _stripe_price_object(
            price_id, amount=4900, lookup_key="outreachai_starter_monthly"
        ),
    )

    raw, signature = stripe_signature(
        _checkout_completed_payload(
            event_id=f"evt_mismatch_{uuid4().hex}",
            session_id=session_id,
            customer_id=customer_id,
            subscription_id=subscription_id,
            workspace_id=workspace["id"],
            user_id=user_id,
            metadata_plan="Agency",
        )
    )
    response = client.post(
        "/webhooks/stripe",
...
def test_checkout_completed_missing_subscription_or_unknown_price_fails_closed(
    monkeypatch, retrieve_subscription: bool
) -> None:
    ...
    if not retrieve_subscription:
        ...
        subscription = _stripe_subscription_object(
            subscription_id,
            customer_id=customer_id,
            workspace_id=workspace["id"],
            user_id=user_id,
            price_id="price_retired_unknown",
        )
        monkeypatch.setattr(
            "app.api.webhooks.stripe.Subscription.retrieve",
        ...
        monkeypatch.setattr(
            "app.services.billing.stripe.Price.retrieve",
            lambda price_id: _stripe_price_object(
                price_id, amount=1, lookup_key="retired_price"
            ),
        )

    raw, signature = stripe_signature(
        _checkout_completed_payload(
            event_id=f"evt_fail_closed_{uuid4().hex}",
            session_id=session_id,
            customer_id=customer_id,
            subscription_id=subscription_id,
            workspace_id=workspace["id"],
...
def test_valid_subscription_webhook_repairs_earlier_degraded_checkout_completion(
    monkeypatch,
) -> None:
    ...
    monkeypatch.setattr(
        "app.api.webhooks.stripe.Subscription.retrieve", lambda subscription_id: None
    )

    raw, signature = stripe_signature(
        _checkout_completed_payload(
            event_id=f"evt_repair_pending_{uuid4().hex}",
            session_id=session_id,
            customer_id=customer_id,
            subscription_id=subscription_id,
            workspace_id=workspace["id"],
    ...
    assert (
        client.get("/api/billing/status", headers=headers).json()["entitlement_source"]
        != "stripe"
    )
    ...
    monkeypatch.setattr(
        "app.services.billing.stripe.Price.retrieve",
        lambda price_id, **_kwargs: _stripe_price_object(
            price_id, amount=14900, lookup_key="outreachai_pro_monthly"
        ),
    )
    raw, signature = stripe_signature(update_payload)
...
def test_stripe_webhook_marks_pending_checkout_completed_and_expired() -> None:
    ...
    with get_sessionmaker()() as db:
        completed_row = db.scalar(
            select(BillingCheckoutSession).where(
                BillingCheckoutSession.stripe_session_id == completed_session_id
            )
        )
        expired_row = db.scalar(
            select(BillingCheckoutSession).where(
                BillingCheckoutSession.stripe_session_id == expired_session_id
            )
        )
...
def test_stripe_webhook_unknown_price_fails_closed_without_starter_fallback() -> None:
    ...
    with get_sessionmaker()() as db:
        ...
        assert (
            rejected.metadata_json["reason"]
            == "stripe_subscription_price_product_binding_mismatch"
        )
    status = client.get("/api/billing/status", headers=headers)
    assert status.status_code == 200
    assert status.json()["status"] == "active"
...
def test_billing_checkout_creates_pending_subscription_session(monkeypatch) -> None:
    ...
    checkout_headers = {
        "Authorization": "Bearer dev",
        "X-Test-User-Email": checkout_user_id,
    }
    captured = {}
    ...
    def fake_checkout(
        ...
        return {
            "url": "https://checkout.stripe.test/session",
            "id": "cs_test_pending",
            "customer_id": customer_id or "cus_pending",
    ...
    monkeypatch.setattr("app.api.routes.create_checkout_session", fake_checkout)
    response = client.post(
        "/api/billing/checkout", headers=checkout_headers, json={"plan": "Starter"}
    )
    assert response.status_code == 200
    _assert_url_components(
        response.json()["url"],
    ...
    try:
        ...
        pending = (
            ...
            .filter(BillingCheckoutSession.workspace_id == UUID(workspace["id"]))
            .one()
        )
...
def test_billing_checkout_reuses_open_pending_session(monkeypatch) -> None:
    ...
    monkeypatch.setattr("app.api.routes.create_checkout_session", fake_checkout)
    first = client.post(
        "/api/billing/checkout", headers=headers, json={"plan": "Starter"}
    )
    second = client.post(
        "/api/billing/checkout", headers=headers, json={"plan": "Starter"}
    )
...
def test_billing_checkout_replaces_expired_pending_session(monkeypatch) -> None:
    ...
    def fake_checkout(
        ...
        return {
            "url": f"https://checkout.stripe.test/session-{calls['count']}",
            "id": f"cs_expired_{calls['count']}",
...
def test_billing_checkout_skips_when_stripe_subscription_active(monkeypatch) -> None:
    ...
    workspace = client.get("/api/workspace/me", headers=headers).json()
    _grant_subscription_for_test(
        workspace["id"], user_id=checkout_user_id, plan="Starter", status="trialing"
    )
    calls = {"count": 0}
    ...
    def fake_checkout(
        ...
        calls["count"] += 1
        return {
            "url": "https://checkout.stripe.test/should-not-create",
            "id": "cs_should_not_create",
            "customer_id": "cus_should_not_create",
        }
...
def test_billing_checkout_blocks_duplicate_active_stripe_subscriptions(
    monkeypatch,
) -> None:
    ...
    with get_sessionmaker()() as db:
        settings = (
            db.query(AppSettings).filter(AppSettings.workspace_id == workspace_id).one()
        )
    ...
    assert any(
        ...
        and item["active_or_trialing_count"] == 2
        for item in diagnostics["duplicate_active_subscriptions"]
    )


def test_subscription_change_upgrade_records_pending_without_local_entitlement_change(
    monkeypatch,
) -> None:
    ...
    captured: dict[str, Any] = {}
    monkeypatch.setattr(
        "app.api.routes.retrieve_bound_stripe_subscription",
        lambda local_subscription, workspace_id, user_id: {
    ...
    with get_sessionmaker()() as db:
        row = (
            db.query(BillingSubscriptionTransition)
            .filter(
                BillingSubscriptionTransition.stripe_subscription_id
                == "sub_change_upgrade"
            )
            .one()
        )
...
def test_subscription_change_downgrade_schedules_for_period_end_and_reuses_open_transition(
    monkeypatch,
) -> None:
    ...
    def fake_schedule(
        *,
        ...
        calls.append(
            {"to_plan": to_plan, "idempotency_key": transition.idempotency_key}
        )
        return {"id": "sched_change_downgrade"}

    monkeypatch.setattr("app.api.routes.schedule_downgrade", fake_schedule)
    first = client.post(
        "/api/billing/subscription/change", headers=headers, json={"plan": "Pro"}
    )
    second = client.post(
        "/api/billing/subscription/change", headers=headers, json={"plan": "Starter"}
    )
    assert first.status_code == 200
    assert second.status_code == 200
...
def test_subscription_change_blocks_annual_duplicate_and_same_plan(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "app.api.routes.retrieve_bound_stripe_subscription",
        lambda *args, **kwargs: calls.update({"retrieve": calls["retrieve"] + 1}),
    )
    annual = client.post(
        "/api/billing/subscription/change",
        headers=headers,
        json={"plan": "Pro", "billing_period": "annual"},
    )
    duplicate = client.post(
        "/api/billing/subscription/change", headers=headers, json={"plan": "Pro"}
    )
...
def test_subscription_cancel_only_uses_cancel_at_period_end_and_can_undo(
    monkeypatch,
) -> None:
    ...
    calls: list[str] = []
    monkeypatch.setattr(
        "app.api.routes.request_cancel_at_period_end",
        lambda subscription, workspace_id, user_id: calls.append("request"),
    )
    monkeypatch.setattr(
        "app.api.routes.undo_cancel_at_period_end",
        lambda subscription, workspace_id, user_id: calls.append("undo"),
    )
...
def test_subscription_transition_stripe_payloads_are_idempotent_and_do_not_mutate_quantity_or_promotions(
    monkeypatch,
) -> None:
    transition = BillingSubscriptionTransition(
        workspace_id=uuid4(),
    ...
    monkeypatch.setattr(
        "app.services.subscription_transitions.price_for_plan",
        lambda plan, billing_period="monthly": "price_pro_test",
    )
    ...
    monkeypatch.setattr(
        "app.services.subscription_transitions.stripe.Subscription.modify",
    ...
    apply_upgrade_now(
        ...
        stripe_subscription={
            "items": {
                "data": [{"id": "si_payload", "price": {"id": "price_starter_test"}}]
            }
        },
        to_plan="Pro",
    )
    ...
    assert captured_upgrade["items"] == [
        {"id": "si_payload", "price": "price_pro_test"}
    ]
    ...
    monkeypatch.setattr(
        "app.services.subscription_transitions.stripe.SubscriptionSchedule.create",
        fake_schedule_create,
    )
    monkeypatch.setattr(
        "app.services.subscription_transitions.stripe.SubscriptionSchedule.modify",
        fake_schedule_modify,
    )
    schedule_downgrade(
        transition=transition,
        stripe_subscription={
    ...
    assert captured_schedule["create"] == {
        "from_subscription": "sub_payload",
        "idempotency_key": "sub_change_schedule",
    }
    assert captured_schedule["modify"]["idempotency_key"].startswith("sub_sched_mod_")
    assert captured_schedule["modify"]["metadata"]["transition_id"] == str(
        transition.id
    )
    assert all(
        "quantity" not in item
        for phase in captured_schedule["modify"]["phases"]
        for item in phase["items"]
    )


def test_billing_portal_configuration_disables_subscription_switching(
    monkeypatch,
) -> None:
    ...
    monkeypatch.setattr(
        "app.services.billing.stripe.billing_portal.Configuration.create",
        fake_configuration_create,
    )
    monkeypatch.setattr(
        "app.services.billing.stripe.billing_portal.Session.create", fake_session_create
    )
    from app.services.billing import create_billing_portal_session

    result = create_billing_portal_session(
        "cus_portal", "https://app.example/dashboard/billing"
    )
    assert result["url"] == "https://billing.stripe.test/session"
    assert (
        captured["configuration"]["features"]["subscription_update"]["enabled"] is False
    )
    assert (
        captured["configuration"]["features"]["subscription_cancel"]["mode"]
        == "at_period_end"
    )
...
def test_subscription_cancel_webhook_reconciles_cancel_at_period_end() -> None:
    ...
    for value in (True, False):
        ...
        assert (
            client.get("/api/billing/status", headers=headers).json()[
...
def test_subscription_change_webhook_confirmation_applies_entitlement_and_stale_event_does_not_revert() -> (
    None
):
...
def test_subscription_upgrade_waits_for_successful_invoice_before_entitlement_change(
    monkeypatch,
) -> None:
                    ...
                    "data": [
                        {
                            "price": {
                                ...
                                "product": {
                                    "id": "prod_pro_test",
                                    "metadata": {"plan": "Pro", "brand": "OutreachAI"},
                                },
                            }
                        }
                    ]
    ...
    assert (
        client.get("/api/billing/status", headers=headers).json()["plan"] == "Starter"
    )
    ...
    monkeypatch.setattr(
        "app.api.webhooks.stripe.Price.retrieve",
        lambda price_id, **_kwargs: _stripe_price_object(price_id),
    )
            ...
            data=(
                SimpleNamespace(
                    price=SimpleNamespace(
                        id="price_starter_test", product="prod_starter_test"
                    )
                ),
                SimpleNamespace(
                    price=SimpleNamespace(id="price_pro_test", product="prod_pro_test")
                ),
            )
        )
    )
...
def test_subscription_webhook_rejects_forged_workspace_user_customer_and_product_metadata() -> (
    None
):
    ...
    forged_events = [
        ...
        (
            ...
            {
                "workspace_id": workspace["id"],
                "user_id": f"attacker-{uuid4()}@example.com",
                "customer": "cus_binding",
                "price_product_plan": "Pro",
            },
        ),
                        ...
                        "data": [
                            {
                                "price": {
                                    ...
                                    "product": {
                                        "id": "prod_forged",
                                        "metadata": {
                                            "plan": values["price_product_plan"],
                                            "brand": "OutreachAI",
                                        },
                                    },
                                }
    ...
    with get_sessionmaker()() as db:
        rejected = db.scalars(
            select(AuditLog).where(AuditLog.action == "stripe.webhook_rejected")
        ).all()
        assert (
            len(
                [
                    row
                    for row in rejected
                    if row.metadata_json.get("object_id") == "sub_binding"
                ]
            )
            == 4
        )
...
def test_stale_invoice_paid_webhook_does_not_reactivate_newer_canceled_subscription() -> (
    None
):
...
def test_stripe_invoice_payment_failed_records_reason_and_keeps_access_inactive() -> (
    None
):
    ...
    try:
        subscription = (
            db.query(Subscription)
            .filter(Subscription.stripe_subscription_id == "sub_payment_failure_test")
            .one()
        )
    ...
    status = client.get(
        ...
        headers={
            "Authorization": "Bearer dev",
            "X-Test-User-Email": "payment-failure@example.com",
        },
    )
    assert status.status_code == 200
    data = status.json()
...
def test_starter_plan_blocks_sales_employee_limits_and_semi_auto_mode() -> None:
    ...
    db = get_sessionmaker()()
    try:
        db.query(AISalesEmployee).filter(
            AISalesEmployee.workspace_id == UUID(workspace["id"])
        ).delete()
        db.query(Subscription).filter(
            Subscription.workspace_id == UUID(workspace["id"])
        ).delete()
    ...
    assert first.status_code == 200
    second = client.post(
        "/api/sales-employees", headers=AUTH, json={**payload, "name": "Second Ava"}
    )
    assert second.status_code == 402
    assert second.json()["detail"]["code"] == "plan_limit_exceeded"
    assert second.json()["detail"]["metric"] == "sales_employees"
    assert "Upgrade in Billing" in second.json()["detail"]["message"]
    semi_auto = client.put(
        f"/api/sales-employees/{first.json()['id']}",
        headers=AUTH,
        json={**payload, "sending_mode": "Semi-Auto Mode"},
    )
    assert semi_auto.status_code == 402
    assert "Semi-Automatic Campaigns" in semi_auto.json()["detail"]


def test_sales_employee_concurrent_creation_cannot_exceed_plan_limit() -> None:
    user_id = f"sales-employee-concurrent-{uuid4()}@example.com"
    headers = {"Authorization": "Bearer dev", "X-Test-User-Email": user_id}
    workspace = client.get("/api/workspace", headers=headers).json()
    _grant_subscription_for_test(
        workspace["id"], user_id=user_id, plan="Starter", status="active"
    ...
    def create_once(index: int) -> int:
        response = client.post(
            "/api/sales-employees",
            headers=headers,
            json={**payload, "name": f"Concurrent Ava {index}"},
        )
        return response.status_code

    with ThreadPoolExecutor(max_workers=3) as executor:
        statuses = list(executor.map(create_once, range(3)))
    ...
    with get_sessionmaker()() as db:
        employee_count = (
            db.scalar(
                ...
                .where(AISalesEmployee.workspace_id == UUID(workspace["id"]))
            )
            or 0
        )
...
def test_billing_sync_latest_subscription_repairs_paid_workspace(monkeypatch) -> None:
    ...
    def fake_subscription_diagnostics(
    ...
    monkeypatch.setattr(
        ...
        fake_subscription_diagnostics,
    )

    forged_email = client.post(
        "/api/billing/sync-latest-subscription",
        headers=sync_headers,
        json={"customer_email": "buyer@example.com"},
    )
    assert forged_email.status_code == 403
    forged_customer = client.post(
        "/api/billing/sync-latest-subscription",
        headers=sync_headers,
        json={"stripe_customer_id": "cus_attacker"},
    )
    ...
    try:
        ...
        settings.billing = {
            **(settings.billing or {}),
            "stripeCustomerId": "cus_sync_live",
            "checkoutSessionId": "cs_server_created",
        }
        db.commit()
    finally:
        db.close()
    ...
    response = client.post(
        "/api/billing/sync-latest-subscription", headers=sync_headers, json={}
    )
    assert response.status_code == 200
    data = response.json()
    ...
    try:
        subscription = (
            db.query(Subscription)
            .filter(Subscription.stripe_subscription_id == "sub_sync_live")
            .one()
        )
    ...
    second = client.post(
        "/api/billing/sync-latest-subscription",
        headers=sync_headers,
        json={"stripe_customer_id": "cus_sync_live"},
    )
    assert second.status_code == 200
    assert calls[-1]["customer_id"] == "cus_sync_live"
...
def test_billing_sync_unknown_price_requires_owner_review(monkeypatch) -> None:
    ...
    with get_sessionmaker()() as db:
        ...
        settings.billing = {
            ...
            "checkoutSessionId": "cs_server_created_unknown",
        }
        db.commit()
    ...
    monkeypatch.setattr(
        "app.api.routes.subscription_diagnostics_for_customer",
        lambda customer_id="", customer_email="": SimpleNamespace(
            customer=customer, subscriptions=(stripe_subscription,)
        ),
    )
    ...
    with get_sessionmaker()() as db:
        subscription = (
            db.query(Subscription)
            .filter(Subscription.stripe_subscription_id == "sub_sync_unknown_price")
            .one()
        )
...
def test_billing_sync_canceled_duplicate_cannot_override_canonical_active(
    monkeypatch,
) -> None:
    ...
    monkeypatch.setattr(
        "app.api.routes.subscription_diagnostics_for_customer",
        lambda customer_id="", customer_email="": SimpleNamespace(
            customer=customer, subscriptions=(canceled_duplicate,)
        ),
    )
    response = client.post(
        "/api/billing/sync-latest-subscription", headers=sync_headers, json={}
...
def test_billing_sync_multiple_active_subscriptions_returns_degraded_409(
    monkeypatch,
) -> None:
    ...
    monkeypatch.setattr(
        ...
        lambda customer_id="", customer_email="": SimpleNamespace(
            customer=customer,
            subscriptions=(
                stripe_subscription("sub_sync_duplicate_one"),
                stripe_subscription("sub_sync_duplicate_two"),
            ),
        ),
    )
    response = client.post(
...
def test_subscription_diagnostics_for_customer_does_not_silently_choose_duplicates(
    monkeypatch,
) -> None:
    app_settings = get_settings()
    ...
    monkeypatch.setattr(
        "app.services.billing.stripe.Customer.retrieve", lambda customer_id: customer
    )
    monkeypatch.setattr(
        "app.services.billing.stripe.Subscription.list",
        lambda customer, status, limit: subscriptions,
    )
    try:
        diagnostics = subscription_diagnostics_for_customer(
            customer_id="cus_diagnostic"
        )
    finally:
        monkeypatch.setattr(app_settings, "stripe_secret_key", original_key)
    assert diagnostics.customer is customer
    assert [item.id for item in diagnostics.subscriptions] == [
        "sub_active_one",
        "sub_trialing_two",
        "sub_canceled",
    ]
    assert diagnostics.duplicate_active_or_trialing is True
    assert [item.id for item in diagnostics.active_or_trialing] == [
        "sub_active_one",
        "sub_trialing_two",
    ]
...
def test_growth_engine_returns_briefing_and_persists_goal() -> None:
    ...
    goal = client.post(
        "/api/growth-engine/goal",
        headers=AUTH,
        json={"goal": "I want 12 meetings this month."},
    )
...
def test_autonomous_acquisition_run_imports_qualifies_prepares_and_blocks_send(
    ...
    monkeypatch.setattr(
        "app.services.acquisition.collect_website",
        lambda url: type(
            "Snapshot",
            (),
            {
                ...
                "meta_description": "Revenue team",
                "text": "Book a demo Contact us",
                "technologies": ["Next.js"],
            },
        )(),
    )
...
def test_campaign_automation_send_obeys_outbound_provider_kill_switch(
    monkeypatch,
) -> None:
    ...
    try:
        workspace = Workspace(
            owner_user_id="automation-guard-owner", name="Automation Guard Workspace"
        )
        db.add(workspace)
        db.flush()
        campaign = Campaign(
            ...
            workspace_id=workspace.id,
            name="Automation Guard Campaign",
        ...
        lead = Lead(
            ...
            workspace_id=workspace.id,
            campaign_id=campaign.id,
            company="Automation Guard Buyer",
            email="buyer@automation-guard.example",
            status=LeadStatus.qualified,
        )
        db.add(lead)
        db.flush()
...
def test_ai_employee_task_results_persist_csv_and_block_external_send(
    monkeypatch,
) -> None:
    monkeypatch.setattr(
    ...
    plan = client.post(
        f"/api/sales-employees/{employee_id}/plan",
        headers=AUTH,
        json={
            "command": "Find 3 rental companies in Monaco",
            "transcript_source": "text",
        },
    )
    assert plan.status_code == 200
    approved = client.post(
        f"/api/sales-employees/{employee_id}/approve-plan",
        headers=AUTH,
        json={"plan_id": plan.json()["id"], "action": "approve"},
    )
    assert approved.status_code == 200
    executed = client.post(
        f"/api/sales-employees/{employee_id}/execute-plan",
        headers=AUTH,
        json={"plan_id": plan.json()["id"], "action": "approve"},
    )
    ...
    csv_response = client.get(
        f"/api/sales-employees/tasks/{task['id']}/csv", headers=AUTH
    )
    assert csv_response.status_code == 200
    assert (
        "company_name,website,country,city,industry,phone,email,source,confidence_score,short_description,why_matched"
        in csv_response.text
    )
    assert "Rental Prospect 1" in csv_response.text

    send_approval = client.post(
        f"/api/sales-employees/tasks/{task['id']}/approve-send", headers=AUTH
    )
    ...
    empty_plan = client.post(
        f"/api/sales-employees/{employee_id}/plan",
        headers=AUTH,
        json={"command": "Analyse my last campaign", "transcript_source": "text"},
    )
    assert empty_plan.status_code == 200
    empty_approved = client.post(
        f"/api/sales-employees/{employee_id}/approve-plan",
        headers=AUTH,
        json={"plan_id": empty_plan.json()["id"], "action": "approve"},
    )
    assert empty_approved.status_code == 200
    empty_executed = client.post(
        f"/api/sales-employees/{employee_id}/execute-plan",
        headers=AUTH,
        json={"plan_id": empty_plan.json()["id"], "action": "approve"},
    )
    ...
    empty_details = client.get(
        f"/api/sales-employees/tasks/{empty_executed.json()['id']}", headers=AUTH
    )
    ...
    try:
        sent = (
            db.query(EmailMessage)
            .filter(
                EmailMessage.tags["task_id"].as_string() == task["id"],
                EmailMessage.sent_at.is_not(None),
            )
            .count()
        )
...
def test_ai_ceo_voice_briefing_persists_history_and_stays_read_only() -> None:
    for length in ["30 sec", "1 min", "3 min", "10 min"]:
        for language in [
            ...
            briefing = client.post(
                "/api/ai-ceo/briefings",
                headers=AUTH,
                json={"length": length, "language": language},
            )
            assert briefing.status_code == 200
            data = briefing.json()
    ...
    answer = client.post(
        "/api/ai-ceo/question",
        headers=AUTH,
        json={"question": "How much revenue did we create?", "language": "English"},
    )
...
def test_ai_sales_employee_review_mode_imports_qualifies_drafts_and_approves(
    monkeypatch,
) -> None:
    monkeypatch.setattr(
    ...
    insight_response = client.post(
        f"/api/sales-employees/{employee['id']}/leads/{lead['id']}/qualify",
        headers=AUTH,
    )
    ...
    draft_response = client.post(
        f"/api/sales-employees/{employee['id']}/leads/{lead['id']}/draft-email",
        headers=AUTH,
    )
    assert draft_response.status_code == 200
    draft = draft_response.json()
    ...
    approve_response = client.post(
        f"/api/sales-employees/{employee['id']}/emails/{draft['id']}/approve",
        headers=AUTH,
    )
    ...
    run_response = client.post(
        f"/api/sales-employees/{employee['id']}/run", headers=AUTH
    )
    assert run_response.status_code == 200
    assert run_response.json()["mode"] == "Review Mode"


def test_ai_sales_employee_voice_task_plans_requires_approval_and_executes(
    monkeypatch,
) -> None:
    monkeypatch.setattr(
        "app.api.routes.plan_sales_employee_task",
    ...
    plan_response = client.post(
        ...
        headers=AUTH,
        json={
            "command": "Find 5 construction companies in Germany.",
...
def test_ai_team_router_splits_multi_employee_task_and_requires_approval(
    monkeypatch,
) -> None:
    monkeypatch.setattr(
    ...
    response = client.post(
        ...
        headers=AUTH,
        json={
            "command": "Find clients and create marketing posts",
            "transcript_source": "text",
        },
    )
    assert response.status_code == 200
...
def _autopilot_fixture(
    user_id: str = "autopilot-owner",
    workspace_name: str = "Autopilot Workspace",
    recipient: str = "buyer@testmail.local",
):
    settings = get_settings()
    ...
    campaign = Campaign(
        user_id=user_id,
        workspace_id=workspace.id,
        name=f"{workspace_name} Campaign",
        status=CampaignStatus.running,
        timezone="UTC",
    )
    db.add(campaign)
    ...
    lead = Lead(
        ...
        notes=json.dumps(
            {"confidence_score": 88, "source_url": "https://buyer.example/contact"}
        ),
    )
    db.add(lead)
    db.flush()
    db.add(
        Company(
            ...
            metadata_json={"public_source": "https://buyer.example/contact"},
        )
    )
    email = EmailMessage(
        user_id=user_id,
        ...
        delivery_status="approved",
        tags={"autopilot_approved": True},
    )
    db.add(email)
    db.flush()
    job = enqueue_autopilot_email_job(
        db,
        user_id=user_id,
        workspace_id=workspace.id,
        lead=lead,
...
def test_autopilot_worker_blocks_send_and_survives_reprocessing(monkeypatch) -> None:
    ...
    db, workspace, _campaign, _lead, email, job = _autopilot_fixture(
...
def test_autopilot_pause_stop_and_workspace_isolation(monkeypatch) -> None:
    ...
    db, workspace_a, campaign_a, _lead_a, email_a, job_a = _autopilot_fixture(
        "autopilot-tenant-a", "Tenant A", "a@testmail.local"
    )
    db_b, workspace_b, campaign_b, _lead_b, email_b, job_b = _autopilot_fixture(
        "autopilot-tenant-b", "Tenant B", "b@testmail.local"
    )
    ...
    try:
        ...
        assert (
            db.query(EmailMessage)
            .filter(
                EmailMessage.workspace_id == workspace_a.id,
...
def test_autopilot_suppression_and_staging_domain_block_keep_crm_review(
    monkeypatch,
) -> None:
    monkeypatch.setattr(
        "app.services.emailer._send_gmail_email",
    ...
    db, workspace, _campaign, lead, email, job = _autopilot_fixture(
        "autopilot-suppression", "Suppression", "blocked@real-company.com"
    )
    try:
        assert process_autopilot_email_job(db, job)
        db.refresh(email)
        ...
        company = (
            db.query(Company)
            .filter(Company.workspace_id == workspace.id, Company.lead_id == lead.id)
            .one()
        )
        assert email.delivery_status == "needs_review"
        assert lead.status == LeadStatus.qualified
...
def test_autopilot_worker_obeys_outbound_provider_kill_switch(monkeypatch) -> None:
    ...
    db, workspace, _campaign, lead, email, job = _autopilot_fixture(
        "autopilot-guard", "Autopilot Guard", "buyer@testmail.local"
    )
    try:
        assert process_autopilot_email_job(db, job)
        db.refresh(email)
...
def _agent_runtime_registry_with_provider_call_recorder(provider_calls: list[dict[str, Any]]):
    ...
    def forbidden_send(context, payload):
        provider_calls.append(payload.model_dump(mode="json"))
        raise AssertionError("send provider must not be called by this regression test")

    handlers["send_email"] = forbidden_send
    return build_default_tool_registry(handlers)
...
def test_agent_runtime_generate_email_draft_keeps_execution_body_and_redacts_api(
    monkeypatch,
) -> None:
    ...
    provider_calls: list[dict[str, Any]] = []
    registry = _agent_runtime_registry_with_provider_call_recorder(provider_calls)
    subject = f"Agent Runtime Raw Draft {uuid4()}"
    body = f"Unique private email body {uuid4()} that must reach the local draft."
    _install_agent_runtime(
        monkeypatch,
        [
            {
                ...
                "tool_name": "generate_email_draft",
                "arguments": {
                    "recipient_email": "buyer@agent-runtime-raw-draft.example",
                    "subject": subject,
                    "body": body,
                    "dry_run": False,
                },
    ...
    response = _create_agent_run(
        USER_A_AUTH,
        "Create a local draft without sending.",
        dry_run=False,
    )
    payload = response.json()
    run_id = payload["run"]["id"]
    ...
    with get_sessionmaker()() as db:
        ...
        draft_step = db.scalar(
            select(AgentStep).where(
                AgentStep.workspace_id == workspace_id,
                AgentStep.run_id == UUID(run_id),
                AgentStep.tool_name == "generate_email_draft",
            )
        )
    ...
    assert email.body != "[REDACTED_CONTENT]"
    assert draft_step is not None
    assert draft_step.input_json["body"] == body
    assert detail.status_code == 200
    assert detail.json()["steps"][0]["input"]["body"] == "[REDACTED_CONTENT]"
    assert body not in detail_text
    assert body not in trace_text
    assert "[REDACTED_CONTENT]" in detail_text
    assert "[REDACTED_CONTENT]" in trace_text
    assert provider_calls == []


def test_agent_runtime_resume_keeps_later_draft_arguments_after_approval_pause(
    monkeypatch,
) -> None:
    from app.models.entities import AgentStep
    ...
    provider_calls: list[dict[str, Any]] = []
    registry = _agent_runtime_registry_with_provider_call_recorder(provider_calls)
    company_name = f"Agent Runtime Resume CRM {uuid4()}"
    subject = f"Agent Runtime Resume Draft {uuid4()}"
    body = f"Resume-only private draft body {uuid4()} must survive approval pause."
    _install_agent_runtime(
        monkeypatch,
        [
            {
                ...
                "arguments": {
                    "company_name": company_name,
                    "website": "https://agent-runtime-resume-crm.example",
                    "contact_email": "buyer@agent-runtime-resume-crm.example",
                    "dry_run": False,
                },
            },
            {
                "id": "draft",
                "title": "Generate draft after approval",
                "tool_name": "generate_email_draft",
                "arguments": {
                    "recipient_email": "buyer@agent-runtime-resume-crm.example",
                    "subject": subject,
                    "body": body,
                    "dry_run": False,
                },
    ...
    created = _create_agent_run(
        USER_A_AUTH,
        "Pause for CRM approval, then create the draft after resume.",
        dry_run=False,
    )
    payload = created.json()
    run_id = payload["run"]["id"]
    approval_id = payload["approvals"][0]["id"]
    before_resume_text = json.dumps(payload, sort_keys=True)
    approved = client.post(
        f"/api/workspace-app/agent-runs/{run_id}/approve",
        headers=USER_A_AUTH,
    ...
    assert created.status_code == 202
    assert payload["run"]["status"] == "waiting_approval"
    assert body not in before_resume_text
    assert payload["steps"][1]["input"]["body"] == "[REDACTED_CONTENT]"
    assert approved.status_code == 200
    assert approved.json()["status"] == "waiting_approval"
    assert resumed.status_code == 200
    assert resumed.json()["run"]["status"] == "completed"
    ...
    assert draft_step is not None
    assert draft_step.input_json["body"] == body
    assert detail.json()["steps"][1]["input"]["body"] == "[REDACTED_CONTENT]"
    assert body not in detail_text
    assert body not in trace_text
    assert "[REDACTED_CONTENT]" in detail_text
    assert "[REDACTED_CONTENT]" in trace_text
